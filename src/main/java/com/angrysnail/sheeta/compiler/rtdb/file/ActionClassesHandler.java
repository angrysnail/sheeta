...
import com.angrysnail.sheeta.compiler.rtdb.FileIterator;
import com.angrysnail.sheeta.library.URIEncoderDecoder;

/**
 * the class handling the copy of the actions classes in the output actions
 * directory
 * 
...
public class ActionClassesHandler {
	...
	/**
	 * copies all the action classes that can be found in the project and its
	 * referenced projects into the compiled action classes directory
	 * 
	 * @param workspacePath
	 *            the workspace path
	...
	public void copyActionClasses(String workspacePath, String outputDirectory,
			LinkedList<String> projectNamesList)
			throws DuplicateActionFileException {
		...
		try {
			destActionDir = new File(new URI(outputDirectory + "/"
	...
	protected File getActionDirectory(File projectFile) {
		...
		if (projectFile != null) {
			...
			// getting the actions directory among the child files of the
			// project directory
	...
	/**
	 * copies all the action files and directory that can be found in the given
	 * source action directory
	 * 
	 * @param sourceActionDirectory
	 *            the source action directory
	 * @param destinationActionDirectory
	 *            the destination action directory
	 * @throws DuplicateActionFileException
	 *             the exception raised when two or more action java classes
	 *             have the same name
	 */
	protected void copyActionFile(File sourceActionDirectory,
			File destinationActionDirectory)
			throws DuplicateActionFileException {

		File srcFile = null;
		LinkedList<String> segments = null;

		// iterating over each directory and file in the source action directory
		// and
		// copying them into the destination directory
		for (FileIterator it = new FileIterator(sourceActionDirectory); it
				.hasNext();) {
			...
			if (srcFile != null) {
				...
				if (srcFile.isDirectory()) {
					...
					// the current file is a directory, it is created in the
					// destination directory
					createDirectory(destinationActionDirectory, segments);

				} else if (srcFile.isFile()
						&& srcFile.getName().endsWith(
								FileManager.CLASS_FILE_EXTENSION)) {

					// the source current file will be written in the
					// destination directory
					createFile(sourceActionDirectory,
							destinationActionDirectory, segments);
				}
			}
		}
	}
	...
	/**
	 * creates the directory denoted by the given segments list under the
	 * destination action directory
	 * 
	 * @param destinationActionDirectory
	 *            the output directory containing the actions
	 * @param segments
	 *            the list of the path segments relatively to the actions
	 *            directory
	 */
	protected void createDirectory(File destinationActionDirectory,
	...
	/**
	 * creates the files denoted by the given segments list and copies the
	 * content of the source file into the destination ones
	 * 
	 * @param sourceActionDirectory
	 * @param destinationActionDirectory
	 * @param segments
	 ...
	 *             have the same name
	 */
	protected void createFile(File sourceActionDirectory,
			File destinationActionDirectory, LinkedList<String> segments)
			throws DuplicateActionFileException {
		...
		for (int i = 0; i < segments.size() - 1; i++) {
			...
			// getting the path of the directory for the source and destination
			// action directories
		...
		if (currentSrcDir != null && currentDestDir != null) {
			...
			if (srcFile.exists()) {
				...
				if (destFile.exists()) {
					...
					throw new DuplicateActionFileException(srcFile.toURI()
							.toASCIIString());
				}

				// copying the content of the source file into the destination
				// file
				try {
					FileChannel srcChannel = new FileInputStream(srcFile)
							.getChannel();
					FileChannel destChannel = new FileOutputStream(destFile)
							.getChannel();
					ByteBuffer buffer = ByteBuffer.allocate((int) srcChannel
							.size());
	 ...
	 *            the base action directory for the project that contains the
	 *            given file
	 * @param file
	 *            a file in the actions directroy of a project
	...
	protected LinkedList<String> getPathSegments(File actionDirectory, File file) {
		...
		// getting the relativized path of the given file against the action
		// directory
		...
		try {
			relativeURI = URIEncoderDecoder.relative(actionDirectoryURI,
					fileURI);
		} catch (Exception ex) {
		}
