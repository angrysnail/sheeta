...
public class ColorsBlinkingsFile {
	 ...
	 */
	private final static String name = "colorsBlinkings"
			+ FileManager.XML_FILE_EXTENSION;
	...
	/**
	 * the map associating an old id (having the pattern :
	 ...
	 *            the name of the project that is handled
	 */
	public ColorsBlinkingsFile(FileManager fileManager, String workspacePath,
			String projectName) {
		...
		try {
			// creating the file
			colorsBlinkingsFile = new File(new URI(workspacePath + "/"
					+ projectName + "/" + name));
	...
	/**
	 * handles (i.e.: make the id unique) all the ids that can be found in the
	 * document
	 */
	public void handleIds() {
			...
			if (cur != null) {
				...
				if (cur instanceof Element) {
					...
					if (oldId != null) {
						...
						// if the old id already exists, it is replaced by the
						// new id
						if (allIds.contains(oldId)) {
							...
							oldIdToNewId.put(projectName
									+ FileManager.separator + oldId, newId);
		...
		// modifies the referenced blinking ids that can be found in the
		// blinking colors
		Element parentColorsElement = getColorsElement();

		if (parentColorsElement != null) {

			for (cur = parentColorsElement.getFirstChild(); cur != null; cur = cur
					.getNextSibling()) {

				if (cur instanceof Element
						&& cur.getNodeName().equals("blinkingColor")) {
	...
	public CompiledColorsBlinkingsFile mergeColorsBlinkingsFiles() {
		...
		// creating the related colors and blinkings files in the other projects
		// that have been
		// found in the lib path files
		...
		for (String cbProjectName : projectNames) {
			...
			if (cbProjectName != null && !cbProjectName.equals("")
					&& !cbProjectName.equals(projectName)) {

				// creating the colors and blinkings file corresponding to the
				// project name
				file = new ColorsBlinkingsFile(fileManager, workspacePath,
						cbProjectName);
				...
				if (file.exists()) {
					...
					if (parent != null) {
						...
						for (cur = parent.getFirstChild(); cur != null; cur = cur
								.getNextSibling()) {

							if (cur instanceof Element) {

								parentBlinkingElement.appendChild(doc
										.importNode(cur, true));
							}
						}
					}
					...
					if (parent != null) {
						...
						for (cur = parent.getFirstChild(); cur != null; cur = cur
							...
							if (cur instanceof Element) {
								...
								parentColorElement.appendChild(doc.importNode(
										cur, true));
							}
						}
					}
		...
		compiledFile = new CompiledColorsBlinkingsFile(fileManager,
				fileManager.getOutputDirectory() + "/" + name, doc,
				oldIdToNewId);
	...
	protected Element getBlinkingsElement() {
		...
		if (nodeList != null && nodeList.getLength() > 0) {
			...
			parentBlinkingElement = doc.createElementNS(doc
	...
	protected Element getColorsElement() {
		...
		if (nodeList != null && nodeList.getLength() > 0) {
			...
			parentColorElement = doc.createElementNS(doc.getDocumentElement()
					.getNamespaceURI(), parentColorsElementName);
