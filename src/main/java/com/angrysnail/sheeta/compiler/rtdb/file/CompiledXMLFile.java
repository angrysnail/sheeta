...
public class CompiledXMLFile extends CompiledFile {
	...
	protected static final String viewNodeName = "view", tagNodeName = "tag",
			alarmAckAttName = "alarm.ack", projectAttName = "project",
			locationAttName = "location";
	...
	/**
	 * the list of the tags that can be found in the xml file and that will be
	 * printed into the tags text file
	 */
	...
	/**
	 * the map associating the name of a destination data base file to a source
	 * data base file
	 */
	 ...
	 * @param dataBaseFiles
	 */
	public CompiledXMLFile(FileManager fileManager, String path, Document doc,
			String rootView, Map<String, File> dataBaseFiles) {

		super(fileManager, path, doc, XML_FILE);

		this.rootView = rootView;
		this.ipAddress = doc.getDocumentElement().getAttribute(
				serverAddressName);
	...
	protected void writeFile() {
		...
		try {
			...
			tagListFilePath = tagListFilePath.substring(0, tagListFilePath
					.lastIndexOf(FileManager.GLIPS_VIEW_EXTENSION))
	...
	/**
	 * handles the view elements that can be found in this compiled xml file and
	 * records all the tags that can be found in the xml file
	 */
		...
		for (NodeIterator it = new NodeIterator(doc.getDocumentElement()); it
			...
			if (cur != null && cur instanceof Element) {
				...
				if (cur.getNodeName().equals(viewNodeName)) {
					...
					// getting the project name, the location and the xml path
					// of the view
					...
					if (projectName != null && !projectName.equals("")
							...
							&& xmlPath != null && !xmlPath.equals("")) {

						// creating the svg file corresponding to the view
						svgFile = new SVGFile(fileManager,
								fileManager.getWorkspacePath(), projectName,
								location, xmlPath);
	...
	/**
	 * @return the map associating the absolute xml path of a svg file to this
	 *         svg file
	 */
	...
	/**
	 * adds the link items in this compiled xml file and sets the path for the
	 * view nodes location attribute
	 */
				...
				if (svgFile != null) {
					...
					for (String tagName : usedTags) {
						...
						if (tagName != null && !tagName.equals("")) {
							...
							linkItem = view.getOwnerDocument().createElement(
									"linkItem");
