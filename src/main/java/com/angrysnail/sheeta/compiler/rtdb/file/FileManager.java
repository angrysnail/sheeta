...
import com.angrysnail.sheeta.compiler.rtdb.DuplicateActionFileException;
import com.angrysnail.sheeta.compiler.rtdb.InvalidInheritedNodeException;
import com.angrysnail.sheeta.compiler.rtdb.InvalidLibPathException;
import com.angrysnail.sheeta.compiler.rtdb.MissingFileException;
...
public class FileManager {
	...
	/**
	 * the map associating the name of a project to the list of the projects
	 * that it references
	 */
	...
	/**
	 * the map of the file paths that are used by all the svg files, it
	 * associates the path of the file found in the svg file to the path used
	 * for copying it
	 */
	 ...
	 *            the root view path
	 * @param isTestMode
	 *            whether the compiler is in the test mode
	 */
	public FileManager(String workspacePath, String projectName,
			String fileName, String outputDirectory, String rootViewPath,
			boolean isTestMode) {
	 ...
	 * 
	 * @throws InvalidLibPathException
	 *             the exception raised if there is a cycle in the lib path
	 *             files
	 * @throws InvalidInheritedNodeException
	 *             the exception raised if there is a cycle in the inheritance
	 *             node
	 * @throws MissingFileException
	 *             the exception raised if a required data base file is missing
	 * @throws Exception
	 *             the exception raised if something wrong happens
	 * @throws DuplicateActionFileException
	 *             the exception raised when two or more action java classes
	 *             have the same name
	 */
	public void compile() throws InvalidLibPathException,
			InvalidInheritedNodeException, MissingFileException,
			DuplicateActionFileException, Exception {

		// filling the list of the lib paths
		fillLibPathFileNamesMap();

		// compiled the colors and blinkings files
		ColorsBlinkingsFile colorsBlinkingsFile = new ColorsBlinkingsFile(this,
				workspacePath, projectName);

		if (colorsBlinkingsFile.exists()) {

			this.compiledColorsBlinkingsFile = colorsBlinkingsFile
					.mergeColorsBlinkingsFiles();
		}
		...
		if (compiledXMLFile != null) {
			...
			actionClassesHandler.copyActionClasses(workspacePath,
					outputDirectory, projectNamesList);
	...
	protected void compileSVGFiles() {
		...
		// handles the references and replaces the ids in the references of all
		// the svg files
		for (SVGFile file : svgFiles.values()) {
		...
		if (svgFile != null) {
			...
			// creating the compiled files of the svg files that are specified
			// in the "loadView"
			// animation nodes
	...
	/**
	 * creates and returns the svg file corresponding to the given absolute xml
	 * path
	 * 
	 * @param xmlPath
	 *            an absolute xml path
	...
	/**
	 * returns the name of the project into which the file denoted by the given
	 * file name can be found
	 * 
	 * @param sourceProjectName
	 *            the project name from which the file path has been found
	 * @param relativeFileName
	 *            a file name relatively to a project path
	 * @return the name of the project into which the file denoted by the given
	 *         file name can be found
	 */
	public String getProjectName(String sourceProjectName,
		...
		if (relativeFileName != null && !relativeFileName.equals("")) {
			...
			// getting the list of the projects names that are referenced by the
			// given project name
			LinkedList<String> libPathList = new LinkedList<String>(
					libPathsMap.get(sourceProjectName));
			libPathList.add(sourceProjectName);

			File file = null;

			// searching for the first project name containing the file denoted
			// by the given file
			// name
			for (String currentProjectName : libPathList) {
				...
				if (currentProjectName != null
					...
					// creating a file corresponding to the given file name in
					// the current project
					// and checking
					// if this exists or not
					try {
						file = new File(new URI(workspacePath + "/"
								+ currentProjectName + "/" + relativeFileName));
					} catch (Exception ex) {
						file = null;
						ex.printStackTrace();
					}
					...
					if (file != null && file.exists()) {
						...
						// as the file denoted by the given file name can be
						// found in the current
						// project, the project name is returned
	...
	/**
	 * @return the map associating an old id (having the pattern :
	 *         "projectName":::"oldId" to a new id
	 */
	...
	/**
	 * copy the used files in the output directory if it is different from the
	 * workspace directory
	 */
					...
					if (newPath != null && !newPath.equals("")) {
						...
						try {
							...
							out = new BufferedOutputStream(
									new FileOutputStream(new File(new URI(
											newPath))));
	...
	protected void writeClasses() {
		...
		try {
			destinationActionDirectory = new File(new File(new URI(
					outputDirectory)), ACTIONS_DIRECTORY_NAME);
				...
				if (pName != null && !pName.equals("")) {
					...
					if (pFile != null) {
						...
						if (actionDirectory != null) {
							...
							// adding all the files that can be found in the
							// actions directory
							// recursively
							copyClassFiles(actionDirectory,
									destinationActionDirectory);
						}
					}
				}
			}
	...
	protected void copyClassFiles(File initFile, File destFile) {
		...
		if (initFile != null && destFile != null) {
			...
			// copying the files that are children of the initial file as
			// children of the
			// destination file
			for (File child : children) {
				...
				if (child != null) {
					...
					if (child.isDirectory()) {
						...
						// checking if this directory exists among the
						// destination child files
						childDestinationDirectory = fileExistAsChild(destFile,
								child.getName());

						if (childDestinationDirectory == null) {

							// creates the directory having the same name as the
							// current file as
							// a child of the destination directory
							try {
								childDestinationDirectory = new File(destFile,
										child.getName());
	...
	/**
	 * returns the file having the given name among the child files of the given
	 * parent file
	 * 
	 * @param parentFile
	 *            the parent file
	 * @param name
	 *            the name of a child file
	 * @return the file having the given name among the child files of the given
	 *         parent file
	 */
	...
	public static Document getSVGDocument(File file) {
		...
		if (file != null) {
			...
			try {
				doc = documentFactory.createSVGDocument(file.toURI()
						.toASCIIString());
			} catch (Exception ex) {
				ex.printStackTrace();
			}
	...
	public static File createFile(String path) {
		...
		if (!path.equals("")) {
			...
			if (pos > 0) {
				...
				// getting the path of the directory into file the file denoted
				// by the path could be
				// found
				String parentDirectoryPath = path.substring(0, pos);

				if (parentDirectoryPath != null
						&& !parentDirectoryPath.equals("")) {
	...
	protected File getLibPathsFile(String libProjectName) {
		...
		if (libProjectName != null && !libProjectName.equals("")) {
			...
			try {
				projectFile = new File(new URI(workspacePath + "/"
						+ libProjectName));
			} catch (Exception ex) {
				ex.printStackTrace();
			}
			...
			if (projectFile != null && projectFile.exists()) {
				...
				// getting the files under the project directory whose extension
				// is ".libpath"
	 ...
	 * 
	 * @param libPathsFile
	 *            the file containing the names of the referenced projects
					...
					if (children != null) {
						...
						for (j = 0; j < children.getLength(); j++) {
							...
							if (children.item(j) != null
									&& children.item(j) instanceof Text
									&& !children.item(j).getNodeValue()
											.equals("")) {

								list.add(URIEncoderDecoder.encode(children
										.item(j).getNodeValue()));
							}
						}
					}
	 ...
	 *             the exception raised if something wrong happens
	 */
	protected void fillLibPathFileNamesMap() throws InvalidLibPathException,
			Exception {
		...
		// filling the list of the lib paths for the current project and its
		// referenced projects
		do {
			...
			if (name != null && !name.equals("")) {
				...
				// getting the list of the names of the projects referenced by
				// the current project
				...
				// adding the name of the referenced projects to the list of the
				// lib paths
				for (String nm : list) {

					if (nm != null && !nm.equals("")
							&& !projectNamesList.contains(nm)) {
	 ...
	 * 
	 * @param projectList
	 *            the list of the project we have already been through
	 ...
	 *             the exception raised if something wrong happens
	 */
	protected void checkCycle(LinkedList<String> projectList,
			String currentProjectName) throws InvalidLibPathException,
			Exception {

		if (projectList != null && currentProjectName != null
				&& !currentProjectName.equals("")) {
			...
			if (!projectList.contains(currentProjectName)) {
				...
				// getting the list of the project names associated with the
				// current project name
				LinkedList<String> list = libPathsMap.get(currentProjectName);

				if (list != null && list.size() > 0) {

					// for each referenced project name, checks if they create a
					// cycle
					for (String name : list) {

						if (name != null && !name.equals("")) {

							checkCycle(new LinkedList<String>(projectList),
									name);
						}
					}
				}
