...
public class RtdaTagHandler {
	...
	/**
	 * the set of the names of the animation attributes that require a blinking
	 * name ( format :
	 * "parent name attribute value"+separator+"blinkingAttributeName")
	 */
	private HashSet<String> blinkingAttributes = new HashSet<String>();

	/**
	 * the set of the strings used to check if the value of a tag attribute is a
	 * tag name or not
	 */
	private static HashSet<String> notTagValues = new HashSet<String>();

	/**
	 * the map associating the name of an animation tag attribute to the
	 * attribute child node containing one of the tag values ( format :
	 * "parent name attribute value"+separator+"tagAttributeName")
	 */
	private final HashMap<String, String> enumeratedTagToTagValue = new HashMap<String, String>();

	/**
	 * the map associating the name of an animation attribute that require the
	 * values of an enumerated tag to the attribute name that references this
	 * enumerated tag (format :
	 * "parent name attribute value"+separator+"tagAttributeName")
	 */
	...
	static {
		...
		// adding the strings used to check if the value of a tag attribute is a
		// tag name or not
			...
			for (NodeIterator it = new NodeIterator(doc.getDocumentElement()); it
				...
				if (cur != null && cur instanceof Element) {
					...
					if (type != null && type.equals("blinkingchooser")) {
						...
						wholeName = ((Element) el.getParentNode())
								.getAttribute("name")
								+ separator
						...
						if (name != null && !name.equals("")) {
							...
							wholeName = ((Element) el.getParentNode())
									.getAttribute("name") + separator + name;
					...
					} else if (el.hasAttribute("constraint")
							&& el.getAttribute("constraint").equals("tagvalue")) {

						name = el.getAttribute("name");

						// getting the attribute name of the parent node
						// corresponding to this tag
						// value attribute
						elementName = ((Element) el.getParentNode())
								.getAttribute("name");

						// getting the attribute name of the tag attribute
						// corresponding to this tag
						// value
						tagAttribute = "";

						// the attribute containing the tag value can be found
						// in the child node of
						// an animation node
						for (node = cur.getParentNode().getParentNode()
								.getFirstChild(); node != null; node = node
								.getNextSibling()) {

							if (node instanceof Element
									&& node.getNodeName().equals("attribute")
									&& ((Element) node).getAttribute(
											"constraint").equals("tag")) {

								tagAttribute = ((Element) node)
										.getAttribute("name");
								break;
							}
						}

						tagAttribute = ((Element) el.getParentNode()
								.getParentNode()).getAttribute("name")
								+ separator + tagAttribute;
						...
						if (wholeName != null
								&& !wholeName.equals("")
								&& !enumeratedTagToTagValue
										.containsKey(tagAttribute)) {

							enumeratedTagToTagValue
									.put(tagAttribute, wholeName);
						}
						...
						isChildNodeAttribute = cur.getParentNode()
								.getNodeName().equals("child");
						...
						if (isChildNodeAttribute) {
							...
							tagAttribute = elementName + separator
									+ tagAttribute;
						}
						...
						if (wholeName != null
							...
							refAttributeToEnumeratedTag.put(wholeName,
									tagAttribute);
						}
					}
				}
	 ...
	 * 
	 * @param nodeName
	 *            the name of the node containing the given attribute
	 * @param attributeName
	...
	/**
	 * computes and returns the absolute tag name corresponding to the given tag
	 * name, by checking if the given tag name is a real tag name and by
	 * concatenating the tag name to the ref path
	 * 
	 * @param xmlPath
	...
	public static String getTagName(String xmlPath, String attValue) {// TODO
		...
		if (xmlPath != null && !xmlPath.equals("") && attValue != null
				&& !attValue.equals("")) {
			...
			if (!notTagValues.contains(attValue)) {
				...
				if (Double.isNaN(d)) {
					...
					// checking if the string is a tag name or if it contains a
					// tag name
					if (attValue.indexOf(TagEventsManager.separator) != -1) {

						// splitting the value
						String[] splitValues = attValue
								.split(TagEventsManager.separatorRegex);

						if (splitValues != null && splitValues.length > 1
								&& !splitValues[1].equals("")) {

							splitValues[1] = normalizeTagName(xmlPath,
									splitValues[1]);
							...
							for (int i = 0; i < splitValues.length; i++) {
								...
								attValue += splitValues[i]
										+ TagEventsManager.separator;
							}
						}
						...
						while (pos != -1) {
							...
							beforeTagName = cValue.substring(0, pos
									+ Toolkit.startTagInRequest.length());
							cValue = cValue.substring(pos
									+ Toolkit.startTagInRequest.length(),
									cValue.length());
							...
							if (pos2 != -1) {
								...
								cValue = cValue.substring(pos2
										+ Toolkit.endTagInRequest.length(),
										cValue.length());
	...
	/**
	 * computes and returns the tag name that could be found in the provided
	 * string
	 * 
	 * @param value
	...
	/**
	 * returns the name of the attribute containing in the children of the node
	 * whose name is nodeName, this attribute has one of the enumerated tag
	 * values as value
	 * 
	 * @param nodeName
	 *            the name of a node
	 * @param tagAttributeName
	 *            the name of the attribute containing the tag
	 * @return the name of the attribute containing in the children of the node
	 *         whose name is nodeName, this attribute
	 */
	public String getEnumeratedTagValueAttributeName(String nodeName,
			String tagAttributeName) {

		String enumeratedTagValueAttributeName = "";

		if (nodeName != null && !nodeName.equals("")
				&& tagAttributeName != null && !tagAttributeName.equals("")) {

			enumeratedTagValueAttributeName = enumeratedTagToTagValue
					.get(nodeName + separator + tagAttributeName);
		}
	...
	/**
	 * returns whether the given attribute name corresponds to an enumerated tag
	 * value
	 * 
	 * @param nodeName
	 *            the name of a node
	 * @param attributeName
	 *            the name of an attribute
	 * @return whether the given attribute name corresponds to an enumerated tag
	 *         value
	 */
	public boolean isEnumeratedTagValueReferenceAttribute(String nodeName,
		...
		if (nodeName != null && !nodeName.equals("") && attributeName != null
			...
			isEnumeratedTagValueReferenceAttribute = refAttributeToEnumeratedTag
					.containsKey(nodeName + separator + attributeName);
		}
	 ...
	 */
	public String getEnumeratedTagValueReferenceAttribute(String nodeName,
			String attributeName) {
		...
		if (nodeName != null && !nodeName.equals("") && attributeName != null
			...
			enumeratedTagValueReferenceAttribute = refAttributeToEnumeratedTag
					.get(nodeName + separator + attributeName);
		}
	...
	/**
	 * returns whether the given attribute name corresponds to a blinking
	 * attribute
	 * 
	 * @param nodeName
	 *            the name of a node
	 * @param attributeName
	 *            the name of an attribute
	 * @return whether the given attribute name corresponds to a blinking
	 *         attribute
	 */
	public boolean isBlinkingAttribute(String nodeName, String attributeName) {
		...
		if (nodeName != null && attributeName != null) {
			...
			isBlinkingAttribute = blinkingAttributes.contains(nodeName
					+ separator + attributeName);
		}
	...
	/**
	 * normalizes the given path, i.e., removes the '/' character at the start
