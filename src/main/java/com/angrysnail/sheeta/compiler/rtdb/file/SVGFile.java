 ...
 */
package com.angrysnail.sheeta.compiler.rtdb.file;
...
import com.angrysnail.sheeta.compiler.rtdb.NodeIterator;
...
public class SVGFile {
	 ...
	 * strings
	 */
	protected static final String imageTagName = "image",
			styleAttribute = "style",
			rectStyleValue = "fill:#ffffff;stroke:none;opacity:0;",
			noneValue = "none", rectTagName = "rect", xAtt = "x", yAtt = "y",
			wAtt = "width", hAtt = "height";
	...
	/**
	 * the path that will be used to build the path of all the links that are
	 * contained in this svg
	 */
	 ...
	 */
	protected static final HashMap<String, String> locationToXmlPath = new HashMap<String, String>();

	/**
	 * the map associating a xml path to the location of the svg file
	 */
	protected static final HashMap<String, String> xmlPathToLocation = new HashMap<String, String>();
	...
	/**
	 * the map associating an old id to a new id that is not a duplicated id in
	 * the application (all the ids are absolute)
	 */
	protected static final Map<String, String> oldIdToNewId = new HashMap<String, String>();

	/**
	 * the map associating an old id to a new id for a jwidget that is not a
	 * duplicated id in the application
	 */
	protected static final Map<String, String> jwidgetOldIdToNewId = new HashMap<String, String>();

	/**
	 * the map associating the path of a used file to its saved path
	 */
	protected static final HashMap<String, String> usedFilesPaths = new HashMap<String, String>();
	 ...
	 */
	protected Map<Element, SVGWidgetFile> includedWidgetSVGFiles = new HashMap<Element, SVGWidgetFile>();
	...
	/**
	 * the set of the elements that should be replaced by a whole svg file
	 * document
	 */
	private final HashSet<Element> viewOrWidgetNodes = new HashSet<Element>();

	/**
	 * the map associating an absolute path to its corresponding relative path
	 * (relative to the current svg file path)
	 */
	...
	/**
	 * checks if the given document contains the rtda name space, if not, the
	 * namespace is added
	 * 
	 * @param doc
	 *            a svg document
	...
	protected String getUniqueSavedPath(String path) {
		...
		if (spath != null && !spath.equals("")) {
			...
			// checks whether the save path does not already exists among the
			// other view files and
			// the used files
			while (locationToXmlPath.containsKey(fileManager
					.getOutputDirectory() + "/" + spath + extension)
					|| usedFilesPaths.containsValue(fileManager
							.getOutputDirectory() + "/" + spath + extension)) {
	 ...
	 * 
	 * @param files
	 *            the set of files used to compute the file that should be saved
	 */
		...
		if (files != null) {
			...
			for (SVGFile file : filesToBeSaved) {
				...
				if (file != null && !files.contains(file)) {
					...
					try {
						file.computeFilesToBeSaved(files);
					} catch (Throwable ex) {
						ex.printStackTrace();
					}
				}
	...
	/**
	 * checking if all the ids are unique in the application, and if not,
	 * replaces them
	 */
	public void handleIds() {
		...
		if (!hasHandledIds && doc != null) {
			...
			// for each node, checks if the id has already been used in the
				...
				if (cur != null) {
					...
					if (cur instanceof Element) {
						...
						if (el.getNodeName().equals(imageTagName)
							...
							rect = doc.createElementNS(doc.getDocumentElement()
									.getNamespaceURI(), "rect");

							rect.setAttributeNS(null, xAtt,
									el.getAttribute(xAtt));
							rect.setAttributeNS(null, yAtt,
									el.getAttribute(yAtt));
							rect.setAttributeNS(null, wAtt,
									el.getAttribute(wAtt));
							rect.setAttributeNS(null, hAtt,
									el.getAttribute(hAtt));
							...
							LinkedList<Element> childNodes = Toolkit
									.getChildrenElements(el);

							// copying all the child nodes of the element into
							// the rect element
							...
							// finds and stores in a set the attributes that
							// should be
	...
	protected void handleJWidgetIds(Element element) {
		...
		if (jwidgetElements != null && jwidgetElements.getLength() > 0) {
			...
			for (int i = 0; i < jwidgetElements.getLength(); i++) {
				...
				jwidgetId = jwidgetElement
						.getAttribute(JWidgetRuntime.jwidgetIdAttributeName);
	...
	/**
	 * finds and stores in a set the attributes that should be checked and the
	 * attributes that contains a uri and handle all the tag values that could
	 * be found in the given element
	 * 
	 * @param element
	 *            an element
	 */
	protected void findAttributesWithUriAndHandleTags(Element element) {
		...
		if (element != null) {
			...
			boolean isJwidgetElement = element.getNodeName().equals(
					JWidgetRuntime.jwidgetTagName);

			if (isJwidgetElement) {

				if (element.getAttribute(
						JWidgetRuntime.jwidgetNameAttributeName).equals(
						"ViewBrowserWidget")) {

					// getting the value of the initial view
					String initialViewValue = element
							.getAttribute("initialView");

					if (initialViewValue != null
							&& !initialViewValue.equals("")) {

						// computing the absolute view path
						initialViewValue = RtdaTagHandler.getTagName(
								svgXmlPath, initialViewValue);
						element.setAttribute("initialView", initialViewValue);

						// getting the file corresponding to the xml path and
						// registering so that it
						// is saved
					...
					tag1Value = RtdaTagHandler
							.getTagName(svgXmlPath, tag1Value);
					element.setAttribute("tag1", tag1Value);

					tag2Value = RtdaTagHandler
							.getTagName(svgXmlPath, tag2Value);
				...
				boolean isRtdaNode = element.getNodeName().startsWith(
						rtdaPrefix)
						&& !isJwidgetElement;
				...
				if (type == WIDGET || type == VIEW) {
					...
					// modifying the "view" attribute if this node links to a
					// view
					if (type == VIEW) {
						...
						if (att != null) {
							...
							newTagName = RtdaTagHandler.getTagName(svgXmlPath,
									att.getNodeValue());
						...
						// getting the relative path of the widget file to this
						// svg file
						location = element.getAttributeNS(
								FileManager.xmlnsXLinkNS, "href");
						...
						if (prjName != null && !prjName.equals("")
							...
							NodeList nodes = element
									.getElementsByTagName(RtdaTagHandler.widgetDataBaseRootNodeName);

							if (nodes != null && nodes.getLength() > 0) {

								widgetDataBaseRootNode = (Element) nodes
										.item(0);
							}
							...
							if (location.startsWith(startPath)) {
								...
								location = location.substring(startPath
										.length());
				...
				} else if (type == REGULAR || type == JWIDGET
					...
					if (isRtdaNode) {
						...
						if (type == REGULAR) {
							...
							currentHandler = fileManager
									.getTagHandlersManager().getTagHandler(
											element);

						} else if (type == JWIDGET_ANIMATION) {

							// getting the id of the jwidget corresponding to
							// this element
							...
							if (jwidgetId != null && !jwidgetId.equals("")) {
								...
								currentHandler = fileManager
										.getTagHandlersManager()
										.getJWidgetTagHandler(jwidgetId);
							}
						}
					}
					...
					for (i = 0; i < attributes.getLength(); i++) {
						...
						if (isRtdaNode && currentHandler != null) {
							...
							if (att.getNodeValue().indexOf(
									JWidgetRuntime.jwidgetPrefixtId) != -1) {

								translateJWidgetIds(att);

							} else if (currentHandler.isBlinkingAttribute(
									element.getNodeName(), att.getNodeName())) {

								// the attribute defines a blinking, creating
								// the whole name of the
								// id of the blinking
								wholeName = this.projectName
										+ FileManager.separator
										+ att.getNodeValue();

								// checking if the value of the attribute is the
								// name of a blinking
								// that has been modified
								if (fileManager
										.getColorsBlinkingsOldIdToNewIdMap()
										.containsKey(wholeName)) {

									newValue = fileManager
											.getColorsBlinkingsOldIdToNewIdMap()
											.get(wholeName);
							...
							} else if (currentHandler.isTagAttribute(
									element.getNodeName(), att.getNodeName())
									|| currentHandler.isViewAttribute(
											element.getNodeName(),
											att.getNodeName())) {

								// computing the new name for the tag (the
								// absolute name of the tag
								// relatively to
								// the reference node of this svg file)
								newTagName = RtdaTagHandler.getTagName(
										svgXmlPath, att.getNodeValue());
								...
								if (newTagName != null
									...
									// getting the real tag name that can be
									// found in the current
									// tag name
									// if the provided string contains a tag
									// name and other
									// information
									newTagName = RtdaTagHandler
											.getRealTagName(newTagName);// TODO

									// adding this tag name into the set of the
									// tag names that are
									// used by this svg file
									if (currentHandler.isViewAttribute(
											element.getNodeName(),
											att.getNodeName())) {

										// registers the referenced views in the
										// attribute value
										registerReferencedViews(att);

									} else if (!usedTagNames
											.contains(newTagName)) {
						...
						if (!isTagAttribute && att != null) {
							...
							if (att.getNodeValue().indexOf(
									ColorsBlinkingsFile.start) != -1) {

								// handles the colors ids//
								newValue = att.getNodeValue();

								// getting all the color ids that can be found
								// in the attribute
								// value
								colorIds = getAllColorIds(newValue);

								// for each found id, checking if a new id
								// exists for it, and then
								// replaces it in the attribute value
								for (String foundId : colorIds) {
									...
									if (foundId != null && !foundId.equals("")) {
										...
										if (newId != null && !newId.equals("")) {
											...
											// replacing all the old id values
											// by the new ones
											newValue = newValue
													.replaceAll(
															"/[*]" + foundId
																	+ "[*]/",
															ColorsBlinkingsFile.start
																	+ newId
																	+ ColorsBlinkingsFile.end);
										}
									}
								}
							...
							if (att.getNodeName().equals("xlink:href")
									|| (!att.getNodeName().equals("id") && att
											.getNodeValue().indexOf("url(") != -1)) {

								// handles the attributes containing a url
								attributesToBeChecked.add(att);

								// retrieving the potential list of paths used
								// in this attribute
								// value
								...
								for (String usedPath : usedPaths) {
									...
									if (usedPath != null
											&& !usedPath.equals("")
											&& !usedFilesPaths
													.containsKey(usedPath)) {

										// computing the path that will be used
										// for saving the used
										// file
										...
										if (savePath != null
											...
											// putting the path of the used file
											// and its save path
											// into the map
											usedFilesPaths.put(usedPath,
													savePath);
										}
									}
								}
	...
	protected void registerReferencedViews(Node att) {
		...
		if (parent != null) {
			...
			// getting the value of the attribute specifying the xml path of the
			// view to go
	...
	protected String getUsedFileUniqueSavePath(String path) {
		...
		if (savePath != null && !savePath.equals("")) {
			...
			if (pos != -1) {
				...
				// getting the extension of the path and removing it from this
				// path
			...
			while (locationToXmlPath.containsKey(fileManager
					.getOutputDirectory() + "/" + savePath + extension)
					|| usedFilesPaths.containsValue(fileManager
							.getOutputDirectory() + "/" + savePath + extension)) {
			...
			savePath = fileManager.getOutputDirectory() + "/" + savePath
					+ extension;
		}
	...
	/**
	 * puts the references into their absolute form and replaces the old ids in
	 * the references by the new ids
	 */
			...
			for (Node att : attributesToBeChecked) {
				...
				if (att != null) {
					...
					if (value != null && !value.equals("")) {
						...
						// getting the collection of the references that can be
						// found in the current
						// attribute value
						...
						if (col != null && col.size() > 0) {
							...
							// all the found references will be modified by the
							// new ones
							for (String ref : col) {

								if (ref != null) {

									oldId = svgXmlPath + FileManager.separator
											+ ref;
									...
									if (oldIdToNewId.containsKey(oldId)) {
										...
										if (newRef != null
											...
											// replacing the old references by
											// the new ones
											if (att.getNodeName().equals(
													"xlink:href")) {
	 ...
	 * 
	 * @param parentSVGFile
	 *            the file into which the cloned element will be inserted
	...
	protected Element importElement(SVGFile parentSVGFile) {
		...
		if (parentSVGFile != null && parentSVGFile.exists() && doc != null
				&& parentSVGFile.getDocument() != null) {

			// cloning the root element of the current svg file
			returnElement = (Element) parentSVGFile.getDocument().importNode(
	...
	public void insertViewsAndWidgets() {
		...
		if (!hasBeenInserted && doc != null) {
			...
			// handles the insertion of all the included view files in this svg
			// file
			...
			// for each image node, a svg node is inserted and replaces the
			// image node
			for (Element cur : viewOrWidgetNodes) {
				...
				if (cur != null && cur.getParentNode() != null) {
					...
					if (type == VIEW) {
						...
						if (xmlPath != null
								&& includedViewSVGFilesPath.contains(xmlPath)) {
					...
					if (file != null) {
						...
						if (elementToBeInserted != null) {
							...
							if (type == VIEW) {
								...
								// adding all the referenced svg files to the
								// files to be saved of
								// this svg file
								filesToBeSaved.addAll(file.filesToBeSaved);
							}

							// modifying all the tag names that can be found in
							// the imported view
							// so that they are absolute in this svg file
							for (it2 = new NodeIterator(elementToBeInserted); it2
									.hasNext();) {
								...
								if (node != null && node instanceof Element) {
									...
									isRtdaElement = node.getNodeName()
											.startsWith(rtdaPrefix);

									// getting the tag handler for this element
									if (isRtdaElement) {

										// getting the current type of the
										// element
										...
										if (elementType == REGULAR) {
											...
											if (jwidgetId != null
												...
												currentHandler = fileManager
														.getTagHandlersManager()
														.getJWidgetTagHandler(
																jwidgetId);
											}
										}
									}

									// getting the list of the attributes of the
									// element
									...
									for (i = 0; i < attributes.getLength(); i++) {
										...
										// for each attribute that contains a
										// tag name
										if (att != null) {
											...
											if (type == VIEW
											...
											} else if (isRtdaElement
													&& currentHandler != null
													&& (currentHandler
															.isTagAttribute(
																	node.getNodeName(),
																	att.getNodeName()) || currentHandler
															.isViewAttribute(
																	node.getNodeName(),
																	att.getNodeName()))) {// TODO
												...
												if (currentHandler
													...
													// if the value not only
													// contains a tag name but
													// other information,
													// only the tag name is
													// stored
													value = RtdaTagHandler
															.getRealTagName(value);
												...
												// modifies the uri that can be
												// found in this
												// attribute value, ie, replaces
												// the path of the
												// imported file by the path of
												// this svg file
												if (att.getNodeName().equals(
														"xlink:href")
														&& value.equals(file
							...
							// setting all the attributes of the image to the
							// svg file
							...
							for (int j = 0; j < attributes.getLength(); j++) {
								...
								if (att != null
									...
									elementToBeInserted.setAttribute(
											att.getNodeName(),
											att.getNodeValue());
								}
							}

							group = elementToBeInserted
									.getOwnerDocument()
									.createElementNS(
											elementToBeInserted
													.getNamespaceURI(),
							...
							group.setAttributeNS(null, "transform",
									cur.getAttribute("transform"));
							group.appendChild(elementToBeInserted);

							// inserts all the child nodes of the view or the
							// widget but the
							...
							for (Element childNode : childNodes) {
								...
								if (!childNode
										.getNodeName()
										.equals(RtdaTagHandler.widgetDataBaseRootNodeName)) {
	...
	/**
	 * handles all the uris that can be found in this svg file after all linked
	 * views or widgets have been added, i.e., all the absolute uris are turned
	 * into relative uris.
	 */
				...
				if (att != null) {
					...
					if (value != null && !value.equals("")) {
						...
						if (col != null && col.size() > 0) {
							...
							// all the found references will be modified so that
							// their path is
							// relative to the current svg file
							for (String ref : col) {
								...
								if (ref != null && !ref.equals("")) {
									...
									if (ref.equals(currentPath)) {
										...
										// corresponding to this
										// reference in its saved form
										savePath = usedFilesPaths.get(ref);

										if (savePath != null) {

											// computing the new reference that
											// is relative to this
											// svg file save path
											newRef = getSaveRelativePath(savePath)
													+ fragment;

										} else {

											newRef = getSaveRelativePath(ref)
													+ fragment;
										}
									}
			...
			if (attributeValue != null && !attributeValue.equals("")) {
				...
				if (attribute.getNodeName().equals("xlink:href")) {
					...
					if (!str.startsWith("#")) {
						...
						// if the path is not absolute it is added turned into
					...
					// parsing the string so that all the referenced ids can be
					// found
	...
	/**
	 * creates and returns the list of the absolute references that are used in
	 * the given attribute value
	 * 
	 * @param attribute
	 *            an attribute with normalized references
		...
		if (attribute != null) {
			...
			if (attributeValue != null && !attributeValue.equals("")) {
				...
				if (attribute.getNodeName().equals("xlink:href")) {
					...
					// parsing the string so that all the references can be
					// found
					while (str.indexOf("url") != -1) {
	...
	/**
	 * creates and returns the list of the references that are used in the given
	 * attribute value and modifies the uri so that their format is the same
	 * 
	 * @param attribute
	 *            an attribute an attribute whith not normalized references
	 * @return the list of the ids that are used in the given attribute value
	 */
	protected Collection<String> normalizeAndGetReferences(Node attribute) {
		...
		if (attribute != null) {
			...
			if (attributeValue != null && !attributeValue.equals("")) {
				...
				String str = new String(attributeValue), newValue = new String(
						attributeValue);
						...
						if (pos != -1) {
							...
							if (reference != null && !reference.equals("")) {
								...
								if (reference.startsWith("#")) {
									...
									fragment = reference.substring(pos2,
											reference.length());
									reference = getAbsolutePath(reference
											.substring(0, pos2)) + fragment;
							...
							if (reference != null) {
								...
								// modifies the current reference in the value
								// string
								newValue = newValue.replaceAll("url[(]"
										+ lastReference + "[)]", "url("
										+ reference + ")");
							}
						}
	...
	protected String getAbsolutePath(String path) {
		...
		if (path != null) {
			...
			if (fileManager.isAbsolutePath(path)) {
				...
				// creating the path segment of the current svg file relatively
				// to the project name
				...
				// splitting the path and creating the list of its different
				// parts
				String[] splitPath = FileManager.getSplitPath(absolutePath);
				LinkedList<String> pathList = new LinkedList<String>();

				// adding each segment of the path to the list, taking into
				// account ".." segments,
				// i.e. : each ".." found segment implies that the previous
	...
	protected String getSaveRelativePath(String path) {
		...
		if (path != null) {
			...
			if (refPathSplit != null && imagePathSplit != null) {
				...
				// for each part of the reference path, tests if this part is
				// the same as the
				// corresponding part
				// of the image path
				for (i = 0; i < refPathSplit.length
						&& i < imagePathSplit.length; i++) {
		...
		if (node != null) {
			...
			if (node.getNodeName().equals("image") && node.hasAttribute("view")) {
				...
				type = JWIDGET;
					...
					(node.getParentNode() instanceof Element && isJWidgetAnimation((Element) node
							.getParentNode()))) {
				...
				// getting the list of the child elements of the node that are
				// jwidgets
				NodeList jwidgetNodes = node
						.getElementsByTagName(JWidgetRuntime.jwidgetTagName);
	...
	protected boolean isJWidgetAnimation(Element element) {
		...
		if (element != null && element.getNodeName().startsWith(rtdaPrefix)) {
			...
			if (!parentElement.getNodeName().equals("svg")) {
				...
				// getting the list of the jwidget elements that are siblings of
				// the given element
				NodeList jwidgetElements = parentElement
						.getElementsByTagName(JWidgetRuntime.jwidgetTagName);
	...
	/**
	 * returns the name of the jwidget to which the given element that is an
	 * animation or action or a child of them belongs
	 * 
	 * @param element
	 *            a jwidget animation or action or animation child or action
	 *            child
	 * @return the name of the jwidget to which the given element that is an
	 *         animation or action or a child of them belongs
	 */
	protected String getJWidgetName(Element element) {
		...
		if (element.getParentNode() != null
			...
			NodeList jwidgetElements = parentNode
					.getElementsByTagName(JWidgetRuntime.jwidgetTagName);
			...
			if (jwidgetElements != null && jwidgetElements.getLength() > 0) {
				...
				if (parentNode.getParentNode() != null
						&& parentNode.getParentNode() instanceof Element) {

					// getting the parent node of the parent node
					Element superParentNode = (Element) parentNode
							.getParentNode();

					if (superParentNode != null) {

						jwidgetElements = superParentNode
								.getElementsByTagName(JWidgetRuntime.jwidgetTagName);
	...
	/**
	 * creates and return the set of the ids of the colors that can be found in
	 * the given string
	 * 
	 * @param value
	 *            a string
	 * @return the set of the ids of the colors that can be found in the given
	 *         string
	 */
	protected Set<String> getAllColorIds(String value) {
		...
		if (value != null && !value.equals("")) {
			...
			int pos = str.indexOf(ColorsBlinkingsFile.start), pos2 = str
					.indexOf(ColorsBlinkingsFile.end);
	...
	/**
	 * creates and returns a unique jwidget id in the application given a base
	 * string
	 * 
	 * @param baseString
	 *            the base string for the id
	...
	public static String getStyleProperty(Element element, String name) {
		...
		if (element != null && name != null && !name.equals("")) {
			...
			if (!styleValue.equals("") && rg != -1) {
				...
				value = styleValue.substring(rg + name.length() + 1,
						styleValue.length());
				rg = value.indexOf(";");
		...
		if (element != null && name != null && !name.equals("")) {
			...
			for (int i = 0; i < splitValues.length; i++) {
				...
				if (splitValues[i] != null && !splitValues[i].equals("")) {
					...
					svalue = splitValues[i].substring(
							pos + nameToValueSep.length(),
