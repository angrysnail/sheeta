...
public class SVGWidgetFile extends SVGFile {
	...
	/**
	 * the map associating a widget tag name to the object containing
	 * information on the widget database
	 */
	...
	public SVGWidgetFile(FileManager fileManager, String workspacePath,
		...
		// creating the objects associating a tag name to the objects containing
		// information on each
		// node
		Node cur = null;
		...
		if (widgetDataBaseRoot != null) {
			...
			for (NodeIterator it = new NodeIterator(widgetDataBaseRoot); it
					.hasNext();) {
	...
	protected void translateTags() {
		...
		Element el = null;
		String nodeName = "", attributeName = "", attributeValue = "", viewValue = "", associatedChildAttributeName = "", referencedTagName = "", tagAttributeNodeName = "", tagAttributeName = "", referencedTagAttribute = "", newValue = "", childNodeName = "", childAttributeName = "";
		...
		// the set of the animation nodes that will be found during the first
		// iteration
		...
		for (NodeIterator it = new NodeIterator(doc.getDocumentElement()); it
			...
			if (cur != null && cur instanceof Element
				...
				if (type == JWIDGET_ANIMATION) {
					...
					// getting the id of the jwidget corresponding to this
					// element
					...
					if (jwidgetId != null && !jwidgetId.equals("")) {
						...
						tagHandler = fileManager.getTagHandlersManager()
				...
				if (tagHandler != null) {
					...
					for (i = 0; i < attributes.getLength(); i++) {
						...
						if (tagHandler.isEnumeratedTagValueReferenceAttribute(
								nodeName, attributeName)) {

							// getting the tag name corresponding to the
							// referenced tag name of this
							// attribute
							referencedTagAttribute = tagHandler
									.getEnumeratedTagValueReferenceAttribute(
											nodeName, attributeName);

							if (referencedTagAttribute != null
									&& referencedTagAttribute
											.indexOf(RtdaTagHandler.separator) != -1) {

								// getting the name of the element containing
								// the tag attribute
								pos = referencedTagAttribute
										.indexOf(RtdaTagHandler.separator);
								tagAttributeNodeName = referencedTagAttribute
										.substring(0, pos);

								// getting the name of the attribute containing
								// the tag itself
								tagAttributeName = referencedTagAttribute
										.substring(
												pos
														+ RtdaTagHandler.separator
																.length(),
												referencedTagAttribute.length());

								// getting the value of the tag attribute
								if (nodeName.equals(tagAttributeNodeName)) {

									// the tag attribute is an attribute of the
									// current node
									referencedTagName = ((Element) cur)
											.getAttribute(tagAttributeName);
								...
								// getting the object containing information on
								// the found tag name
								if (referencedTagName != null
										&& !referencedTagName.equals("")) {

									tagObject = widgetDataBase
											.get(referencedTagName);

									if (tagObject != null) {

										// getting the map of the values of the
										// enumerated tag
										values = tagObject
												.getWidgetToViewValues();

										// modifying the value of the attribute
										if (attributeValue.indexOf("|") == -1) {

											// the attribute only contains one
											...
											// the attribute contains a list of
											// the values of the
											// enumerated tag
											splitValues = attributeValue
													.split("|");
											newValue = "";
		...
		for (Node node : animationNodes) {
			...
			if (type == JWIDGET_ANIMATION) {
				...
				tagHandler = fileManager.getTagHandlersManager().getTagHandler(
						(Element) node);
			}
				...
				for (i = 0; i < attributes.getLength(); i++) {
					...
					if (tagHandler.isTagAttribute(nodeName, attributeName)) {
						...
						if (tagObject != null) {
							...
							// if the tag is an enumerated tag, the values of
							// this tag, that can be
							// found in the child nodes
							// of the animation node, are replaces by the view
							// tag values
							if (tagObject.getWidgetToViewValues().size() > 0) {

								// getting the name of the attribute of the
								// child nodes that
								// contains one of the values of the enumerated
								// tag
								associatedChildAttributeName = tagHandler
										.getEnumeratedTagValueAttributeName(
												node.getNodeName(),
												attributeName);

								if (associatedChildAttributeName != null
										&& !associatedChildAttributeName
												.equals("")) {

									// getting the node name of the child nodes
									// of the animation
									// node and the name of the attribute
									// containing a value of the current tag
									pos = associatedChildAttributeName
											.indexOf(RtdaTagHandler.separator);
									childNodeName = associatedChildAttributeName
											.substring(0, pos);
									childAttributeName = associatedChildAttributeName
											.substring(
													pos
															+ RtdaTagHandler.separator
																	.length(),
									...
									for (child = node.getFirstChild(); child != null; child = child
										...
										if (child instanceof Element
											...
											// checking all the attributes of
											// the child node, to get
											// the attribute
											// that use one of the values of the
											// previously computed
											// tag
											childAttributes = child
													.getAttributes();

											for (j = 0; j < childAttributes
													.getLength(); j++) {
												...
												if (childAttribute != null
													...
													if (viewValue != null) {
														...
														// setting the view tag
														// value for the
														// attribute
														childAttribute
																.setNodeValue(viewValue);
													}
	...
	protected Element importElement(SVGFile parentSVGFile) {
		...
		if (returnElement != null) {
			...
			// getting the widget database nodes that can be found in the
			// element
			NodeList nodes = returnElement
	...
	/**
	 * the class containing information on the widget database, and enabling to
	 * make the correspondance between the widget tag names and the view tag
	 * names
	 * 
	 * @author ITRIS, Jordi SUC
	 */
	protected class TagObject {
		 ...
		 */
		private LinkedHashMap<String, String> widgetValuesToViewValues = new LinkedHashMap<String, String>();
		...
		protected TagObject(Element tagElement) {
			...
			if (tagElement.getNodeName().indexOf("tag.enumerated") != -1) {
				...
				// getting the view and widget values, as the tag is an
				// enumerated one
				String widgetValue = "", viewValue = "";
				Element el = null;

				for (Node cur = tagElement.getFirstChild(); cur != null; cur = cur
						.getNextSibling()) {
					...
					if (cur instanceof Element) {
						...
						if (widgetValue != null && !widgetValue.equals("")) {
							...
							widgetValuesToViewValues
									.put(widgetValue, viewValue);
						}
					}
				}
			}
		...
		/**
		 * computes and returns the absolute tag names for the widget and the
		 * view
		 * 
		 * @param tagElement
		 *            a node of the widget database corresponding to a tag
		...
		protected String[] getAbsoluteTagPaths(Element tagElement) {
			...
			if (tagElement != null) {
				...
				String widgetTagPath = RtdaTagHandler.normalizePath(tagElement
						.getAttribute("id")), viewTagPath = RtdaTagHandler
						.normalizePath(tagElement.getAttribute("value"));
				...
				while (parent != null
					...
					widgetTagPath = RtdaTagHandler
							.normalizePath(((Element) parent)
									.getAttribute("id"))
							+ "/" + widgetTagPath;
					viewTagPath = RtdaTagHandler
							.normalizePath(((Element) parent)
									.getAttribute("value"))
							+ "/" + viewTagPath;
		...
		/**
		 * @return the map associating the value for a widget to the value for a
		 *         view
		 */
