...
public class XMLFile {
	...
	/**
	 * the map of the files specified in each inherit attribute, associating the
	 * value of the attribute to its related xml file
	 */
	private static final Map<String, XMLFile> inheritedFilesMap = new HashMap<String, XMLFile>();

	/**
	 * the map associating the name of a database file (when it is saved) to the
	 * source data base file
	 */
	private static final Map<String, File> dataBaseFilesMap = new HashMap<String, File>();

	/**
	 * the set of the path of the files that are specified in the "inherit"
	 * attributes
	 */
	 ...
	 *            the name of the xml file
	 */
	public XMLFile(FileManager fileManager, String workspacePath,
		...
		try {
			// creating the file
			xmlFile = new File(new URI(workspacePath + "/" + projectName + "/"
					+ name));
	 ...
	 * associating the value of the attribute to its related xml file. Also
	 * fills the map of the data base files specified in each inherited xml file
	 * 
	...
	public void initialize() throws MissingFileException {
		...
		if (doc != null) {
			...
			// for each node in the given document, checks if it has an
			...
			do {
				...
				if (cur != null && cur instanceof Element) {
					...
					if (el.getNodeName().equals("view")) {
						...
						// setting the project name associated with the svg file
						// that is referenced
						// in this view node
						...
						if (!dataBaseFileName.equals("")) {
							...
							if (!savedDataBaseFileName.equals("")) {
								...
								// setting the new value for the database file
								// name attribute
								el.setAttribute("description",
										savedDataBaseFileName);

								// getting the data base file name
								String parentXMLFilePath = xmlFile.toURI()
										.toASCIIString();
								...
								if (pos != -1) {
									...
									parentXMLFilePath = parentXMLFilePath
											.substring(0, pos);
								}
								...
								if (dataBaseFileName.startsWith("/")) {
									...
									dataBaseFileName = dataBaseFileName
											.substring(1,
													dataBaseFileName.length());
								}

								String absoluteDataBaseFileName = parentXMLFilePath
										+ dataBaseFileName;
								...
								try {
									dataBaseFile = new File(new URI(
											absoluteDataBaseFileName));
								} catch (Exception ex) {
								}

								if (dataBaseFile != null) {

									dataBaseFilesMap.put(savedDataBaseFileName,
											dataBaseFile);
					...
					if (el.hasAttribute("inherit")) {
						...
						// getting the file name specified in the "inherit"
						// attribute
						fileName = el.getAttribute("inherit");

						// adding the filename to the set of the file names of
						// the inherited files
						...
						if (!inheritedFilesMap.containsKey(fileName)) {
							...
							// getting the xml file associated with this file
							// name
							file = getXMLFile(fileName);

							if (file != null) {

								// putting the file name and the file into the
								// map
								inheritedFilesMap.put(fileName, file);
							}
						}

						// adding the element to the list of the elements that
						// have an "inherit"
						// attribute
	...
	/**
	 * creates and returns the single xml file that is the union of all the
	 * linked xml files
	 * 
	 * @param destinationDirectory
	 *            the directory into which the xml file will be written
	 ...
	 *             node
	 */
	public CompiledXMLFile compile(String destinationDirectory)
			throws InvalidInheritedNodeException {
		...
		if (destinationDirectory != null && !destinationDirectory.equals("")) {
			...
			// creating the list containing the file name of each previously
			// handled files
			LinkedList<String> trajectory = new LinkedList<String>();
			trajectory.add(name);

			// handles the "inherit" attributes so that they are replaced by the
			// accurate subtree
			// and values for each
			...
			compiledFile = new CompiledXMLFile(fileManager, savedPath, doc,
					fileManager.getRootViewPath(), new HashMap<String, File>(
							dataBaseFilesMap));
		}
	 ...
	 * 
	 * @param fileName
	 *            the path of the xml file relatively to the project file
	...
	protected XMLFile getXMLFile(String fileName) throws MissingFileException {
		...
		if (fileName != null && !fileName.equals("")
			...
			String currentProjectName = fileManager.getProjectName(projectName,
					fileName);

			if (currentProjectName != null && !currentProjectName.equals("")) {

				// creating the xml file
				returnFile = new XMLFile(fileManager, workspacePath,
	 ...
	 * 
	 * @param trajectory
	 *            the list of the xml files that have been already parsed
	 * @throws InvalidInheritedNodeException
	 *             the exception raised if there is a cycle in the inheritance
	 *             node
	 */
			...
			for (String fileName : inheritedFileNames) {
				...
				if (fileName != null && !fileName.equals("") && file != null
					...
					if (!trajectory.contains(fileName)) {
						...
						// if the file has not already been handled, the file
						...
						// replaces the "inherit" attribute contained by each
						// node by the accurate
						// sub tree
						file.replaceInheritAttributes();

					} else {

						// if the file has already been handled, an exception is
						// raised
			...
			for (Element cur : elementWithInheritAttribute) {
				...
				if (cur != null) {
					...
					if (file != null && file.getRootElement() != null) {
						...
						// importing the root element of this xml file into the
						// document of the
						// current xml file
						inheritRoot = (Element) doc.importNode(
								file.getRootElement(), true);
	 ...
	 * 
	 * @param element
	 *            the subtree that inherits the inherited tree
	...
	protected void mergeSubTree(Element element, Element inheritedElement) {
		...
		// building the map associating the id of a child of the given element
		// to this node
		...
		LinkedList<Element> children = Toolkit.getLinkedList(inheritedElement
				.getChildNodes());
	...
	protected void mergeAttributes(Element element, Element inheritedElement) {
		...
		for (int i = 0; i < inheritedAttributes.getLength(); i++) {
			...
			// if the attribute is not the id attribute, and the attribute is
			// not already specified
			// in the first element,
			// the attribute is added
			if (!cur.getNodeName().equals("id")
					&& !element.hasAttribute(cur.getNodeName())) {
	...
	/**
	 * returns the unique data base file name corresponding to the given file
	 * name
	 * 
	 * @param fileName
	 *            the name of a data base file
	 * @return the unique data base file name corresponding to the given file
	 *         name
	 */
	protected String getUniqueDataBaseFileName(String fileName) {
		...
		if (fileName != null && !fileName.equals("")) {
			...
			if (dataBaseFilesMap.containsKey(fileName)) {
				...
				while (dataBaseFilesMap.containsKey(root + i
						+ FileManager.DATA_BASE_FILE_EXTENSION)) {

					i++;
				}

				uniqueFileName = root + i
						+ FileManager.DATA_BASE_FILE_EXTENSION;
