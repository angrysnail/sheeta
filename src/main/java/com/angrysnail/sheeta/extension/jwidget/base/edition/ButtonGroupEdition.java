...
public abstract class ButtonGroupEdition extends JWidgetEdition {
	 ...
	 */
	public ButtonGroupEdition(JWidgetManager jwidgetManager, Frame mainFrame,
	...
	protected String getLabel(String baseString, Element element) {
		...
		if (baseString != null) {
			...
			for (Node subWidget = element.getFirstChild(); subWidget != null; subWidget = subWidget
					.getNextSibling()) {

				if (subWidget instanceof Element) {

					labels.add(getProperty(
							(Element) subWidget,
	...
	protected class ExtendedJWidgetConfigurationPanel
		...
		public void initializePanel() {
			...
			if (getElement() != null) {
				...
				String orientationValue = getProperty(getElement(),
				...
				if (buttonsChooserTable.getRowCount() > 0) {
					...
					buttonsChooserTable.getSelectionModel()
				...
				if (mainType == TOGGLE_BUTTON_BAR) {
					...
					isOpaqueChooser
							.removeActionListener(isOpaqueChooserListener);
					isOpaqueChooser.setSelected(Boolean
		...
		public void buildPanel() {
			...
			// getting the labels
			String orientationLabel = "", horizontalLabel = "", verticalLabel = "", titleBorderLabel = "", lookLabel = "";
			...
			buttonsListener = new ActionListener() {
				...
				public void actionPerformed(ActionEvent evt) {
					...
					if (evt.getSource().equals(newButton)) {
						...
						tableModel.removeButton(buttonsChooserTable
								.getSelectedRow());
			...
			buttonsChooserTable.getSelectionModel().addListSelectionListener(
			...
			comboBoxListener = new ActionListener() {
				...
				public void actionPerformed(ActionEvent evt) {
					...
					if (item != null) {
						...
						setProperty(getElement(), propertiesList.get(0), item
			...
			if (mainType == TOGGLE_BUTTON_BAR) {
				...
				isOpaqueChooser = new JCheckBox(
						bundle.getString("isOpaqueLabel"));
				...
				isOpaqueChooserListener = new ActionListener() {
					...
					public void actionPerformed(ActionEvent e) {
						...
						setProperty(getElement(), propertiesList.get(6),
								Boolean.toString(isOpaqueChooser.isSelected()),
								true);
					}
				};
				...
				/*
				 * sameSizeForButtonsChooser=new
				 * JCheckBox(bundle.getString("sameSizeForButtonsLabel"));
				 * 
				 ...
				 * Boolean.toString(sameSizeForButtonsChooser.isSelected()),
				 * true); } };
				 */
			}
			...
			fontStyleListener = new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					for (int i = 0; i < newValues.length; i++) {
						...
						setProperty(getElement(), propertiesList.get(i + 1),
								newValues[i], true);
					}
				}
			};
		 ...
		 * 
		 * @param subJWidgetElement
		 *            a sub widget element
		 * @param newLabel
		...
		protected class ButtonsChooserTableModel extends AbstractTableModel {
			...
			protected void removeButton(int pos) {
				...
				if (pos >= 0 && pos <= buttonJwidgetElements.size()) {
					...
					removeSubWidgetElement(tableJwidgetElement,
							buttonJwidgetElements.get(pos), true);
			...
			protected void putUp() {
				...
				if (row > 0 && row < buttonJwidgetElements.size()) {
					...
					ButtonGroupEdition.this.putUp(
							buttonJwidgetElements.get(row), true);
					setCurrentJWidgetElement(tableJwidgetElement);
					table.getSelectionModel().setSelectionInterval(row - 1,
							row - 1);
				}
			}
			...
			protected void putDown() {
				...
				if (row >= 0 && row < buttonJwidgetElements.size()) {
					...
					ButtonGroupEdition.this.putDown(
							buttonJwidgetElements.get(row), true);
					setCurrentJWidgetElement(tableJwidgetElement);
					table.getSelectionModel().setSelectionInterval(row + 1,
							row + 1);
			...
			public void setValueAt(Object value, int row, int col) {
				...
				if (col == 0 && tableJwidgetElement != null && row >= 0
					...
					setButtonLabel(buttonJwidgetElements.get(row),
							value.toString());
				}
			}
		 ...
		 * @author ITRIS, Jordi SUC
		 */
		protected class ButtonChooserTableCellEditor extends AbstractCellEditor
				implements
					TableCellEditor {
			...
			public Component getTableCellEditorComponent(JTable table,
					Object value, boolean selected, int row, int col) {
	 ...
	 * @author ITRIS, Jordi SUC
	 */
	protected class ButtonGroupSourceChooser
			extends
		...
		protected void buildWidget() {
			...
			comboListener = new ActionListener() {
				...
				public void actionPerformed(ActionEvent evt) {
					...
					ComboListItem item = (ComboListItem) combo
							.getSelectedItem();
		...
		protected void updateWidgets() {
			...
			if (getJwidgetElement() != null) {
				...
				String currentSource = RtdaAnimationsAndActionsModule
						.getStateRecord().getSourceName();

				for (Node node = getJwidgetElement().getFirstChild(); node != null; node = node
						.getNextSibling()) {
					...
					if (node instanceof Element) {
						...
						if (value != null && !value.equals("")) {
							...
							// creating the combo item and adding it to the
							// combo box
				...
				if (currentSource.equals("")) {
					...
					if (combo.getItemCount() > 0) {
						...
						setSource(((ComboListItem) combo.getItemAt(0))
								.getValue().toString());
