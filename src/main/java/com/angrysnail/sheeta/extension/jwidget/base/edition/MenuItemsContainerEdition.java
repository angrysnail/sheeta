package com.angrysnail.sheeta.extension.jwidget.base.edition;
...
import com.angrysnail.sheeta.library.widgets.FontFamilyChooserWidget;
import com.angrysnail.sheeta.rtda.toolkit.TagToolkit;
import com.angrysnail.sheeta.rtdaeditor.anim.components.RtdaAnimationsAndActionsModule;
import com.angrysnail.sheeta.rtdaeditor.jwidget.AnimationChooserJWidgetSourceChooser;
import com.angrysnail.sheeta.rtdaeditor.jwidget.JWidgetConfigurationPanel;
...
import com.angrysnail.sheeta.rtdaeditor.widget.FontStyleChooser;
import com.angrysnail.sheeta.svgeditor.Editor;
import com.angrysnail.sheeta.svgeditor.display.canvas.dom.SVGDOMListener;
...
public abstract class MenuItemsContainerEdition extends JWidgetEdition {
	 ...
	 */
	protected static ImageIcon menuBarIcon, popUpMenuIcon, menuIcon,
			menuItemIcon;

	static {

		// loading the icons
		ImageIcon[] icons = getIcons(MenuItemsContainerEdition.class,
				"MenuBarWidget");
	 ...
	 *            the main frame
	 * @param mainType
	 *            the type of the jwidget
	 * @param id
	 *            the id of the jwidget type
	 * @param position
	 *            the position of the toggle button of the jwidget
	 */
	...
	public Element createJWidgetElement(Element parentElement) {
		...
		jwidgetElement.setAttribute(
				com.angrysnail.sheeta.library.Toolkit.labelAttribute,
				description);
	...
	protected class ExtendedJWidgetConfigurationPanel
		...
		public void buildPanel() {
			...
			try {
				configurationLabel = bundle
						.getString("configurationPanelLabel");
			...
			MenuItemTreeCellRenderer renderer = new MenuItemTreeCellRenderer(
					tree);
			tree.setCellRenderer(renderer);

			// the tree cell editor
			MenuItemTreeCellEditor editor = new MenuItemTreeCellEditor(tree,
					renderer);
				...
				public void actionPerformed(ActionEvent evt) {
					...
					if (mousePosition != null) {
						...
						if (path != null) {
							...
							MenuItemTreeNode treeNode = (MenuItemTreeNode) path
						...
						popMenu.show(newButton, mousePosition.x,
								mousePosition.y);
					}
				}
			});
				...
				public void actionPerformed(ActionEvent evt) {
					...
					if (path != null) {
						...
						if (treeNode.getParent() != null) {
							...
							((MenuItemTreeNode) treeNode.getParent())
			...
			treeSelectionListener = new TreeSelectionListener() {
				...
				public void valueChanged(TreeSelectionEvent evt) {
					...
					if (evt.getPath() != null) {
						...
						treeNode = (MenuItemTreeNode) evt.getPath()
								.getLastPathComponent();
					}
			...
			fontStyleListener = new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					for (int i = 0; i < newValues.length; i++) {
						...
						setProperty(getElement(), propertiesList.get(i),
								newValues[i], true);
					}
				}
			};
		...
		protected void buildPopUpMenu() {
			...
			tree.addTreeSelectionListener(new TreeSelectionListener() {
				...
				public void valueChanged(TreeSelectionEvent evt) {
					...
					if (selectedTreeNode != null
							&& (selectedTreeNode.getType() == MENU
									|| selectedTreeNode.getType() == MENU_BAR || selectedTreeNode
									.getType() == POP_UP)) {
		...
		protected MenuItemTreeNode getSelectedTreeNode() {
			...
			if (tree.getSelectionPath() != null) {
				...
				treeNode = (MenuItemTreeNode) tree.getSelectionPath()
						.getLastPathComponent();
	...
	protected class MenuItemTreeNode extends DefaultMutableTreeNode {
		 ...
		 *            the jwidget element
		 */
		public MenuItemTreeNode(JTree nodeTree, MenuItemTreeModel treeModel,
				Element element) {
			...
			// adding a listener to the element so that the tree node can be
			// notified when the
			// element changes
			domListener = new SVGDOMListener(currentElement) {
				...
				public void nodeChanged() {
					...
					// checking if the given node equals this tree path element,
					// and notifies the
					// listener that this node
					// has been modified
					if (getNode() != null && getNode() instanceof Element
							&& getNode().equals(currentElement)) {

						model.nodeChanged(MenuItemTreeNode.this);
						currentTree.setSelectionPath(MenuItemTreeNode.this
								.getTreePath());
					}
				}
				...
				public void nodeInserted(Node insertedNode) {
					...
					// checking if the inserted node has not already been
					// inserted
					if (!childElements.contains(insertedNode)) {

						// creating and adding the new tree node
						MenuItemTreeNode childTreeNode = new MenuItemTreeNode(
								currentTree, model, (Element) insertedNode);
						model.insertNodeInto(childTreeNode,
								MenuItemTreeNode.this, getChildCount());
						currentTree.setSelectionPath(MenuItemTreeNode.this
								.getTreePath());
					...
					if (removedNode != null) {
						...
						if (childTreeNode != null) {
							...
							if (getChildCount() > 0) {
								...
								currentTree.scrollPathToVisible(firstChild
										.getTreePath());
								currentTree.setSelectionPath(firstChild
										.getTreePath());
				...
				public void structureChanged(final Node lastModifiedNode) {
					...
					// removing all the child node of this tree node and
					// creating them
					createChildren();
					...
					if (treeNode != null) {
						...
						currentTree.getSelectionModel().setSelectionPath(
								treeNode.getTreePath());
				...
				if (currentElement != null && currentElement.hasChildNodes()) {
					...
					for (int i = 0; i < childrenNodes.getLength(); i++) {
						...
						if (child != null && child instanceof Element) {
							...
							childTreeNode = new MenuItemTreeNode(currentTree,
									model, (Element) child);
							add(childTreeNode);
						}
					}
		...
		public void add(MutableTreeNode newTreeNode) {
			...
			if (newTreeNode != null && newTreeNode instanceof MenuItemTreeNode) {
				...
				childElements.add(((MenuItemTreeNode) newTreeNode)
						.getJWidgetElement());
			}
		}
		...
		public void remove(MutableTreeNode oldTreeNode) {
			...
			if (oldTreeNode instanceof MenuItemTreeNode) {
				...
				childElements.remove(((MenuItemTreeNode) oldTreeNode)
						.getJWidgetElement());
			}
		}
		...
		protected MenuItemTreeNode getTreeNode(Element childElement) {
			...
			if (childElement != null) {
				...
				for (Iterator<?> it = children.iterator(); it.hasNext();) {
					...
					if (child != null
							&& child.getJWidgetElement().equals(childElement)) {
		...
		public String getNodeId() {
			...
			return currentElement
					.getAttribute(com.angrysnail.sheeta.library.Toolkit.idAttribute);
		}
		 ...
		 *            the parent element
		 * @param element
		 *            the element from which siblings should be found not
		 * @return the set of the labels of the jwidget element
		 */
		protected HashSet<String> getSiblingLabels(Element parentElement,
				Element element) {

			// creating the set of the labels of the siblings of the current
			// node
			...
			for (Node node = parentElement.getFirstChild(); node != null; node = node
				...
				if (node instanceof Element
						&& (element == null || !node.equals(element))) {

					currentLabel = ((Element) node)
							.getAttribute(com.angrysnail.sheeta.library.Toolkit.labelAttribute);
		...
		public boolean isDuplicatedLabel0(String foundLabel) {
			...
			if (foundLabel != null && !foundLabel.equals("") && !isRoot()) {
				...
				return getSiblingLabels(
						(Element) getJWidgetElement().getParentNode(),
						getJWidgetElement()).contains(foundLabel);
			}
		...
		/**
		 * creates and returns a unique label in the application given a base
		 * string
		 * 
		 * @param baseString
		 *            the base string for the label
		...
		protected String getLabel(String baseString) {
			...
			if (baseString != null) {
				...
				// getting the set of the ids of the siblings of the current
				// node
				HashSet<String> siblingLabels = getSiblingLabels(
						getJWidgetElement(), null);
		...
		public void removeChildNode(MenuItemTreeNode child) {
			...
			if (topLevelElement != null) {
				...
				removeSubWidgetElement(currentElement,
						child.getJWidgetElement(),
		...
		protected void insertChildNode(int nodeType) {
			...
			if (topLevelElement != null) {
				...
				createSubWidgetElement(currentElement, name, widgetLabel,
		...
		protected void putUp() {
			...
			if (topLevelElement != null) {
				...
				MenuItemsContainerEdition.this.putUp(currentElement,
						topLevelElement.equals(currentElement));
			}
		}
		...
		protected void putDown() {
			...
			if (topLevelElement != null) {
				...
				MenuItemsContainerEdition.this.putDown(currentElement,
						topLevelElement.equals(currentElement));
			}
		}
	 ...
	 * @author ITRIS, Jordi SUC
	 */
	protected class MenuItemTreeModel extends DefaultTreeModel
			implements
				TreeModel {
		...
		public void insertNodeInto(MutableTreeNode newChild,
			...
			if (parent != null && parent instanceof MenuItemTreeNode
				...
				((MenuItemTreeNode) parent).getChildElements().add(
						((MenuItemTreeNode) newChild).getJWidgetElement());
			}
		}
		...
		public void removeNodeFromParent(MutableTreeNode childNode) {
			...
			if (childNode != null && childNode instanceof MenuItemTreeNode) {
				...
				((MenuItemTreeNode) childNode.getParent()).getChildElements()
						.remove(((MenuItemTreeNode) childNode)
								.getJWidgetElement());
			}
		...
		public void valueForPathChanged(TreePath path, Object newValue) {
			...
			if (path != null && newValue != null && newValue instanceof String) {
				...
				MenuItemTreeNode node = (MenuItemTreeNode) path
						.getLastPathComponent();
				...
				if (node != null) {
					 ...
					 * 
					 * }else if(newValue.toString()==null ||
		...
		@Override
		public void fireTreeStructureChanged(Object arg0, Object[] arg1,
		...
		protected void showDuplicatedIdDialog() {
			...
			if (editorBundle != null) {
				...
				try {
					...
					duplicatedIdLabel = editorBundle
							.getString("rtdaanim_duplicatedId");
				} catch (Exception ex) {
				}
			}

			JOptionPane.showMessageDialog(Editor.getParent(),
					duplicatedIdLabel, errorLabel, JOptionPane.ERROR_MESSAGE);
		}
		...
		protected void showMalformedIdDialog() {
			...
			if (editorBundle != null) {
				...
				try {
					errorLabel = editorBundle.getString("labelerror");
					malformedIdLabel = editorBundle
							.getString("rtdaanim_malformedId");
				} catch (Exception ex) {
				}
			}

			JOptionPane.showMessageDialog(Editor.getParent(), malformedIdLabel,
	...
	public class MenuItemTreeCellRenderer extends DefaultTreeCellRenderer {
		...
		public MenuItemTreeCellRenderer(JTree tree) {
			...
			backgroundNonSelectionColor = UIManager
					.getColor("Tree.textBackground");
			borderSelectionColor = UIManager
	 ...
	 * @author ITRIS, Jordi SUC
	 */
	public class MenuItemTreeCellEditor extends AbstractCellEditor
			implements
				TreeCellEditor {
		 ...
		 */
		public MenuItemTreeCellEditor(JTree widgetTree,
				MenuItemTreeCellRenderer renderer) {
			...
			labelAndTextField.setLayout(new BorderLayout(renderer
					.getIconTextGap(), 0));
		...
		public boolean isCellEditable(EventObject event) {
			...
			if (event instanceof MouseEvent) {
				...
				if (SwingUtilities.isLeftMouseButton(evt)
					...
					TreePath hitPath = widgetTree.getPathForLocation(
							evt.getX(), evt.getY());
		 ...
		 */
		@Override
		public Component getTreeCellEditorComponent(JTree theTree,
	...
	protected class TreeAnimationChooserJWidgetSourceChooser
			extends
				AnimationChooserJWidgetSourceChooser {
			...
			treeSelectionListener = new TreeSelectionListener() {
				...
				public void valueChanged(TreeSelectionEvent evt) {
					...
					if (evt.getPath() != null) {
						...
						MenuItemTreeNode selectedNode = (MenuItemTreeNode) evt
								.getPath().getLastPathComponent();

						// getting the source id of the selected node
						String sourceId = getSourceId(selectedNode
								.getJWidgetElement());
		...
		protected String getSourceId(Element node) {
			...
			if (node != null) {
				...
				while (com.angrysnail.sheeta.library.Toolkit
						.isJWidgetElement(element)) {

					sourceId = sourceSeparator
							+ element
									.getAttribute(com.angrysnail.sheeta.library.Toolkit.idAttribute)
							+ sourceId;
		 ...
		 */
		protected TreePath getTreePathFromSourceId(MenuItemTreeNode rootNode,
				String sourceId) {
		...
		/**
		 * returns the tree node corresponding to the given id and that is a
		 ...
		 * @return the tree node corresponding to the given id and that is a
		 *         child of the given parent node
		 */
		protected MenuItemTreeNode getTreeNodeFromId(MenuItemTreeNode treeNode,
				String elementId) {

			try {
				for (MenuItemTreeNode node = (MenuItemTreeNode) treeNode
						.getFirstChild(); node != null; node = (MenuItemTreeNode) node
						.getNextSibling()) {
		...
		protected void updateWidgets() {
			...
			if (getJwidgetElement() != null) {
				...
				if (RtdaAnimationsAndActionsModule.getStateRecord()
						.getSourceName().equals("")) {
					...
					String source = RtdaAnimationsAndActionsModule
							.getStateRecord().getSourceName();
					setSource(source);

					// getting the tree path corresponding to the source id
					TreePath treePath = getTreePathFromSourceId(
							(MenuItemTreeNode) treeModel.getRoot(), source);
