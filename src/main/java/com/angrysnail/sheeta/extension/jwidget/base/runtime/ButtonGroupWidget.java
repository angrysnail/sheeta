...
public class ButtonGroupWidget extends JToolBar {
	 ...
	 */
	private LinkedHashMap<AbstractButton, ActionListener> buttonsToListeners = new LinkedHashMap<AbstractButton, ActionListener>();

	/**
	 * the map associating the source id of a button to
	 */
	private Map<String, AbstractButton> sourceNameToButtonMap = new HashMap<String, AbstractButton>();
	...
	protected void initialize(Element jwidgetElement) {
		...
		boolean isToolBar = jwidgetElement.getAttribute(
				...
				"ToggleButtonBarWidget");

		// creating the buttons
		NodeList jwidgetElements = jwidgetElement
		...
		for (int i = 0; i < jwidgetElements.getLength(); i++) {
			...
			if (el != null) {
				...
				label = el
						.getAttribute(JWidgetRuntime.jwidgetLabelAttributeName);
				sourceName = el
		 ...
		 * if(isToolBar){
		 * 
		 * //checking whether all the buttons should have the same size boolean
		 * sameSizeForButtons=false;
		 * 
		 ...
		 * 
		 * setLayout(new GridLayout(buttonsToListeners.size(), 1)); break;
		 * 
		 * case SwingConstants.VERTICAL :
		 * 
		 * setLayout(new GridLayout(1, buttonsToListeners.size())); break; }
		 * 
		 * }else{
		 * 
		 * //setting the properties of the component setLayout(new
		 * BoxLayout(this, (orientation==SwingConstants.HORIZONTAL)?
	 ...
	 * @param enable
	 *            whether the button denoted by the given id should be enabled
	 *            or not
	 */
