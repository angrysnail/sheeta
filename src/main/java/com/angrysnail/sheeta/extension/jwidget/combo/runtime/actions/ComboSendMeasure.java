...
public class ComboSendMeasure extends AbstractSendMeasure {
	 ...
	 *            jwidget
	 */
	public ComboSendMeasure(SVGPicture picture, String projectName,
			...
			for (Node child = actionElement.getFirstChild(); child != null; child = child
				...
				if (child instanceof Element) {
					...
					if (childElement.getAttribute(usedAttName).equals(
						...
						tagToWriteValues.add(AnimationsToolkit.getNumber(label,
								tagType));
					}
				}
			}
	...
	public void performAction(Object evt) {
		...
		if (isEntitled() && isAuthorized && showConfirmationDialog()) {
			...
			Object currentValue = getData(actionElement
					.getAttribute(tagToWriteName));
