package com.angrysnail.sheeta.extension.jwidget.combo.runtime.actions;
...
import com.angrysnail.sheeta.rtda.widget.ComboListItem;
...
public class ComboSimpleSendCommand extends AbstractSimpleSendCommand {
	 ...
	 *            jwidget
	 */
	public ComboSimpleSendCommand(SVGPicture picture, String projectName,
	...
	protected void initializeAction() {
			...
			for (Node child = actionElement.getFirstChild(); child != null; child = child
				...
				if (child instanceof Element) {
					...
					if (childElement.getAttribute(usedAttName).equals(
						...
						realTagToWriteValues.add(childElement
								.getAttribute(valueName));
						tagToWriteValues.add(AnimationsToolkit
		...
		// filling the combo box with the combo items corresponding to each tag
		// to write value
	...
	public void performAction(Object evt) {
		...
		if (isEntitled() && isAuthorized && showConfirmationDialog()) {
			...
			String tagValue = "";
			ComboListItem item = (ComboListItem) ((JComboBox) component)
					.getSelectedItem();
