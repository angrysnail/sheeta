package com.angrysnail.sheeta.extension.jwidget.popup.runtime;
...
import com.angrysnail.sheeta.extension.jwidget.popup.edition.PopUpEdition;
...
public class PopUpRuntime extends JWidgetRuntime {
	...
	public void initialize() {
		...
		// when clicking on a canvas zone defined by the previously computed
		// bounds
		mouseListener = new MouseAdapter() {
			...
			public void mousePressed(MouseEvent evt) {
				...
				boolean isPopupEvent = (evt.isPopupTrigger() || SwingUtilities
						.isRightMouseButton(evt));

				if (isPopupEvent && componentBounds.contains(evt.getPoint())) {

					((JPopupMenu) menuItemsContainerWidget.getComponent())
							.show(picture.getPictureComponent(),
