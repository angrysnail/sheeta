package com.angrysnail.sheeta.extension.jwidget.progressbar.runtime.anim;
...
public class ProgressBarBarGraph extends JWidgetAnimation {
	 ...
	 * the string names
	 */
	private static String tagAttributeName = "tag",
			tagMinAttributeName = "tagMin", tagMaxAttributeName = "tagMax";
	...
	public ProgressBarBarGraph(JWidgetRuntime jwidgetRuntime,
		...
		if (jwidgetRuntime.getPicture().getMainDisplay().isTestVersion()) {
			...
			TestTagInformation info = new TestTagInformation(
					...
					animationElement.getAttribute(tagAttributeName), null);
	...
	public Runnable dataChanged(DataEvent evt) {
		...
		if (animationElement != null) {
			...
			String tagAttributeValue = animationElement
			...
			if (!Double.isNaN(tagValue) && !Double.isNaN(tagMaxValue)
				...
				final double value = tagValue / (tagMaxValue - tagMinValue)
						* (maxValue - minValue);
