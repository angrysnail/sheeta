...
import com.angrysnail.sheeta.extension.jwidget.table.edition.TableEdition;
import com.angrysnail.sheeta.extension.jwidget.table.runtime.actions.TableSendMeasure;
import com.angrysnail.sheeta.extension.jwidget.table.runtime.actions.TableSendString;
import com.angrysnail.sheeta.extension.jwidget.table.runtime.actions.TableSimpleSendCommand;
...
public class TableRuntime extends JWidgetRuntime {
	...
	public void initialize() {
		...
		boolean showHLines = element.getAttribute("showHorizontalLines")
				.equals(Boolean.toString(true));
		boolean showVLines = element.getAttribute("showVerticalLines").equals(
			...
			if (tableWidget.cellExist(sourceId)) {
				...
				if (tagName.equals("rtda:simpleSendCommand")) {
					...
					action = new TableSimpleSendCommand(picture, projectName,
							picture.getCanvas(), component, null,
							actionElement, this);
	...
	public static int getCol(String sourceId) {
		...
		try {
			col = Integer.parseInt(sourceId.substring(pos + 1,
					sourceId.length()));
		} catch (Exception ex) {
		}
	...
	public void refreshComponentState() {
		...
		for (String sourceId : subCmpIdToActions.keySet()) {
			...
			if (row >= 0 && col >= 0) {
				...
				enabled = (actionsSet != null && !actionsInactive(actionsSet));
