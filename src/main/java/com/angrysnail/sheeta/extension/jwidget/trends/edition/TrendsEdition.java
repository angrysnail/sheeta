package com.angrysnail.sheeta.extension.jwidget.trends.edition;
...
public class TrendsEdition extends JWidgetEdition {
	...
	protected BufferedImage createImage(Element jwidgetElement, Dimension size) {
		...
		boolean displayToolBar = Boolean.parseBoolean(getProperty(
				jwidgetElement, propertiesList.get(30)));
		...
		if (displayToolBar) {
			...
			JToggleButton switchButton = new JToggleButton(TrendsIcons.getIcon(
					"RealTimeMode", false));
			JToggleButton playButton = new JToggleButton(TrendsIcons.getIcon(
					"Play", false));
			JToggleButton pauseButton = new JToggleButton(TrendsIcons.getIcon(
					"Pause", false));

			JButton datePickersDialogButton = new JButton(TrendsIcons.getIcon(
					"DatePickersDialog", false));

			JToggleButton cursorButton = new JToggleButton(TrendsIcons.getIcon(
					"Regular", false));
			JToggleButton lineCursorButton = new JToggleButton(
					TrendsIcons.getIcon("LineCursor", false));
			JToggleButton crossCursorButton = new JToggleButton(
					TrendsIcons.getIcon("CrossCursor", false));

			JButton timeZoomInitialButton = new JButton(TrendsIcons.getIcon(
					"TimeZoomInitial", false));
			JButton timeZoomInButton = new JButton(TrendsIcons.getIcon(
					"TimeZoomIn", false));
			JButton timeZoomOutButton = new JButton(TrendsIcons.getIcon(
					"TimeZoomOut", false));
			JButton timeZoomDialogButton = new JButton(TrendsIcons.getIcon(
					"TimeZoomDialog", false));
			JButton mouseTimeZoomButton = new JButton(TrendsIcons.getIcon(
					"MouseTimeZoom", false));
			JButton verticalZoomInitialButton = new JButton(
					TrendsIcons.getIcon("VerticalZoomInitial", false));
			JButton mouseZoomButton = new JButton(TrendsIcons.getIcon(
					"MouseZoom", false));

			JButton displayCurvesPropertiesButton = new JButton(
					TrendsIcons.getIcon("DisplayCurvesProperties", false));
		...
		boolean displayScrollBar = Boolean.parseBoolean(getProperty(
				jwidgetElement, propertiesList.get(23)));
		...
		Color curvesPanelColor = ColorsAndBlinkingsToolkit
