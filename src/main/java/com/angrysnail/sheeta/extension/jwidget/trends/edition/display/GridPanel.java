...
public class GridPanel extends JPanel {
	...
	public void initialize(Element element) {
		...
		try {
			gridCheckBox.setSelected(Boolean.parseBoolean(jwidgetEdition
	...
	protected void build() {
		...
		try {
			...
			displayGridLabel = jwidgetEdition.getBundle().getString(
					"displayGrid");
		} catch (Exception ex) {
			ex.printStackTrace();
		}
		...
		checkBoxActionListener = new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				jwidgetEdition.setProperty(currentElement, jwidgetEdition
						.getPropertiesList().get(12), Boolean
						.toString(gridCheckBox.isSelected()), true);
	...
	/**
	 * handles the modifications that should be done when the checkbox state
	 * changes
	 */
