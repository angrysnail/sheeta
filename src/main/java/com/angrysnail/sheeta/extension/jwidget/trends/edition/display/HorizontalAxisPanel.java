...
public class HorizontalAxisPanel extends JPanel {
	...
	/**
	 * the time chooser widget used for specifying the whole time that will be
	...
	public void initialize(Element element) {
		...
		displayedTimeChooser.init(jwidgetEdition.getProperty(element,
				jwidgetEdition.getPropertiesList().get(0)));
		displayedTimeChooser.addListener(displayedTimeChooserListener);

		displayTimeGraduationCheckBox
				.removeActionListener(checkBoxesActionListener);
		displayHorodateCheckBox.removeActionListener(checkBoxesActionListener);

		try {
			displayTimeGraduationCheckBox.setSelected(Boolean
					.parseBoolean(jwidgetEdition.getProperty(element,
							jwidgetEdition.getPropertiesList().get(1))));
		} catch (Exception ex) {
			ex.printStackTrace();
		}

		try {
			displayHorodateCheckBox.setSelected(Boolean
					.parseBoolean(jwidgetEdition.getProperty(element,
							jwidgetEdition.getPropertiesList().get(2))));
		} catch (Exception ex) {
			ex.printStackTrace();
		}

		displayTimeGraduationCheckBox
				.addActionListener(checkBoxesActionListener);
		...
		horodateFormatTextField.setText(jwidgetEdition.getProperty(element,
				jwidgetEdition.getPropertiesList().get(3)));
	...
	protected void build() {
		...
		// getting the labels
		String horizontalAxisLabel = "", displayedTimeLabel = "", displayGraduationLabel = "", horodateLabel = "", displayHorizontalHorodatesLabel = "", horizontalHorodatesFormatLabel = "";

		try {
			horizontalAxisLabel = jwidgetEdition.getBundle().getString(
					"horizontalAxis");
			displayedTimeLabel = jwidgetEdition.getBundle().getString(
					"displayedTime");
			displayGraduationLabel = jwidgetEdition.getBundle().getString(
					"displayGraduation");

			horodateLabel = jwidgetEdition.getBundle().getString("horodate");
			displayHorizontalHorodatesLabel = jwidgetEdition.getBundle()
					.getString("displayHorizontalHorodates");
			horizontalHorodatesFormatLabel = jwidgetEdition.getBundle()
					.getString("horizontalHorodatesFormat");
		} catch (Exception ex) {
			ex.printStackTrace();
		}
		...
		displayedTimeChooserLbl.setHorizontalAlignment(SwingConstants.RIGHT);
		...
		displayedTimeChooserListener = new ActionListener() {
			...
			public void actionPerformed(ActionEvent evt) {
				...
				jwidgetEdition.setProperty(currentElement, jwidgetEdition
						.getPropertiesList().get(0), displayedTimeChooser
						.getValue(), true);
			}
		};
		...
		checkBoxesActionListener = new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (e.getSource().equals(displayTimeGraduationCheckBox)) {
					...
					jwidgetEdition.setProperty(currentElement, jwidgetEdition
							.getPropertiesList().get(1), Boolean
							.toString(displayTimeGraduationCheckBox
									.isSelected()), true);

				} else if (e.getSource().equals(displayHorodateCheckBox)) {

					jwidgetEdition.setProperty(currentElement, jwidgetEdition
							.getPropertiesList().get(2), Boolean
							.toString(displayHorodateCheckBox.isSelected()),
							true);
		...
		horodateFormatLbl.setHorizontalAlignment(SwingConstants.RIGHT);
		...
		caretListener = new CaretListener() {
			...
			public void caretUpdate(CaretEvent evt) {
				...
				jwidgetEdition.setProperty(currentElement, jwidgetEdition
						.getPropertiesList().get(3), horodateFormatTextField
						.getText(), true);
			}
		};
	...
	/**
	 * handles the state of the horodate check box, ie, enabling or disabling
	 * the format textfield according to its state
	 */
	protected void handleHorodateCheckBoxState() {

		horodateFormatLbl.setEnabled(displayHorodateCheckBox.isSelected());
		horodateFormatTextField
				.setEnabled(displayHorodateCheckBox.isSelected());
	}
}
