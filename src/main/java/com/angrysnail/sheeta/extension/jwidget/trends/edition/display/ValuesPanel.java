...
public class ValuesPanel extends JPanel {
	...
	public void initialize(Element element) {
		...
		decimalChooser.init(jwidgetEdition.getProperty(element, jwidgetEdition
				.getPropertiesList().get(24)));
		decimalChooser.addListener(decimalChooserListener);

		higherChooser.removeListener(higherChooserListener);
		higherChooser.init(jwidgetEdition.getProperty(element, jwidgetEdition
				.getPropertiesList().get(25)));
		higherChooser.addListener(higherChooserListener);

		lowerChooser.removeListener(lowerChooserListener);
		lowerChooser.init(jwidgetEdition.getProperty(element, jwidgetEdition
				.getPropertiesList().get(26)));
	...
	protected void build() {
		...
		try {
			...
			decimalNumberLabel = jwidgetEdition.getBundle().getString(
					"decimalNumber");
			limitLabel = jwidgetEdition.getBundle().getString("limit");
			higherLimitLabel = jwidgetEdition.getBundle().getString(
					"higherLimit");
			lowerLimitLabel = jwidgetEdition.getBundle()
		...
		decimalNumberLbl.setHorizontalAlignment(SwingConstants.RIGHT);
		...
		limitLbl.setHorizontalAlignment(SwingConstants.LEFT);

		// creating the jlabel for the higher limit chooser
		JLabel higherLimitLbl = new JLabel(higherLimitLabel + " : ");
		higherLimitLbl.setHorizontalAlignment(SwingConstants.RIGHT);

		// creating the higher limit chooser
		higherChooser = new DoubleSpinnerWidget(100000, 0, 1000000000000.0, 1,
				false);
		...
		lowerLimitLbl.setHorizontalAlignment(SwingConstants.RIGHT);

		// creating the lower limit chooser
		lowerChooser = new DoubleSpinnerWidget(100000, 0, 1000000000000.0, 1,
				false);
