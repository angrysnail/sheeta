...
public class VerticalGridLinesPanel extends JPanel {
	...
	protected void build() {
		 ...
		 * 
		 * try { verticalGridLinesLabel=jwidgetEdition.getBundle().getString(
		 * "verticalGridLines"); }catch (Exception ex) {ex.printStackTrace();}
		 */

		// creating the vertical grid lines divisions panel
		verticalGridLinesDivisionsPanel = new VerticalGridLinesDivisionsPanel(
				jwidgetEdition);

		// creating the vertical grid lines sub divisions panel
		verticalGridLinesSubDivisionsPanel = new VerticalGridLinesSubDivisionsPanel(
				jwidgetEdition);
