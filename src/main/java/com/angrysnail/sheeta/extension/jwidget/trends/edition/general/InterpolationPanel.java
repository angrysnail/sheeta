...
public class InterpolationPanel extends JPanel {
	...
	protected void build() {
		...
		try {
			interpolationLabel = jwidgetEdition.getBundle().getString(
					"interpolation");
			interpolationWithPeriodLabel = jwidgetEdition.getBundle()
					.getString("interpolationWithPeriod");
		} catch (Exception ex) {
			ex.printStackTrace();
		}
		...
		checkBoxActionListener = new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				jwidgetEdition.setProperty(currentElement, jwidgetEdition
						.getPropertiesList().get(21), Boolean.toString(checkBox
						.isSelected()), true);
			}
		};
