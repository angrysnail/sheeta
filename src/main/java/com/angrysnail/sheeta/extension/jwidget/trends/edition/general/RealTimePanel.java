package com.angrysnail.sheeta.extension.jwidget.trends.edition.general;
...
public class RealTimePanel extends JPanel {
	...
	public void initialize(Element element) {
		...
		refreshPeriodTimeChooser.init(jwidgetEdition.getProperty(element,
				jwidgetEdition.getPropertiesList().get(6)));
		refreshPeriodTimeChooser.addListener(refreshPeriodChooserListener);

		pointsNumberSpinner.removeListener(spinnerListener);
		pointsNumberSpinner.init(jwidgetEdition.getProperty(element,
				jwidgetEdition.getPropertiesList().get(7)));
		pointsNumberSpinner.addListener(spinnerListener);

		maxDisplayableTimeChooser
				.removeListener(maxDisplayableTimeChooserListener);
		maxDisplayableTimeChooser.init(jwidgetEdition.getProperty(element,
				jwidgetEdition.getPropertiesList().get(22)));
		maxDisplayableTimeChooser
				.addListener(maxDisplayableTimeChooserListener);
	}
	...
	protected void build() {
		...
		// getting the labels
		String realTimeLabel = "", refreshPeriodLabel = "", pointsNumberLabel = "", maxDisplayableTimeLabel = "";

		try {
			realTimeLabel = jwidgetEdition.getBundle().getString("realTime");
			refreshPeriodLabel = jwidgetEdition.getBundle().getString(
					"refreshPeriod");
			pointsNumberLabel = jwidgetEdition.getBundle().getString(
					"pointsNumber");
			maxDisplayableTimeLabel = jwidgetEdition.getBundle().getString(
					"maxDisplayableTime");
		} catch (Exception ex) {
			ex.printStackTrace();
		}
		...
		JLabel refreshPeriodTimeChooserLbl = new JLabel(refreshPeriodLabel
				+ " : ");
		refreshPeriodTimeChooserLbl.setHorizontalAlignment(SwingConstants.RIGHT);
		...
		refreshPeriodChooserListener = new ActionListener() {
			...
			public void actionPerformed(ActionEvent evt) {
				...
				jwidgetEdition.setProperty(currentElement, jwidgetEdition
						.getPropertiesList().get(6), refreshPeriodTimeChooser
						.getValue(), false);
			}
		};
		...
		pointsNumberSpinnerLbl.setHorizontalAlignment(SwingConstants.RIGHT);
		...
		// creating the jlabel for the time chooser
		JLabel maxDisplayableTimeChooserLbl = new JLabel(
				maxDisplayableTimeLabel + " : ");
		maxDisplayableTimeChooserLbl.setHorizontalAlignment(SwingConstants.RIGHT);
		...
		maxDisplayableTimeChooserListener = new ActionListener() {
			...
			public void actionPerformed(ActionEvent evt) {
				...
				jwidgetEdition.setProperty(currentElement, jwidgetEdition
						.getPropertiesList().get(22), maxDisplayableTimeChooser
						.getValue(), false);
			}
		};
