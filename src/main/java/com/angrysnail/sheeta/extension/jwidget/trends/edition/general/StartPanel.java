...
public class StartPanel extends JPanel {
	...
	public void initialize(Element element) {
		...
		String value = jwidgetEdition.getProperty(element, jwidgetEdition
				.getPropertiesList().get(9));

		// getting the index of the combo item corresponding to the property
		// value
		int i = 0;
	...
	protected void build() {
		...
		comboLbl.setHorizontalAlignment(SwingConstants.RIGHT);
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (combo.getSelectedItem() != null) {
					...
					jwidgetEdition.setProperty(currentElement, jwidgetEdition
							.getPropertiesList().get(9), ((ComboItem) combo
	...
	protected class ComboItem {
		...
		private ComboItem(String value) {
			...
			try {
				label = jwidgetEdition.getBundle().getString(
						labelPrefix + value);
			} catch (Exception ex) {
			}
		}
