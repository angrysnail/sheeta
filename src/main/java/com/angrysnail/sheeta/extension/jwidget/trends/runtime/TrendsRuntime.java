...
import com.angrysnail.sheeta.extension.jwidget.trends.edition.TrendsEdition;
...
public class TrendsRuntime extends JWidgetRuntime {
	...
	public void initializeWhenCanvasDisplayed() {
		...
		if (controller.getConfiguration().getStartMode() == TrendsConfiguration.HISTORY_MODE
	 ...
	 *            a tag name
	 * @param isEnumerated
	 *            whether this tag is an enumerated one
	 */
	public void registerTag(String tagName, boolean isEnumerated) {
		...
		if (picture.getMainDisplay().isTestVersion()) {
			...
			if (isEnumerated) {
				...
				// getting the trends curve configuration corresponding to the
				// tag name
				TrendsCurveConfiguration trendsCurveConfiguration = controller
						.getConfiguration()
						.getTrendsCurveConfiguration(tagName);
