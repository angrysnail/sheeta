package com.angrysnail.sheeta.extension.jwidget.trends.runtime.configuration;
...
import com.angrysnail.sheeta.library.widgets.TimeChooserWidget;
...
/**
 * the class of the objects that store and provide the configuration values for
...
public class TrendsConfiguration {
	 ...
	 */
	private LinkedList<TrendsCurveConfiguration> curvesConfigurationList = new LinkedList<TrendsCurveConfiguration>();

	/**
	 * the map associating a tag name to a trends curve configuration object
	 */
	private Map<String, TrendsCurveConfiguration> curvesConfigurationMap = new HashMap<String, TrendsCurveConfiguration>();

	/**
	 * the set of the listeners
	 */
	private Set<ConfigurationChangeListener> listeners = new HashSet<ConfigurationChangeListener>();
	...
	/**
	 * the number of digits of the decimal part that should be displayed when
	 * displaying a string representation of a value of a tag
	 */
	...
	/**
	 * whether to keep on recording the tag values of the real time mode when
	 * switching to the history mode
	 */
	...
	/**
	 * the trends curve configuration corresponding to the curve whose grid
	 * horizontal lines and/or cursor value should be displayed
	 */
	 ...
	 * 
	 * @param controller
	 *            the trends runtime controller
	 * @param jwidgetElement
	 *            the jwidget element defining the configuration
	 */
	public TrendsConfiguration(TrendsRuntimeController controller,
		...
		confDoc = picture.getMainDisplay().getPictureManager()
	...
	/**
	 * retrieves and computes all the non string values that are specified in
	 * the jwidget element
	 */
	protected void computeValues() {
		...
		try {
			bufferPointsNumber = Integer.parseInt(jwidgetElement
					.getAttribute("pointsNumber"));
		} catch (Exception ex) {
		}

		// getting the maximum time that can be displayed when in real time mode
		maxDisplayableTime = getTime(jwidgetElement
				.getAttribute("maxDisplayableTime"));
		...
		backgroundColor = ColorsAndBlinkingsToolkit.getColor(jwidgetElement
				.getAttribute("backgroundColor"));

		// getting the font
		try {
			float fontSize = Float.parseFloat(jwidgetElement
					.getAttribute("fontSize"));
			font = font.deriveFont(fontSize);
		} catch (Exception ex) {
		}

		// getting the distance between two vertical grid division lines in
		// milliseconds
		verticalLinesDivisionDuration = getTime(jwidgetElement
				.getAttribute("verticalLinesDivisionDuration"));

		// getting the color of the grid vertical division lines
		verticalLinesDivisionColor = ColorsAndBlinkingsToolkit
				.getColor(jwidgetElement
						.getAttribute("verticalLinesDivisionColor"));

		// getting the distance between two vertical grid sub division lines in
		// milliseconds
		verticalLinesSubDivisionDuration = getTime(jwidgetElement
				.getAttribute("verticalLinesSubDivisionDuration"));

		// getting the color of the grid vertical sub division lines
		verticalLinesSubDivisionColor = ColorsAndBlinkingsToolkit
				.getColor(jwidgetElement
						.getAttribute("verticalLinesSubDivisionColor"));

		// getting the number of digits of the decimal part that should be
		// displayed
		// when displaying a string representation of a value of a tag
		try {
			valuesDecimalDigitsNumber = Integer.parseInt(jwidgetElement
					.getAttribute("decimalDigitsNumber"));
		} catch (Exception ex) {
			valuesDecimalDigitsNumber = 0;
		}

		// getting the higher limit from which the values should be written with
		// an exponent
		try {
			higherLimit = format.parse(
					jwidgetElement.getAttribute("higherLimit")).doubleValue();
		} catch (Exception ex) {
		}

		// getting the lower limit under which the values should be written with
		// an exponent
		try {
			lowerLimit = format
					.parse(jwidgetElement.getAttribute("lowerLimit"))
					.doubleValue();
		} catch (Exception ex) {
		}

		keepTagRecordInHistory = Boolean.parseBoolean(jwidgetElement
				.getAttribute("keepTagRecordInHistory"));
		...
		displayToolBar = Boolean.parseBoolean(jwidgetElement
				.getAttribute("displayToolBar"));
	}
	...
	protected void createCurvesConfigurationObjects() {
		...
		for (int i = 0; i < childNodes.getLength(); i++) {
			...
			if (node != null
					...
					&& !node.getNodeName()
							.equals(JWidgetRuntime.jwidgetTagName)
					&& node.getNodeName().startsWith(
							AnimationsToolkit.rtdaPrefix)) {

				element = (Element) node;
				trendsCurveConfig = new TrendsCurveConfiguration(controller,
						this, element);
				curvesConfigurationList.add(trendsCurveConfig);
				curvesConfigurationMap.put(trendsCurveConfig.getTagName(),
						trendsCurveConfig);
			}
		}
	...
	/**
	 * computes the trends curve configuration corresponding to the curve whose
	 * grid horizontal lines and/or cursor value should be displayed
	 */
	protected void computeCurrentTrendsCurveConfiguration() {
		...
		LinkedList<TrendsCurveConfiguration> displayedScaleCurves = new LinkedList<TrendsCurveConfiguration>();
	...
	/**
	 * @return the trends curve configuration corresponding to the curve whose
	 *         grid horizontal lines and/or cursor value should be displayed
	 */
	...
	/**
	 * returns the trends curve configuration corresponding to the given tag
	 * name
	 * 
	 * @param tagName
	 *            a tag name
	 * @return the trends curve configuration corresponding to the given tag
	 *         name
	 */
	 ...
	 * 
	 * @param listener
	 *            a new configuration change listener
	 */
	 ...
	 * 
	 * @param listener
	 *            a configuration change listener
	 */
	...
	/**
	 * @return whether to allow or not the dispacth of configuration
	 *         modification events
	 */
	...
	/**
	 * sets whether to allow or not the dispatch of configuration modification
	 * events
	 * 
	 * @param allowEventDispatch
	 *            whether to allow or not the dispacth of configuration
	 *            modification events
	 */
	...
	/**
	 * @return the time that will be displayed in the trends window in
	 *         milliseconds
	 */
	...
	/**
	 * @return the initial time that will be displayed in the trends window in
	 *         milliseconds
	 */
	 ...
	 *            the new vertical zoom factor
	 * @param notify
	 *            whether to notify or not that the zoom factor has changed
	 */
	 ...
	 * 
	 * @param verticalZoomOrigin
	 *            the new vertical zoom origin
	 */
	...
	public boolean displayHorizontalAxisTimeGraduations() {
		...
		return Boolean.parseBoolean(jwidgetElement
				.getAttribute("displayTimeGrad"));
	}
	...
	public boolean displayHorizontalAxisHorodates() {
		...
		return Boolean.parseBoolean(jwidgetElement
				.getAttribute("displayHorizHorodate"));
	}
	...
	public boolean displayCurvesLegend() {
		...
		return Boolean.parseBoolean(jwidgetElement
				.getAttribute("displayCurvesLegend"));
	}
	...
	/**
	 * @return whether to automatically compute the distance between two
	 *         vertical grid division lines
	 */
	public boolean isVerticalLinesDivisionAutomaticPeriod() {

		return Boolean.parseBoolean(jwidgetElement
				.getAttribute("verticalLinesDivisionAutomaticPeriod"));
	}

	/**
	 * @return the distance between two vertical grid division lines in
	 *         milliseconds
	 */
	...
	/**
	 * @return whether to automatically compute the distance between two
	 *         vertical grid sub division lines
	 */
	public boolean isVerticalLinesSubDivisionAutomaticPeriod() {

		return Boolean.parseBoolean(jwidgetElement
				.getAttribute("verticalLinesSubDivisionAutomaticPeriod"));
	}

	/**
	 * @return the distance between two vertical grid sub division lines in
	 *         milliseconds
	 */
	...
	/**
	 * @return whether to integrate the values that are constant for a moment
	 *         when computing the interpolation
	 */
	public boolean interpolationWithPeriod() {

		return Boolean.parseBoolean(jwidgetElement
				.getAttribute("interpolationWithPeriod"));
	}
	...
	/**
	 * @return the higher limit from which the values should be written with an
	 *         exponent
	 */
	...
	/**
	 * @return the lower limit under which the values should be written with an
	 *         exponent
	 */
	 ...
	 * 
	 * @param strValue
	 *            a value representating a duration
	...
	public static long getTime(String strValue) {
		...
		if (strValue != null && !strValue.equals("")) {
			...
			if (pos != -1) {
				...
				// getting the time value relatively to the unit that can be
				// found in the string
				long time = -1;
