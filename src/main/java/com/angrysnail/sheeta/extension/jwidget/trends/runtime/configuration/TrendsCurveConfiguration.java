...
import com.angrysnail.sheeta.library.PreferencesStore;
...
public class TrendsCurveConfiguration {
	 ...
	 */
	protected static final String colorPrefId = "Color",
			displayScalePrefId = "DisplayScale", dashesPrefId = "Dashes",
	...
	/**
	 * computes the values (that could not change) that are specified in the
	 * jwidget element
	 */
	...
	public void computeDatabaseValues() {
		...
		if (confDoc != null) {
			...
			// getting the xml element corresponding to the tag in the xml data
			// base
			...
			if (tagElement != null) {
				...
				String historyManagerName = tagElement
						.getAttribute("historymanager");
				String historyTableName = tagElement
						.getAttribute("historytable");
				String historyTagName = tagElement
						.getAttribute("historytagname");

				// getting the history manager and the history table handling
				// this tag name
				HistoryManager historyManager = controller.getModel()
						.getHistoryManager(historyManagerName);

				if (historyManager != null) {

					// getting the table history object corresponding to the
					// table name
					HistoryTable historyTable = historyManager
							.getHistoryTable(historyTableName);

					if (historyTable != null) {

						// creating the history field
						historyField = new HistoryField(historyTable, tagName,
								historyTagName);
					}
				}
			}
	...
	/**
	 * retrieves and computes all the values that are specified in the jwidget
	 * element
	 */
	protected void computeValues() {
		...
		colorForInvalid = ColorsAndBlinkingsToolkit.getColor(curveElement
				.getAttribute("colorForInvalid"));

		// getting the thickness of the curve when it is in an invalid state
		try {
			thicknessForInvalid = Integer.parseInt(curveElement
					.getAttribute("thicknessForInvalid"));
		} catch (Exception ex) {
		}
		...
		dashes = PreferencesStore.getPreference(getPreferencesNodeId(),
				dashesPrefId);
		...
		pointType = PreferencesStore.getPreference(getPreferencesNodeId(),
				pointsTypePrefId);
		...
		minLimit = new TrendsDataLimit(controller, tagName,
				curveElement.getAttribute("tagMin"), true);
		maxLimit = new TrendsDataLimit(controller, tagName,
				curveElement.getAttribute("tagMax"), true);
	}
	 ...
	 * 
	 * @param listener
	 *            a new curves configuration change listener
	 */
	 ...
	 * 
	 * @param listener
	 *            the curves configuration change listener
	 */
	...
	protected void notifyStyleChanged() {
		...
		for (CurvesConfigurationChangeListener listener : new HashSet<CurvesConfigurationChangeListener>(
				listeners)) {
	...
	/**
	 * @return the list of the enumerated tag values, if this tag is an
	 *         enumerated one
	 */
	...
	public String getPreferencesNodeId() {
		...
		return controller.getJwidgetRuntime().getPicture()
				.getAnimActionsHandler().getViewPath()
				+ "::" + getTagName();
	}
	...
	/**
	 * @param displayScale
	 *            whether the scale of this curve should be displayed
	 */
	...
	public void setColor(Color color) {
		...
		PreferencesStore.setPreference(getPreferencesNodeId(), colorPrefId,
				ColorsAndBlinkingsToolkit.getColorString(color));
	}
	...
	public void setThickness(int thickness) {
		...
		PreferencesStore.setPreference(getPreferencesNodeId(), thicknessPrefId,
				thickness + "");
	}
	...
	public void setDash(String dash) {
		...
		PreferencesStore.setPreference(getPreferencesNodeId(), dashesPrefId,
				dash);
	}
	...
	/**
	 * @param dashForInvalid
	 *            the dash of the curve when it is in an invalid state
	 */
	...
	public void reinitialize() {
		...
		try {
			Preferences pref = PreferencesStore
					.getPreferenceNode(getPreferencesNodeId());
			pref.clear();
		} catch (Exception ex) {
		}
