...
public class TrendsDataLimit {
	 ...
	 *            the tag name whose limit is computed
	 * @param limitValue
	 *            the string value of the limit
	 * @param isMinValue
	 *            whether this limit stands for a min value
	 */
	public TrendsDataLimit(TrendsRuntimeController controller,
			String referenceTagName, String limitValue, boolean isMinValue) {

		this.controller = controller;

		if (limitValue != null && !limitValue.equals("")) {

			// gets the value of the limit according to the content of
			// "limitValue"
			if (limitValue.equals(DataLimit.AUTO_LIMIT)) {
				...
				try {
					SVGPicture picture = controller.getJwidgetRuntime()
							.getPicture();
					tagElement = picture.getMainDisplay().getPictureManager()
							.getConfigurationDocument(picture)
							.getElement(referenceTagName);
				} catch (Exception ex) {
				}

				if (tagElement != null) {

					String valStr = tagElement.getAttribute(isMinValue
							? "min"
							: "max");
