package com.angrysnail.sheeta.extension.jwidget.trends.runtime.model;
...
import com.angrysnail.sheeta.extension.jwidget.trends.runtime.configuration.ConfigurationChangeListener;
import com.angrysnail.sheeta.extension.jwidget.trends.runtime.configuration.TrendsConfiguration;
import com.angrysnail.sheeta.extension.jwidget.trends.runtime.configuration.TrendsCurveConfiguration;
import com.angrysnail.sheeta.extension.jwidget.trends.runtime.controller.TrendsRuntimeController;
import com.angrysnail.sheeta.extension.jwidget.trends.runtime.model.buffer.CurveBuffer;
import com.angrysnail.sheeta.extension.jwidget.trends.runtime.model.buffer.DoubleCurveBuffer;
import com.angrysnail.sheeta.extension.jwidget.trends.runtime.model.buffer.StringCurveBuffer;
import com.angrysnail.sheeta.extension.jwidget.trends.runtime.model.database.HistoryField;
import com.angrysnail.sheeta.extension.jwidget.trends.runtime.model.database.HistoryManager;
import com.angrysnail.sheeta.extension.jwidget.trends.runtime.model.database.HistoryTable;
import com.angrysnail.sheeta.library.Toolkit;
import com.angrysnail.sheeta.library.widgets.WaitDialog;
import com.angrysnail.sheeta.rtda.AnimationsHandler;
import com.angrysnail.sheeta.rtda.config.ConfigurationDocument;
...
public class TrendsRuntimeModel {
	 ...
	 * some labels
	 */
	protected static String receivingValuesLabel, normalizingValuesLabel,
			refreshingDisplayLabel;

	static {

		receivingValuesLabel = TrendsBundle.bundle.getString("receivingData");
		normalizingValuesLabel = TrendsBundle.bundle
				.getString("normalizingValues");
		refreshingDisplayLabel = TrendsBundle.bundle
				.getString("refreshingDisplay");
	}
	...
	/**
	 * the date before which no tag values can be displayed for the real time
	 * mode
	 */
	private long firstRTDate = 0;

	/**
	 * the date after which no tag values can be displayed for the real time
	 * mode
	 */
	...
	public void initialize() {
		...
		for (TrendsCurveConfiguration curveConfiguration : controller
				.getConfiguration().getCurvesConfigurationList()) {

			if (curveConfiguration.isEnumeratedTag()) {

				rtBuffer = new StringCurveBuffer(
						curveConfiguration.getTagName(),
						curveConfiguration.isEnumeratedTag(), controller
								.getConfiguration().getBufferPointsNumber());

				historyBuffer = new StringCurveBuffer(
						curveConfiguration.getTagName(),
						curveConfiguration.isEnumeratedTag(), controller
								.getConfiguration().getBufferPointsNumber());
				...
				rtBuffer = new DoubleCurveBuffer(
						curveConfiguration.getTagName(),
						curveConfiguration.isEnumeratedTag(), controller
								.getConfiguration().getBufferPointsNumber());

				historyBuffer = new DoubleCurveBuffer(
						curveConfiguration.getTagName(),
						curveConfiguration.isEnumeratedTag(), controller
								.getConfiguration().getBufferPointsNumber());
			}
		...
		controller.getConfiguration().addListener(
	 ...
	 * 
	 * @param firstDate
	 *            the date before which no tag values can be displayed
	 */
	 ...
	 * 
	 * @param lastDate
	 *            the date after which no tag values can be displayed
	 */
	public synchronized void setLastDate(long lastDate) {
		...
		if (controller.getConfiguration().getCurrentMode() == TrendsConfiguration.REAL_TIME_MODE) {
			...
			if (lastRTDate - firstRTDate > controller.getConfiguration()
					.getMaxDisplayableTime()) {

				firstRTDate = lastRTDate
						- controller.getConfiguration().getMaxDisplayableTime();
			}
	 ...
	 * 
	 * @param dataMap
	 *            the map associating a tag name to its new value
	 */
	public void updateTags(Map<String, Object> dataMap) {

		AnimationsHandler handler = controller.getJwidgetRuntime().getPicture()
				.getMainDisplay().getAnimationsHandler();

		// checking if the new values should be stored
		if (controller.getConfiguration().getCurrentMode() == TrendsConfiguration.HISTORY_MODE) {

			String startDateTag = controller.getConfiguration()
					.getStartDateTag();
			...
			if (dataMap.containsKey(startDateTag)
				...
				try {
					startDate = Long.parseLong(handler.getData(startDateTag)
							.toString());
				} catch (Exception ex) {
				}

				try {
					endDate = Long.parseLong(handler.getData(endDateTag)
							.toString());
				} catch (Exception ex) {
				}
		...
		if ((controller.getConfiguration().getCurrentMode() == TrendsConfiguration.HISTORY_MODE && controller
				.getConfiguration().keepTagRecordInHistory())
				|| controller.getConfiguration().getCurrentMode() == TrendsConfiguration.REAL_TIME_MODE) {
	...
	/**
	 * returns all the values that have been recorded by the model from the
	 * given start date to the given end date.Tthe value corresponding to the
	 * date should be in the buffer, no values will be gotten from the history
	 * 
	 * @param tagName
	 *            the name of a tag
	 * @param startDate
	 ...
	 * @param endDate
	 *            the end date
	 * @return all the values that have been recorded by the model from the
	 *         given start date to the given end date
	 */
	public TreeMap<Long, Object> getValues(String tagName, long startDate,
			long endDate) {
	...
	/**
	 * requests the tag value corresponding to the tag name and that is the
	 * closest value corresponding to the horodate and that occured before this
	 * horodate
	 * 
	 * @param tagName
	 *            a tag name
	 ...
	 * 
	 * @param dbRequestCancelled
	 *            whether the data base request has been cancelled
	 */
	...
	public void requestFromDataBase(final long startDate, final long endDate) {
		...
		Thread thread = new Thread() {
			 ...
			 */
			private Set<DataBaseRequest> dataBaseRequestsSet = new HashSet<DataBaseRequest>();

			@Override
			public void run() {

				// getting the wait dialog
				WaitDialog waitDialog = controller.getView()
						.getComponentsHandler().getWaitDialog();
				waitDialog.setProgressMessageInAwtThread(receivingValuesLabel,
						0, 0, 100);

				DataBaseRequest dataBaseRequest;

				// creating the data base request thread for each table of each
				// history manager
				for (HistoryManager historyManager : historyManagersSet) {

					if (historyManager != null
							&& historyManager.getHistoryTables().size() > 0) {

						for (HistoryTable historyTable : historyManager
								.getHistoryTables()) {

							// creating the data base request thread
							dataBaseRequest = new DataBaseRequest(
									historyTable.getHistoryFields(), startDate,
									endDate);
				...
				while (!requestsDone()) {
					...
					if (dbRequestCancelled) {
						...
						// getting the number of threads that have received
						// their values
						nb = 0;
						...
						waitDialog.setProgressMessageInAwtThread(
								receivingValuesLabel, 50 * nb / requestsNumber,
								0, 100);
						...
						waitDialog.setProgressMessageInAwtThread(
								normalizingValuesLabel, 50 + 30 * nb
										/ requestsNumber, 0, 100);
					}
				...
				waitDialog.setProgressMessageInAwtThread(
						refreshingDisplayLabel, 90, 0, 100);

				// notifying the view that new history values are available for
				// display
				controller.getView().refreshDisplay(
						startDate,
						startDate
								+ controller.getConfiguration()
										.getHorizontalAxisDuration());
	...
	protected void createDataBaseConfig() {
		...
		ConfigurationDocument confDoc = controller.getConfiguration()
		...
		if (confDoc != null && confDoc.getRootElement() != null) {
			...
			NodeList historyManagerElements = confDoc.getRootElement()
					.getElementsByTagName(Toolkit.historyManagerAttName);

			if (historyManagerElements != null) {

				// creating a history manager for each node and its contained
				// tables
				Node node;
				NodeList tableNodes;
				...
				for (int i = 0; i < historyManagerElements.getLength(); i++) {
					...
					if (node != null && node instanceof Element) {
						...
						historyManager = new HistoryManager(
								histEl.getAttribute(Toolkit.idAttName),
								histEl.getAttribute(Toolkit.driverNameAttName),
								histEl.getAttribute(Toolkit.subProtocolAttName),
								histEl.getAttribute(Toolkit.hostAttName),
								histEl.getAttribute(Toolkit.dbNameAttName),
								histEl.getAttribute(Toolkit.userAttName),
								histEl.getAttribute(Toolkit.passwordAttName));
						historyManagersSet.add(historyManager);

						// getting all the table child nodes of this history
						// manager element
						tableNodes = histEl
								.getElementsByTagName(Toolkit.historyTableAttName);

						if (tableNodes != null) {

							// creating the history table objects corresponding
							// to each node
							...
							for (int j = 0; j < tableNodes.getLength(); j++) {
								...
								if (node != null && node instanceof Element) {
									...
									tableName = tableEl
											.getAttribute(Toolkit.idAttName);

									if (!tableName.equals("")) {

										new HistoryTable(historyManager,
												tableName);
									}
								}
							}
			...
			LinkedList<TrendsCurveConfiguration> configs = controller
					.getConfiguration().getCurvesConfigurationList();
	...
	protected class DataBaseRequest extends Thread {
		 ...
		 * 
		 * @param fields
		 *            the fields whose values should be requested
		 * @param startDate
		 *            the start date
		 * @param endDate
		 *            the end date
		 */
		public DataBaseRequest(Set<HistoryField> fields, long startDate,
				long endDate) {
		...
		public void run() {
			...
			if (fields != null && fields.size() > 0 && !dbRequestCancelled) {
				...
				try {
					...
					connection = firstField.getHistoryTable()
							.getHistoryManager().openConnection();
					...
					if (resultSet != null) {
						...
						if (state) {
							...
							// creating the map associating a tag name to the
							// map of the values//
							values = new HashMap<String, TreeMap<Long, Object>>();

							// creating the maps corresponding to each field
							// name
							for (HistoryField field : fields) {

								values.put(field.getTagName(),
							...
							// creating the set of all the row index that should
							// not be handled so
							// that
							// the number of values in the buffer is the buffer
							// capacity
							Set<Integer> removeIndices = new HashSet<Integer>();

							if (rowsNumber > controller.getConfiguration()
									.getBufferPointsNumber()) {

								// the number of rows to remove
								int nbToRemove = rowsNumber
										- controller.getConfiguration()
												.getBufferPointsNumber();
							...
							while (state) {
								...
								if (removeIndices.size() == 0
										|| (removeIndices.size() != 0 && !removeIndices
												.contains(resultSet.getRow()))) {
		...
		/**
		 * @return the map associating tag name to the map associating a
		 *         horodate to a tag value
		 */
