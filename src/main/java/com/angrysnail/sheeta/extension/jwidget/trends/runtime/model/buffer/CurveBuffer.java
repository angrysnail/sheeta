package com.angrysnail.sheeta.extension.jwidget.trends.runtime.model.buffer;
...
public abstract class CurveBuffer {
	...
	/**
	 * returns the sub map of the buffer associating a time value (long) to a
	 * tag value
	 * 
	 * @param startDate
	 *            the date from which the elements in the returned map are added
	 * @param endDate
	 *            the date before which the elements in the returned map are
	 *            added
	 * @return the sub map of the buffer associating a time value (long) to a
	 *         tag value
	 */
	public TreeMap<Long, Object> getBufferPart(long startDate, long endDate) {
		...
		try {
			...
			synchronized (buffer) {
				map = new TreeMap<Long, Object>(buffer.subMap(startDate,
						endDate + 1));
			}

			// getting the key in the buffer that lies just before the first key
			// of the computed map
			// and putting the key and its associated value to the computed map
			SortedMap<Long, Object> headMap = null;

			synchronized (buffer) {
				headMap = buffer.headMap(map.size() > 0
						? map.firstKey()
						: startDate);
			}
			...
			// getting the key in the buffer that lies just after the last key
			// of the computed map
			// and putting the key and its associated value to the computed map
			SortedMap<Long, Object> tailMap = null;

			synchronized (buffer) {
				tailMap = buffer.tailMap(map.size() > 0
						? map.lastKey() + 1
						: endDate);
			}
	...
	public TreeMap<Long, Object> getBufferPart(long date) {
		...
		if (buffer.size() > 0) {
			...
			if (date >= buffer.firstKey() && date <= buffer.lastKey()) {
				...
				// if the date is included in the map dates, a part of the
				// buffer is returned
				try {
					...
					// getting the key in the buffer that lies just before the
					// first key of the
					// computed map
					// and putting the key and its associated value to the
					// computed map
					SortedMap<Long, Object> headMap = null;

					synchronized (buffer) {

						headMap = buffer.headMap(map.size() > 0 ? map
								.firstKey() : date);
					}
				...
				// if the date is not included in the map dates, and is lower
				// than the first date,
	...
	/**
	 * returns the value corresponding to the horodate, i.e., the closest value
	 * that occured before the horodate
	 * 
	 * @param horodate
	 *            the horodate
	 * @return the value corresponding to the horodate, i.e., the closest value
	 *         that occured before the horodate
	 */
	 ...
	 * 
	 * @param values
	 *            a map associating a horodate to a tag value
	 */
	public abstract void addTagValues(Map<Long, Object> values);

	/**
	 * @return the date of the first recorded tag value or <code>-1</code> if
	...
	/**
	 * @return the date of the last recorded tag value or <code>-1</code> if the
	 *         buffer contains no value
	 */
