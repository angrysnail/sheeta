...
import com.angrysnail.sheeta.extension.jwidget.trends.runtime.view.component.HorizontalBarComponent;
import com.angrysnail.sheeta.extension.jwidget.trends.runtime.view.component.ToolBarComponent;
import com.angrysnail.sheeta.extension.jwidget.trends.runtime.view.component.curvesproperties.CurvesPropertiesComponent;
import com.angrysnail.sheeta.extension.jwidget.trends.runtime.view.component.dialog.DatePickersDialog;
import com.angrysnail.sheeta.library.widgets.WaitDialog;

/**
 * the class used to handle all the updates, actions and modifications that can
 * occur on one of the components
 * 
...
public class ComponentsHandler {
	...
	public ComponentsHandler(TrendsRuntimeView view) {
		...
		TrendsConfiguration configuration = view.getController()
				.getConfiguration();
		...
		exponentFormat = new DecimalFormat("0." + decimalPattern + "E0####",
				symbols);
	...
	protected void initialize() {
		...
		if (toolBarComponent != null) {
			...
			location = new Point(toolBarComponent
					.getDisplayCurvesPropertiesTool().getWidth() / 2,
		...
		curvesPropertiesComponent.setVisible(view.getController()
				.getConfiguration().displayCurvesLegend(), location);
		...
		waitDialog.setTitleMessage(TrendsBundle.bundle
				.getString("historyDataLoading"));
		waitDialog.setMessage(TrendsBundle.bundle
				.getString("historyDataLoadingInfo"));
		waitDialog.setErrorMessage(TrendsBundle.bundle
				.getString("historyDataErrorLoading"));
	}
	 ...
	 * @param deselectAll
	 *            whether to deselect all the mouse action
	 */
	 ...
	 *            the name of a tag
	 * @param tagValues
	 *            the map associating a time value to a tag value
	 * @param startDate
	 *            the start date of the tag values
	 * @param endDate
	 *            the end date of the tag values
	 */
	...
	/**
	 * validates all the updates that have been done, so that the curves panel
	 * can be refreshed
	 * 
	 * @param originDate
	 *            the date before which no tag value should be displayed
	 * @param lastDate
	 *            the date after which no tag value should be displayed
	 * @param startDate
	 *            the start date of the tag values
	 * @param endDate
	 *            the end date of the tag values
	 */
	public void validateUpdates(long originDate, long lastDate, long startDate,
			long endDate) {
		...
		if (horizontalBarComponent != null) {
			...
			horizontalBarComponent.validateUpdates(originDate, lastDate,
					startDate, endDate);
		}
	}
	 ...
	 *            the end date of the tag values
	 */
	public void updateScrollBar(long firstDate, long lastDate, long startDate,
			long endDate) {

		if (horizontalBarComponent != null) {

			horizontalBarComponent.updateScrollBar(firstDate, lastDate,
					startDate, endDate);
		}
	}
	...
	public String getStringRepresentation(double value) {
		...
		if (absValue > 0
				&& (absValue <= view.getController().getConfiguration()
						.getLowerLimit() || absValue >= view.getController()
						.getConfiguration().getHigherLimit())) {
