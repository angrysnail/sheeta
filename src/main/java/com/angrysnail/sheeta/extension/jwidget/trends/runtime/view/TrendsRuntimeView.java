package com.angrysnail.sheeta.extension.jwidget.trends.runtime.view;
...
import com.angrysnail.sheeta.extension.jwidget.trends.runtime.configuration.CurvesConfigurationChangeListener;
...
import com.angrysnail.sheeta.extension.jwidget.trends.runtime.model.TrendsRuntimeModel;
...
public class TrendsRuntimeView {
	...
	/**
	 * the object used to handle all the updates, actions and modifications that
	 * can occur on one of the components
	 */
	...
	public void initialize() {
		...
		// creating the listener to the changes of the modes and of the sub
		// modes
				...
				new ConfigurationChangeListener() {
					...
					public void modeOrSubModeChanged() {
						...
						if (controller.getConfiguration().getCurrentMode() == TrendsConfiguration.HISTORY_MODE) {
							...
							refreshDisplay(System.currentTimeMillis() - 1,
					...
					public void durationChanged() {
						...
						int mode = controller.getConfiguration()
								.getCurrentMode();
						int submode = controller.getConfiguration()
								.getCurrentSubMode();

						if ((mode == TrendsConfiguration.REAL_TIME_MODE && submode != TrendsConfiguration.UPDATE)
		...
		for (TrendsCurveConfiguration trendsCurveConfiguration : controller
				.getConfiguration().getCurvesConfigurationList()) {

			trendsCurveConfiguration
					.addCurvesConfigurationChangeListener(new CurvesConfigurationChangeListener() {
	...
	public void setTagValue(String tagName, Object value) {
		...
		componentsHandler.getCurvesPropertiesComponent().setTagValue(tagName,
				value);

		if (componentsHandler.getToolBarComponent() != null) {

			componentsHandler.getToolBarComponent().getCursorLineTool()
					.setTagValue(tagName, value);
			componentsHandler.getToolBarComponent().getCursorCrossTool()
					.setTagValue(tagName, value);
		}
	}

	/**
	 * refreshes the values of the tags for the given start date and to the
	 * given end date
	 * 
	 * @param startDate
	 *            the start date
	...
	protected void refreshValues(final long startDate, final long endDate) {
		...
		final Set<String> tagNames = controller.getConfiguration()
				.getTagNames();
		...
		for (String tagName : tagNames) {
			...
			tagValues = controller.getModel().getValues(tagName, startDate,
					endDate);

			// updating the tag values
			componentsHandler.updateValues(tagName, tagValues, startDate,
					endDate);
		}
		...
		try {
			SwingUtilities.invokeAndWait(new Runnable() {
				...
				public void run() {
					...
					componentsHandler.validateUpdates(firstDate, lastDate,
							startDate, endDate);
				}
			});
		} catch (Exception ex) {
	...
	protected class UpdateRunnable implements Runnable {
		...
		public void run() {
			...
			final TrendsConfiguration configuration = controller
					.getConfiguration();
			...
			long refreshPeriod = controller.getConfiguration()
					.getRefreshPeriod();
	...
	protected class ScrollRunnable implements Runnable {
		...
		/**
		 * requests to display the tag values from the start date to the end
		 * date
		 * 
		 * @param startDate
		 *            the start date
		...
		public synchronized void requestSimpleRefresh() {
			...
			long duration = controller.getConfiguration()
					.getHorizontalAxisDuration();
		...
		public synchronized void requestDisplayWhenZoomChanged() {
			...
			if (endDateSet) {
				...
				if (this.refreshStartDate < controller.getModel()
						.getFirstDate()) {

					this.refreshStartDate = controller.getModel()
							.getFirstDate();
