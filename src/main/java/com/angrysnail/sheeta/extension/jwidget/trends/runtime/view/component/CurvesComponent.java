...
import com.angrysnail.sheeta.extension.jwidget.trends.runtime.view.component.painters.CurvePaintersManager;
import com.angrysnail.sheeta.extension.jwidget.trends.runtime.view.component.painters.DivisionPainter;
import com.angrysnail.sheeta.extension.jwidget.trends.runtime.view.component.painters.ScalesPainter;
...
public class CurvesComponent extends JPanel {
	 ...
	 *            the object used to handle all the updates, actions and
	...
	protected void paintComponent(Graphics g) {
		...
		if (offscreenImage != null) {
			...
			// painting the curves
			((Graphics2D) g).drawRenderedImage(offscreenImage,
					new AffineTransform());
		}
	...
	public void initiateUpdate() {
		...
		if (availableHeight > 0) {
			...
			scalesPainter.initiateUpdate((Graphics2D) getGraphics(),
					availableHeight);
	...
	public void updateValues(String tagName, TreeMap<Long, Object> tagValues,
		...
		int availableWidth = getWidth() - leftOffset
				- ComponentsHandler.rightOffset;
		int availableHeight = getHeight() - 2 * topOffset - bottomOffset;

		if (availableWidth > 0 && availableHeight > 0) {

			curvePaintersManager.updateValues(tagName, tagValues, startDate,
					endDate, leftOffset, ComponentsHandler.rightOffset,
					topOffset, availableWidth, availableHeight);
		}
	}
	...
	public void validateUpdates(long startDate, final long endDate) {
		...
		if (availableWidth > 0 && availableHeight > 0) {
			...
			g.fillRect(0, 0, offscreenImage.getWidth(),
			...
			if (divisionsPainter != null) {
				...
				if (configuration.displayGrid()) {
					...
					divisionsPainter.paintGridVerticalLines(g, startDate,
							endDate, leftOffset, availableWidth, getHeight()
									- bottomOffset);
				}
				...
				if (configuration.displayHorizontalAxisHorodates()) {
					...
					divisionsPainter.paintGraduations(g, startDate, endDate,
							getHeight() - bottomOffset, leftOffset,
							availableWidth, bottomOffset);
				}
			}
			...
			// drawing the scales
			scalesPainter.validateUpdates(g, topOffset, availableHeight,
					availableWidth, leftOffset);
			g.dispose();
	...
	public Rectangle getCurvesZoneBounds() {
		...
		return new Rectangle(leftOffset, topOffset, getWidth() - leftOffset
				- ComponentsHandler.rightOffset, getHeight() - 2 * topOffset
				- bottomOffset);
	}
