...
public class HorizontalBarComponent extends JPanel {
	...
	/**
	 * validates all the updates that have been done, so that the horizontal bar
	 * panel can be refreshed
	 * 
	 * @param firstDate
	 ...
	 */
	public void validateUpdates(long firstDate, long lastDate, long startDate,
			long endDate) {
		...
		scrollbar.setValues((int) (startDate - baseDate),
				(int) (endDate - startDate), 0, (int) (lastDate - baseDate));
	...
	public void updateScrollBar(long firstDate, long lastDate, long startDate,
		...
		if (!isAdjusting) {
			...
			// getting the model
			TrendsRuntimeModel model = componentsHandler.getView()
			...
			if (duration > (model.getLastDate() - model.getFirstDate())) {
				...
				scrollbar.setValues((int) (startDate - firstDate),
						(int) (lastDate - startDate), 0,
				...
				scrollbar.setValues((int) (lastStartDate - firstDate),
						(int) (lastEndDate - lastStartDate), 0,
						(int) (lastDate - firstDate));
			...
			// setting the new values
			int unitIncrement = (int) (configuration
					.getHorizontalAxisDuration() / 20);
