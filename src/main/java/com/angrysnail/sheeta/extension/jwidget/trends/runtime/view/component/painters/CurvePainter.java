...
public class CurvePainter {
	...
	/**
	 * the object storing all the configuration property values for the current
	 * curve
	 */
	...
	/**
	 * whether the last drawn point should be drawn at each period when no point
	 * is drawn
	 */
	 ...
	 * 
	 * @param painterManager
	 *            the curve painters manager
	 * @param config
	 *            the object storing all the configuration property values for
	 *            the current curve
	 */
	public CurvePainter(CurvePaintersManager painterManager,
			TrendsCurveConfiguration config) {

		this.config = config;

		// getting the controller part of the trends widget
		TrendsRuntimeController controller = painterManager
				.getCurvesComponent().getComponentsHandler().getView()
				.getController();

		// registering the tag of this curve
		controller.registerTag(config.getTagName(), config.isEnumeratedTag());

		// computing whether an interpolation shape should be drawn between two
		// points
		String interpolationType = config.getInterpolation();
		useInterpolation = (!interpolationType.equals("") && !interpolationType
				.equals("dot"));

		// computing whether the last drawn point should
		// be drawn at each period when no point is drawn
		interpolationWithPeriod = config.getConfiguration()
				.interpolationWithPeriod();
	...
	/**
	 * prepares and returns the general paths that will be drawn on the trends
	 * component when the real time mode is started
	 * 
	 * @param tagValues
	 *            the map associating a horodate to a tag value
	 * @param leftOffset
	 *            the left offset
	 * @param rightOffset
	 *            the right offset
	 ...
	 *            the available width to draw
	 * @param availableHeight
	 *            the available height to draw
	 * @param startDate
	 *            the start date
	 * @param endDate
	 *            the end date
	 */
	public void preparePoints(TreeMap<Long, Object> tagValues, int leftOffset,
			int rightOffset, int topOffset, int availableWidth,
			int availableHeight, long startDate, long endDate) {
		...
		if (tagValues != null) {
			...
			if (((config.isEnumeratedTag() && enumTagValues.size() > 0) || (!config
					.isEnumeratedTag())) && tagValues.size() > 0) {
				...
				if (!Double.isNaN(minValue) && !Double.isNaN(maxValue)) {
					...
					// adding the points into the general path and interpolating
					// between the points
					int x = 0;
					int y = 0;
					...
					for (long time : tagValues.keySet()) {
						...
						x = (int) Math.floor(availableWidth
								- (endDate - time)
								* availableWidth
						...
						if (useInterpolation && interpolationWithPeriod) {
							...
							if (previousTime > 0
									&& time - previousTime >= period) {

								// if the time delta between the current point
								// and the previous
								// point is higher than the refresh period, then
								// an interpolation
								// line is drawn
								addInterpolation(paths, pX, pY, x, pY,
										isPreviousInvalid);
								pX = x;
							}
							...
							if (i == size - 1 && endDate - time > period) {
								...
								// an interpolation
								// line is drawn
								addInterpolation(paths, x, y, availableWidth
										+ leftOffset, y, isInvalid);
								pX = x;
							}
						}
						...
						if (useInterpolation && pX != Integer.MAX_VALUE
								&& pY != Integer.MAX_VALUE) {

							addInterpolation(paths, pX, pY, x, y,
									isPreviousInvalid);
						}
	 ...
	 *            a graphics object
	 * @param bounds
	 *            the bounds for painting the curves
	 */
	 ...
	 * 
	 * @param currentPaths
	 *            the array of the paths
	 * @param x1
	 *            the absciss of the first point to paint
	 * @param y1
	 *            the ordinate of the first point to paint
	 * @param x2
	 *            the absciss of the second point to paint
	 * @param y2
	 *            the ordinate of the second point to paint
	 * @param isInvalid
	 *            whether the first given point is invalid
	 */
	protected void addInterpolation(GeneralPath[] currentPaths, int x1, int y1,
			int x2, int y2, boolean isInvalid) {
	 ...
	 *            the array of the paths
	 * @param x
	 *            the absciss of the location point
	 * @param y
	 *            the ordinate of the location point
	 * @param isInvalid
	 *            whether the given point is invalid
	 */
	protected void addPoint(GeneralPath[] currentPaths, int x, int y,
			boolean isInvalid) {
		...
		if (shapeName.equals("plus")) {
			...
			filledPath.moveTo(x - pointShapeSize.width / 2, y
					- pointShapeSize.height / 2);
			filledPath.lineTo(x + pointShapeSize.width / 2, y
					- pointShapeSize.height / 2);
			filledPath.lineTo(x + pointShapeSize.width / 2, y
					+ pointShapeSize.height / 2);
			filledPath.lineTo(x - pointShapeSize.width / 2, y
		...
		} else if (shapeName.equals("filledTriangle")
				|| shapeName.equals("triangle")) {

			int[] xPoints = {x, x + pointShapeSize.width / 2,
					x - pointShapeSize.width / 2};
			int[] yPoints = {y - pointShapeSize.height / 2,
					y + pointShapeSize.height / 2,
					y + pointShapeSize.height / 2};

			GeneralPath path = shapeName.equals("filledTriangle")
					? filledPath
					: drawnPath;
