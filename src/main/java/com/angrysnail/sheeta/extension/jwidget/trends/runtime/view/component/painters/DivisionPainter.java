...
/**
 * the class used to compute the division and subdivision values for the
 * graduations and the grid
 * 
...
public class DivisionPainter {
	 ...
	 */
	public static long[] possibleDivisionPeriods = {1L, 5L, 10L, 100L,
			250L,
			500L, // ms
			1000L, 5000L,
			10000L, // sec
			...
			86400000L,
			1296000000L, // day
			2592000000L, 7776000000L,
			15552000000L, // month
			31536000000L, 157680000000L, 315360000000L, 1576800000000L,
			3153600000000L // year
	};
	...
	public void initialize() {
		...
		configuration = componentsHandler.getView().getController()
				.getConfiguration();
		...
		if (configuration.displayHorizontalAxisTimeGraduations()
			...
			dateFormat = new SimpleDateFormat(
					configuration.getHorizontalAxisHorodatesFormat());
			...
			if (dateString != null) {
				...
				horodatesWidth = componentsHandler.getTrendsComponent()
						.getGraphics().getFontMetrics(configuration.getFont())
		...
		if (configuration.displayGrid()) {
			...
			float[] dashArray = TrendsCurveConfiguration
					.getDashArray(configuration.getVerticalLinesDivisionDash());

			if (dashArray != null) {

				verticalGridLinesDivisionStroke = new BasicStroke(1,
						BasicStroke.CAP_BUTT, BasicStroke.JOIN_BEVEL, 1.0f,
						dashArray, 0);
			}

			// getting the stroke of the vertical grid subdivision lines
			dashArray = TrendsCurveConfiguration.getDashArray(configuration
					.getVerticalLinesSubDivisionDash());

			if (dashArray != null) {

				verticalGridLinesSubDivisionStroke = new BasicStroke(1,
						BasicStroke.CAP_BUTT, BasicStroke.JOIN_BEVEL, 1.0f,
						dashArray, 0);
			}

			// if the grid division and subdivision lines should not be handled
			// automatically,
			// the value of their periods will be retrieved
			if (!configuration.isVerticalLinesDivisionAutomaticPeriod()) {

				verticalGridLinesDivisionDuration = configuration
						.getVerticalLinesDivisionDuration();
			}

			if (!configuration.isVerticalLinesSubDivisionAutomaticPeriod()) {

				verticalGridLinesSubDivisionDuration = configuration
						.getVerticalLinesSubDivisionDuration();
			}
		}
	}

	/**
	 * validates all the updates that have been done, so that the divisions can
	 * be refreshed
	 * 
	 * @param endDate
	 *            the date for the right part
	 * @param availableWidth
	 *            the available width for painting the graduations and the grid
	 *            vertical lines
	 */
	public void validateUpdates(long endDate, int availableWidth) {
		...
		if (configuration.displayHorizontalAxisTimeGraduations()
				...
				&& (configuration.isVerticalLinesDivisionAutomaticPeriod() || configuration
						.isVerticalLinesSubDivisionAutomaticPeriod())) {

			// getting the array of the division periods
			divisions = getDivisionPeriods(availableWidth,
					configuration.getHorizontalAxisDuration());
	 ...
	 *            a graphics object
	 * @param startDate
	 *            the last time the update happened
	 * @param endDate
	 *            the date for the right part
	 * @param topOffset
	 *            the top offset
	 ...
	 *            the left offset
	 * @param availableWidth
	 *            the available width for painting the graduations
	 * @param availableHeight
	 *            the available height
	 */
	public void paintGraduations(Graphics2D g, long startDate, long endDate,
			int topOffset, int leftOffset, int availableWidth,
			int availableHeight) {
		...
		if (divisions != null) {
			...
			// clearing the zone
			g.fillRect(leftOffset, topOffset, availableWidth
					+ ComponentsHandler.rightOffset, availableHeight);
			...
			if (horodatesWidth > 0) {
				...
				// computing the time on the curves component that the horodate
				// string should take
			...
			// getting the closest higher date of the current date corresponding
			// to a division
			long closestHigherDate = (long) Math.floor(endDate / divisions[0]
					+ 1)
					* divisions[0];
			...
			for (long time = closestHigherDate;; time -= divisions[0]) {
				...
				position = (int) Math.floor(availableWidth - (endDate - time)
						* availableWidth
						/ configuration.getHorizontalAxisDuration())
						+ leftOffset;
				...
				if (position >= leftOffset
					...
					for (int i = divisions.length - 1; i >= 0; i--) {
						...
						if ((time % divisions[i]) == 0) {
							...
							g.setColor((divisions[i] % horodateDivision == 0)
									? Color.black
									: Color.darkGray);
							g.drawLine(position, topOffset, position, topOffset
									+ graduationDivisionMinHeight + i * 2);
							break;
						}
					}
					...
					if (configuration.displayHorizontalAxisHorodates()
						...
						if (dateString != null && !dateString.equals("")) {
							...
							if (stringWidth > 0) {
								...
								// drawing the string
								g.drawString(dateString, position - stringWidth
										/ 2, topOffset
										+ horodateVerticalPosition);
							}
						}
					}
	 ...
	 *            a graphics object
	 * @param startDate
	 *            the date from which the lines should be painted
	 * @param endDate
	 *            the date for the right part
	 * @param leftOffset
	 *            the left offset
	 * @param availableWidth
	 *            the available width for painting the vertical grid lines
	 * @param availableHeight
	 *            the available height for painting the vertical grid lines
	 */
	public void paintGridVerticalLines(Graphics2D g, long startDate,
			long endDate, int leftOffset, int availableWidth,
			int availableHeight) {
		...
		if (divisions != null) {
			...
			// getting the closest higher date of the current date corresponding
			// to a division
			long closestHigherDate = (long) Math.floor(endDate
			...
			// getting the closest higher date of the current date corresponding
			// to a subdivision
			closestHigherDate = (long) Math.floor(endDate
					/ verticalGridLinesSubDivisionDuration)
					* verticalGridLinesSubDivisionDuration;

			// painting each vertical sub division line
			for (long time = closestHigherDate;; time -= verticalGridLinesSubDivisionDuration) {
	...
	protected long[] getDivisionPeriods(int availableWidth, long duration) {
		...
		if (duration > 0) {
			...
			// getting the first period of the table that could fit the given
			// duration
			int periodWidth = 0;
			...
			for (int index = 0; index < possibleDivisionPeriods.length; index++) {
				...
				periodWidth = (int) Math.floor(possibleDivisionPeriods[index]
						* ratio);
			...
			if (firstPeriodIndex != -1
					&& firstPeriodIndex < possibleDivisionPeriods.length) {

				// filling the array of the division periods that will be used
				currentDivisions = new long[5];

				for (int i = firstPeriodIndex; i < possibleDivisionPeriods.length
						&& (i - firstPeriodIndex) < currentDivisions.length; i++) {
