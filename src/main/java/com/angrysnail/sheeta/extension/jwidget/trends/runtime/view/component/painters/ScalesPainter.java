package com.angrysnail.sheeta.extension.jwidget.trends.runtime.view.component.painters;
...
import com.angrysnail.sheeta.extension.jwidget.trends.runtime.view.component.CurvesComponent;
...
public class ScalesPainter {
	 ...
	 */
	private Map<TrendsCurveConfiguration, double[]> curvesGraduations = new HashMap<TrendsCurveConfiguration, double[]>();
	 ...
	 *            the available height for drawing the curve
	 */
	public void initiateUpdate(Graphics2D g, int availableHeight) {
		...
		if (g != null) {
			...
			ComponentsHandler componentsHandler = curvesComponent
					.getComponentsHandler();
			LinkedList<TrendsCurveConfiguration> configs = curvesComponent
			...
			for (TrendsCurveConfiguration config : configs) {
				...
				if (config.displayScale()) {
					...
					verticalZoomFactor = config.getConfiguration()
							.getVerticalZoomFactor();

					if (config.isEnumeratedTag()) {

						// getting the array of the graduations
						graduations = getGraduations(minValue, maxValue,
								availableHeight,
								(int) (availableHeight * verticalZoomFactor),
								true);
						...
						if (graduations.length > 0) {
							...
							// getting the maximum size of the strings that will
							// be displayed
							for (String val : config.getEnumeratedTagValues()) {

								width = (int) g.getFontMetrics()
										.getStringBounds(val, g).getWidth();
							...
							scaleWidth = maxStringWidth + graduations.length
									* graduationSize + 2 + 5;
						...
						if (graduations.length > 0) {
							...
							g.setColor(config.getConfiguration()
									.getBackgroundColor());
							g.setFont(font);
							...
							// getting the width of the string representation of
							// the value
							// corresponding to the first graduations item
							maxStringWidth = 0;
							double[] values = {
									(int) (minValue + delta / 2)
											+ graduations[graduations.length - 1],
									maxValue, minValue,
									(int) (minValue + delta / 2)};
							valueStr = componentsHandler
									.getStringRepresentation(maxValue);

							// getting the maximum size of the strings that will
							// be displayed
							maxStringWidth = g.getFontMetrics().stringWidth(
									valueStr);

							for (double val : values) {

								valueStr = componentsHandler
										.getStringRepresentation(val);
								width = (int) g.getFontMetrics()
										.getStringBounds(valueStr, g)
										.getWidth();
	 ...
	 *            a graphics object
	 * @param topOffset
	 *            the top offset of the component of the curves
	 * @param availableHeight
	 *            the available height for drawing the curve
	 * @param gridAvailableWidth
	 *            the available width for drawing the grid
	 * @param gridLeftOffset
	 *            the left offset for drawing the grid
	 */
	public void validateUpdates(Graphics2D g, int topOffset,
			int availableHeight, int gridAvailableWidth, int gridLeftOffset) {
		...
		for (TrendsCurveConfiguration config : configs) {
			...
			if (config.displayScale()) {
				...
				paintScale(config, g, topOffset, availableHeight,
						currentXPosition, curvesScaleWidth[i],
						curvesGraduations.get(config),
						((paintedScaleNumber == (curveScaleNb - 1)) && config
								.getConfiguration().displayGrid()),
	 ...
	 *            the available height for drawing the curve
	 * @param xPosition
	 *            the position from which the scale should be painted
	 * @param scaleWidth
	 *            the width of this scale
	 * @param graduations
	 *            the array of the graduations
	 * @param paintHorizontalGridLines
	 *            whether or not to paint the horizontal grid lines
	 * @param gridAvailableWidth
	 *            the available width for drawing the grid
	 * @param gridLeftOffset
	 *            the left offset for drawing the grid
	...
	protected void paintScale(TrendsCurveConfiguration config, Graphics2D g,
			int topOffset, int availableHeight, int xPosition, int scaleWidth,
			double[] graduations, boolean paintHorizontalGridLines,
		...
		if (graduations != null && graduations.length > 0) {
			...
			if (lastGraduationItemValue < 1) {
				...
				factor = (int) Math.pow(10,
						Math.floor(-Math.log10(lastGraduationItemValue) + 1));
			}
			...
			int maxStringWidth = scaleWidth - normalizedGraduations.length
					* graduationSize - 2;
			...
			// getting the zoom factor
			double verticalZoomFactor = config.getConfiguration()
					.getVerticalZoomFactor();

			// getting the index in the graduations panel after which no grid
			...
			for (int i = 0; i < normalizedGraduations.length; i++) {
				...
				itemHeight = (int) Math
						.floor((availableHeight * verticalZoomFactor)
								* normalizedGraduations[i] / delta);
			...
			g.fillRect(xPosition, 0, scaleWidth, availableHeight + 2
					* topOffset);
			...
			g.drawLine(xPosition + scaleWidth - 1, topOffset, xPosition
					+ scaleWidth - 1, availableHeight + topOffset);
			...
			if (paintHorizontalGridLines) {
				...
				gridColor = new Color(color.getRed(), color.getGreen(),
						color.getBlue(), 150);
			}

			// getting the higher graduation item value
			double higherGraduationItemValue = ((int) (maxValue / lastGraduationItemValue))
					* lastGraduationItemValue;

			// getting the zoom origin
			double verticalZoomOrigin = config.getConfiguration()
					.getVerticalZoomOrigin();
			...
			for (int index = 0; value >= minValue; index--) {
				...
				value = higherGraduationItemValue + index
						* lastGraduationItemValue;
				...
				// getting the y position of the item corresponding to this
				// value
				y = (int) Math.round(((higherGraduationItemValue - value)
						* availableHeight / delta - verticalZoomOrigin)
						* verticalZoomFactor)
						+ topOffset;
				...
				if (y >= topOffset && y <= topOffset + availableHeight + 1) {
					...
					if (config.isEnumeratedTag()) {
						...
						strValue = config.getEnumeratedTagValues().get(
								config.getEnumeratedTagValues().size() - count
										- 1);
						...
						if (width >= maxStringWidth) {
							...
							g2.setClip(xPosition, 0, maxStringWidth
									- nextStringWidth, 2 * topOffset
									+ availableHeight);
							g2.drawString(strValue, xPosition, y + stringHeight
									/ 2);
							g2.dispose();
							g.drawString(next, xPosition + maxStringWidth
									- nextStringWidth, y + stringHeight / 2);

						} else {

							g.drawString(strValue, xPosition + maxStringWidth
									- width, y + stringHeight / 2);
						}
						...
						if (paintHorizontalGridLines) {
							...
							g.drawLine(gridLeftOffset, y, gridLeftOffset
									+ gridAvailableWidth
									+ ComponentsHandler.rightOffset, y);
						}

					} else {

						// getting the index of the type of the graduation item
						...
						for (int i = 0; i < normalizedGraduations.length; i++) {
							...
							if (value % normalizedGraduations[i] == 0) {
								...
								if (i < gridGraduationsIndex) {
									...
									// drawing the string representation of the
									// value
									Graphics g2 = g.create();
									g2.setClip(xPosition, 0, maxStringWidth,
											availableHeight + 2 * topOffset);
									g2.setColor(config.getColor());
									g2.drawString(strValue, xPosition
											+ maxStringWidth
											- (width > maxStringWidth
													? maxStringWidth
													: width), y + stringHeight
											/ 2);
									g2.dispose();
					...
					x2 = xPosition + maxStringWidth
							+ normalizedGraduations.length * graduationSize;
	 ...
	 * 
	 * @param minValue
	 *            the min value for the graduation
	 * @param maxValue
	 *            the max value for the graduation
	 * @param baseHeight
	 *            the unscaled height
	 * @param availableHeight
	 *            the available height
	 * @param useOnlyInteger
	 *            whether the only graduation items that should be kept are
	 *            integers
	...
	protected double[] getGraduations(double minValue, double maxValue,
		...
		double[] dividers = {2, 4, 8, 10, 20, 50, 100, 200, 500, 1000, 2000,
		...
		do {
			...
			if (currentNumber == Math.floor(currentNumber)
					|| (!useOnlyInteger && currentNumber < 1)) {

				itemHeight = (int) Math.floor(availableHeight * currentNumber
						/ delta);
