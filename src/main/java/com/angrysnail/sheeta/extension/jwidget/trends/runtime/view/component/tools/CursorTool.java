...
public class CursorTool extends JToggleButton {
			...
			public void actionPerformed(ActionEvent evt) {
				...
				if (isSelected()) {
					...
					if (configuration.getCurrentMode() == TrendsConfiguration.REAL_TIME_MODE
						...
						configuration
								.setCurrentSubMode(TrendsConfiguration.SCROLL);
					}
	...
	public void clear() {
		...
		if (!cleared) {
			...
			// repainting the curves component
			componentsHandler.getCurvesComponent().repaint(
					vlinePainter.getBounds());

			if (isCross) {

				componentsHandler.getCurvesComponent().repaint(
						hlinePainter.getBounds());
			}
	 ...
	 * @author ITRIS, Jordi SUC
	 */
	protected class CursorToolListener extends MouseAdapter
			implements
				MouseMotionListener {
		...
		public void mouseReleased(MouseEvent evt) {
			...
			boolean allowAction = componentsHandler.getCurrentExclusiveAction()
					.equals(id);
			...
			if (!allowAction) {
				...
				if (curvesBounds != null && evt.getX() >= curvesBounds.x
					...
					long horodate = computeHorodate(evt.getPoint(),
							curvesBounds);
		...
		protected void drawLine(Point point, Rectangle curvesBounds) {
			...
			if (point != null) {
				...
				vlinePainter.getPath().lineTo(point.x,
						componentsHandler.getCurvesComponent().getHeight());
				...
				if (isCross) {
					...
					hlinePainter.getPath().lineTo(
							curvesBounds.x + curvesBounds.width, point.y);
		 ...
		 *            a point
		 * @param curvesBounds
		 *            the bounds of the curves panel
		...
		protected long computeHorodate(Point point, Rectangle curvesBounds) {
			...
			long startDate = componentsHandler.getView().getCurrentEndDate()
					- currentDuration;

			// getting the horodate corresponding to the point
			horodate = startDate
					+ (long) Math.floor((point.x - curvesBounds.x)
		...
		protected void requestTagValue(long horodate) {
			...
			if (configuration.getTrendsCurveConfiguration() != null) {
				...
				componentsHandler
						...
						.getModel()
						.requestTagValue(
								configuration.getTrendsCurveConfiguration()
										.getTagName(), horodate);
			}
		}
		...
		protected void removeListener() {
			...
			componentsHandler.getCurvesComponent().removeMouseListener(
					cursorToolListener);
	...
	protected class TextCursorPainter extends AnyPainter {
		...
		/**
		 * the location of the rectangle that will be painted, i.e. the mouse
		 * point
		 */
		...
		public void setValueString(Object value) {
			...
			if (value != null) {
				...
				if (value instanceof String) {
					...
					valueString = componentsHandler
							.getStringRepresentation((Double) value);
				}
			}
		}

		/**
		 * sets the location of the rectangle that will be painted, i.e. the
		 * mouse point
		 * 
		 * @param location
		 *            the new location
		...
		public void validatePath() {
			...
			// computing the bounds of the rectangle and the location of the
			// strings to be painted
			if (drawVal) {

				width += Math.max(fm.stringWidth(horodateString),
						fm.stringWidth(valueString));
		...
		public void paint(Graphics2D g) {
			...
			if (path != null) {
				...
				g.drawString(horodateString, location.x
						+ relativeHorodateLocation.x, location.y
						+ relativeHorodateLocation.y);

				if (drawVal) {

					g.drawString(valueString, location.x
							+ relativeValueLocation.x, location.y
							+ relativeValueLocation.y);
				}
