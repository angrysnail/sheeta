...
import com.angrysnail.sheeta.extension.jwidget.trends.runtime.view.component.painters.AnyPainter;
...
public class MouseZoomTool extends JButton {
	...
	public MouseZoomTool(ToolBarComponent toolBar) {
		...
		this.configuration = toolBar.getComponentsHandler().getView()
	...
	protected void build() {
		...
		listener = new ActionListener() {
			...
			public void actionPerformed(ActionEvent evt) {
				...
				componentsHandler.getCurvesComponent().addMouseListener(
						mouseZoomToolListener);
				componentsHandler.getCurvesComponent().addMouseMotionListener(
						mouseZoomToolListener);
			}
		};
	 ...
	 * @author ITRIS, Jordi SUC
	 */
	protected class MouseZoomToolListener extends MouseAdapter
			implements
				MouseMotionListener {
		...
		public void mousePressed(MouseEvent evt) {
			...
			if (componentsHandler.getCurrentAction().equals(id)) {
				...
				Rectangle curvesBounds = componentsHandler.getCurvesComponent()
						.getCurvesZoneBounds();

				// normalizing the value of the coordinates
				currentFirstPoint = getNormalizedPoint(evt.getPoint(),
						curvesBounds);

				componentsHandler.getCurvesComponent().addAnyPainter(
						firstLinePainter);
				componentsHandler.getCurvesComponent().addAnyPainter(
						secondLinePainter);
				componentsHandler.getCurvesComponent().addAnyPainter(
		...
		public void mouseReleased(MouseEvent evt) {// TODO
			...
			if (currentFirstPoint != null) {
				...
				// checking if the duration is longer than the delta between the
				// first and the last
				// date
				long newDuration = 0;
				...
				long currentDuration = configuration
						.getHorizontalAxisDuration();
				long previousStartDate = componentsHandler.getView()
						.getCurrentEndDate() - currentDuration;
				...
				if (componentsHandler.getHorizontalBarComponent() != null) {
					...
					date = new Date(componentsHandler
							.getHorizontalBarComponent().getStartDate());

				} else {

					date = new Date(componentsHandler.getView().getController()
							.getModel().getFirstDate());
				}
				...
				long firstClickedDate = previousStartDate
						+ (long) Math.floor((x1 - curvesBounds.x)
								* currentDuration / curvesBounds.width);

				long secondClickedDate = previousStartDate
						+ (long) Math.floor((x2 - curvesBounds.x)
								* currentDuration / curvesBounds.width);
				...
				double verticalZoomFactor = curvesBounds.height
						/ Math.abs(scaledYSecondPoint - scaledYFirstPoint);
				double verticalZoomOrigin = scaledYFirstPoint < scaledYSecondPoint
						? scaledYFirstPoint
						: scaledYSecondPoint;
				...
				componentsHandler.getCurvesComponent().removeAnyPainter(
						firstLinePainter);
				componentsHandler.getCurvesComponent().removeAnyPainter(
						secondLinePainter);
				componentsHandler.getCurvesComponent().removeAnyPainter(
				...
				componentsHandler.getView()
						.setCurrentEndDate(secondClickedDate);
		...
		public void mouseDragged(MouseEvent evt) {
			...
			if (currentFirstPoint != null) {
				...
				drawLine(firstLinePainter, currentFirstPoint, new Point(
						currentFirstPoint.x, point.y));
				drawLine(secondLinePainter, new Point(currentFirstPoint.x,
						point.y), point);
				drawLine(thirdLinePainter, point, new Point(point.x,
						currentFirstPoint.y));
				drawLine(fourthLinePainter, new Point(point.x,
						currentFirstPoint.y), currentFirstPoint);
			}
		}

		/**
		 * returns the point corresponding to the given point but that is
		 ...
		 * @param bounds
		 *            the bounds
		 * @return the point corresponding to the given point but that is
		 *         bounded by the given rectangle
		 */
		 ...
		 *            the second point
		 */
		protected void drawLine(AnyPainter anyPainter, Point firstPoint,
				Point secondPoint) {
		...
		protected void removeListener() {
			...
			componentsHandler.getCurvesComponent().removeAnyPainter(
					secondLinePainter);
			componentsHandler.getCurvesComponent().removeMouseListener(this);
			componentsHandler.getCurvesComponent().removeMouseMotionListener(
					this);
		}
	}
}
