...
/**
 * the class of the tool used to choose the displayed time on the horizontal
 * axis
 * 
...
public class TimeZoomDialogTool extends JButton {
	...
	public void initialize() {
		...
		if (topLevelAncestor instanceof Frame) {
			...
			dialog = new DurationChooserDialog(componentsHandler.getView()
					...
					.getCanvas().getPicture().getMainDisplay()
					.getTopLevelFrame());
	...
	protected class DurationChooserDialog extends TitledDialog {
		...
		protected JPanel buildContentPanel() {
			...
			// getting the labels
			String title = TrendsBundle.bundle
					.getString("TimeZoomDialogWidget");
			String informationMessage = TrendsBundle.bundle
					.getString("TimeZoomDialogWidgetMessage");
			String durationLabel = TrendsBundle.bundle
					.getString("TimeZoomDialogWidgetTime");
			final String tooLongLabel = TrendsBundle.bundle
					.getString("TimeZoomDialogWidgetTimeTooLong");
			...
			ActionListener widgetListener = new ActionListener() {
				...
				public void actionPerformed(ActionEvent evt) {
					...
					long chosenTime = TrendsConfiguration
							.getTime(timeChooserWidget.getValue());
			...
			ActionListener buttonsListener = new ActionListener() {
				...
				public void actionPerformed(ActionEvent evt) {
					...
					if (evt.getSource().equals(okButton)) {
						...
						configuration
								.setHorizontalAxisDuration(timeChooserWidget
										.getValue());
					}
