...
public class Toolkit {
	 ...
	 */
	public static final String jwidgetElementName = com.angrysnail.sheeta.library.Toolkit.rtdaPrefix
			+ "jwidget";
	 ...
	 */
	public static final String jwidgetChildElementName = com.angrysnail.sheeta.library.Toolkit.rtdaPrefix
			+ "property";
	...
	/**
	 * the name of the attribute contained in an image node, meaning that the
	 * image represents an inserted widget
	 */
	 ...
	 */
	public static final String[] properties = {"fill", "stroke", "opacity",
			"visibility", "stroke-opacity", "stroke-width", "stroke-dasharray",
			"stroke-dashoffset", "stroke-linecap", "stroke-linejoin",
			"stroke-miterlimit", "fill-opacity", "marker-start", "marker-mid",
			"marker-end", "font-family", "font-size", "font-weight",
			"font-style", "font-stretch", "letter-spacing", "word-spacing",
			"text-decoration", "writing-mode", "glyph-orientation-horizontal",
			"text-anchor"};
	...
	public static boolean isDocumentAView(Document doc) {
		...
		if (doc != null) {
			...
			if (root != null) {
				...
				String att1 = root.getAttribute("referenceFile"), att2 = root
						.getAttribute("referenceNode");

				if (att1 != null && !att1.equals("") && att2 != null
						&& !att2.equals("")) {
	...
	public static boolean isJWidgetElement(Element element) {
		...
		if (element != null
				&& element.getNodeName().equals(Toolkit.jwidgetElementName)) {
	...
	public static boolean containsRtdaElements(Element element) {
		...
		if (element.getNodeName().startsWith(rtdaPrefix)
				|| (element.getNodeName().equals(imageNodeName) && (element
						.hasAttribute(viewAtt) || element
						.hasAttribute(widgetAttribute)))) {
			...
			for (NodeIterator iterator = new NodeIterator(element); iterator
					.hasNext();) {
	 ...
	 */
	public static LinkedList<Element> getChildrenElements(Element parentElement) {
	...
	public static void setStyleProperty(Element element, String name,
		...
		if (element != null && name != null && !name.equals("")) {
			...
			if (rg != -1) {
				...
				// computing the first part of the value before the current
				// property
				...
				if (nextPos != -1) {
					...
					secondPart = styleValue.substring(nextPos + 1,
							styleValue.length()).trim();
