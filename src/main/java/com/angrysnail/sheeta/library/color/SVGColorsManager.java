package com.angrysnail.sheeta.library.color;
...
public class SVGColorsManager {
	...
	static {
		...
		try {
			...
			doc = docBuild.parse(SVGColorsManager.class
					.getResourceAsStream("xml/svgColors.xml"));
		} catch (Exception ex) {
		}
	...
	protected static void retrieveW3CColors(Document svgColorsDocument) {
		...
		if (svgColorsDocument != null
				&& svgColorsDocument.getDocumentElement() != null) {
			...
			// the list
			for (Node cur = svgColorsDocument.getDocumentElement()
					.getFirstChild(); cur != null; cur = cur.getNextSibling()) {

				if (cur instanceof Element
						&& cur.getNodeName().equals("svgColor")) {
	...
	public static Color getColor(String colorString) {
		...
		if (color == null) {
			...
			if (color == null && colorString.length() == 7) {
				...
				try {
					r = new Integer(tmp.substring(0, tmp.indexOf(",")))
							.intValue();
					tmp = tmp.substring(tmp.indexOf(",") + 1, tmp.length());

					g = new Integer(tmp.substring(0, tmp.indexOf(",")))
							.intValue();
					tmp = tmp.substring(tmp.indexOf(",") + 1, tmp.length());
