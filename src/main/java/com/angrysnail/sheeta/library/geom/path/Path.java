package com.angrysnail.sheeta.library.geom.path;
...
import com.angrysnail.sheeta.library.geom.path.segment.ArcToSegment;
import com.angrysnail.sheeta.library.geom.path.segment.ClosePathSegment;
import com.angrysnail.sheeta.library.geom.path.segment.CubicToSegment;
import com.angrysnail.sheeta.library.geom.path.segment.LineToSegment;
import com.angrysnail.sheeta.library.geom.path.segment.MoveToSegment;
import com.angrysnail.sheeta.library.geom.path.segment.QuadraticToSegment;
import com.angrysnail.sheeta.library.geom.path.segment.Segment;
...
public class Path implements Shape {
	...
	/**
	 * the constant corresponding to the case when no arc can be found in the
	 * path
	 */
	public static final int REGULAR_PATH = 0;

	/**
	 * the constant corresponding to the case when one or more arcs can be found
	 * in the path
	 */
	public static final int PATH_WITH_ARC = 1;

	/**
	 * the constant corresponding to the case when only a moveTo and an arcTo
	 * instructions can be found in the path
	 */
	public static final int ARC_PATH = 2;

	/**
	 * the comma separator that could be used to separate the two coordinates of
	 * a point
	 */
	 ...
	 * 
	 * @param aPath
	 *            a path from which the new path will be initialized
	 */
	 ...
	 * 
	 * @param pathShape
	 *            a path shape from which the new path will be initialized
	 */
	...
	/**
	 * @return one of the following constants : REGULAR_PATH, PATH_WITH_ARC,
	 *         ARC_PATH
	 */
	...
	/**
	 * creates and returns the segments that define the path represented by the
	 * provided string
	 * 
	 * @param pointsString
	 *            the string representation of a path
	 * @return the segments that define the path represented by the provided
	 *         string
	 */
	...
	/**
	 * returns the root segment of the list of the segments corresponding to the
	 * provided path
	 * 
	 * @param thePath
	 *            a path
	 * @return the root segment of the list of the segments corresponding to the
	 *         provided path
	 */
	protected Segment getSegments(ExtendedGeneralPath thePath) {
		...
		if (firstSegment instanceof MoveToSegment) {
			...
			if (secondSegment != null && secondSegment instanceof ArcToSegment
	 ...
	 * 
	 * @param index
	 *            the index of the point of the segment after which a segment
	 *            should be added
	 */
	public void addSegment(int index) {
		...
		if (segment != null) {
			...
			if (segment.getNextSegment() != null
				...
				newPoint = new Point2D.Double(endPoint.getX() + 10,
						endPoint.getY());
			}
			...
			if (segment instanceof QuadraticToSegment) {
				...
				// getting the symetric point of the control point of the
				// previous segment
	 ...
	 * 
	 * @param index
	 *            the index of a point of the segment to be removed
	 */
	...
	public void insertBefore(ExtendedGeneralPath insertedPathShape) {
		...
		// adding all the segments of the current path shape but the first
		// moveTo segment
	...
	/**
	 * inserts all the segments of the pathToAdd into the receiverPath but the
	 * first segment
	 * 
	 * @param pathToAdd
	 *            the path whose segments will be inserted into the receiver
	 *            path
	 * @param receiverPath
	 *            the path that will receive new segments from the pathToAdd
	 */
	protected void insertAllButFirst(ExtendedGeneralPath pathToAdd,
			ExtendedGeneralPath receiverPath) {
		...
		while (!pathIt.isDone()) {
			...
			switch (type) {
				...
				case ExtendedPathIterator.SEG_QUADTO :
					...
					receiverPath.quadTo(values[0], values[1], values[2],
							values[3]);
					break;

				case ExtendedPathIterator.SEG_CUBICTO :

					receiverPath.curveTo(values[0], values[1], values[2],
							values[3], values[4], values[5]);
					break;

				case ExtendedPathIterator.SEG_ARCTO :

					receiverPath.arcTo(values[0], values[1], values[2],
							values[3] == 1, values[4] == 1, values[5],
							values[6]);
					break;
		...
		while (segment != null) {
			...
			if (segment instanceof LineToSegment) {
				...
				reversedPath.curveTo((int) segment.getControlPoint().getX(),
						(int) segment.getControlPoint().getY(), (int) segment
								.getOtherControlPoint().getX(), (int) segment
								.getOtherControlPoint().getY(),
						(int) previousSegment.getEndPoint().getX(),
						(int) previousSegment.getEndPoint().getY());
			}
	...
	/**
	 * @return returns whether this path could apply a transform to its points
	 *         without using the "transform" attribute
	 */
	...
	/**
	 * applies the provided transform to the path if and only if the type of the
	 * path is REGULAR_PATH
	 * 
	 * @param transform
	 *            the new transform to apply to the path
	 */
	 ...
	 * if(rootSegment!=null){
	 * 
	 * Segment currentSeg=rootSegment; int foundType=REGULAR_PATH;
	 * 
	 * //checking if the type is ARC_PATH if(rootSegment instanceof
	 * MoveToSegment){
	 * 
	 * //getting the second segment Segment
	 * secondSegment=currentSeg.getNextSegment();
	 * 
	 * if(secondSegment!=null && secondSegment instanceof ArcToSegment &&
	 * secondSegment.getNextSegment()!=null){
	 * 
	 ...
	 * ExtendedPathIterator pathIt=path.getExtendedPathIterator(); double[]
	 * valuesArray=new double[7]; int type; Segment firstSegment=null; Segment
	 * segment; Segment previousSegment=null;
	 * 
	 * while(! pathIt.isDone()){
	 * 
	 * //getting the values for the current segment
	 ...
	 * previousSegment.setNextSegment(segment); }
	 * 
	 * previousSegment=segment; pathIt.next(); }
	 * 
	 * //creating the string representation of the list of the segments
	 * StringBuffer strRepr=new StringBuffer(); Segment currentSeg=firstSegment;
	 * 
	 * while(currentSeg!=null){
	 * 
	 * strRepr.append(currentSeg.toString());
	...
	/**
	 * returns the segment corresponding to the provided type and having the
	 * provided values
	 * 
	 * @param type
	 *            the type of a segment
	 * @param valuesArray
	 *            the array values for the segment
	 * @param previousSegment
	 *            the segment before the segment that will be returned
	 * @return the segment corresponding to the provided type and having the
	 *         provided values
	 */
