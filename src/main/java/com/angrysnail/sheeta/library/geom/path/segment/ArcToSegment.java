package com.angrysnail.sheeta.library.geom.path.segment;
...
public class ArcToSegment extends Segment {
	 ...
	 * @param values
	 *            the array of the values defining the segment
	 */
	public ArcToSegment(Segment previousSegment, double[] values) {
		...
		if (values != null && values.length >= 7) {
			...
			ArcData arcData = new ArcData(new Point2D.Double(values[0],
					values[1]), values[2], values[3] == 1, values[4] == 1,
	 ...
	 *            the segment that lies just before this segment
	 * @param segmentString
	 *            the string defining the segment
	 */
	...
	protected void parseString(String segmentString) {
		...
		if (valuesArray != null && valuesArray.length > 0) {
			...
			for (int i = 0; i < valuesArray.length; i += 7) {
				...
				theEndPoint = new Point2D.Double(valuesArray[i + 5],
						valuesArray[i + 6]);

				if (isRelative && previousSegment != null) {

					// converting the relative end point into an absolute one
					theEndPoint = computeAbsolute(theEndPoint,
							previousSegment.getEndPoint());
				}

				// creating the arc data object
				arcData = new ArcData(radii, xAxisRotation, largeArcFlag,
						sweepFlag, theEndPoint);
	...
	public void fillPath(ExtendedGeneralPath path) {
		...
		for (ArcData arcData : data) {
			...
			path.arcTo((float) arcData.getRadii().getX(), (float) arcData
					.getRadii().getY(), (float) arcData.getXAxisRotation(),
					arcData.getLargeArcFlag(), arcData.getSweepFlag(),
					(float) arcData.getEndPoint().getX(), (float) arcData
							.getEndPoint().getY());
		}
	...
	public class ArcData {
		...
		public String toString() {
			...
			String rep = absoluteCmdName + FormatStore.format(radii.getX())
