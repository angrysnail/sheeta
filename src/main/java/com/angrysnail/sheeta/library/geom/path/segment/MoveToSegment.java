...
public class MoveToSegment extends Segment {
	...
	protected void parseString(String segmentString) {
		...
		java.util.List<Point2D> pointsList = getPoints(segmentString.substring(
				1, segmentString.length()));

		if (pointsList.size() > 0) {

			point = pointsList.get(0);

			// if the segment is relative, it is transformed into an absolute
			// one
	...
	public String toString() {
		...
		return absoluteCmdName + FormatStore.format(point.getX()) + " "
