...
public abstract class Segment {
	...
	public Segment(Segment previousSegment) {
		...
		this.segmentIndex = (previousSegment != null ? previousSegment
				.getSegmentIndex() + 1 : 0);
	}
	...
	protected java.util.List<Point2D> getPoints(String pointsString) {
		...
		if (coordinatesArray != null && coordinatesArray.length > 0
			...
			for (int i = 0; i < coordinatesArray.length; i += 2) {
				...
				pointsList.add(new Point2D.Double(coordinatesArray[i],
	...
	public static Point2D computeSymetric(Point2D originalPoint, Point2D center) {
		...
		return new Point2D.Double(center.getX()
				- (originalPoint.getX() - center.getX()), center.getY()
	 ...
	 *         provided points
	 */
	public static Point2D computeSymetricRelCenter(Point2D originalPoint,
	 ...
	 */
	public static Segment createSegment(Segment previousSegment,
		...
		switch (command) {
			...
			case 'h' :
				...
				segment = new HorizontalLineToSegment(previousSegment,
						segmentString);
				break;

			case 'v' :

				segment = new VerticalLineToSegment(previousSegment,
						segmentString);
				break;
			...
			case 's' :
				...
				segment = new SmoothCubicToSegment(previousSegment,
						segmentString);
				break;
			...
			case 't' :
				...
				segment = new SmoothQuadraticToSegment(previousSegment,
						segmentString);
				break;
