...
public class SmoothQuadraticToSegment extends Segment {
	 ...
	 */
	public SmoothQuadraticToSegment(Segment previousSegment,
	...
	protected void parseString(String segmentString) {
		...
		java.util.List<Point2D> thePointsList = getPoints(segmentString
				.substring(1, segmentString.length()));
		...
		if (thePointsList.size() > 0) {
			...
			// adding the first control point for each sub segment of the
			// segment//
			...
			for (int i = 0; i < pointsList.size(); i++) {
				...
				if (i == 0) {
					...
					firstControlPoint = isPreviousCubicSegment
							? previousSegment.getControlPoint()
				...
				firstControlPoint = computeSymetric(firstControlPoint,
						centerPoint);
	...
	protected boolean isCubicSegment(Segment segment) {
		...
		return (segment != null && (segment instanceof CubicToSegment || segment instanceof SmoothQuadraticToSegment));
	}
	...
	public String toString() {
		...
		for (int i = 0; i < pointsList.size(); i += 2) {
			...
			rep += absoluteCmdName + FormatStore.format(p2.getX()) + " "
