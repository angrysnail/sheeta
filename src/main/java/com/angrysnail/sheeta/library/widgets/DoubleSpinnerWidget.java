...
public class DoubleSpinnerWidget extends Widget {
	 ...
	 * @param borderless
	 *            whether the spinner should have borders
	 */
	public DoubleSpinnerWidget(double defaultValue, double min, double max,
			double step, boolean borderless) {
	...
	protected void build(double min, double max, double step) {
		...
		SpinnerNumberModel spinnerModel = new SpinnerNumberModel(min, min, max,
				step);
		spinner = new JSpinner(spinnerModel);
		numberEditor = new JSpinner.NumberEditor(spinner, spinnersFormat);
		spinner.setEditor(numberEditor);
		spinnerEditor = ((JSpinner.NumberEditor) spinner.getEditor())
				.getTextField();
		...
		changeListener = new ChangeListener() {
			...
			public void stateChanged(ChangeEvent e) {
				...
				try {
					spinnerValue = Double.parseDouble(spinner.getValue()
							.toString());
				} catch (Exception ex) {
				}
