package com.angrysnail.sheeta.library.widgets;
...
public class PointStyleChooserWidget extends Widget {
	 ...
	 */
	public static final String[] pointStyles = {"", "plus", "cross", "disc",
			"circle", "square", "rect", "filledTriangle", "triangle"};
		...
		comboListener = new ActionListener() {
			...
			public void actionPerformed(ActionEvent evt) {
				...
				if (combo.getSelectedItem() != null) {
					...
					currentValue = ((ComboItem) combo.getSelectedItem())
							.getValue();
				}
			...
			JPanel panel = new JPanel() {
				...
				protected void paintComponent(Graphics g) {
					...
					Point location = new Point(
							(getWidth() - pointShapeSize.width) / 2,
							(getHeight() - pointShapeSize.height) / 2);

					if (pointType.equals("plus")) {

						g2.drawLine(location.x + pointShapeSize.width / 2,
								location.y, location.x + pointShapeSize.width
										/ 2, location.y + pointShapeSize.height);

						g2.drawLine(location.x, location.y
								+ pointShapeSize.height / 2, location.x
								+ pointShapeSize.width, location.y
								+ pointShapeSize.height / 2);

					} else if (pointType.equals("cross")) {

						g2.drawLine(location.x, location.y, location.x
								+ pointShapeSize.width, location.y
								+ pointShapeSize.height);

						g2.drawLine(location.x + pointShapeSize.width,
								location.y, location.x, location.y
										+ pointShapeSize.height);

					} else if (pointType.equals("disc")) {

						g2.fillOval(location.x, location.y,
								pointShapeSize.width, pointShapeSize.height);
						// g2.drawOval(location.x, location.y,
						// pointShapeSize.width,
						// pointShapeSize.height);
						...
						g2.fillRect(location.x, location.y,
								pointShapeSize.width, pointShapeSize.height);
						// g2.drawRect(location.x, location.y,
						// pointShapeSize.width,
						// pointShapeSize.height);
					...
					} else if (pointType.equals("filledTriangle")
							|| pointType.equals("triangle")) {

						int[] xPoints = {location.x + pointShapeSize.width / 2,
								location.x + pointShapeSize.width, location.x};
						int[] yPoints = {location.y,
