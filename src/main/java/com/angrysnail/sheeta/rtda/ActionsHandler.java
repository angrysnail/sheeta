...
/**
 * the class handling the actions that can be found over canvases or other
 * components
 * 
...
public class ActionsHandler {
	 ...
	 */
	private static final Cursor activeZoneCursor = Cursor
			.getPredefinedCursor(Cursor.HAND_CURSOR), defaultCursor = Cursor
	 ...
	 * the font
	 */
	public static final Font theFont = new Font("theFont", Font.ROMAN_BASELINE,
			12);
	...
	/**
	 * the map associating a svg picture to a map associating an element to its
	 * related actions
	 */
	private Map<SVGPicture, Map<Element, LinkedList<Action>>> pictureToElementToActions = new HashMap<SVGPicture, Map<Element, LinkedList<Action>>>();

	/**
	 * the map associating a canvas to a set of key actions
	 */
	private Map<SVGPicture, LinkedList<Action>> pictureToKeyActions = new HashMap<SVGPicture, LinkedList<Action>>();
	...
	public void addAction(Action action) {
		...
		if (action != null && action.getActionComponent() instanceof SVGCanvas
			...
			SVGEventsManager eventsManager = (SVGEventsManager) action
					.getEventsManager();

			if (eventsManager.isKeyAction()) {

				// if the action is a key action, it is added to the map of the
				// key actions
				...
				Map<Element, LinkedList<Action>> elementToActions = pictureToElementToActions
						.get(picture);
				...
				LinkedList<Action> actions = elementToActions.get(action
						.getActionObject());
				...
				if (actions == null) {
					...
					actions = new LinkedList<Action>();
					elementToActions.put((Element) action.getActionObject(),
							actions);
				}
	...
	public void removeAction(Action action) {
		...
		if (action != null && action.getActionComponent() instanceof SVGCanvas
			...
			if (eventsManager.isKeyAction()) {
				...
				// if the action is a key action, it is removed from the map of
				// the key actions
				LinkedList<Action> actions = pictureToKeyActions.get(picture);
				...
				if (elementToActions != null) {
					...
					if (actions != null) {
						...
						if (actions.size() == 0) {
							...
							action.getActionComponent().removeMouseListener(
									eventsListener);
							action.getActionComponent()
									.removeMouseMotionListener(eventsListener);
						}
					}
				}
			}
	...
	/**
	 * the class of the listener of the events that can be made on registered
	 * components
	 * 
	 * @author ITRIS, Jordi SUC
	 */
	protected final class EventsListener extends MouseMotionAdapter
			implements
				MouseListener,
				KeyListener {
		...
		/**
		 * the label defining that the action is not entitled and/or not
		 * authorized
		 */
		protected String notAuthorizedLabel = "", notEntitledLabel = "",
				notEntitledAndnotAuthorizedLabel = "";
		...
		protected EventsListener() {
			...
			modifiersMap.put(InputEvent.ALT_GRAPH_MASK,
					InputEvent.ALT_GRAPH_DOWN_MASK);
			modifiersMap.put(InputEvent.ALT_MASK, InputEvent.ALT_DOWN_MASK);
			modifiersMap.put(InputEvent.BUTTON1_MASK,
					InputEvent.BUTTON1_DOWN_MASK);
			modifiersMap.put(InputEvent.BUTTON2_MASK,
					InputEvent.BUTTON2_DOWN_MASK);
			modifiersMap.put(InputEvent.BUTTON3_MASK,
			...
			// getting the labels
			notEntitledLabel = RtdaResources.bundle
					.getString("tooltip_notEntitled");
			notAuthorizedLabel = RtdaResources.bundle
					.getString("tooltip_notAuthorized");
			notEntitledAndnotAuthorizedLabel = RtdaResources.bundle
					.getString("tooltip_notEntitledAndnotAuthorized");
		}
		 ...
		 * @param mousePosition
		 *            the mouse position while the event occured
		 * @param eventType
		...
		protected void performMouseAction(JComponent component,
			...
			if (canListen()) {
				...
				if (element != null) {
					...
					Element rootElement = element.getOwnerDocument()
							.getDocumentElement();
				...
				if (allActions.size() > 0) {
					...
					if (eventType == SVGEventsManager.MOUSE_MOVE) {
						...
						for (Action action : allActions) {
							...
							eventsManager = (SVGEventsManager) action
									.getEventsManager();

							if (eventsManager.isEvent(eventType, modifiers, -1)) {

								eventsManager.performAction(eventType,
										modifiers, -1);
							}

							i++;
							cannotExecuteAction = !action.isAuthorized()
									|| !action.isEntitled();
							...
							tooltipText += (cannotExecuteAction
						...
						if (!tooltipText.equals("")) {
							...
							tooltipText = "<html><body>" + tooltipText
									+ "</body></html>";
		 ...
		 */
		protected void performKeyAction(JComponent component, int eventType,
				int modifiers, int eventKey) {
			...
			if (canListen()) {
				...
				LinkedList<Action> actions = pictureToKeyActions
						.get(((SVGCanvas) component).getPicture());
				SVGEventsManager eventsManager = null;
				...
				for (Action action : actions) {
					...
					if (eventsManager.isEvent(eventType, modifiers, eventKey)) {
						...
						eventsManager.performAction(eventType, modifiers,
								eventKey);
					}
				}
			}
		...
		public void mouseMoved(MouseEvent evt) {
			...
			if (canListen()) {
				...
				performMouseAction((JComponent) evt.getSource(),
						evt.getPoint(), SVGEventsManager.MOUSE_MOVE,
						evt.getModifiers());
			}
		}
		...
		public void keyPressed(KeyEvent evt) {
			...
			performKeyAction((JComponent) evt.getSource(),
					SVGEventsManager.KEY_DOWN, evt.getModifiers(),
					evt.getKeyCode());
		}
		...
		public void keyReleased(KeyEvent evt) {
			...
			performKeyAction((JComponent) evt.getSource(),
					SVGEventsManager.KEY_UP, evt.getModifiers(),
					evt.getKeyCode());
		}
		...
		public void keyTyped(KeyEvent evt) {
			...
			performKeyAction((JComponent) evt.getSource(),
					SVGEventsManager.KEY_TYPED, evt.getModifiers(),
					evt.getKeyCode());
		}
		...
		protected boolean canListen() {
			...
			return !(animationsHandler.isStopped() || animationsHandler
					.isPaused());
		}
	}
}
