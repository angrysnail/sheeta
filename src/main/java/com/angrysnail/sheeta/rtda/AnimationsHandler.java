...
import com.angrysnail.sheeta.rtda.animaction.ListenerAction;
import com.angrysnail.sheeta.rtda.components.picture.SVGCanvas;
...
import com.angrysnail.sheeta.rtda.rtdp.RealTimeDataProvider;
import com.angrysnail.sheeta.rtda.rtdp.RealTimeDataProviderTestSimulator;
import com.angrysnail.sheeta.rtda.rtdp.ServerRealTimeDataProvider;
import com.angrysnail.sheeta.rtda.test.TestTagInformation;
import com.angrysnail.sheeta.rtda.toolkit.TagToolkit;

/**
 * the class of the animations handler that will animate nodes by means of the
 * values it receives from the rtdp, the functions handler, the blinking
 * handler, ...
 * 
...
public class AnimationsHandler {
	 ...
	 */
	private Set<AnimationsStateListener> animationsStateListeners = new CopyOnWriteArraySet<AnimationsStateListener>();

	/**
	 * the set of the listeners to the requests of the display of views
	 */
	private Set<RequireViewDisplayListener> requireViewDisplayListeners = new CopyOnWriteArraySet<RequireViewDisplayListener>();

	/**
	 * the list of all the data changed listeners and actions
	 */
	private List<ListenerAction> allListenersAndActions = new CopyOnWriteArrayList<ListenerAction>();
	 ...
	 */
	private ConcurrentHashMap<String, Integer> dataNameToTypeMap = new ConcurrentHashMap<String, Integer>();

	/**
	 * the map associating an insensitive case tag to a sensitive case tag
	 */
	private Map<String, String> insensitiveToSensitiveMap = new ConcurrentHashMap<String, String>();

	/**
	 * the map associating the name of a data to its value, this map stores all
	 * the tags that are handled with theur current value
	 */
	private Map<String, Object> persistentDataMap = new ConcurrentHashMap<String, Object>();

	/**
	 * the map associating the name of a data to the set of information objects
	 * associated with this name
	 */
	private Map<String, Set<TestTagInformation>> dataToInformationMap = new ConcurrentHashMap<String, Set<TestTagInformation>>();

	/**
	 * the map associating the name of a data to the list of listeners or
	 * actions associated with it
	 */
	private Map<String, List<ListenerAction>> listenersActionsMap = new ConcurrentHashMap<String, List<ListenerAction>>();

	/**
	 * the map of the data providers that are used by this class, associating
	 * the id of a data provider to this data provider
	 */
	private Map<String, RealTimeDataProvider> realTimeDataProviders = new ConcurrentHashMap<String, RealTimeDataProvider>();
	 ...
	 *            whether we are in the debug mode
	 */
	public AnimationsHandler(MainDisplay mainDisplay, boolean isTestVersion,
			boolean debugMode) {
			...
			public void run() {
				...
				while (true) {
					...
					if (dataMap.size() > 0 && !isPaused() && !isStopped()) {
						...
						// creating the map associating a runnable queue to the
						// list of runnables
						for (ListenerAction listenerAction : allListenersAndActions) {
							...
							if (listenersActionsSet.contains(listenerAction)) {
								...
								// creating the map associating the name of a
								// tag to its new value
								...
								// creating the runnable returned by the
								// method's listener and that
								// will be
								// executed in a runnable queue
								try {
									runnable = listenerAction
											.dataChanged(new DataEvent(
													nameToValue, null));
								} catch (Exception ex) {
									runnable = null;
									ex.printStackTrace();
								...
								if (runnable != null) {
									...
									runnablesHandler.addRunnables(
											listenerAction.getPicture(),
											runnable);
								}
							}
						}
	...
	/**
	 * sets the value of a data specified by its name (this method should be
	 * used by a rtdp object)
	 * 
	 * @param name
	 *            the name of a data
	 * @param value
	 ...
	 * @param serverIPAddress
	 *            the IP address of the server
	 * @param listenerOrAction
	 *            a listener or an action
	 */
	public void registerListenerOrAction(String serverIPAddress,
		...
		if (listenerOrAction != null) {
			...
			for (String name : listenerOrAction.getDataNames()) {
				...
				if (name != null && !name.equals("")) {
					...
					// adds the listener or action to the listeners or actions
					// map
					list = listenersActionsMap.get(name);
			...
			if (listenerOrAction instanceof DataChangedListener) {
				...
				// of the listeners that will be used to compute the value of
				// the tag
				...
				Collection<BlinkingValueModifier> blinkingValueModifiers = listener
						.getBlinkingValueModifiers();

				if (blinkingValueModifiers != null
						&& blinkingValueModifiers.size() > 0) {
			...
			if (isTestVersion) {
				...
				// adds the information objects linked with the given listener
				// or action
				Map<String, TestTagInformation> dataNamesToInfo = listenerOrAction
						.getDataNamesToInformation();
	...
	public void unregistersListenerOrAction(String serverIPAddress,
		...
		if (listenerOrAction != null) {
			...
			if (listenerOrAction instanceof DataChangedListener) {
				...
				blinkingsHandler.removeBlinkingValueModifiers(listener
						.getBlinkingValueModifiers());
			...
			if (isTestVersion) {
				...
				// removes the information objects linked with the given
				// listener
				Map<String, TestTagInformation> dataNamesToInfo = listenerOrAction
						.getDataNamesToInformation();
				TestTagInformation info = null;
	 ...
	 */
	public void setDataValue(String serverIPAddress, String dataName,
			Object value) {
	 ...
	 * 
	 * @param id
	 *            the id of a data provider
	 * @return the data provider linked with the given name
	 */
	protected RealTimeDataProvider getRealTimeDataProvider(String id) {

		RealTimeDataProvider realTimeDataProvider = realTimeDataProviders
				.get(id);

		// if no rtdp has been found, a test simulator or a regular simulator is
		// used
	...
	/**
	 * notifies the accurate data provider to refresh the value of the data
	 ...
	 * 
	 * @param isPaused
	 *            whether the animations are paused or not
	 */
	 ...
	 * 
	 * @param isStopped
	 *            whether the animations are stopped or not
	 */
	 ...
	 *            an animations state listener
	 */
	public void addRequireViewDisplayListener(
			RequireViewDisplayListener listener) {
	 ...
	 *            an animations state listener
	 */
	public void removeRequireViewDisplayListener(
			RequireViewDisplayListener listener) {
	 ...
	 * @param viewPath
	 *            the path of the view to be displayed
	 */
	...
	/**
	 * notifies that a request to display the last displayed view before the
	 * current one has been done
	 * 
	 * @param viewBrowserId
	 *            the id of the view browser
	 */
	...
	/**
	 * notifies that data changed listeners have been added to the animations
	 * handler
	 */
	...
	class AnimationsRunnablesHandler {
		 ...
		 */
		private HashMap<SVGPicture, AnimationsRunnable> animationsRunnables = new HashMap<SVGPicture, AnimationsRunnable>();
		...
		public void addRunnables(SVGPicture picture, Runnable runnable) {
			...
			if (picture != null) {
				...
				AnimationsRunnable animationsRunnable = animationsRunnables
						.get(picture);
		...
		public void clear() {
			...
			// for each picture, checks if it is used, and then clears its
