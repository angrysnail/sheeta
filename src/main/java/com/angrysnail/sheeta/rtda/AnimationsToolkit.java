...
import com.angrysnail.sheeta.library.geom.path.Path;
...
public class AnimationsToolkit {
	 ...
	 */
	private static final String dbNodeName = "equipment.historymanager",
			dbNameAtt = "DBname", tableNodeName = "historymanager.table",
			tableNameAtt = "id";
	...
	/**
	 * returns the value (a double between -1 and 1) of the sin function given
	 * the t parameter
	 * 
	 * @param t
	 *            the parameter
	 ...
	 * @return the value (a double between -1 and 1) of the triangle function
	...
	public static double sin(double t, double initialValue, double period) {
		...
		if (!Double.isNaN(t) && !Double.isNaN(period)) {
			...
			t = AnimationsToolkit.getModuloValue(t, period)
					+ AnimationsToolkit.getModuloValue(initialValue - period
							/ 4, period);
			t = AnimationsToolkit.getModuloValue(t, period);
	...
	/**
	 * returns the value (a double between -1 and 1) of the triangle function
	 * given the t parameter
	 * 
	 * @param t
	 *            the parameter
	 ...
	 *            the parameter
	 * @param t1
	 *            the instant when the function go from -1 to 1
	 * @param t2
	 *            the instant when the function go from 1 to -1
	 * @param period
	 *            the period for the function
	...
	public static double square(double t, double t1, double t2, double period) {
		...
		if (!Double.isNaN(t) && !Double.isNaN(period)) {
			...
			if (!Double.isNaN(t) && t >= 0 && t <= period && !Double.isNaN(t1)
					&& t1 >= 0 && t1 <= period && !Double.isNaN(t2) && t2 >= 0
					&& t2 <= period) {
	...
	public static boolean hasAnimationNodes(Node node) {
		...
		if (node != null && node.hasChildNodes()) {
			...
			for (Node cur = node.getFirstChild(); cur != null; cur = cur
				...
				// for each child of the given node, checks if it is an
				// animation node
	...
	public static boolean hasActiveAnimationNodes(Node node) {
		...
		if (node != null && node.hasChildNodes()) {
			...
			for (Node cur = node.getFirstChild(); cur != null; cur = cur
				...
				// animation node
				if (cur.getNodeName().startsWith("rtda:")
						&& ListenerActionBuilder
								.isActionNode(cur.getNodeName())) {
	 ...
	 * @param exact
	 *            whether the exact node corresponding to the given point should
	 *            be returned or if a parent of this node that contains
	 *            animation nodes should be returned
	...
	public static Node getNodeAt(SVGPicture picture, Point point, boolean exact) {
		...
		if (picture != null && point != null && picture.getCanvas() != null) {
			...
			BridgeContext ctxt = picture.getCanvas().getUpdateManager()
					.getBridgeContext();
			...
			if (ctxt != null) {
				...
				try {
					gnode = ctxt.getGraphicsNode(picture.getDocument()
							.getDocumentElement());
				} catch (Exception e) {
				}
				...
				if (gnode != null) {
					...
					try {
						graphicsNode = gnode.nodeHitAt(point);
					} catch (Exception ex) {
					}
					...
					if (graphicsNode != null) {
						...
						// that contains either animation nodes or active
						// animation nodes
						while (node != null) {
	...
	public static Rectangle2D getNodeBounds(SVGPicture picture, Node node) {
		...
		if (picture != null && picture.getCanvas() != null && node != null) {
			...
			if (picture.getCanvas().getUpdateManager() != null) {
				...
				ctxt = picture.getCanvas().getUpdateManager()
						.getBridgeContext();
			}
			...
			if (ctxt != null) {
				...
				try {
					gnode = ctxt.getGraphicsNode(node);
				} catch (Exception e) {
				}
				...
				if (gnode != null) {
					...
					if (bounds != null) {
						...
						if (gnode.getGlobalTransform() != null) {
							...
							transform
									.preConcatenate(gnode.getGlobalTransform());
						}
	 ...
	 * @return a rectangle representing the position and size of the given node
	 */
	public static Rectangle2D getGeometryNodeBounds(SVGPicture picture,
			Node node) {
			...
			if (ctxt != null) {
				...
				if (gnode != null) {
					...
					if (shape != null) {
						...
						AffineTransform vt = picture.getCanvas()
								.getViewingTransform();
	...
	public static String getId(SVGPicture picture, String baseString) {
		...
		if (picture != null && picture.getCanvas() != null) {
			...
			if (doc != null) {
				...
				// adds to the list all the ids found among the children of the
				// root element
	...
	public static Element convertToPath(SVGPicture picture, Element baseElement) {
		...
		if (picture != null && picture.getCanvas() != null
			...
			if (doc != null) {
				...
				// for each command in the path, the command and its values are
				// added to the string
				// value
				for (PathIterator pit = gpath
						.getPathIterator(new AffineTransform()); !pit.isDone(); pit
						.next()) {
					...
					if (type == PathIterator.SEG_CLOSE) {
						...
						pathValues.append("C " + values[0] + " " + values[1]
								+ " " + values[2] + " " + values[3] + " "
								+ values[4] + " " + values[5] + " ");
						...
						values = new double[4];
						pit.currentSegment(values);
						pathValues.append("Q " + values[0] + " " + values[1]
								+ " " + values[2] + " " + values[3] + " ");
				...
				pathElement = doc.createElementNS(doc.getDocumentElement()
	...
	/**
	 * creates the "d" attribute value of a path that is the intersection of the
	 * provided shape and rectangle
	 * 
	 * @param picture
	 *            a svg picture
	 * @param shape
	 *            an element
	 * @param rect
	 *            the rectangle with which the shape will be intersected
	 * @return the "d" attribute value of a path that is the intersection of the
	 *         provided shape and rectangle
	 */
	public static String intersectPath(SVGPicture picture, Element shape,
			Rectangle rect) {

		String dAtt = "";

		if (picture != null && picture.getCanvas() != null && shape != null
				&& rect != null) {
	...
	public static String getText(Element element) {
		...
		if (element != null) {
			...
			// for each child of the given element, builds the value of the text
			// nodes
	...
	/**
	 * sets the new new text values for an element (removes all the previous
	 * text values before)
	 * 
	 * @param element
	 *            an element
	 ...
	 *            a node
	 * @param newTransform
	 *            the new transform for the element
	 */
	public static void setTransform(SVGPicture picture, Node node,
		...
		if (picture != null && picture.getCanvas() != null && node != null
			...
			if (ctxt != null) {
				...
				if (gnode != null) {
					...
					final SVGTransformMatrix matrix = new SVGTransformMatrix(1,
							0, 0, 1, 0, 0);
					matrix.concatenateTransform(newTransform);

					((Element) node).setAttribute("transform",
	 ...
	 *            the uri of a file
	 * @param isEditionMode
	 *            whether the current mode is the edition or the runtime mode
		...
		if (fileUri != null) {
			...
			if (isEditionMode) {
				...
				while (file != null) {
					...
					// checking if one of the children of the file is a
					// ".project" file
					...
					if (children != null) {
						...
						for (i = 0; i < children.length; i++) {
							...
							if (children[i] != null
									&& children[i].getName().equals(".project")) {
				...
				// getting the parent directory of the given file, as in the
				// runtime version, all
	...
	/**
	 * returns the map associating the name of a db to the set of the names of
	 * all the tables in this db
	 * 
	 * @param dbRootElement
	 *            the root element of the xml db
	 * @return the map associating the name of a db to the set of the names of
	 *         all the tables in this db
	 */
	public static LinkedHashMap<String, Set<String>> getDBsAndTables(
			Element dbRootElement) {
	...
	/**
	 * returns the double number corresponding to the provided object and that
	 * is a value of the type of the provided type
	 * 
	 * @param valStr
	 *            the string defining the value
	 * @param tagType
	 *            the type of the tag whose value is computed
