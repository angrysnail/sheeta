...
public class BlinkingValueModifier implements Comparable<BlinkingValueModifier> {
	 ...
	 *            the name of a property or an attribute
	 * @param value1
	 *            the first value for the blinking
	 * @param value2
	 *            the second value for the blinking
	 */
	public BlinkingValueModifier(MainDisplay mainDisplay, SVGPicture picture,
			File projectFile, String blinkingId, Element element,
		...
		if (picture != null && blinkingId != null && element != null
			...
			Map<String, Object> blinkings = mainDisplay
					.getColorsAndBlinkingsToolkit()
					.getBlinkingsMap(projectFile);
	...
	/**
	 * applies the changes of the value of the blinking on the attribute or
	 * property this object is linked with
	 * 
	 * @return runnable the runnable used to modify the value of an attribute or
	 *         a property
	 */
	public Runnable applyBlinkingChanges() {
		...
		if (blinking != null && isActive) {
			...
			// test whether the blinking value has changed since the last time
			// it has been set
			if (currentModifierValue == null
					|| (currentModifierValue != null && !currentModifierValue
							.equals(currentValue))) {

				final String fcurrentValue = currentValue;

				// creating the runnable that will be executed to apply the
				// blinking value
