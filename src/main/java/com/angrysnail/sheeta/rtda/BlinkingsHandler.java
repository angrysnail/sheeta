...
/**
 * the class of the object handling the computation of the blinking values and
 * the dispatch of them
 * 
...
public class BlinkingsHandler {
	...
	/**
	 * the map associating a svg picture to the set of blinking value modifiers
	 * it is linked with
	 */
	private Map<SVGPicture, Set<BlinkingValueModifier>> blinkingValueModifiers = new ConcurrentHashMap<SVGPicture, Set<BlinkingValueModifier>>();

	/**
	 * the map associating a blinking object to the counter specifying the
	 * number of modifiers that have registered this blinking
	 */
	private Map<Blinking, Integer> blinkings = new ConcurrentHashMap<Blinking, Integer>();
	...
	public BlinkingsHandler(AnimationsHandler animationsHandler) {
		...
		blinkingThread = new Thread() {
			...
			public void run() {
				...
				// the list of the runnables that will be executed in each
				// runnable queue
				Collection<Runnable> runnablesList = null;
				Runnable runnable = null, blinkingRunnable = null;

				// the set of runnables that are to be executed in each runnable
				// queue
				final Set<Runnable> blinkingRunnableSet = new CopyOnWriteArraySet<Runnable>();

				while (true) {

					if (!getAnimationsHandler().isPaused()
							&& !getAnimationsHandler().isStopped()) {
						...
						if (blinkingRunnableSet.size() > 0) {
							...
							// computing the number of seconds since the
							// animations
							// were started (initially or after they were
							// stopped)
							time = 0;
							...
							// for svg picture, computes the values of each
							// function tag in each
							// listener
							for (SVGPicture picture : blinkingValueModifiers
									.keySet()) {

								if (picture != null) {

									// retrieving the set of the listeners
									final Set<BlinkingValueModifier> fset = blinkingValueModifiers
											.get(picture);
									...
									if (fset != null) {
										...
										// computes the new values for the tags
										// and adds the
										// returned runnable to the list of
										// runnables
										...
										// the runnable into which all the
										// runnables from the
										// runnable list will be modified so
										// that
										...
										blinkingRunnableSet
												.add(blinkingRunnable);

										// adding the runnable to the runnable
										// queue
										try {
											picture.enqueue(blinkingRunnable,
													true);
										} catch (Exception ex) {
										}
									}
	...
	public void addBlinkingValueModifier(BlinkingValueModifier modifier) {
		...
		if (modifier != null) {
			...
			// getting the set linked with the runnable queue of the given
			// modifier
			Set<BlinkingValueModifier> set = blinkingValueModifiers
					.get(modifier.getPicture());
	 ...
	 * 
	 * @param blinkingValueModifiersList
	 *            the list of the blinking value modifiers
	 */
	public void addBlinkingValueModifiers(
