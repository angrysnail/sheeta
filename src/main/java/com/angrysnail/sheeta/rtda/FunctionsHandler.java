...
/**
 * the class of the object that will handle the computation of the funcion
 * values and the dispatch of these values
 * 
...
public class FunctionsHandler {
	...
	/**
	 * the map associating a svg picture to the set of listeners it is linked
	 * with
	 */
	private Map<SVGPicture, List<DataChangedListener>> functionTagListeners = new ConcurrentHashMap<SVGPicture, List<DataChangedListener>>();
	...
	public FunctionsHandler(AnimationsHandler animationsHandler) {
		...
		// the thread refreshing the values of the tags that are described by a
		// function
		functionThread = new Thread() {
			...
			public void run() {
				...
				// queue
				final Set<Runnable> animationsRunnableSet = new CopyOnWriteArraySet<Runnable>();
				...
				while (true) {
					...
					if (!getAnimationsHandler().isPaused()
						...
						if (animationsRunnableSet.size() > 0) {
							...
							// animations were started
							// (initially or after they were stopped)
							time = 0;
							...
							// for each svg picture, computes the values of each
							// function tag in
							// each listener
							for (SVGPicture picture : functionTagListeners
									.keySet()) {

								if (picture != null) {

									// retrieving the set of the listeners
									final List<DataChangedListener> listeners = functionTagListeners
											.get(picture);
									...
									if (listeners != null) {
										...
										animationsRunnableSet
												.add(animationsRunnable);
										picture.enqueue(animationsRunnable,
												true);
									}
								}
							}
	...
	public void addFunctionTagListener(DataChangedListener listener) {
		...
		if (listener != null) {
			...
			// listener
			List<DataChangedListener> list = functionTagListeners.get(listener
					.getPicture());
	...
	public void removeFunctionTagListener(DataChangedListener listener) {
		...
		if (listener != null) {
			...
			// getting the list linked with the runnable queue of the given
			// listener
			List<DataChangedListener> list = functionTagListeners.get(listener
					.getPicture());
