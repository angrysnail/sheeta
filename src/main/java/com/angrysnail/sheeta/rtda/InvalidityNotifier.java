...
import com.angrysnail.sheeta.rtda.components.picture.Painter;
...
public class InvalidityNotifier {
	 ...
	 */
	private static ImageIcon invalidIcon = RtdaResources.getIcon("invalidIcon",
			false);

	/**
	 * the set of the listeners that are in an invalid state
	 */
	private CopyOnWriteArraySet<DataChangedListener> listeners = new CopyOnWriteArraySet<DataChangedListener>();

	/**
	 * the current painters map
	 */
	private ConcurrentHashMap<SVGPicture, CopyOnWriteArrayList<Painter>> pictureToPaintersMap = new ConcurrentHashMap<SVGPicture, CopyOnWriteArrayList<Painter>>();
		...
		if (!isUpdating) {
			...
			TimerTask task = new TimerTask() {
				...
				public void run() {
					...
					synchronized (InvalidityNotifier.this) {
						isUpdating = false;
					}
				}
			};
				...
				painter = new Painter() {
					...
					public void paint(Graphics2D g) {
						...
						Rectangle2D bounds = AnimationsToolkit.getNodeBounds(
								picture, listener.getParentElement());
						...
						if (bounds != null) {
							...
							Point location = new Point(
									(int) (bounds.getX() + bounds.getWidth()
											/ 2 - invalidIcon.getIconWidth() / 2),
									(int) (bounds.getY() + bounds.getHeight()
											/ 2 - invalidIcon.getIconHeight() / 2));

							// painting the image
							g.drawImage(invalidIcon.getImage(), location.x,
									location.y, picture.getCanvas());
						}
					}
				};
	...
	protected void paint() {
		...
		for (final SVGPicture picture : pictureToPaintersMap.keySet()) {
			...
			if (invalidMarkersShown) {
				...
				picture.getCanvas().addPainters(
						pictureToPaintersMap.get(picture));
