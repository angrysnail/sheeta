...
public abstract class AbstractLoadData
	...
	private static final String dbAttributeName = "dbName",
			...
			dataNameAttributeName = "dataName", tagAttributeName = "tag",
			childNodeName = "rtda:match";
	 ...
	 *            jwidget
	 */
	public AbstractLoadData(SVGPicture picture, String projectName,
		...
		if (pos != -1) {
			...
			while (pos != -1) {
				...
				beforeTagName = cValue.substring(0, pos
						+ Toolkit.startTagInRequest.length() - 1);
				cValue = cValue.substring(
						pos + Toolkit.startTagInRequest.length(),
						cValue.length());
				...
				if (pos2 != -1) {
					...
					cValue = cValue.substring(
							pos2 + Toolkit.endTagInRequest.length(),
							cValue.length());
	...
	protected void receiveData(final LinkedList<Object[]> data) {
		...
		if (data.size() > 0) {
			...
			if (data.size() > 0) {
				...
				// getting the configuration document of the picture of this
				// action
				...
				for (String fieldName : columnNames) {
					...
					if (tagName != null && !tagName.equals("")) {
						...
						if (value instanceof Timestamp || value instanceof Date) {
							 ...
							 * value=(int)((Date)value).getTime(); break;
							 */
	...
	protected class LoadDataDataBaseAccess extends DataBaseAccess {
		 ...
		 *            the table name
		 */
		public LoadDataDataBaseAccess(SVGPicture picture, String dbName,
				String tableName) {
			...
			if (requestWithTags) {
				...
				for (String part : requestParts) {
					...
					if (i < requestTagNames.size()) {
						...
						obj = picture.getMainDisplay().getAnimationsHandler()
								.getData(tagName);
