...
public abstract class AbstractLoadView
	 ...
	 * the string names
	 */
	protected static final String targetAttributeName = "target",
			viewAttributeName = "view",
			closePopupDialogValue = "/**closePopupDialog**/",
			quitValue = "/**quit**/",
			returnToPreviousValue = "/**returnToPrevious**/",
	 ...
	 *            jwidget
	 */
	public AbstractLoadView(SVGPicture picture, String projectName,
	...
	public void performAction(Object evt) {
		...
		if (isEntitled() && isAuthorized && showConfirmationDialog()) {
			...
			if (viewPath.equals(quitValue)) {
				...
				Component topMostComponent = picture.getCanvas()
						.getTopLevelAncestor();

				if (topMostComponent != null
						&& topMostComponent instanceof ViewBrowserPopupDialog) {

					ViewBrowserPopupDialog dialog = (ViewBrowserPopupDialog) topMostComponent;
					dialog.dispose();
				}
				...
				if (targetId.equals(popupTargetOption)) {
					...
					Container parentContainer = picture.getCanvas()
							.getTopLevelAncestor();

					if (parentContainer instanceof Frame) {

						new ViewBrowserPopupDialog(picture.getMainDisplay(),
								(Frame) parentContainer, viewPath, picture
										.getCanvas().getProjectName(),
								component);
						...
						new ViewBrowserPopupDialog(picture.getMainDisplay(),
								...
								viewPath, picture.getCanvas().getProjectName(),
								component);

					} else {

						new ViewBrowserPopupDialog(picture.getMainDisplay(),
								(JDialog) parentContainer, viewPath, picture
										.getCanvas().getProjectName(),
								component);
					}
