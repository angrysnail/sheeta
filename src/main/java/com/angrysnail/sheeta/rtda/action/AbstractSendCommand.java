...
public abstract class AbstractSendCommand extends Action {
	...
	protected static String tagToWriteName = "tagToWrite",
			...
			valueName = "value", commandToSendName = "commandToSend",
			usedAttName = "used";
	 ...
	 *            jwidget
	 */
	public AbstractSendCommand(SVGPicture picture, String projectName,
	...
	protected void initializeAction() {
		...
		if (picture.getMainDisplay().isTestVersion()) {
			...
			if (actionElement.hasChildNodes()) {
				...
				for (Node child = actionElement.getFirstChild(); child != null; child = child
						.getNextSibling()) {
					...
					if (child instanceof Element) {
						...
						if (childElement.getAttribute(usedAttName).equals(
								Boolean.toString(true))) {

							referenceTagValues.add(childElement
									.getAttribute(valueName));
							tagToWriteValues.add(childElement
									.getAttribute(commandToSendName));
						}
					}
				}
			}
			...
			info = new TestTagInformation(picture,
					actionElement.getAttribute(referenceTagName),
					referenceTagValues);
			dataNamesToTestTagInfo.put(
					actionElement.getAttribute(referenceTagName), info);
		}
	...
	public void performAction(Object evt) {
		...
		if (isEntitled() && isAuthorized && showConfirmationDialog()) {
			...
			if (obj != null) {
				...
				if (actionElement.hasChildNodes()
						&& !referenceTagValue.equals("")) {

					// getting the tag to write value corresponding to the
					// reference tag value
					Element childElement = null;
					String value = "";
				...
				if (tagValue.equals("")) {
					...
					tagValue = AnimationsToolkit
							.normalizeEnumeratedValue(actionElement
									.getAttribute(defaultValueName));
				}
			...
			String returnToInitialValue = actionElement
					.getAttribute(returnToInitialValueMethodName);
			...
			if (returnToInitialValue.equals(mouseUpEventName)) {
				...
				if (!Double.isNaN(timer)) {
					...
					final String tagToWrite = actionElement
					...
					AnimationsHandler.getTimer().schedule(timerTask,
							(long) (timer * 1000));
				}
			}
		}
