...
import com.angrysnail.sheeta.rtda.config.DataBaseAccess;
 ...
 */
public abstract class AbstractWriteDataToFile
		extends
	...
	private static final String dbAttributeName = "dbName",
			requestAttributeName = "request",
			directoryAttributeName = "directory",
			fileNamePatternAttributeName = "fileNamePattern",
			writeOnSameFileAttributeName = "writeOnSameFile",
			sepBetweenFieldsAttributeName = "sepBetweenFields",
			displayColumnNamesAttributeName = "displayColumnNames",
			descriptionTypeSuffix = ")", regularTypePrefix = "string(",
			dateTypePrefix = "date(";

	/**
	 * the date format
	 */
	protected static DateFormat dateFormat = DateFormat
			.getDateInstance(DateFormat.SHORT);

	/**
	 * the date and time format
	 */
	protected static DateFormat dateTimeFormat = DateFormat
	 ...
	 *            jwidget
	 */
	public AbstractWriteDataToFile(SVGPicture picture, String projectName,
	...
	protected void initializeAction() {
		...
		try {
			writeOnSameFile = Boolean.parseBoolean(actionElement
					.getAttribute(writeOnSameFileAttributeName));
		} catch (Exception ex) {
		}

		try {
			displayColumnNames = Boolean.parseBoolean(actionElement
					.getAttribute(displayColumnNamesAttributeName));
		} catch (Exception ex) {
		}
	 ...
	 * 
	 * @param data
	 *            the data found in the db, the first array is the array of the
	 *            fields of the table, the second array is the array of the
	 *            labels of the fields of the table,
	 */
	protected void receiveData(final LinkedList<Object[]> data) {
		...
		try {
			...
			String sep = actionElement
					.getAttribute(sepBetweenFieldsAttributeName);
			...
			if (displayColumnNames) {
				...
				byteBuffer = ByteBuffer.wrap(buffer.toString()
						.getBytes("UTF-8"));
			...
			if (data.size() > 0) {
				...
				for (Object[] array : data) {
					...
					buffer.append(lineSep);
					byteBuffer = ByteBuffer.wrap(buffer.toString().getBytes(
							"UTF-8"));
					channel.write(byteBuffer);
					byteBuffer.clear();
				}
	...
	protected String getFileName() {
		...
		String fileName = actionElement.getAttribute(
				fileNamePatternAttributeName).trim();

		if (!writeOnSameFile && fileName.length() > 0) {

			try {
				fileName = fileName.substring(0, fileName.length()
						- descriptionTypeSuffix.length());
			} catch (Exception ex) {
			}

			if (fileName.startsWith(regularTypePrefix)) {

				fileName = fileName.substring(regularTypePrefix.length(),
						fileName.length());

			} else if (fileName.startsWith(dateTypePrefix)) {

				fileName = fileName.substring(dateTypePrefix.length(),
						fileName.length());
	...
	protected class LoadDataToWriteDataBaseAccess extends DataBaseAccess {
		 ...
		 *            the data base name
		 */
		public LoadDataToWriteDataBaseAccess(SVGPicture picture, String dbName) {
