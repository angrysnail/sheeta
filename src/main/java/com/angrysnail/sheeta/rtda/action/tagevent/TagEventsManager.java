...
/**
 * the events manager for the actions triggered on the modification of tags
 * value
 * 
 * @author Jordi SUC
 */
public class TagEventsManager extends EventsManager {
	 ...
	 * 
	 * @param value
	 *            the value of the tag event attribute
	 */
	protected void registerTag(String value) {
		...
		if (splitValues != null) {
			...
			if (splitValues.length == 4
				...
				tagConditionHandler = new AnalogicTagConditionHandler(
						splitValues);
				...
				tagConditionHandler = new EnumeratedTagConditionHandler(
						splitValues);
			}
		}
	}
	...
	protected class EnumeratedTagConditionHandler extends TagConditionHandler {
		 ...
		 */
		protected Set<String> possibleValuesSet = new HashSet<String>();

		/**
		 * the constructor of the class
		 * 
		 * @param splitValues
		 *            the array of the segments of the string defining the
		 *            condition
		 */
		protected EnumeratedTagConditionHandler(String[] splitValues) {
			...
			if (splitValues.length > 2) {
				...
				for (int i = 2; i < splitValues.length; i++) {
					...
					possibleValuesSet.add(AnimationsToolkit
			...
			if (action.getPicture().getMainDisplay().isTestVersion()) {
				...
				TestTagInformation info = new TestTagInformation(
						action.getPicture(), tagName, realValues);
	...
	protected class AnalogicTagConditionHandler extends TagConditionHandler {
		...
		protected AnalogicTagConditionHandler(String[] splitValues) {
			...
			if (action.getPicture().getMainDisplay().isTestVersion()) {
				...
				// registering the tag
				TestTagInformation info = new TestTagInformation(
						action.getPicture(), tagName, null);
				action.getDataNamesToInformation().put(tagName, info);
			}
		}
			...
			if (evt.getDataNameToValue().containsKey(tagName)
				...
				if (currentValue != null) {
					...
					if (!Double.isNaN(dValue)
							&& EqualityChooserWidget.checkCondition(dValue,
									definedValue, equalityMode)) {
	...
	protected class StringTagConditionHandler extends TagConditionHandler {
		...
		protected void dataChanged(DataEvent evt) {
			...
			if (evt.getDataNameToValue().containsKey(tagName)) {
				...
				if (currentValue != null
						&& definedTagValue.equals(currentValue)) {
