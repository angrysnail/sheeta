...
import com.angrysnail.sheeta.rtda.animaction.eventsmanager.EventsManager;
...
public abstract class Action implements ListenerAction {
	...
	/**
	 * the label used when an action occurs triggered by a user that has no
	 * right
	 */
	...
	protected static final String authorizationTagName = "authorizationTag",
			authorizationValueName = "authorizationValue",
			valueSeparatorRegex = "[|]";
	...
	static {
		...
		try {
			bundle = ResourceBundle
					.getBundle("com.angrysnail.sheeta.rtda.resources.properties.strings");
		} catch (Exception ex) {
		}
	...
	/**
	 * the id of the sub component linked to this action (if the component
	 * contains sub components)
	 */
	protected String sourceId = "";

	/**
	 * the object to which the action applies, if it is not the provided
	 * component
	 */
	 ...
	 */
	protected Map<String, TestTagInformation> dataNamesToTestTagInfo = new HashMap<String, TestTagInformation>();
	 ...
	 *            jwidget
	 */
	public Action(SVGPicture picture, String projectName, Container parent,
	...
	/**
	 * @see com.angrysnail.sheeta.rtda.animaction.ListenerAction#getPicture()
	 */
	@Override
	...
	protected void computeRightsForTagsModif() {
		...
		try {
			rightLevel = Integer.parseInt(actionElement
					.getAttribute(authorizationLevelAtt));
		} catch (Exception ex) {
		}
	}
	...
	public boolean isEntitled() {
		...
		switch (rightType) {
			...
			case UserRights.RIGHT_FOR_TAG_MODIFICATION :
				...
				isEntitled = (userRights.getRightForTagModification() >= rightLevel);
				break;

			case UserRights.RIGHT_FOR_VIEW_LOADING :

				isEntitled = (userRights.getRightForViewLoading() >= rightLevel);
				break;

			case UserRights.RIGHT_FOR_RECIPE_LOADING :

				isEntitled = (userRights.getRightForRecipeLoading() >= rightLevel);
				break;

			case UserRights.RIGHT_FOR_RECIPE_MODIFICATION :

				isEntitled = (userRights.getRightForRecipeModification() >= rightLevel);
				break;

			case UserRights.RIGHT_FOR_PROGRAM_LAUNCHING :

				isEntitled = (userRights.getRightForProgramLaunching() >= rightLevel);
				break;
		}
	...
	protected void initializeAuthorizationTag() {
		...
		if (authorizationTag != null && !authorizationTag.equals("")) {
			...
			if (splitValues != null) {
				...
				for (int i = 0; i < splitValues.length; i++) {
					...
					if (splitValues[i] != null && !splitValues[i].equals("")) {
						...
						authorizationTagValues.add(AnimationsToolkit
								.normalizeEnumeratedValue(splitValues[i]));
			...
			if (picture.getMainDisplay().isTestVersion()) {
				...
				// adding the information object for the test for the
				// authorization tag
				TestTagInformation info = new TestTagInformation(picture,
						authorizationTag, realAuthorizationValues);
	...
	protected void checkIsAuthorized() {
		...
		if (authorizationTag != null && !authorizationTag.equals("")
			...
			isAuthorized = authorizationTagValues
					.contains(getData(authorizationTag));
		}
	}
	...
	public Object getData(String tagName) {
		...
		return picture.getMainDisplay().getAnimationsHandler().getData(tagName);
	}
	...
	public final void putTagValue(String tagName, Object tagValue) {
		...
		picture.getMainDisplay()
				...
				.setDataValue(
						picture.getAnimActionsHandler().getServerIPAddress(), // TODO
						tagName, tagValue);
	}
	...
	/**
	 * @return the id of the sub component linked to this action (if the
	 *         component contains sub components)
	 */
	...
	/**
	 * @return the object to which the action applies, if it is not the
	 *         component
	 */
	...
	/**
	 * @return the rectangle corresponding to the area for which the action
	 *         should be triggered
	...
	/**
	 * shows a confirmation dialog if specified in the action element and
	 * returns whether the action should be done
	 * 
	...
	protected boolean showConfirmationDialog() {
		...
		String confirmationMessage = actionElement
				.getAttribute(confirmationDialogAtt);

		if (!confirmationMessage.equals("")) {

			int result = JOptionPane.showConfirmDialog(component,
					confirmationMessage, "", JOptionPane.YES_NO_OPTION);
	...
	/**
	 * @return the point where the mouse can be found or the center point of the
	 *         parent element of this action
	 */
	...
	/**
	 * shows a message notifying the user that he is not authorized to execute
	 * the action
	 */
	protected void showUnAuthorizedMessage() {

		String tooltipText = "<html><body>" + unauthorizedLabel
				+ "</body></html>";
	...
	/**
	 * shows a message notifying the user that he is not entitled to execute the
	 * action
	 */
	protected void showNotEntitledMessage() {

		String tooltipText = "<html><body>" + notEntitledLabel
				+ "</body></html>";
