...
public class ActionsLoader {
	...
	/**
	 * the name of the directory of the action classes that can be found in each
	 * project
	 */
	...
	/**
	 * the map associating the name of a project to the set of all the classes
	 * that can be found in this project
	 */
	public static Map<String, Set<Class<?>>> projectClasses = new ConcurrentHashMap<String, Set<Class<?>>>();
	...
	/**
	 * returns the list of the action class names that can be found in the
	 * project and its referenced projects
	 * 
	 * @param superClass
	 *            the super class of the classes whose name should be retrieved
	 * @param projectFile
	 *            the project file
	 * @param isEditionMode
	 *            whether the mode, into which the actions are retrieved, is the
	 *            edition or runtime mode
	 * @return the list of the action class names
	 */
	public static LinkedList<String> getClassNames(Class<?> superClass,
			File projectFile, boolean isEditionMode) {

		LinkedList<String> classNames = new LinkedList<String>();

		// getting the set of the available classes
		Set<Class<?>> classes = getClasses(superClass, projectFile,
				isEditionMode);
	...
	/**
	 * return the class object corresponding to the given class name in the
	 * given project
	 * 
	 * @param superClass
	 *            the super class of the classes whose name should be retrieved
	 ...
	 *         given project
	 */
	public static Class<?> getClass(Class<?> superClass, File projectFile,
			String className, boolean isEditionMode) {
	...
	/**
	 * returns the list of the classes of the actions that can be found in the
	 * project and its referenced projects
	 * 
	 * @param superClass
	 *            the super class of the classes whose name should be retrieved
		...
		if (projectFile != null) {
			...
			if (projectClasses.containsKey(projectName)) {
				...
				if (isEditionMode) {
					...
					// getting the set of the names of all the referenced
					// projects by the given
					// project
					Set<String> allProjectNames = new HashSet<String>(
							DataBaseToolkit.getLibPaths(DataBaseToolkit
									.getLibPathsFile(projectFile
											.getParentFile().toURI()
											.toASCIIString(),
											Toolkit.getFileName(projectFile))));
							...
							for (File file : workspaceChildren) {
								...
								if (file != null && file.isDirectory()) {
									...
									if (pos != -1) {
										...
										theName = theURI.substring(pos + 1,
												theURI.length());
					...
					// getting the classes associated with each found project
					// file
					...
					// getting all the action classes that can be found in the
					// given project file
					Set<Class<?>> foundClasses = retrieveClasses(superClass,
							projectFile);
				...
				// putting the classes into the map associating a project name
				// to its set of classes
			...
			// modifying the classes set, by only keeping those who extend the
			// given super class
	...
	/**
	 * computes the set of the classes specified in the project denoted by the
	 * given project file
	 * 
	 * @param superClass
	 *            the super class of the classes whose name should be retrieved
	 * @param projectFile
	 *            a project file
	 * @return the set of the classes specified in the project denoted by the
	 *         given project file
	 */
	protected static Set<Class<?>> retrieveClasses(Class<?> superClass,
			File projectFile) {
		...
		if (projectFile != null) {
			...
			if (children != null) {
				...
				for (File file : children) {
					...
					if (file != null
							&& file.getName()
									.equals(actionClassesDirectoryName)) {
			...
			if (classesDirectory != null) {
				...
				// creating the class loader associated with the classes
				// directory
				ActionURLClassLoader classLoader = null;

				try {
					classLoader = new ActionURLClassLoader(
				...
				if (classLoader != null) {
					...
					// getting all the class files that can be found in this
							...
							if (files != null) {
								...
								for (int j = 0; j < files.length; j++) {
									...
									if (subFile != null) {
										...
										} else if (subFile.getName().endsWith(
												classSuffix)) {
					...
					for (File classFile : classFiles) {
						...
						try {
							// getting the byte code of the file to create the
							// class
							...
							out.close();
							in.close();
							foundClass = classLoader.defineNewClass(out
									.toByteArray());
	 ...
	 * 
	 * @param rootFile
	 *            the file from which the class file name should be computed
	 * @param classFile
	 *            the class file
	...
	protected static class ActionURLClassLoader extends URLClassLoader {
		 ...
		 * 
		 * @param content
		 *            the array of bytes for this class
