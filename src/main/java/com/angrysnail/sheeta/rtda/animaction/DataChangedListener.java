...
public abstract class DataChangedListener implements ListenerAction {
	...
	/**
	 * the map associating the name of the datas used by the resource node to
	 * their respective value, the function tags are not included in this map
	 */
	protected Set<String> dataNamesWithoutFunctionTags = new CopyOnWriteArraySet<String>();

	/**
	 * the list of the computers of the function values
	 */
	protected Set<FunctionValueComputer> functionValueComputers = new CopyOnWriteArraySet<FunctionValueComputer>();
	 ...
	 */
	protected Map<String, BlinkingValueModifier> itemNameToBlinkingValueModifier = new ConcurrentHashMap<String, BlinkingValueModifier>();
	...
	public DataChangedListener(SVGPicture picture, Element animationElement) {
		...
		if (animationElement != null
				&& animationElement.getParentNode() != null
	 ...
	 * 
	 * @param modifierId
	 *            the id of a blinking value modifier
	 */
	public void activateBlinkingValueModifier(String modifierId) {
		...
		if (modifierId != null && !modifierId.equals("")) {
			...
			BlinkingValueModifier modifier = itemNameToBlinkingValueModifier
					.get(modifierId);
	...
	public void deactivateBlinkingValueModifiers() {
		...
		for (BlinkingValueModifier modifier : itemNameToBlinkingValueModifier
				.values()) {
	...
	/**
	 * if the property values describes a blinking color, a new blinking value
	 * computer is created
	 * 
	 * @param element
	 *            the element onto which the blinking color should be applied
	 * @param modifierId
	 *            the id of a modifier
	 * @param value
	 *            the value from which the blinking color will be computed
	 * @param propertyName
	...
	protected void addBlinkingColorValueComputer(Element element,
		...
		if (element != null && modifierId != null && !modifierId.equals("")
			...
			// getting the blinking color linked with the value of this
			// property, if it exists
			BlinkingColor blinkingColor = mainDisplay
					.getColorsAndBlinkingsToolkit().getBlinkingColor(
							picture.getAnimActionsHandler().getProjectFile(),
							value);

			if (blinkingColor != null) {

				// creates a new blinkg value modifier and adds it to the map of
				// the modifiers
				BlinkingValueModifier modifier = new BlinkingValueModifier(
						mainDisplay, picture, picture.getAnimActionsHandler()
								.getProjectFile(),
						blinkingColor.getBlinkingId(), element, propertyName,
						blinkingColor.getColorValue(),
	 ...
	 *            the name of a property
	 * @param value1
	 *            the first value of the blinking
	 * @param value2
	 *            the second value of the blinking
	 */
	protected void addBlinkingValue(String modifierId, String blinkingId,
		...
		if (modifierId != null && !modifierId.equals("")
				&& propertyName != null && !propertyName.equals("")
				&& blinkingId != null && !blinkingId.equals("")
				&& value1 != null && value2 != null) {

			// creates a new blinkg value modifier and adds it to the map of the
			// modifiers
			BlinkingValueModifier modifier = new BlinkingValueModifier(
					picture.getMainDisplay(), picture, picture
							.getAnimActionsHandler().getProjectFile(),
