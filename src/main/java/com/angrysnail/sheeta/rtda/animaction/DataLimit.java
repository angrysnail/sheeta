...
public class DataLimit {
	 ...
	 * 
	 * @param picture
	 *            the picture to which animation or action using this limit
	 *            object belongs
	 * @param refTagName
	 *            the name of the tag whose min or max is computed
	 * @param limitValue
	 *            the value of the limit
	 * @param isMin
	 *            whether the min or the max is handled in this object
	 */
	public DataLimit(SVGPicture picture, String refTagName, String limitValue,
			boolean isMin) {
			...
			if (limitValue.equals(AUTO_LIMIT)) {
				...
				try {
					tagElement = picture.getMainDisplay().getPictureManager()
							...
							.getElement(refTagName);
				} catch (Exception ex) {
				}

				if (tagElement != null) {

					String valStr = tagElement.getAttribute(isMin
							? "min"
							: "max");
