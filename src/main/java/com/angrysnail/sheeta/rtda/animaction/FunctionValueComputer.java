...
/**
 * the class allowing to compute the value of a tag that is described by a
 * function
 * 
 * @author ITRIS, Jordi SUC
 */
public class FunctionValueComputer implements Comparable<FunctionValueComputer> {
	...
	public FunctionValueComputer(SVGPicture picture, String tagName,
		...
		// analysing the function string and retrieving the name of the
		// function, the period and the
		// initial value
		String function = new String(tagName);
		function = function.substring(function.indexOf("function(") + 9,
				function.length());
			...
			if (minVal < maxVal) {
				...
				if (functionName.equals(AnimationsToolkit.SIN_FUNCTION)) {
					...
					dVal = AnimationsToolkit.sin(time, initialValue, period);
						...
						.equals(AnimationsToolkit.TRIANGLE_FUNCTION)) {

					dVal = AnimationsToolkit.triangle(time, initialValue,
							period);
