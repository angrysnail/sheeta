...
import com.angrysnail.sheeta.rtda.colorsblinkings.Blinking;
import com.angrysnail.sheeta.rtda.components.picture.SVGPicture;
import com.angrysnail.sheeta.rtda.test.TestTagInformation;

/**
 * the listener to the changes of data for the "attribute on interval" animation
 * node
 * 
...
public class AttributeOnInterval extends DataChangedListener {
	 ...
	 */
	private String invalidValue1 = "", invalidValue2 = "",
			outOfRangeValue1 = "", outOfRangeValue2 = "",
			invalidValueBlinkingId = "", outOfRangeValueBlinkingId = "";
	 ...
	 */
	private Blinking invalidValueBlinking = null,
			outOfRangeValueBlinking = null;

	/**
	 * the id of the modifiers for the invalid value and the out of range value
	 */
	private final String invalidValueBVModifierId = "InvalidValue",
			outOfRangeBVModifierId = "OutOfRange";
	...
	public AttributeOnInterval(SVGPicture picture, Element animationElement) {
		...
		invalidValueBlinkingId = animationElement
				.getAttribute("invalidValueBlinking");
		invalidValueBlinking = picture
				.getMainDisplay()
				.getColorsAndBlinkingsToolkit()
				.getBlinking(picture.getAnimActionsHandler().getProjectFile(),
						invalidValueBlinkingId);

		if (invalidValueBlinking != null) {

			// creating the blinking value modifier for the invalid value
			addBlinkingValue(invalidValueBVModifierId,
					invalidValueBlinking.getId(), attributeName, invalidValue1,
					invalidValue2);
		}

		// getting the out of range blinking id and its corresponding blinking
		outOfRangeValueBlinkingId = animationElement
				.getAttribute("outOfRangeValueBlinking");
		outOfRangeValueBlinking = picture
				.getMainDisplay()
				.getColorsAndBlinkingsToolkit()
				.getBlinking(picture.getAnimActionsHandler().getProjectFile(),
		...
		if (outOfRangeValueBlinking != null) {
			...
			addBlinkingValue(outOfRangeBVModifierId,
					outOfRangeValueBlinking.getId(), attributeName,
					outOfRangeValue1, outOfRangeValue2);
		}
				...
				if (!Double.isNaN(tagValue) && !Double.isNaN(tagMinValue)
					...
					if (tagValue >= tagMinValue && tagValue <= tagMaxValue) {
						...
						for (AnimationChildValues child : childrenAttributeValues) {
							...
							if (child != null
									&& child.isInInterval(dataNameToValue,
											tagValue)) {

								isInInterval = true;

								// sets the value of the attribute corresponding
								// to the value of the
								// tag
								if (child.getBlinking() != null) {
									...
									// activating the modifier corresponding to
									// this value
									activateBlinkingValueModifier(child
											.getChildId());
					...
					// the tag value is out of range, as it is not in any
					// interval
					if (!isInInterval || tagValue < tagMinValue
						...
						if (outOfRangeValueBlinking != null) {
							...
							// activating the blinking value modifier
							// corresponding to the out of
							// range value
					...
					if (invalidValueBlinking != null) {
						...
						// activating the blinking value modifier corresponding
				...
				if (newValue != null
					...
					runnable = new Runnable() {
						...
						public void run() {
							...
							parentElement
									.setAttribute(attributeName, fnewValue);
						}
					};
				}

				// stores that the state of the listener is invalid as some of
				// its tags are invalid
	...
	protected class AnimationChildValues {
		...
		/**
		 * the possible values of the attribute if the value of the tag of the
				...
				blinking = picture
						...
						.getBlinking(
								picture.getAnimActionsHandler()
										.getProjectFile(), blinkingId);

				// creating the blinking value modifiers for the both values
				if (blinking != null) {

					addBlinkingValue(childId, blinking.getId(), attributeName,
							value1, value2);
				}
