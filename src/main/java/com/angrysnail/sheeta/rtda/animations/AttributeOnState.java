...
/**
 * the listener to the changes of data for the "attribute on state" animation
 * node
 * 
...
public class AttributeOnState extends DataChangedListener {
	...
	protected final static String tagAtt = "tag", attributeAtt = "attribute",
			invalidValue1Att = "invalidValue1",
			invalidValue2Att = "invalidValue2",
			defaultValue1Att = "defaultValue1",
			defaultValue2Att = "defaultValue2",
			invalidValueBlinkingAtt = "invalidValueBlinking",
			defaultValueBlinkingAtt = "defaultValueBlinking", usedAtt = "used",
			valueAtt = "value", blinkingAtt = "blinking",
			attributeValue1Att = "attributeValue1",
	...
	private final String invalidValueBVModifierId = "InvalidValue",
			defaultValueBVModifierId = "default";

	/**
	 * the map associating a the value of a tag to its corresponding child
	 */
	private Map<String, ChildrenAttributeOnState> childrenMap = new HashMap<String, ChildrenAttributeOnState>();
	...
	public AttributeOnState(SVGPicture picture, Element animationElement) {
		...
		if (defaultValueBlinking != null) {
			...
			addBlinkingValue(defaultValueBVModifierId,
					defaultValueBlinking.getId(),
					animationElement.getAttribute(attributeAtt),
		...
		for (Node cur = animationElement.getFirstChild(); cur != null; cur = cur
			...
			if (cur instanceof Element) {
				...
				isUsed = Boolean.parseBoolean(childElement
						.getAttribute(usedAtt));
				...
				if (isUsed) {
					...
					childrenMap.put(AnimationsToolkit
							.normalizeEnumeratedValue(childElement
									.getAttribute(valueAtt)), child);
	...
	public Runnable dataChanged(DataEvent evt) {
		...
		if (!animationElement.getAttribute(attributeAtt).equals("")) {
			...
			String value = (String) getData(animationElement
					.getAttribute(tagAtt));
			...
			if (value == null) {
				...
				if (child.getBlinking() != null) {
					...
					// activates the blinking of the fill attribute for the
			...
			if (canSetAttribute
					...
					&& !newValue.equals(parentElement
							.getAttribute(animationElement
									.getAttribute(attributeAtt)))) {
	...
	protected class ChildrenAttributeOnState {
		...
		protected ChildrenAttributeOnState(Element valueElement) {
			...
			blinkingModifierId = valueElement.getAttribute(valueAtt)
					+ "-blinking";
			blinking = picture
					.getMainDisplay()
					.getColorsAndBlinkingsToolkit()
