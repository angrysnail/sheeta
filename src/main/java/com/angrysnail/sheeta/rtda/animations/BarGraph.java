...
public class BarGraph extends DataChangedListener {
	...
	/**
	 * the foreground, the background, the rectangle that will be used for
	 * intersecting with the foreground element to create the accurate shape
	 */
	...
	public BarGraph(SVGPicture picture, Element animationElement) {
		...
		String transparentBackground = animationElement
				.getAttribute("transparentBackground");

		// whether the background is transparent or not
		isBackgroundTransparent = (transparentBackground != null && transparentBackground
				.equals("true"));
		...
		foregroundColor = (foregroundColor.equals("")
				? "none"
				: foregroundColor);
		addBlinkingColorValueComputer(foregroundElement, "foregroundColor",
				foregroundColor, "fill");

		backgroundColor = animationElement.getAttribute("backgroundColor");
		backgroundColor = (backgroundColor.equals("")
				? "none"
				: backgroundColor);

		if (!isBackgroundTransparent) {

			addBlinkingColorValueComputer(parentElement, "backgroundColor",
					backgroundColor, "fill");
		}

		invalidColor = animationElement.getAttribute("invalidColor");
		invalidColor = (invalidColor.equals("") ? "none" : invalidColor);
		addBlinkingColorValueComputer(foregroundElement, "invalidColor",
				invalidColor, "fill");
		...
		foregroundElement = doc.createElementNS(doc.getDocumentElement()
		...
		if (parentElement.getNextSibling() != null) {
			...
			parentElement.getParentNode().insertBefore(foregroundElement,
		...
		if (dataNameToValue != null) {
			...
			runnable = new Runnable() {
				...
				public void run() {
					...
					Rectangle2D theBounds = AnimationsToolkit
							.getGeometryNodeBounds(picture, parentElement);
					...
					if (theBounds != null) {
						...
						if (!Double.isNaN(tagValue)
							...
							// computing the percentage of the foreground
							// element facing the
							// background element
							double ratio = 0, amplitude = Math.abs(tagMaxValue
									- tagMinValue);

							if (amplitude > 0) {

								ratio = Math.abs((tagValue - tagMinValue)
										/ amplitude);
							}

							// computing the new bounds for the rectangle in the
							// mask
							if (direction.equals("left")) {

								newBounds.x = (int) Math.round(newBounds.x
										+ newBounds.width * (1 - ratio));

							} else if (direction.equals("right")) {

								newBounds.width = (int) Math
										.round(newBounds.width * ratio);

							} else if (direction.equals("bottom")) {

								newBounds.height = (int) Math
										.round(newBounds.height * ratio);

							} else {

								newBounds.y = (int) Math.round(newBounds.y
										+ newBounds.height * (1 - ratio));
							}

							// getting the path element attribute value that is
							// the intersection
							// between
							// the parent element and the computed bounds for
							// the foreground element
							pathPoints = AnimationsToolkit.intersectPath(
									picture, parentElement, newBounds);

						} else {

							// the state is invalid//
							if (itemNameToBlinkingValueModifier
									.containsKey("invalidColor")) {
						...
						if (canSetForeground
								&& !newForegroundValue
										.equals(lastForegroundValue)) {

							foregroundElement.setAttribute("fill",
									newForegroundValue);
							lastForegroundValue = newForegroundValue;
						}

						// setting the new points for the foreground element
						// path
