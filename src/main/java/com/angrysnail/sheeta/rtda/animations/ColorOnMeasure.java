...
import com.angrysnail.sheeta.rtda.animaction.DataLimit;
import com.angrysnail.sheeta.rtda.animaction.FunctionValueComputer;
...
public class ColorOnMeasure extends DataChangedListener {
	 ...
	 */
	private String invalidValueFill = "", outOfRangeFill = "",
			invalidValueStroke = "", outOfRangeStroke = "";

	/**
	 * the list of the objects containing all information about the child of an
	 * animation node
	 */
	...
	public ColorOnMeasure(SVGPicture picture, Element animationElement) {
		...
		// getting the name specified in the "tag" attribute and adding it to
		// the set of the data
		// names
		...
		// storing the values of the attributes of the animation node and
		// creating the associated
		// blinking color value computer
		invalidValueFill = animationElement.getAttribute("invalidValueFill");
		invalidValueFill = (invalidValueFill.equals("")
				? "none"
				: invalidValueFill);
		addBlinkingColorValueComputer(parentElement, "invalidValueFill",
				invalidValueFill, "fill");

		outOfRangeFill = animationElement.getAttribute("outOfRangeFill");
		outOfRangeFill = (outOfRangeFill.equals("") ? "none" : outOfRangeFill);
		addBlinkingColorValueComputer(parentElement, "outOfRangeFill",
				outOfRangeFill, "fill");

		invalidValueStroke = animationElement
				.getAttribute("invalidValueStroke");
		invalidValueStroke = (invalidValueStroke.equals("")
				? "none"
				: invalidValueStroke);
		addBlinkingColorValueComputer(parentElement, "invalidValueStroke",
				invalidValueStroke, "stroke");

		outOfRangeStroke = animationElement.getAttribute("outOfRangeStroke");
		outOfRangeStroke = (outOfRangeStroke.equals("")
				? "none"
				: outOfRangeStroke);
		addBlinkingColorValueComputer(parentElement, "outOfRangeStroke",
				outOfRangeStroke, "stroke");

		// getting children information
		int i = 0;
		AnimationChildValues animationChildValues = null;

		for (Node cur = animationElement.getFirstChild(); cur != null; cur = cur
				.getNextSibling()) {
		...
		if (isTagFunction) {
			...
			// creates the function value computer if the tag is described by a
			// function
			FunctionValueComputer computer = new FunctionValueComputer(picture,
					tagAttributeValue, tagMinValue, tagMaxValue);

			// adds the computer to the list of the function values computer
			functionValueComputers.add(computer);
		}

		// if we are in the test version, we store information on the tag, the
		// minTag and the maxTag
	...
	public Runnable dataChanged(DataEvent evt) {
		...
		if (dataNameToValue != null) {
			...
			// whether the fill or stroke attribute can be set, i.e. whether the
			// tag value
			// corresponds to a blinking color,
			...
			if (!Double.isNaN(tagValue) && !Double.isNaN(tagMinValue)
				...
				if (tagValue >= tagMinValue && tagValue <= tagMaxValue) {
					...
					// for each child of the animation node, checks if the
					// current value is inside a
					// child's interval//
					for (AnimationChildValues child : childrenAttributeValues) {
						...
						if (child != null) {
							...
							isInvalidState = isInvalidState
									|| child.isChildInvalid();
						}

						if (child != null
								&& child.isInInterval(dataNameToValue, tagValue)) {
							...
							String fillIdModifier = child.getChildId()
									+ "-fill", strokeIdModifier = child
									.getChildId() + "-stroke";

							if (itemNameToBlinkingValueModifier
									.containsKey(fillIdModifier)) {

								// activates the blinking of the fill attribute
								// for the current
								// value of the tag
							...
							if (itemNameToBlinkingValueModifier
									.containsKey(strokeIdModifier)) {

								// activates the blinking of the stroke
				...
				if (!isInInterval || tagValue < tagMinValue
						|| tagValue > tagMaxValue) {

					if (itemNameToBlinkingValueModifier
							.containsKey("outOfRangeFill")) {
				...
				if (itemNameToBlinkingValueModifier
						.containsKey("invalidValueStroke")) {
				...
				runnable = new Runnable() {
					...
					public void run() {
						...
						if (fcanSetFillAttribute
								&& !fnewFillValue.equals(parentElement
										.getAttribute("fill"))) {
						...
						if (fcanSetStrokeAttribute
								&& !fnewStrokeValue.equals(parentElement
										.getAttribute("stroke"))) {

							parentElement.setAttribute("stroke",
									fnewStrokeValue);
						}
					}
				};
			...
			// stores that the state of the listener is invalid as some of its
			// tags are invalid
	...
	/**
	 * the class storing information on a the child of an animation node that
	...
	protected class AnimationChildValues {
		...
		/**
		 * whether the value of the tag of the animation node could be equal to
		 * the min
		 */
		...
		/**
		 * the possible fill and stroke values if the value of the tag of the
		 * animation node is in this interval
		 */
		 ...
		 * @param childId
		 *            the id of the child
		 * @param animationChildElement
		 *            a child of the animation node
		 */
		protected AnimationChildValues(String childId,
				Element animationChildElement) {
			...
			if (animationChildElement != null) {
				...
				String eq1 = animationChildElement.getAttribute("equal1"), eq2 = animationChildElement
						.getAttribute("equal2");
				...
				addBlinkingColorValueComputer(parentElement, childId + "-fill",
						fill, "fill");

				stroke = animationChildElement.getAttribute("stroke");
				stroke = (stroke.equals("") ? "none" : stroke);
				addBlinkingColorValueComputer(parentElement, childId
						+ "-stroke", stroke, "stroke");

				// getting the min and max
				String minStr = animationChildElement.getAttribute("min"), maxStr = animationChildElement
						.getAttribute("max");
				...
				// if we are in the test version, we store information on the
				// tag
				if (picture.getMainDisplay().isTestVersion()) {
					...
					if (min.isTag()) {
						...
						info = new TestTagInformation(picture, min.getTag(),
								null);
					...
					if (max.isTag()) {
						...
						info = new TestTagInformation(picture, max.getTag(),
								null);
		...
		public void refreshValues(Map<String, Object> dataNameToInfo) {
			...
			if (!Double.isNaN(tagMinValue) && !Double.isNaN(tagMaxValue)) {
				...
				if (max.isTag() && dataNameToInfo.containsKey(max.getTag())) {
					...
					System.out.println("SETTING MAX=" + getData(max.getTag())
							+ " " + Toolkit.getNumber(getData(max.getTag())));

					setMaxValue(Toolkit.getNumber(getData(max.getTag())));
				}

				System.out.println("CHILD=" + childId + " " + minValue + " "
						+ maxValue + " " + min.isTag() + " " + max.isTag());
			}
		}
		 ...
		 * 
		 * @param dataNameToValue
		 *            the map containing the new values
		 * @param theTagValue
		 *            the value of the main tag of the animation node
		 * @return whether the given value is in this interval
		 */
		public boolean isInInterval(Map<String, Object> dataNameToValue,
				double theTagValue) {
