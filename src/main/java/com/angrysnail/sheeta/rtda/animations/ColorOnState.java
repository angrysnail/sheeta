...
public class ColorOnState extends DataChangedListener {
	 ...
	 * attribute names
	 */
	private final static String tagAtt = "tag",
			invalidValueFillAtt = "invalidValueFill",
			invalidValueStrokeAtt = "invalidValueStroke",
			defaultValueFillAtt = "defaultValueFill",
			defaultValueStrokeAtt = "defaultValueStroke", usedAtt = "used",
			valueAtt = "value", noneVal = "none", fillAtt = "fill",
			strokeAtt = "stroke";
	...
	/**
	 * the map of the fill colors associating a possible value of a tag to its
	 * corresponding color
	 */
	private Map<String, String> fillMap = new HashMap<String, String>();

	/**
	 * the map of the stroke colors associating a possible value of a tag to its
	 * corresponding color
	 */
	...
	public ColorOnState(SVGPicture picture, Element animationElement) {
		...
		invalidValueFill = (invalidValueFill.equals("")
				? noneVal
				: invalidValueFill);
		addBlinkingColorValueComputer(parentElement, invalidValueFillAtt,
				invalidValueFill, fillAtt);

		defaultValueFill = animationElement.getAttribute(defaultValueFillAtt);
		defaultValueFill = (defaultValueFill.equals("")
				? noneVal
				: defaultValueFill);
		addBlinkingColorValueComputer(parentElement, defaultValueFillAtt,
				defaultValueFill, fillAtt);

		invalidValueStroke = animationElement
				.getAttribute(invalidValueStrokeAtt);
		invalidValueStroke = (invalidValueStroke.equals("")
				? noneVal
				: invalidValueStroke);
		addBlinkingColorValueComputer(parentElement, invalidValueStrokeAtt,
				invalidValueStroke, strokeAtt);

		defaultValueStroke = animationElement
				.getAttribute(defaultValueStrokeAtt);
		defaultValueStroke = (defaultValueStroke.equals("")
				? noneVal
				: defaultValueStroke);
		addBlinkingColorValueComputer(parentElement, defaultValueStrokeAtt,
				defaultValueStroke, strokeAtt);
			...
			if (cur instanceof Element) {
				...
				if (isUsed) {
					...
					if (!value.equals("")) {
						...
						// getting the fill and stroke values for the given tag
						// value and creating
						// the associated blinking color value computer
						fill = ((Element) cur).getAttribute(fillAtt);
						fill = (fill.equals("") ? noneVal : fill);
						addBlinkingColorValueComputer(parentElement, value
								+ "-fill", fill, fillAtt);

						stroke = ((Element) cur).getAttribute(strokeAtt);
						stroke = (stroke.equals("") ? noneVal : stroke);
						addBlinkingColorValueComputer(parentElement, value
								+ "-stroke", stroke, strokeAtt);

						realValuesNames.add(value);
						value = AnimationsToolkit
								.normalizeEnumeratedValue(value);
		...
		if (picture.getMainDisplay().isTestVersion()) {
			...
			dataNamesToInformation.put(animationElement.getAttribute(tagAtt),
					info);
		}
	}
	...
	public Runnable dataChanged(DataEvent evt) {
		...
		String value = (String) getData(animationElement.getAttribute(tagAtt));
		...
		if (value == null) {
			...
			if (itemNameToBlinkingValueModifier
					.containsKey(invalidValueFillAtt)) {
			...
			if (itemNameToBlinkingValueModifier
					.containsKey(invalidValueStrokeAtt)) {
			...
			String fillIdModifier = value + "-fill", strokeIdModifier = value
					+ "-stroke";

			if (itemNameToBlinkingValueModifier.containsKey(fillIdModifier)) {

				// activates the blinking of the fill attribute for the current
				// value of the tag
			...
			if (itemNameToBlinkingValueModifier.containsKey(strokeIdModifier)) {
				...
				// activates the blinking of the stroke attribute for the
			...
			if (itemNameToBlinkingValueModifier
					.containsKey(defaultValueFillAtt)) {
			...
			if (itemNameToBlinkingValueModifier
					.containsKey(defaultValueStrokeAtt)) {
