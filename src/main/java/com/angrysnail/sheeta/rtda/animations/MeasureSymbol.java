...
/**
 * the listener to the changes of data for the "symbol on measure" animation
 * node
 * 
...
public class MeasureSymbol extends DataChangedListener {
	...
	public MeasureSymbol(SVGPicture picture, Element animationElement) {
		...
		for (Node child = parentElement.getFirstChild(); child != null; child = child
			...
			if (child instanceof Element
					&& !child.getNodeName().startsWith("rtda:")) {

				childrenSet.add((Element) child);

				// getting the id of the child
				theId = Toolkit.toUnURLValue(((Element) child)
						.getAttribute("id"));
		...
		invalidSymbol = Toolkit.toUnURLValue(animationElement
				.getAttribute("invalidSymbol"));
		outOfRangeSymbol = Toolkit.toUnURLValue(animationElement
				.getAttribute("outOfRangeSymbol"));
	...
	public Runnable dataChanged(DataEvent evt) {
		...
		if (!Double.isNaN(tagValue) && !Double.isNaN(tagMinValue)
			...
			// for each child of the animation node, checks if the current value
			// is inside a child's
			// interval//
			for (AnimationChildValues child : childrenAttributeValues) {
		...
		if (newSymbol != null && !newSymbol.equals("")
			...
			runnable = new Runnable() {
				...
				public void run() {
					...
					// getting the last and new children corresponding to the
					// ids
					try {
	...
	protected class AnimationChildValues {
		...
		protected AnimationChildValues(String childId,
			...
			if (animationChildElement != null) {
				...
				symbol = Toolkit.toUnURLValue(animationChildElement
						.getAttribute("symbol"));
