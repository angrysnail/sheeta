...
public class Rotation extends DataChangedListener {
	 ...
	 */
	private Point2D centerPoint, absoluteCenterPoint, refPoint, minPoint,
			maxPoint;
	...
	public Rotation(SVGPicture picture, Element animationElement) {
		...
		String xCenter = animationElement.getAttribute("xCenter"), yCenter = animationElement
				.getAttribute("yCenter"), xRef = animationElement
				.getAttribute("xRef"), yRef = animationElement
				.getAttribute("yRef"), xMin = animationElement
				.getAttribute("xMin"), yMin = animationElement
				.getAttribute("yMin"), xMax = animationElement
				.getAttribute("xMax"), yMax = animationElement
				.getAttribute("yMax");
	...
	public void initializeWhenCanvasDisplayed() {
		...
		absoluteCenterPoint = new Point2D.Double(centerPoint.getX()
				+ bounds.getX(), centerPoint.getY() + bounds.getY());
	}
			...
			if (amplitude > 0) {
				...
				if (clockwise) {
					...
					if (minAngle < maxAngle) {
						...
						currentAngle = -(minAngle - ratio
								* (minAngle - (maxAngle - 2 * Math.PI)));
					...
					if (minAngle < maxAngle) {
						...
						currentAngle = -(ratio
								* (maxAngle - (minAngle - 2 * Math.PI)) + (minAngle - 2 * Math.PI));
				...
				affineTransform.preConcatenate(AffineTransform
						.getTranslateInstance(-absoluteCenterPoint.getX(),
								-absoluteCenterPoint.getY()));
				affineTransform.preConcatenate(AffineTransform
						.getRotateInstance(currentAngle));
				affineTransform.preConcatenate(AffineTransform
						.getTranslateInstance(absoluteCenterPoint.getX(),
								absoluteCenterPoint.getY()));
