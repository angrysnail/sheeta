...
public class Scale extends DataChangedListener {
	 ...
	 */
	private Point2D.Double absoluteCenterPoint, centerPoint, refPoint,
			minPoint, maxPoint;
	...
	public Scale(SVGPicture picture, Element animationElement) {
		...
		preserveAspectRatio = Boolean.parseBoolean(animationElement
		...
		try {
			x = Double.parseDouble(animationElement.getAttribute("xRef"))
					- centerPoint.x;
			y = Double.parseDouble(animationElement.getAttribute("yRef"))
					- centerPoint.y;
		} catch (Exception ex) {
			x = 0;
			y = 0;
		...
		try {
			x = Double.parseDouble(animationElement.getAttribute("xMin"))
					- centerPoint.x;
			y = Double.parseDouble(animationElement.getAttribute("yMin"))
					- centerPoint.y;
		} catch (Exception ex) {
			x = 0;
			y = 0;
		...
		try {
			x = Double.parseDouble(animationElement.getAttribute("xMax"))
					- centerPoint.x;
			y = Double.parseDouble(animationElement.getAttribute("yMax"))
					- centerPoint.y;
		} catch (Exception ex) {
			x = 0;
			y = 0;
		...
		if (parametersCorrect && refPoint.getX() != 0 && refPoint.getY() != 0) {
			...
			if (isTagFunction) {
				...
				// creates the function value computer if the tag is described
				// by a function
			...
			if (picture.getMainDisplay().isTestVersion()) {
				...
				if (!isTagFunction) {
					...
					info = new TestTagInformation(picture, tagAttributeValue,
							null);
				...
				if (tagMin.isTag()) {
					...
					info = new TestTagInformation(picture, tagMin.getTag(),
							null);
				...
				if (tagMax.isTag()) {
					...
					info = new TestTagInformation(picture, tagMax.getTag(),
							null);
	...
	public void initializeWhenCanvasDisplayed() {
		...
		initialAffineTransform = AnimationsToolkit.getTransform(picture,
				parentElement);

		// getting the bounds of the parent element
		Rectangle2D bounds = AnimationsToolkit.getGeometryNodeBounds(picture,
				parentElement);

		// computing the absolute coordinates of the center point
		absoluteCenterPoint = new Point2D.Double(centerPoint.x + bounds.getX(),
				centerPoint.y + bounds.getY());
	}
			...
			if (!Double.isNaN(tagValue) && !Double.isNaN(tagMinValue)
				...
				if (amplitude > 0) {
					...
					if (reversedX) {
						...
						newPoint.x = minPoint.x - tagValue
								* Math.abs(minPoint.getX() - maxPoint.getX())
								/ amplitude;
						...
						newPoint.x = tagValue
								* Math.abs(minPoint.getX() - maxPoint.getX())
								/ amplitude + minPoint.x;
					}

					if (reversedY) {

						newPoint.y = minPoint.y - tagValue
								* Math.abs(minPoint.getY() - maxPoint.getY())
								/ amplitude;
					...
					affineTransform.preConcatenate(AffineTransform
							.getScaleInstance(scaleFactors.x, scaleFactors.y));
					affineTransform.preConcatenate(AffineTransform
							.getTranslateInstance(absoluteCenterPoint.x,
									absoluteCenterPoint.y));

					// getting the new affine transform for the element
					final AffineTransform newAffineTransform = picture
							.getMainDisplay().getAnimationsHandler()
							.getTransformHandler()
							.computeTransform(parentElement);

					if (!newAffineTransform.equals(AnimationsToolkit
							.getTransform(picture, parentElement))) {
						...
						runnable = new Runnable() {
							...
							public void run() {
								...
								AnimationsToolkit.setTransform(picture,
										parentElement, newAffineTransform);
							}
						};
					}
				}
