...
public class StateSymbol extends DataChangedListener {
	...
	private final static String tagAtt = "tag", idAtt = "id",
			invalidSymbolAtt = "invalidSymbol",
			defaultSymbolAtt = "defaultSymbol", symbolAtt = "symbol",
			usedAtt = "used", valueAtt = "value", opacityAtt = "opacity";
	...
	/**
	 * the map of the ids associating a possible value of a tag to the id of a
	...
	public StateSymbol(SVGPicture picture, Element animationElement) {
		...
		for (Node child = parentElement.getFirstChild(); child != null; child = child
			...
			if (child instanceof Element
					&& !child.getNodeName().startsWith(Toolkit.rtdaPrefix)) {
		...
		invalidSymbol = Toolkit.toUnURLValue(animationElement
				.getAttribute(invalidSymbolAtt));
		defaultSymbol = Toolkit.toUnURLValue(animationElement
				.getAttribute(defaultSymbolAtt));
			...
			if (cur instanceof Element) {
				...
				if (isUsed) {
					...
					if (!value.equals("")) {
						...
						symbol = Toolkit.toUnURLValue(el
								.getAttribute(symbolAtt));
