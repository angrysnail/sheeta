 ...
 */
package com.angrysnail.sheeta.rtda.colorsblinkings;
...
import com.angrysnail.sheeta.library.color.SVGColorsManager;
import com.angrysnail.sheeta.rtda.AnimationsToolkit;
import com.angrysnail.sheeta.rtda.BlinkingValueModifier;
import com.angrysnail.sheeta.rtda.MainDisplay;
import com.angrysnail.sheeta.rtda.NodeIterator;
...
public class ColorsAndBlinkingsToolkit {
	 ...
	 */
	public static String colorsBlinkingsFileName = "colorsBlinkings"
			+ AnimationsToolkit.XML_FILE_EXTENSION;
	...
	/**
	 * the map associating the name of a project to its map of the colors and
	 * blinkings
	 */
	private Map<String, Map<Integer, Map<String, Object>>> projectNameToColorsAndBlinkingsMap = new ConcurrentHashMap<String, Map<Integer, Map<String, Object>>>();
	...
	/**
	 * returns the map associating the type of a data defined in the colors and
	 * blinkings file
	 * 
	 * @param projectFile
	 *            a project file
	 * @return the map associating the type of a data defined in the colors and
	 *         blinkings file
	 */
	protected Map<Integer, Map<String, Object>> getColorsAndBlinkings(
			File projectFile) {
		...
		if (projectFile != null && projectName != null
			...
			if (file != null && file.exists()) {
				...
				// retrieving the xml document corresponding to the computed
				// path
				...
				if (doc != null && doc.getDocumentElement() != null) {
					...
					for (parent = doc.getDocumentElement().getFirstChild(); parent != null; parent = parent
							.getNextSibling()) {
						...
						if (parent instanceof Element) {
							...
							if (parent.getNodeName().equals("blinkings")) {
								...
								for (blinking = parent.getFirstChild(); blinking != null; blinking = blinking
										.getNextSibling()) {

									if (blinking instanceof Element
											&& blinking.getNodeName().equals(
													"blinking")) {
										...
										blinkingsMap.put(id, new Blinking(id,
												dT1, dT2, dPeriod));
									}
								}
								...
								for (color = parent.getFirstChild(); color != null; color = color
										.getNextSibling()) {
									...
									if (color instanceof Element) {
										...
										if (color.getNodeName().equals("color")) {
											...
											if (cColor1 != null) {
												...
												predefinedColorsMap.put(id,
														new PredefinedColor(id,
																cColor1));
											}

										} else if (color.getNodeName().equals(
												"blinkingColor")) {
											...
											if (cColor1 != null
												...
												blinkingColorsMap.put(id,
														new BlinkingColor(id,
																cColor1,
																cColor2,
																blinkingName));
											}
				...
				// putting this map in the map associating a canvas to the map
				// of the colors and
				// blinkings
		...
		} else if (projectFile != null && projectName != null
				&& !projectName.equals("")) {
	...
	public Map<String, Object> getPredefinedColorsMap(File projectFile) {
		...
		if (projectFile != null) {
			...
			Map<Integer, Map<String, Object>> colorsAndBlinkings = getColorsAndBlinkings(projectFile);
	 ...
	 *            a project file
	 * @param value
	 *            a string that may contain a blinking color id
	 * @return the blinking color or null if the string does not describe a
	 *         blinking color
	 */
	...
	/**
	 * returns the collection of the blinking value modifiers that handle
	 * blinking colors that are not in rtda animation nodes
	 * 
	 * @param doc
	 *            a document
	 * @param picture
	 ...
	 * @param projectFile
	 *            the file of a project
	 * @return the collection of the blinking value modifiers that handle
	 *         blinking colors that are not in rtda animation nodes
	 */
							...
							for (i = 0; i < attributes.getLength(); i++) {
								...
								if (value != null && !value.equals("")
									...
									if (att.getNodeName().equals("style")) {
										...
										blinkingValueModifierList
												.addAll(getNotInAnimationsNodeBlinkingValueModifiersFromStyleAttribute(
														picture, projectFile,
														el, value));
										...
										modifier = getNotInAnimationsNodeBlinkingValueModifiersFromAttribute(
												picture, projectFile, el,
												att.getNodeName(), value);
	...
	/**
	 * returns a list of blinking value modifiers that handle the blinking
	 * colors that could be found in the given string
	 * 
	 * @param picture
	 *            a svg picture
	 * @param projectFile
	 *            the file of a project
	 * @param element
	 *            the element to which the styleValue belongs
	 * @param styleValue
	 *            the value of style attribute
	 * @return a list of blinking value modifiers that handle the blinking
	 *         colors that could be found in the given string
	 */
	private java.util.List<BlinkingValueModifier> getNotInAnimationsNodeBlinkingValueModifiersFromStyleAttribute(
			...
			if (propertyValues != null) {
				...
				for (int i = 0; i < propertyValues.length; i++) {
					...
					if (propertyValues[i] != null) {
						...
						// getting the position of the limit between the name
						...
						if (pos != -1) {
							...
							value = propertyValues[i].substring(pos + 1,
									propertyValues[i].length());

							if (name != null && value != null
									&& !name.equals("") && !value.equals("")) {

								// retrieving the associated blinking color
								blinkingColor = getBlinkingColor(projectFile,
										value);
	...
	/**
	 * returns a blinking value modifier that handles the blinking color that
	 ...
	 * @return a blinking value modifier that handles the blinking color that
	 *         could be found in the given string
	 */
	private BlinkingValueModifier getNotInAnimationsNodeBlinkingValueModifiersFromAttribute(
		...
		if (picture != null && attributeValue != null
				&& !attributeValue.equals("")) {

			// retrieving the blinking color
			BlinkingColor blinkingColor = getBlinkingColor(projectFile,
					attributeValue);

			if (blinkingColor != null) {

				modifier = getNotInAnimationsNodeBlinkingValueModifier(picture,
						projectFile, blinkingColor, element, attributeName);
			}
		}
	...
	private BlinkingValueModifier getNotInAnimationsNodeBlinkingValueModifier(
		...
		if (picture != null && blinkingColor != null && element != null
			...
			modifier = new BlinkingValueModifier(mainDisplay, picture,
					projectFile, blinkingColor.getBlinkingId(), element,
					propertyName, blinkingColor.getColorValue(),
					blinkingColor.getColorValue2());
	...
	public static Color getColor(String colorString) {
		...
		if (color == null && colorString.length() == 7) {
			...
			try {
				r = Integer.decode("#" + colorString.substring(1, 3))
						.intValue();
				g = Integer.decode("#" + colorString.substring(3, 5))
						.intValue();
				b = Integer.decode("#" + colorString.substring(5, 7))
						.intValue();
	...
	public static String getColorString(Color color) {
		...
		String sr = Integer.toHexString(color.getRed()), sg = Integer
				.toHexString(color.getGreen()), sb = Integer.toHexString(color
				.getBlue());
