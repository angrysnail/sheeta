...
public class SVGPicture {
	 ...
	 * 
	 * @param isDisplayed
	 *            whether the picture is displayed
	 */
	...
	public void dispose() {
		...
		if (!isDisposed) {
			...
			mainDisplay.getAnimationsHandler().removeAnimationsStateListener(
					animationsStateListener);
	...
	/**
	 * enqueues the provided runnable so that it is executed into the AWT thread
	 * 
	 * @param runnable
	 *            a runnable
	 * @param wait
	 *            whether to wait in the calling thread while executing the
	 *            runnable
	 */
	public void enqueue(final Runnable runnable, boolean wait) {

		if (runnable != null && pictureComponent != null && getCanvas() != null
				&& getCanvas().getUpdateManager() != null) {
				...
				if (wait) {
					...
					try {
						if (queue.getRunHandler() != null) {
							...
							queue.getRunHandler()
									.runnableStart(queue, runnable);
							SwingUtilities.invokeAndWait(runnable);
							queue.getRunHandler().runnableInvoked(queue,
									runnable);
						}
					} catch (Exception ex) {
						ex.printStackTrace();
