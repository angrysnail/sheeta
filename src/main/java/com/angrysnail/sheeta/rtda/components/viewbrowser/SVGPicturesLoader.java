package com.angrysnail.sheeta.rtda.components.viewbrowser;
...
import com.angrysnail.sheeta.compiler.rtdb.RTDBCompiler;
import com.angrysnail.sheeta.rtda.components.picture.SVGPicture;
import com.angrysnail.sheeta.rtda.database.DataBaseToolkit;
import com.angrysnail.sheeta.rtda.widget.ProgressBarDialog;
...
public class SVGPicturesLoader {
	 ...
	 * 
	 * @param viewBrowser
	 *            the picture loader linked to the view browser
	 */
	 ...
	 * @param uri
	 *            the uri of a canvas
	 * @param disposeAllPreviousPictures
	 *            whether the previously loaded pictures should be disposed or
	 *            not
	 */
	public void setCurrentPicture(String uri, boolean disposeAllPreviousPictures) {
		...
		if (uri != null && !uri.equals("")) {
			...
			SVGPicture picture = viewBrowser.mainDisplay.getPictureManager()
					.getPicture(uri);
			...
			if (picture == null) {
			...
			} else if (!picture.equals(viewBrowser.canvasContainer
					.getCurrentPicture())
					&& !picture.isDisplayed()
					&& picture.getAnimActionsHandler().isEntitled()) {
				...
				viewBrowser.mainDisplay.getViewBrowsersManager()
						.fireViewLoaded(projectName, viewBrowser.viewBrowserId,
								uri);
			}
		}
	...
	protected void disposeUnusedPictures() {
		...
		Set<SVGPicture> allPictures = viewBrowser.mainDisplay
				.getPictureManager().getPictures();

		for (SVGPicture picture : allPictures) {

			if (picture != null && !picture.isDisplayed()) {

				viewBrowser.mainDisplay.getPictureManager().removePicture(
						picture);
				picture.dispose();
			}
		}
	 ...
	 *            not
	 */
	public void setCurrentPicture(final SVGDocument doc, String uri,
			boolean disposeAllPreviousPictures) {
		...
		if (doc != null && uri != null) {
			...
			if (!com.angrysnail.sheeta.library.Toolkit.isDocumentAView(doc)) {
				...
				Thread thread = new Thread() {
					...
					public void run() {
						...
						final SVGDocument svgDoc = (SVGDocument) doc
								.cloneNode(true);

						// replacing the jwidget images by a rectangle
						NodeList childNodes = null;
						NodeList jwidgetElements = svgDoc
								.getElementsByTagName("rtda:jwidget");
						...
						for (int i = 0; i < jwidgetElements.getLength(); i++) {
							...
							imageNode = (Element) jwidgetElement
									.getParentNode();
						...
						for (Element imageElement : imagesList) {
							...
							// creating the set of the child nodes of the image
							// element
							...
							rectNode.setAttributeNS(null, "transform",
									imageElement.getAttribute("transform"));
							rectNode.setAttributeNS(null, "style",
									"fill:#ffffff;stroke:none;opacity:0;");

							// adding the children of the image node to the rect
							// node
							...
							// adding all the image child nodes to the rect
							// nodes
						...
						for (com.angrysnail.sheeta.rtda.NodeIterator it = new com.angrysnail.sheeta.rtda.NodeIterator(
								svgDoc.getDocumentElement()); it.hasNext();) {
							...
							if (node != null && node instanceof Element) {
								...
								com.angrysnail.sheeta.library.Toolkit
										.transformFromStyleToAttribute((Element) node);
							}
						}
				...
				// creating the progress bar that will be used to give
				// information on the process to
				// the user
				ProgressBarDialog pbd = null;
				Container container = viewBrowser.canvasContainer
						.getTopLevelAncestor();
				...
				if (container != null) {
					...
					if (container instanceof Frame) {
						...
						pbd = new ProgressBarDialog((Frame) container,
								taskLabel);

					} else if (container instanceof JApplet) {

						pbd = new ProgressBarDialog(
								viewBrowser.mainDisplay.getTopLevelFrame(),
								taskLabel);

					} else {

						pbd = new ProgressBarDialog((JDialog) container,
								taskLabel);
					}
					...
					Thread thread = new Thread() {
						...
						public void run() {
							...
							if (canvasFile != null) {
								...
								// getting the needed piece of information for
								// compiling the
								// application
								String xmlFilePath = doc.getDocumentElement()
										.getAttribute("referenceFile");
								String viewPath = doc.getDocumentElement()
										.getAttribute("viewPath");
								String projectPath = DataBaseToolkit
										.getProjectPath(furi, xmlFilePath, true);
								String startViewPath = doc.getDocumentElement()
										.getAttribute("referenceNode");

								if (startViewPath.endsWith("/")) {

									startViewPath = startViewPath.substring(0,
											startViewPath.length() - 1);
								}

								if (startViewPath.startsWith("/")) {

									startViewPath = startViewPath.substring(1,
											startViewPath.length());
								}

								viewPath = "/" + startViewPath + "/" + viewPath;

								// computing the workspace path and the project
								// name
								...
								// creating the temporary folder that will
								// contain the compiled
								// files
								File outputFolder = null;

								try {
									// creating a temporary file and getting the
									// folder of the
									// temporary files
									...
									outputFolder = new File(tempFolder,
											"GGTemp" + Math.random());
								...
								if (outputFolder != null) {
									...
									RTDBCompiler compiler = new RTDBCompiler(
											...
											xmlFilePath, outputFolder.toURI()
													.toASCIIString(), viewPath,
											true);
									compiler.compile();

									// getting the svg file corresponding to the
									// view
									File compiledSVGFile = null;

									try {
										File parentOutputFolder = new File(
												new URI(outputFolder.toURI()
														.toASCIIString()));

										// getting the output project file
										File outputProject = null;
										File[] children = parentOutputFolder
												.listFiles();
										String theUri = "", name = "";
									...
									// getting the svg document corresponding to
									// this file
									SVGDocument svgDocument = null;

									if (compiledSVGFile != null
											&& compiledSVGFile.exists()) {
										...
										try {
											svgDocument = documentFactory
													.createSVGDocument(compiledSVGFile
															.toURI()
															.toASCIIString());
										} catch (Exception ex) {
											ex.printStackTrace();
										...
										if (svgDocument != null) {
											...
											final String path = compiledSVGFile
													.toURI().toASCIIString();
	...
	public void setCurrentPicture(SVGPicture picture) {
		...
		if (picture != null) {
			...
			// getting the last picture uri that can be found in the history
			// list
			String lastUri = null;
	...
	protected void addInHistory(SVGPicture picture) {
		...
		if (picture != null) {
			...
			// removes the first occurence in the history list if its size is
			// superior to maxSize
			...
			if (history.size() == 0) {
				...
				// removing all the pictures that lie after the current picture
				// position
	...
	public void returnToPreviousPicture() {
		...
		if (history.size() > 1) {
			...
			previousPicture = viewBrowser.mainDisplay.getPictureManager()
					.getPicture(previousUri);
		...
		if (previousPicture != null && !previousPicture.isDisplayed()) {
			...
			viewBrowser.mainDisplay.getViewBrowsersManager().fireViewLoaded(
					projectName, viewBrowser.viewBrowserId,
					previousPicture.getUri());
		}
	...
	public void goToNextPicture() {
		...
		if (history.size() > 0) {
			...
			// getting the previous picture
			nextPicture = viewBrowser.mainDisplay.getPictureManager()
					.getPicture(nextUri);
