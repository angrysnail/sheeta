...
public class ViewBrowser extends JComponent {
	 ...
	 * the font
	 */
	protected static final Font theFont = new Font("smallFont",
	 ...
	 *            the id of the view browser
	 * @param showHeader
	 *            whether the header should be shown in the view browser
	 */
	public ViewBrowser(MainDisplay mainDisplay, String viewBrowserId,
			boolean showHeader) {
	...
	protected void buildComponent() {
		...
		if (showHeader) {
			...
			ImageIcon previousIcon = RtdaResources.getIcon("previousIcon",
					false), dpreviousIcon = RtdaResources.getIcon(
					"previousIcon", true), nextIcon = RtdaResources.getIcon(
					"nextIcon", false), dnextIcon = RtdaResources.getIcon(
					"nextIcon", true);
	...
	protected void handleButtonsState() {
		...
		if (previousButton != null) {
			...
			previousButton
					.setEnabled((pictureLoader.getHistory().size() > 1 && pictureLoader
							.getCurrentHistoryPosition() > 0));
		}

		if (nextButton != null) {

			nextButton
					.setEnabled(pictureLoader.getHistory().size() > 1
							&& pictureLoader.getCurrentHistoryPosition() < pictureLoader
									.getHistory().size() - 1);
		}
	}
