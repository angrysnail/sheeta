package com.angrysnail.sheeta.rtda.config;
...
public class ConfigurationDocument {
	 ...
	 * the strings
	 */
	protected static String rootGvcFileName = "root.gvc",
			enumeratedTagName = "tag.enumerated", itemTagName = "item",
			idAttributeName = "id",
	...
	protected void build() {
		...
		if (xmlFile != null) {
			...
			rootElement = DataBaseToolkit.getDocument(xmlFile)
					.getDocumentElement();

		} else {

			rootElement = DataBaseToolkit.getRtdaWidgetDataBase(picture
					.getDocument());
			isWidget = true;
		}
	...
	protected File getXMLFile(SVGPicture pict) {
		...
		for (int i = 0; i < children.length; i++) {
			...
			if (children[i] != null
					&& children[i].getName().equals(rootGvcFileName)) {
	...
	/**
	 * returns the authorization level for the view denoted by the provided view
	 * path
	 * 
	 * @param viewPath
	 *            the view path
	 * @return the authorization level for the view denoted by the provided view
	 *         path
	 */
	public int getAuthorizationLevelForView(String viewPath) {
		...
		try {
			authorizationLevel = Integer.parseInt(viewElement
					.getAttribute(viewAuthorizationLevelAtt));
		} catch (Exception ex) {
		}
	...
	public Element getElement(String tagName) {
		...
		if (tagName != null) {
			...
			// creating the list of the segments that are non empty string
			// values
			...
			if (element != null
					&& segmentsList.size() > 0
					&& element.getAttribute(idAttributeName).equals(
							segmentsList.get(0))) {
				...
				for (int i = 1; i < segmentsList.size(); i++) {
					...
					for (int j = 0; j < childNodes.getLength(); j++) {
						...
						if (node != null && node instanceof Element) {
							...
							if (cEl.getAttribute(idAttributeName).equals(
									segmentsList.get(i))) {
	...
	/**
	 * computes the map associating a tag name to its type for all the tags that
	...
	protected void computeTagTypes() {
		...
		for (NodeIterator iterator = new NodeIterator(rootElement); iterator
				.hasNext();) {
	...
	public LinkedList<String> getChildValues(String tagName) {
		...
		if (tagElement != null) {
			...
			// getting the item nodes of the element, so that the tag values can
			// be stored
			...
			for (int i = 0; i < items.getLength(); i++) {
				...
				if (node != null && node instanceof Element) {
					...
					if (id != null && !id.equals("")) {
						...
						tagValues.add(AnimationsToolkit
								.normalizeEnumeratedValue(id));
					}
				}
			}
		}
	...
	/**
	 * @return whether the svg document corresponding to this configuration
	 *         document is a widget
	 */
	...
	/**
	 * @return the object storing information on the rights of the application
	 */
