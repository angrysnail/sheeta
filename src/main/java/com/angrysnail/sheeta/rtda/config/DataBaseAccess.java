...
public class DataBaseAccess {
	 ...
	 *            the table name
	 */
	public DataBaseAccess(SVGPicture picture, String dbName, String tableName) {
		...
		if (confDoc != null && confDoc.getRootElement() != null) {
			...
			if (historyManagerElements != null) {
				...
				// getting the history manager element corresponding to the
				// provided db name
				...
				if (dbElement != null) {
					...
					subProtocol = dbElement
							.getAttribute(Toolkit.subProtocolAttName);
					...
					try {
						Class.forName(
								dbElement
										.getAttribute(Toolkit.driverNameAttName))
								.newInstance();
					} catch (Exception ex) {
						ex.printStackTrace();
					}
			...
			public void run() {
				...
				try {
					...
					if (request.toLowerCase().indexOf("{call") != -1) {
						...
						statement = configureCallableRequest(connection,
								request);
						resultSet = ((CallableStatement) statement)
								.executeQuery();
	 ...
	 *            the map associating a field name in the table to its value for
	 *            the row
	 */
	public void recordData(final Map<String, Object> fieldNameToValue) {
		...
		if (fieldNameToValue != null && fieldNameToValue.size() > 0) {
			...
			final boolean containsIdField = fieldNameToValue
				...
				TimerTask task = new TimerTask() {
					...
					public void run() {
						...
						if (doUpdate && idFieldValue != null) {
							...
							// getting the number of rows in the table that have
							// whose id value is
							// the current one
							String countRequest = "SELECT " + idFieldName
									+ " FROM " + tableName + " WHERE "
									+ idFieldName + "=\'"
									+ idFieldValue.toString() + "\'";

							try {
								// executing the request
								statement = connection.createStatement();
								ResultSet resultSet = statement
										.executeQuery(countRequest);
								connection.commit();
								doUpdate = resultSet != null
										&& resultSet.next();
							} catch (Exception ex) {
						...
						try {
							...
							if (doUpdate && idFieldValue != null) {
								...
								for (String fieldName : fieldNameToValue
										.keySet()) {
	...
	/**
	 * configures the provided callable request and creates the associated
	 * callable statement
	 * 
	 * @param request
	 *            a request
	 ...
	 */
	protected CallableStatement configureCallableRequest(Connection connection,
			String request) {
			...
			if (pos2 != -1) {
				...
				if (!paramStr.equals("")) {
					...
					if (newParams.endsWith("?, ")) {
						...
						newParams = newParams.substring(0,
								newParams.length() - 2);
					}
				}
			}
	 ...
	 * 
	 * @param statement
	 *            the callable statement to configure
	 * @param value
	 ...
	 */
	protected void setParameter(CallableStatement statement, String value,
