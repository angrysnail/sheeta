...
public class Profile {
	 ...
	 */
	protected static final String idAtt = "id",
			rightForTagModificationAtt = "rightForTagModification",
			rightForViewLoadingAtt = "rightForViewLoading",
			rightForRecipeLoadingAtt = "rightForRecipeLoading",
			rightForRecipeModificationAtt = "rightForRecipeModificationAtt",
			rightForProgramLaunchingAtt = "rightForProgramLaunching",
			timeOutAtt = "timeout";
	...
	/**
	 * the time out before which the logged user will be disconnected
	 */
	 ...
	 * 
	 * @param element
	 *            the element describing the profile
	 */
	public Profile(Element element) {
		...
		try {
			rightForTagModification = Integer.parseInt(element
					.getAttribute(rightForTagModificationAtt));
		} catch (Exception ex) {
		}

		try {
			rightForViewLoading = Integer.parseInt(element
					.getAttribute(rightForViewLoadingAtt));
		} catch (Exception ex) {
		}

		try {
			rightForRecipeLoading = Integer.parseInt(element
					.getAttribute(rightForRecipeLoadingAtt));
		} catch (Exception ex) {
		}

		try {
			rightForRecipeModification = Integer.parseInt(element
					.getAttribute(rightForRecipeModificationAtt));
		} catch (Exception ex) {
		}

		try {
			rightForProgramLaunching = Integer.parseInt(element
					.getAttribute(rightForProgramLaunchingAtt));
		} catch (Exception ex) {
		}

		try {
			timeOut = Integer.parseInt(element.getAttribute(timeOutAtt));
		} catch (Exception ex) {
		}
	}
	...
	/**
	 * @return the time out before which the logged user will be disconnected
	 */
