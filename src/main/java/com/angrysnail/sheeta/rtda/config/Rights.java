...
public class Rights {
	 ...
	 * the element names
	 */
	protected static final String rightsName = "rights",
			profileName = "profile", userName = "user",
			tagForUserAttName = "tagForUser";
	...
	public Rights(Element rootElement) {
		...
		if (rootElement != null) {
			...
			NodeList rightsElements = rootElement
					.getElementsByTagName(rightsName);
			...
			if (rightsElements != null && rightsElements.getLength() > 0) {
				...
				if (rightsElement != null) {
					...
					NodeList profileElements = rootElement
					...
					NodeList userElements = rootElement
