...
public class UserRights {
	 ...
	 */
	private Set<RightsListener> rightsListenerSet = new HashSet<RightsListener>();
	...
	public void setCurrentPicture(SVGPicture newPicture) {
		...
		if (newPicture != null
				&& (picture == null || !newPicture.getCanvas().getProjectFile()
						.equals(picture.getCanvas().getProjectFile()))) {
	 ...
	 *            the component relatively to which the options dialog should be
	 *            displayed
	 * @param login
	 ...
	 */
	public boolean setCurrentUser(JComponent relativeComponent, String login,
			String password) {
			...
			if (configDoc != null) {
				...
				if (user == null) {
					...
					if (profile != null) {
						...
						rightForTagModification = profile
								.getRightForTagModification();
						rightForViewLoading = profile.getRightForViewLoading();
						rightForRecipeLoading = profile
								.getRightForRecipeLoading();
						rightForRecipeModification = profile
								.getRightForRecipeModification();
						rightForProgramLaunching = profile
								.getRightForProgramLaunching();
						userName = user.getId();
						...
						if (profile.getTimeOut() != 0) {
							...
							timerTask = new TimerTask() {
								...
								public void run() {
									...
									if (fmainPicture != null
											&& fpicture != mainPicture) {
	 ...
	 * 
	 * @param notify
	 *            whether to notify that the rights have changed
	 */
	...
	protected void notifyUserNameChanged() {
		...
		for (RightsListener listener : new HashSet<RightsListener>(
				rightsListenerSet)) {
	...
	protected void refreshJWidgets() {
		...
		Set<JWidgetRuntime> set = new HashSet<JWidgetRuntime>(mainDisplay
				.getJwidgetRuntimeManager().getJWidgetRuntimeObjects());
	...
	protected void showWrongLoginDialog(JComponent relativeComponent) {
		...
		JOptionPane.showMessageDialog(relativeComponent, wrongLoginLabel,
	...
	protected void showWrongPasswordDialog(JComponent relativeComponent) {
		...
		JOptionPane.showMessageDialog(relativeComponent, wrongPasswordLabel,
				errorLabel, JOptionPane.ERROR_MESSAGE);
	}
}
