 ...
 */
package com.angrysnail.sheeta.rtda.database;
...
public class DataBaseToolkit {
	 ...
	 * @param isEditionMode
	 *            whether the current mode is the edition or runtime mode
	 * @return the path of the project of the canvas path
	 */
	public static String getProjectPath(String canvasPath,
			String referenceFileName, boolean isEditionMode) {
		...
		if (canvasPath != null && referenceFileName != null) {
			...
			try {
				projectFile = AnimationsToolkit.getProjectFile(new URI(
						canvasPath), isEditionMode);
			} catch (Exception ex) {
			}
	 ...
	 */
	public static String getProjectName(String canvasPath,
			String referenceFileName, boolean isEditionMode) {
	...
	/**
	 * returns the root element of the widget database (creates it if it does
	 * not exist)
	 * 
	 * @param svgDoc
	 *            a widget svg document
	...
	public static Element getRtdaWidgetDataBase(Document svgDoc) {
		...
		if (svgDoc != null) {
			...
			NodeList nodeList = svgDoc.getDocumentElement()
					.getElementsByTagName("rtda:" + widgetDatabaseRootTagName);
			...
			if (widgetDataBaseRoot == null) {
				...
				// as the svg file does not contain any widget database, the
				// root node of this
				// database is created
				widgetDataBaseRoot = svgDoc.createElementNS(null, "rtda:"
						+ widgetDatabaseRootTagName);
	 ...
	 * found at the given reference file name and having the given reference
	 * node
	 * 
	 * @param canvasPath
	 ...
	 * @param referenceNode
	 *            the reference node
	 * @return the root element of the xml database that can be found at the
	 *         given reference file name and having the given reference node
	 */
	public static Element getRootDataBaseElement(String canvasPath,
			String referenceFileName, String referenceNode) {
		...
		if (canvasPath != null && referenceFileName != null
			...
			if (projectFile != null) {
				...
				try {
					workspacePath = projectFile.getParentFile().toURI()
							.toASCIIString();
				} catch (Exception ex) {
					workspacePath = null;
				}
				...
				if (workspacePath != null && !workspacePath.equals("")
					...
					LinkedHashMap<String, LinkedList<String>> libPathsMap = DataBaseToolkit
							.getLibPathFileNamesMap(workspacePath, projectName);

					if (libPathsMap != null) {

						// creating the xml file that will build the data base
						// tree
						...
						if (doc != null) {
							...
							// retrieving the appropriate node corresponding to
							// the given reference
							// node//

							// getting the list of each segment of the reference
							// node path
							String[] splitNodePath = Toolkit
									.getSplitPath(referenceNode);

							if (splitNodePath != null
									&& splitNodePath.length > 0) {
								...
								if (element != null
									...
									// handling the child nodes under the root
									// node
									start :

									for (i = 1; i < splitNodePath.length
											&& children != null; i++) {

										for (j = 0; j < children.getLength(); j++) {

											if (children.item(j) != null
													&& children.item(j) instanceof Element) {

												element = (Element) children
														.item(j);

												// checking if one of the nodes
												// has the current id
												if (element
														.getAttribute("id")
														.equals(splitNodePath[i])) {

													// setting the new root data
													// base element
													rootDataBaseElement = element;
													break;
												}
										...
										// getting the children of the current
										// node
										children = rootDataBaseElement
												.getChildNodes();
									}
								}
							}
	 ...
	 * 
	 * @param imageViewURI
	 *            the uri of the image of a view
	 * @param canvasSVGURI
	 *            the canvas uri into which the image having the imageViewPath
	...
	public static String getAbsoluteLocation(URI imageViewURI, URI canvasSVGURI) {
		...
		if (canvasSVGPath != null && !canvasSVGPath.equals("")
			...
			// relativizes the given baseSVGPath to the ".project" file of the
			// current project
			int i, j;
			...
			if (splitBaseSVGPath != null && canvasSVGFile != null) {
				...
				// checking the index of the element of the path from which the
				// ".project" file can
				// be found
				for (i = splitBaseSVGPath.length - 1; i >= 0; i--) {
					...
					if (uri != null) {
						...
						// creating the file corresponding to the previously
						// created path
						file = new File(uri);

						if (file.exists()) {

							// getting the file under the project directory
							// whose name is ".project"
							...
							if (files != null && files.length > 0) {
								...
								try {
									...
									location = getAbsolutePath(workspacePath,
											projectName, imageViewPath);
								} catch (Exception ex) {
									location = "";
									ex.printStackTrace();
								}
	...
	protected static String getAbsolutePath(String workspacePath,
		...
		if (workspacePath != null && baseProjectName != null
				&& imagePath != null) {

			// getting the collection of the names of the projects that are used
			// in the given
			// project
			Collection<String> libPaths = getLibPathFileNamesList(
					workspacePath, baseProjectName);
			libPaths.add(baseProjectName);

			File file = null;
			String foundProjectName = "";

			// searching in the projects if the file denoted by the given image
			// path exists
			...
			if (!foundProjectName.equals("")) {
				...
				absolutePath = workspacePath + "/" + foundProjectName + "/"
						+ imagePath;
			}
		}
	 ...
	 * 
	 * @param libPathsFile
	 *            the file of a of the lib paths
					...
					if (children != null) {
						...
						for (j = 0; j < children.getLength(); j++) {
							...
							if (children.item(j) != null
								...
								libPaths.add(URIEncoderDecoder.encode(children
										.item(j).getNodeValue()));
							}
						}
					}
	...
	protected static LinkedList<String> getLibPathFileNamesList(
		...
		// Getting the map of the lib paths
		LinkedHashMap<String, LinkedList<String>> libPathMap = getLibPathFileNamesMap(
				workspacePath, currentProjectName);

		if (libPathMap != null) {

			// getting the list of the lib path for the current project name
			libPathsList = new LinkedList<String>(
					libPathMap.get(currentProjectName));
		}
	 ...
	 */
	public static LinkedHashMap<String, LinkedList<String>> getLibPathFileNamesMap(
			String workspacePath, String currentProjectName) {

		// the map associating a project name to the list of its referenced
		// project names
		LinkedHashMap<String, LinkedList<String>> libPathsMap = new LinkedHashMap<String, LinkedList<String>>();
			...
			if (projectName != null && !projectName.equals("")) {
				...
				for (String name : list) {
					...
					if (name != null && !name.equals("")
							&& !projectNamesList.contains(name)) {
		...
		boolean isCorrect = checkCycle(libPathsMap, new LinkedList<String>(),
				currentProjectName);
	...
	protected static boolean checkCycle(
		...
		if (libPathsMap != null && projectList != null
				&& currentProjectName != null && !currentProjectName.equals("")) {
					...
					for (String name : list) {
						...
						if (name != null && !name.equals("")) {
							...
							isCorrect = isCorrect
									&& checkCycle(
											libPathsMap,
											new LinkedList<String>(projectList),
											name);
						}
					}
				}
