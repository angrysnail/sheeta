...
public class XMLFile {
	...
	public XMLFile(LinkedHashMap<String, LinkedList<String>> libPathsMap,
		...
		try {
			// creating the file
			this.xmlFile = new File(new URI(workspacePath + "/" + projectName
					+ "/" + name));
	...
	public Document resolveInheritance() {
		...
		// creating the list containing the file name of each previously handled
		// files
	...
	/**
	 * fills the map of the files specified in each inherit attribute,
	 * associating the value of the attribute to its related xml file
	 */
	...
	protected XMLFile getXMLFile(String fileName) {
		...
		if (fileName != null && !fileName.equals("")
				&& fileName.endsWith(AnimationsToolkit.GLIPS_VIEW_EXTENSION)) {

			String currentProjectName = getProjectName(this.projectName,
					fileName);

			if (currentProjectName != null && !currentProjectName.equals("")) {

				// creating the xml file
				returnFile = new XMLFile(this.libPathsMap, this.workspacePath,
	...
	public String getProjectName(String sourceProjectName, String fileName) {
		...
		if (fileName != null && !fileName.equals("")) {
			...
			LinkedList<String> libPathList = new LinkedList<String>(
					this.libPathsMap.get(sourceProjectName));
			...
			for (String currentProjectName : libPathList) {
				...
				if (currentProjectName != null
					...
					try {
						file = new File(new URI(this.workspacePath + "/"
								+ currentProjectName + "/" + fileName));
					} catch (Exception ex) {
						file = null;
					}
			...
			for (Element cur : elementWithInheritAttribute) {
				...
				if (cur != null) {
					...
					if (file != null && file.getRootElement() != null) {
						...
						// document of the
						// current xml file
						inheritRoot = (Element) this.doc.importNode(
