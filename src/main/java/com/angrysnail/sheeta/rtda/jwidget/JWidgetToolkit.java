...
/**
 * the abstract class of all the jwidget toolkits, handling features shared by
 * the edition and runtime jwidget objects
 * 
...
public abstract class JWidgetToolkit {
	 ...
	 */
	public static String[] fontAttributesNames = {"foregroundColor",
	 ...
	 */
	public static Rectangle computeBounds(SVGPicture picture,
			Element jwidgetElement) {

		Rectangle2D bounds = AnimationsToolkit.getNodeBounds(picture,
				jwidgetElement.getParentNode());

		Rectangle componentBounds = new Rectangle((int) bounds.getX(),
	 ...
	 *            the array of the attributes for the look
	 * @param component
	 *            the component whose look has to be handled
	 */
	 ...
	 *            a jwidget element
	 * @param attributeName
	 *            the name of the attribute defining the color
	 * @param component
	 *            the component whose font should be modified
	 */
	public static void handleForegroundColor(Element jwidgetElement,
		...
		Set<Component> cmpSet = new HashSet<Component>();
		cmpSet.add(component);
		handleLook(jwidgetElement, new String[]{attributeName, "", "", "", ""},
				cmpSet);
	}

	/**
	 * handles the background and border look attributes of the given jwidget
	 * elements
	 * 
	 * @param jwidgetElement
	 *            a jwidget element
	 ...
	 *            the component whose look has to be handled
	 */
	public static void handleBackgroundAndBorderLook(Element jwidgetElement,
			Component component) {
		...
		try {
			displayBorder = Boolean.parseBoolean(jwidgetElement
					.getAttribute(displayBorderName));
		} catch (Exception ex) {
		}
		...
		if (!displayBorder) {
			...
			if (component instanceof JScrollPane) {
				...
				((JScrollPane) component).setViewportBorder(new EmptyBorder(0,
						0, 0, 0));
			}
	 ...
	 *            a jwidget element
	 * @param textField
	 *            the textField whose alignment has to be handled
	 */
	public static void handleAlignment(Element jwidgetElement,
	 ...
	 *            a jwidget element
	 * @param table
	 *            the table whose row height should be set
	 */
	public static void handleRowHeight(Element jwidgetElement, JTable table) {
		...
		try {
			rowHeight = Integer.parseInt(jwidgetElement
					.getAttribute(rowHeightName));
		} catch (Exception ex) {
		}
	 ...
	 *            a jwidget element
	 * @param components
	 *            the components whose look has to be handled
	 */
	public static void handleLook(Element jwidgetElement,
	...
	public static void handleLook(Element jwidgetElement,
		...
		try {
			isBold = Boolean.parseBoolean(jwidgetElement
					.getAttribute(attributeNames[3]));
		} catch (Exception ex) {
		}
		...
		try {
			isItalic = Boolean.parseBoolean(jwidgetElement
					.getAttribute(attributeNames[4]));
		} catch (Exception ex) {
		}
	...
	public static void computeColumnsWidth(JTable table) {
		...
		if (g != null) {
			...
			if (totalCharSize < parentWidth && columnsCount > 0) {
				...
				remainingSpaceForEachColumn = (parentWidth - totalCharSize)
						/ columnsCount;
			}
