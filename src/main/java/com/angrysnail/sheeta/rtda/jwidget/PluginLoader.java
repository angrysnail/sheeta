...
public class PluginLoader {
	...
	/**
	 * the map associating an abstract class to the map associating the id of a
	 * plugin to this plugin class
	 */
	private static final Map<Class<?>, LinkedHashMap<String, Class<?>>> abstractClassToPlugins = new HashMap<Class<?>, LinkedHashMap<String, Class<?>>>();

	/**
	 * the map associating an id to a document
	 */
	private static final Map<String, Document> jwidgetIdToDoc = new HashMap<String, Document>();
	...
	static {
		...
		if (applicationLocationFile != null && applicationLocationFile.exists()) {
			...
			if (children != null) {
				...
				// for each children of the application directory, retrieves the
				// "plugins" directory
				for (i = 0; i < children.length; i++) {

					if (children[i] != null
							&& children[i].getName().equals("GLIPSSVGPlugins")) {
				...
				if (pluginDirectory != null) {
					...
					if (children != null) {
						...
						// for each child of the plugin directory, if this child
						// is a ".jar" file,
						// it is added to the list of the plugins
						for (i = 0; i < children.length; i++) {

							if (children[i] != null
									&& children[i].getName().endsWith(
											jarExtension)) {
	...
	public static Class<?> getPlugin(Class<?> relatedAbstractClass, String id) {
		...
		if (id != null && !id.equals("")) {
			...
			// checks if the abstract class has already been handled, if not,
			...
			Map<String, Class<?>> map = abstractClassToPlugins
					.get(relatedAbstractClass);
	...
	/**
	 * returns the list of the classes contained in the plugin directory and
	 * that implements the given abstract class
	 * 
	 * @param relatedAbstractClass
	 *            an abstract class
	 * @return the list of the classes contained in the plugin directory and
	 *         that implements the given abstract class
	 */
	public static LinkedList<Class<?>> getPlugins(Class<?> relatedAbstractClass) {
		...
		if (relatedAbstractClass != null && foundJars.size() > 0
			...
			// for each jar file, retrieves the class that implements the given
			// abstract class
			for (File file : foundJars) {

				if (file != null) {

					// getting the class contained in the jar that implements
					// the given abstract
					// class
					try {
						foundClasses = getClasses(file, relatedAbstractClass);
					} catch (Exception ex) {
						foundClasses = null;
					}
	...
	/**
	 * returns the classes contained in the given file and that implements the
	 ...
	 * @param relatedAbstractClass
	 *            an abstract class
	 * @return the classes contained in the given file and that implements the
	 *         given abstract class
	 */
	protected static HashSet<Class<?>> getClasses(File file,
			Class<?> relatedAbstractClass) {
		...
		if (file != null && relatedAbstractClass != null) {
			...
			if (jarFile != null && url != null) {
				...
				if (urlClassLoader != null) {
					...
					// for each file in the jar, checks if it is a class and
					// then, loads it
						...
						if (entry != null) {
							...
							if (name != null && name.endsWith(classExtension)) {
								...
								if (name != null && !name.equals("")) {
									...
									if (loadedClass != null
											&& !Modifier.isAbstract(loadedClass
													.getModifiers())
											&& relatedAbstractClass
													.isAssignableFrom(loadedClass)) {

										// the loaded class extends the abstract
										// class
	...
	public static Map<String, Document> getJWidgetDocuments() {
		...
		if (!jwidgetDocumentsLoaded) {
			...
			// loading all the jwidget documents that can be found in the jar
			// plugins
	...
	protected static Map<String, Document> getJWidgetDocument(File file) {
		...
		if (file != null) {
			...
			if (jarFile != null) {
				...
				// for each file in the jar, checks if it is a class and then,
				// loads it
				while (enumeration.hasMoreElements()) {
					...
					if (entry != null) {
						...
						if (name != null && name.endsWith(xmlExtension)) {
							...
							if (pos != -1) {
								...
								id = name.substring(pos + resourceSep.length(),
										name.length() - xmlExtension.length());

							} else {

								id = name.substring(0, name.length()
										- xmlExtension.length());
							}
							...
							if (id != null && !id.equals("")
								...
								try {
									doc = documentBuilder.parse(jarFile
