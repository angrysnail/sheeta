 ...
 */
package com.angrysnail.sheeta.rtda.rtdp;
...
public class RealTimeDataProviderTestSimulator extends RealTimeDataProvider {
	...
	/**
	 * the map associating the name of a tag to the list of the
	 * "TestSimulationTagValues" objects providing to the simulator the values
	 * of the tags specified by the user
	 */
	private Map<String, TestSimulationTagValues> testSimulationValuesSpecifiersMap = new ConcurrentHashMap<String, TestSimulationTagValues>();
			...
			public void run() {
				...
				while (true) {
					...
					if (isStarted()) {
						...
						for (TestSimulationTagValues testValues : testSimulationValuesSpecifiersMap
								.values()) {
							...
							if (testValues != null) {
								...
								if (dataNames.contains(testValues.getTagName())
									...
									setDataValueForAnimationsHandler(
											testValues.getTagName(),
											testValues.getTagValue());
								}
							}
						}
						...
						if (!RealTimeDataProviderTestSimulator.this.handler
								.isPaused()) {
	...
	public void setDataValue(String name, Object newValue) {
		...
		if (name != null && !name.equals("") && dataNames.contains(name)) {
			...
			// getting the test simulation object corresponding to the short
			// name
			TestSimulationTagValues testValues = testSimulationValuesSpecifiersMap
					.get(name);
	...
	/**
	 * refreshes the list of the "TestSimulationTagValues" describing the values
	 * the user should provide to simulate the animations
	 * 
	 * @return the list of the "TestSimulationTagValues" describing the values
	 *         the user should provide to simulate the animations
	 */
	public LinkedList<TestSimulationTagValues> refreshSimulationValuesSpecifiers() {
		...
		Map<String, Set<TestTagInformation>> infos = handler
				.getDataToInformationMap();

		if (infos != null) {

			// the list of the tag names that will be modified
			List<String> tagNamesToHandle = new LinkedList<String>(
					infos.keySet());
			...
			for (String tagName : tagNamesToHandle) {
				...
				if (tagName != null) {
					...
					if (testInfos != null) {
						...
						// builds the set of the possible values of a tag if it
						// is an enumerated tag
						...
						if (testValues != null) {
							...
							// as the simulator already exists, the enumerated
							// values that already
							// existed
							// and the newly found ones are merged
							enumeratedTagValues.addAll(testValues
									.getEnumeratedValues());
						}
						...
						if (testValues != null) {
							...
							// creating the object that will be used for
							// specifying
							...
							if (rightInfo != null) {
								...
								if (tagType == TagToolkit.ANALOGIC_FLOAT
									...
									// getting the min and max values for the
									// tag
									picture = rightInfo.getPicture();
									configDoc = picture.getMainDisplay()
											.getPictureManager()
											.getConfigurationDocument(picture);

									if (configDoc != null) {

										tagElement = configDoc
												.getElement(tagName);

										if (tagElement != null) {

											// computing the min and the max
											// values
											try {
												minVal = Double
														.parseDouble(tagElement
																.getAttribute("min"));
											} catch (Exception ex) {
												minVal = 0;
											}
								...
								testValues = new TestSimulationTagValues(
										tagName, value, tagType, enumValues,
										minVal, maxVal);
