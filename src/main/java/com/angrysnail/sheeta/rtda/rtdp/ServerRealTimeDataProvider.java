...
public class ServerRealTimeDataProvider extends RealTimeDataProvider {
	 ...
	 *            the IP address of the server
	 */
	public ServerRealTimeDataProvider(AnimationsHandler handler,
			String IPAddress) {
								...
								if (line != null) {
									...
									if (splitLine != null
										...
										setDataValueForAnimationsHandler(
												splitLine[0],
												splitLine[1]
														.equals(AnimationsHandler.invalidKeyWord)
														? null
														: splitLine[1]);// TODO
									}
				...
				if (serverIPAddress != null && !serverIPAddress.equals("")) {
					...
					try {
						...
						if (handler.isDebugMode()) {
							...
							System.out.println("sending tag=" + name + " "
									+ newValue);
						}
		...
		if (serverIPAddress != null && !serverIPAddress.equals("")) {
			...
			Thread thread = new Thread() {
				...
				public void run() {
					...
					try {
						String urlStr = "http://" + serverIPAddress + ":8080"
								+ "/unloadview?View=" + path + "&Port=" + port;
						URL url = new URL(urlStr);
