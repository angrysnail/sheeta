...
public class NumberChooser {
	...
	/**
	 * whether the last display of the number chooser ends with a OK or a CANCEL
	 * action
	 */
	 ...
	 * the labels
	 */
	private String okLabel = "", cancelLabel = "", clearLabel = "",
			titleLabel = "", errorTitle = "", errorMessage = "";
	...
	public NumberChooser(ResourceBundle bundle) {
		...
		displayPanel.setBorder(new CompoundBorder(new EmptyBorder(4, 1, 1, 1),
		...
		final JButton button0 = new JButton("0"), button1 = new JButton("1"), button2 = new JButton(
				"2"), button3 = new JButton("3"), button4 = new JButton("4"), button5 = new JButton(
				"5"), button6 = new JButton("6"), button7 = new JButton("7"), button8 = new JButton(
				"8"), button9 = new JButton("9");
		...
		buttonsPanel.setBorder(new CompoundBorder(new EmptyBorder(1, 1, 1, 1),
				new EtchedBorder(EtchedBorder.LOWERED)));
		...
		final JButton okButton = new JButton(okLabel), cancelButton = new JButton(
				cancelLabel), clearButton = new JButton(clearLabel);
		...
		final ActionListener buttonsListener = new ActionListener() {
			...
			public void actionPerformed(ActionEvent evt) {
				...
				// removing the '0' characters at the beginning of the string
				// buffer
				while (displayString.length() > 0
						&& displayString.charAt(0) == '0') {
				...
				if (displayString.length() <= 20) {
					...
					if (evt.getSource().equals(button0)) {
						...
						if (displayString.length() > 0
								&& displayString.indexOf("E") == -1
								&& displayString
										.charAt(displayString.length() - 1) != '.') {
						...
						if (displayString.length() > 0) {
							...
							if (displayString.indexOf(".") != -1) {
								...
								if (ind != -1
										&& ind < displayString.length() - 1
										&& displayString.lastIndexOf(".") < ind) {
				...
				} else if (evt.getSource().equals(buttonMinus)
				...
				if (dialog != null) {
					...
					if (!evt.getSource().equals(okButton)) {
						...
						if (!Double.isNaN(currentValue)
							...
							// shows a dialog informing the user that the
	 ...
	 * @return whether the display of the number chooser ends with a OK or a
	 *         CANCEL action
	 */
	public int showNumberChooser(Container parent) {
		...
		if (parent != null) {
			...
			if (!(parent instanceof Frame) && !(parent instanceof JDialog)
				...
				// getting the container into which the number chooser will be
				// displayed
			...
			if (parent != null) {
				...
				if (dialog != null) {
					...
					int x = (int) (parent.getLocationOnScreen().getX()
							+ parent.getWidth() / 2 - dialog.getSize()
							.getWidth() / 2), y = (int) (parent
							.getLocationOnScreen().getY()
							+ parent.getHeight()
							/ 2 - dialog.getSize().getHeight() / 2);
