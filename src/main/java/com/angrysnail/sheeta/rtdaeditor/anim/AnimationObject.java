...
public class AnimationObject extends ItemObject {
	...
	public AnimationObject(JWidgetManager jwidgetManager,
		...
		if (Toolkit.hasJWidgetChildElement(parentElement)) {
			...
			if (jwidgetEdition != null) {
				...
				setNewSpecAnimationDocument(jwidgetEdition
		...
		if (returnedObjects != null && returnedObjects[0] != null
		...
		if (specAnimationElement != null) {
			...
			type = EditableItem.getType(specAnimationElement
					.getAttribute("type"));
			...
			for (Node node = specAnimationElement.getFirstChild(); node != null; node = node
					.getNextSibling()) {
				...
				if (node.getNodeName().equals("attribute")
					...
					if (cur.hasAttribute("group")) {
						...
						if (groupElements != null && groupElements.size() > 0
								&& currentGroup != null
								&& !currentGroup.equals(groupValue)) {

							// handles the nodes of the previous group
							tmpAttributes.put(currentGroup,
									new AttributeObject(this, animationElement,
											groupElements));
							groupElements = null;
							currentGroup = null;
						}
						...
						// adding the element to the list of the elements being
						// in the same group
						...
						tmpAttributes.put(name, new AttributeObject(this,
								animationElement, cur));
					}
				}
			}
			...
			if (groupElements != null && groupElements.size() > 0) {
				...
				// handles the nodes of the previous group
				tmpAttributes.put(currentGroup, new AttributeObject(this,
						animationElement, groupElements));
			}
		...
		undoRedoLabel = getObjectType() == ACTION
				? actionUndoRedoLabel
				: animationUndoRedoLabel;
	...
	public void handleChildren() {
		...
		if (specAnimationElement != null) {
			...
			NodeList childNodes = specAnimationElement
			...
			if (childNodes != null && childNodes.getLength() > 0) {
				...
				Element cur = null;
				// whether this child requires a xml view path among all the
				// available ones
				...
				for (Node node = specChildElement.getFirstChild(); node != null; node = node
						.getNextSibling()) {
					...
					if (node.getNodeName().equals("attribute")
						...
						if (cur.getAttribute("constraint").equals(
								viewPathConstraint)) {
				...
				if (hasViewPathConstraint) {
					...
					LinkedList<String> viewPaths = DataBaseNodeToolkit
							.getAllAvailableViewPath(animationElement
									.getOwnerDocument());

					// getting the map associating a view xml path to an
					// existing child node
					HashMap<String, Element> viewPathToElement = new HashMap<String, Element>();
					NodeList childrenElements = animationElement
							.getElementsByTagName(specChildElement
									.getAttribute("name"));
					...
					if (childrenElements != null) {
						...
						for (int i = 0; i < childrenElements.getLength(); i++) {
							...
							// storing the child view path and the element in
							// the map
							child = (Element) childrenElements.item(i);
							childViewPath = child
									.getAttribute(viewPathAttribute);
							viewPathToElement.put(childViewPath, child);
						}
					}

					// creating the child elements corresponding to each found
					// view path
						...
						if (!viewPathToElement.containsKey(viewXMLPath)) {
							...
							newChild = animationElement.getOwnerDocument()
									.createElementNS(
											...
											specChildElement
													.getAttribute("name"));
							newChild.setAttribute(viewPathAttribute,
									viewXMLPath);
				...
				if (childrenElements != null) {
					...
					for (int i = 0; i < childrenElements.getLength(); i++) {
						...
						child = (Element) childrenElements.item(i);
						childrenList.add(new ChildObject(this, child,
								specChildElement));
					}
				}
			}
	...
	public String getChildTagValueAttribute() {
		...
		if (specChildElement != null) {
			...
			NodeList childAttributes = specChildElement
	 ...
	 *            the runnable for the execution of the action
	 * @param undoRunnable
	 *            the undo runnable
	 * @param redoRunnable
	 *            the redo runnable
	...
	public void addUndoRedoAction(Runnable executeRunnable,
		...
		UndoRedoAction action = new UndoRedoAction(undoRedoLabel,
				executeRunnable, undoRunnable, redoRunnable,
				new HashSet<Element>());

		// creates the undo/redo list so that actions can be added to it
		UndoRedoActionList actionlist = new UndoRedoActionList(undoRedoLabel,
				false);
	...
	public String getJWidgetSource() {
		...
		return animationElement
				.getAttribute(com.angrysnail.sheeta.library.Toolkit.sourceAttributeName);
	}
