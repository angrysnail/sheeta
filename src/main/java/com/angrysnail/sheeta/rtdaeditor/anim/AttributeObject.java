package com.angrysnail.sheeta.rtdaeditor.anim;
...
public class AttributeObject extends EditableItem {
	 ...
	 */
	private final static String rtdaAnimLabelPrefix = "rtdaanim_",
			tagMinAtt = "tagMin", tagMaxAtt = "tagMax";
	...
	/**
	 * this list is only used if this attribute is a group one it contains the
	 * list of the attribute objects contained in the group attribute
	 */
	 ...
	 * the svg handle
	 */
	private SVGHandle handle = Editor.getEditor().getHandlesManager()
			.getCurrentHandle();
	 ...
	 *            the parent element
	 * @param specElements
	 *            the list of the spec elements for this object
	 */
	public AttributeObject(AnimationObject animationObject,
		...
		if (specElements != null && specElements.size() > 1) {
			...
			for (Element attSpecElement : specElements) {
				...
				groupAttributes.add(new AttributeObject(animationObject,
						parentElement, attSpecElement));
			}
	 ...
	 *            the parent element
	 * @param specElement
	 *            the spec element for this object
	 */
	public AttributeObject(AnimationObject animationObject,
			Element parentElement, Element specElement) {
	...
	public LinkedHashMap<String, String> getPossibleValues() {
		...
		if (getType().equals(EditableItem.TAG_VALUES_CHOOSER)
			...
			for (AttributeObject attributeObject : animationObject
					.getAttributesList()) {
			...
			if (tag != null && !tag.equals("")) {
				...
				LinkedList<String> tagValues = DataBaseNodeToolkit
						.getEnumeratedTagValues(getParentElement()
								.getOwnerDocument(), tag);
			...
			Map<String, Object> blinkings = ((RtdaColorChooserModule) Editor
					.getColorChooser()).getColorsAndBlinkingsToolkit()
					.getBlinkingsMap(
							Editor.getEditor().getHandlesManager()
									.getCurrentHandle().getScrollPane()
									.getSVGCanvas().getProjectFile());
			...
			for (Node cur = parentElement.getFirstChild(); cur != null; cur = cur
					.getNextSibling()) {
	...
	public String getType() {
		...
		if (isGroupAttribute) {
			...
			if (groupAttributes.size() > 0) {
				...
				value = groupAttributes.getFirst().getSpecElement()
						.getAttribute("type");
			}
	...
	public String getValue() {
		...
		if (isGroupAttribute) {
			...
			for (AttributeObject obj : groupAttributes) {
				...
				value += obj.getParentElement().getAttribute(obj.getName())
						+ separator;
			}
	...
	public void validateChanges() {
		...
		if (currentValue != null) {
			...
			if (isGroupAttribute) {
				...
				String cur = "";
				int pos = -1;
				final HashMap<String, String> attributeNameToValue = new HashMap<String, String>();
				final HashMap<String, String> lastAttributeNameToValue = new HashMap<String, String>();
				...
				for (AttributeObject obj : groupAttributes) {
					...
					if (pos != -1) {
						...
						value = value.substring(pos + separator.length(),
								value.length());
						lastAttributeNameToValue.put(obj.getName(),
								obj.getValue());
				...
				Runnable executeRunnable = new Runnable() {
					...
					public void run() {
						...
						if (fselectedElement != null) {
							...
							hnd.getSelection().handleSelection(
									fselectedElement, false, true);
						}
					}
				};
				...
				animationObject.addUndoRedoAction(executeRunnable,
				...
				final Element animationElement = animationObject
						.getAnimationElement();
				final Element childSpecElement = animationObject
						.getSpecChildElement();
				final SVGHandle hnd = handle;
				...
				if (getType().equals(EditableItem.TAG_CHOOSER)
					...
					final LinkedList<String> tagValues = DataBaseNodeToolkit
							.getEnumeratedTagValues(getDocument(), value);
					final String tagValueAttribute = animationObject
							.getChildTagValueAttribute();
					...
					if (!newValue.equals("")) {
						...
						for (String tagValue : tagValues) {
							...
							newChild = animationElement.getOwnerDocument()
									.createElementNS(
							...
							for (Node node = animationObject
									.getSpecChildElement().getFirstChild(); node != null; node = node
									.getNextSibling()) {
								...
								if (node instanceof Element) {
									...
									newChild.setAttribute(
											att.getAttribute(ItemObject.nameAttribute),
					...
					Runnable executeRunnable = new Runnable() {
						...
						public void run() {
							...
							hnd.getSvgDOMListenerManager()
									.fireStructureChanged(animationElement,
											animationElement);
					...
					Point2D point = DataBaseNodeToolkit.getMinAndMax(
					...
					final String oldMinVal = directParentElement
							.getAttribute(tagMinAtt);
					final String oldMaxVal = directParentElement
							.getAttribute(tagMaxAtt);
					...
					Runnable executeRunnable = new Runnable() {
						...
						public void run() {
							...
							if (!fminVal.equals("")) {
								...
								directParentElement.setAttribute(tagMinAtt,
										fminVal);
							}

							if (!fmaxVal.equals("")) {

								directParentElement.setAttribute(tagMaxAtt,
										fmaxVal);
							}

							directParentElement.setAttribute(name, newValue);
							hnd.getSvgDOMListenerManager().fireNodeChanged(
									directParentElement);
						}
					};
	...
	protected String getSpecAttribute(String attributeName) {
		...
		if (isGroupAttribute) {
			...
			for (AttributeObject obj : groupAttributes) {
				...
				value += obj.getSpecElement().getAttribute(attributeName)
						+ separator;
			}
	...
	/**
	 * @see com.angrysnail.sheeta.rtdaeditor.anim.EditableItem#isEditable()
	 */
	@Override
