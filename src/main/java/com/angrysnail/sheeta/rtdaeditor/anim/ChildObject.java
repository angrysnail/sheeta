...
public class ChildObject extends ItemObject {
	 ...
	 * 
	 * @param anim
	 *            the animation object that contains this child object
	 * @param childAnimationElement
	 *            the element of an animation child
	 * @param specChildAnimationElement
	 *            the specification of the child animation element
	 */
	public ChildObject(AnimationObject anim, Element childAnimationElement,
			Element specChildAnimationElement) {

		this.childAnimationElement = childAnimationElement;

		// creating the attribute objects
		NodeList attributes = specChildAnimationElement
			...
			if (cur != null) {
				...
				if (cur.hasAttribute("group")) {
					...
					if (groupElements != null && groupElements.size() > 0
						...
						tmpAttributes.put(currentGroup, new AttributeObject(
								anim, childAnimationElement, groupElements));
						groupElements = null;
						currentGroup = null;
					}
					...
					// adding the element to the list of the elements being in
					// the same group
					...
					tmpAttributes.put(name, new AttributeObject(anim,
							childAnimationElement, cur));
				}
			}
		}
		...
		if (groupElements != null && groupElements.size() > 0) {
			...
			// handles the nodes of the previous group
			tmpAttributes.put(currentGroup, new AttributeObject(anim,
					childAnimationElement, groupElements));
			attributeNames.add(currentGroup);
		}
