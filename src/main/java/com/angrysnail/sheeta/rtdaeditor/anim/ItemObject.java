...
public abstract class ItemObject {
	...
	static {
		...
		regularRtdaAnimationsAndActionsDocument = RtdaResources
				.getXMLDocument("rtdaAnimationsAndActions.xml");

		tagEventRtdaAnimationsAndActionsDocument = RtdaResources
				.getXMLDocument("rtdaAnimationsAndActionsOnEvents.xml");

		// getting the labels
		animationUndoRedoLabel = ResourcesManager.bundle
				.getString("undoredortdaanim");
		actionUndoRedoLabel = ResourcesManager.bundle
				.getString("undoredortdaaction");
	}
	...
	/**
	 * computes the object type and the spec element corresponding to the
	 * animation element
	 * 
	 * @param animationElement
	 *            an animation element
	 * @return the object type and the spec element corresponding to the
	 *         animation element as an array
	 */
	protected Object[] computeObjectType(Element animationElement) {
		...
		if (name != null && !name.equals("") && specDocument != null) {
			...
			// for each spec animation element, checks if the given element maps
			// one of them
			NodeList items = specDocument
					.getElementsByTagName(animationTagName);
