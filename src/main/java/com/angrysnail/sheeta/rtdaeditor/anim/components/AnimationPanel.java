...
public class AnimationPanel extends JPanel {
	 ...
	 */
	private JSplitPane allPanel = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT,
			true);
	 ...
	 *            the jwidget manager
	 * @param objectType
	 *            the type of the objects that have to be handled
	 */
	 ...
	 * 
	 * @param animationObjects
	 *            the list of the animation objects to be handled
	 */
			...
			if (animationObject.hasChildren()) {
				...
				if (animationObject.getType() == TagToolkit.ANALOGIC) {
					...
					childrenPanel.setBorder(new TitledBorder(
							analogicChildrenLabel));

				} else if (animationObject.getType() == TagToolkit.NEED_CHILDREN) {

					// getting the label
					String nodeName = animationObject.getSpecChildElement()
							.getAttribute("name");

					try {
						nodeName = nodeName.substring(
								Toolkit.rtdaPrefix.length(), nodeName.length());
					} catch (Exception ex) {
					}

					String needChildrenLabel = ResourcesManager.bundle
							.getString("rtdaanim_" + nodeName);

					childrenButtonsPanel.setVisible(true);
					childrenTable.setRowSelectionAllowed(true);
					childrenPanel
							.setBorder(new TitledBorder(needChildrenLabel));
					...
					childrenPanel
							.setBorder(new TitledBorder(viewChildrenLabel));
					...
					childrenTable.setRowSelectionAllowed(false);
					childrenPanel
							.setBorder(new TitledBorder(enumChildrenLabel));
				}
				...
				SwingUtilities.invokeLater(new Runnable() {
					...
					public void run() {
						...
						// setting the new animation object for the children
						// table
				...
				rightPanel
						.setLayout(new BoxLayout(rightPanel, BoxLayout.X_AXIS));
				rightPanel.add(propertiesPanel);
			}
	...
	protected void createChildrenButtonsPanel() {
		...
		// getting the icons
		ImageIcon newIcon = ResourcesManager.getIcon("New", false), dnewIcon = ResourcesManager
				.getIcon("New", true), deleteIcon = ResourcesManager.getIcon(
				"Delete", false), ddeleteIcon = ResourcesManager.getIcon(
				"Delete", true);
		...
		newButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent evt) {
				...
				if (currentAnimationObject != null) {
					...
					// creating the child dom element and appending it to the
					// animation element
					final Element animationElement = currentAnimationObject
							.getAnimationElement();
					Document doc = animationElement.getOwnerDocument();
					Element specChildElement = currentAnimationObject
							.getSpecChildElement();
					final Element newChild = doc.createElementNS(
							animationElement.getNamespaceURI(),
							specChildElement.getAttribute("name"));
					final SVGHandle handle = currentAnimationObject
							.getSVGHandle();

					// getting the currently selected element
					Set<Element> selectedElements = handle.getSelection()
							.getSelectedElements();
					final Element selectedElement = selectedElements.iterator()
							.next();
					...
					currentAnimationObject.addUndoRedoAction(executeRunnable,
			...
			public void actionPerformed(ActionEvent evt) {
				...
				if (currentAnimationObject != null) {
					...
					if (row != -1) {
						...
						if (nextRow < currentAnimationObject.getChildrenList()
								.size()) {

							nextElement = currentAnimationObject
									.getChildrenList().get(nextRow)
									.getChildAnimationElement();
						}

						final Element afterElement = nextElement;
						final Element removedElement = currentAnimationObject
								.getChildrenList().get(row)
								.getChildAnimationElement();
						final Element animationElement = currentAnimationObject
								.getAnimationElement();
						final SVGHandle handle = currentAnimationObject
						...
						Set<Element> selectedElements = handle.getSelection()
								.getSelectedElements();
						final Element selectedElement = selectedElements
								.iterator().next();
							...
							Runnable undoRunnable = new Runnable() {
								...
								public void run() {
									...
									if (afterElement != null) {
										...
										animationElement.insertBefore(
												removedElement, afterElement);

									} else {

										animationElement
												.appendChild(removedElement);
									}
		...
		childrenTable.getSelectionModel().addListSelectionListener(
