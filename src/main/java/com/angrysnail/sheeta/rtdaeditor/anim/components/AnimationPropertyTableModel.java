...
/**
 * the class of the model of the table used for modifying the properties of an
 * animation
 * 
...
public class AnimationPropertyTableModel extends AnimationTableModel {
	...
	public AnimationPropertyTableModel(AnimationTable animationTable) {
		...
		// getting the labels
		try {
			nameLabel = ResourcesManager.bundle
					.getString("rtdaanim_propertyTableName");
			valueLabel = ResourcesManager.bundle
					.getString("rtdaanim_propertyTableValue");
		} catch (Exception ex) {
		}
	}
	...
	public Object getValueAt(int rowIndex, int columnIndex) {
		...
		if (animationObject != null
				&& animationObject.getAttributesList().size() > rowIndex) {
