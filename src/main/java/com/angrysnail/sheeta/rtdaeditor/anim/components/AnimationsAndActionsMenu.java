...
public class AnimationsAndActionsMenu {
	 ...
	 * 
	 * @param isAnimations
	 *            whether the menu should be used forcreating animations or
	 *            actions
	 */
	public AnimationsAndActionsMenu(boolean isAnimations) {
		...
		undoRedoLabel = isAnimations
				? ItemObject.animationUndoRedoLabel
	 ...
	 * 
	 * @param cleanAll
	 *            whether all the object should be cleant
	 */
	 ...
	 * 
	 * @param specDocument
	 *            the specification document used to build the menu items
	 * @param optionalBundle
	 *            the bundle to use when a label can't be found in the default
	 *            bundle
	 * @param selectedElement
	 *            the element that is selected on the canvas
	 */
	protected void buildMenuItems(Document specDocument,
		...
		if (specDocument != null) {
			...
			NodeList groups = rtdaAnimationsDocument.getDocumentElement()
					.getElementsByTagName(
							isAnimations ? "animationgroup" : "actiongroup");

			Element group = null;
			JMenu groupMenu = null;
			String label = "", prefix = com.angrysnail.sheeta.library.Toolkit.rtdaPrefix;
			NodeList nodes = null;
			...
			for (int i = 0; i < groups.getLength(); i++) {
				...
				nodes = group.getElementsByTagName(isAnimations
						? "animation"
						: "action");
				...
				for (int j = 0; j < nodes.getLength(); j++) {
					...
					try {
						label = label.substring(
								label.indexOf(prefix) + prefix.length(),
								label.length());
						label = getLabel("rtdaanim_menuItem_" + label,
								optionalBundle);
					} catch (Exception ex) {
					}

					// creating the menu item corresponding to this animation or
					// action
					...
					if (parent.hasAttribute(restrictionAtt)) {
						...
						if (selectedElement.getNodeName().equals(
								...
								|| (parent.getAttribute(restrictionAtt).equals(
										restrictionOneAttValue) && !Toolkit
										.hasAnimations(selectedElement))) {
	 ...
	 *            the element that is selected on the canvas
	 */
	protected void createAnimationOrAction(Element specElement,
			final Element selectedElement) {

		if (specElement != null && selectedElement != null) {

			// creating the new node and its attributes
			final Element newElement = selectedElement
					.getOwnerDocument()
					.createElementNS(
							com.angrysnail.sheeta.library.Toolkit.rtdaNameSpace,
			...
			for (Node node = specElement.getFirstChild(); node != null; node = node
					.getNextSibling()) {
			...
			if (!currentSourceName.equals("")) {
				...
				newElement
						.setAttribute(
								com.angrysnail.sheeta.library.Toolkit.sourceAttributeName,
								currentSourceName);
			}
			...
			Runnable executeRunnable = new Runnable() {
				...
				public void run() {
					...
					handle.getSelection().handleSelection(selectedElement,
							false, true);
				}
			};
	 ...
	 *            the runnable that is run for executing the action
	 * @param undoRunnable
	 *            the undo runnable
	 * @param redoRunnable
	 *            the redo runnable
