...
public class AnimationsChooser extends JPanel {
	...
	/**
	 * the handlers of the animations and actions creation menu, the first one
	 * handles the jwidget elements and the second one handles the svg elements
	 */
	 ...
	 *            the animation panel
	 * @param isAnimations
	 *            whether this chooser is used for handling animations or
	 *            actions
	 * @param jwidgetManager
	 *            the jwidget manager
	 */
	public AnimationsChooser(AnimationPanel animationPanel,
			boolean isAnimations, JWidgetManager jwidgetManager) {
	...
	protected void buildPanel() {
		...
		MouseListener listListener = new MouseAdapter() {
			...
			/**
			 * checks whether this mouse event denotes a popup trigger event or
			 * not
			 * 
			 * @param evt
			 *            an event
			 * @return whether this mouse event denotes a popup trigger event or
			 *         not
			 */
		...
		sourceChooserPanel.setLayout(new BoxLayout(sourceChooserPanel,
				BoxLayout.X_AXIS));
	}
	...
	public void setAnimations(LinkedList<AnimationObject> list) {
		...
		SVGHandle handle = Editor.getEditor().getHandlesManager()
				.getCurrentHandle();
		LinkedList<Element> selectedNodes = new LinkedList<Element>(handle
		...
		if (selectedNodes.size() >= 1) {
			...
			selectedElement = handle.getCanvas().getDocument()
					.getDocumentElement();
		}
		...
		if (jwidgetElement != null && jwidgetEdition != null) {
			...
			Document specDocument = jwidgetEdition
					.getAnimationsAndActionsDocument();

			animationsAndActionsMenu.createPopUpMenu(specDocument,
					jwidgetEdition.getBundle(), selectedElement);
			popupMenuCreated = true;
		}

		if (!popupMenuCreated) {

			if (selectedElement.equals(handle.getCanvas().getDocument()
					.getDocumentElement())) {

				// creating the popup menu with the tag event animations and
				// actions document
				svgAnimationsAndActionsMenu.createPopUpMenu(ItemObject
						.getTagEventRtdaAnimationsAndActionsDocument(), null,
						selectedElement);

			} else {

				// creating the popup menu with the regular animations and
				// actions document
				svgAnimationsAndActionsMenu
						.createPopUpMenu(ItemObject
								.getRegularRtdaAnimationsAndActionsDocument(),
	 ...
	 *            a jwidget element
	 */
	public void handleSourceWidget(JWidgetEdition jwidgetEdition,
			Element jwidgetElement) {

		// adding the new jwidget source chooser
		AnimationChooserJWidgetSourceChooser sourceChooser = jwidgetEdition
				.getSourceChooser(isAnimations);
	...
	public void setNewSource(String sourceName) {
		...
		RtdaAnimationsAndActionsModule.getStateRecord().setSourceName(
				sourceName);

		if (sourceName != null && !sourceName.equals("")) {

			// creating the list of the animations or actions corresponding to
	...
	protected void handleList() {
		...
		if (currentAnimations.size() > 0) {
			...
			if (animationIndex < 0
					|| animationIndex >= currentAnimations.size()) {

				animationIndex = 0;
			}

			animationList.setSelectedIndex(animationIndex);
			animationPanel.setCurrentAnimation(currentAnimations
					.get(animationIndex));
	...
	protected void handleListSelection() {
		...
		if (isAnimations) {
			...
			RtdaAnimationsAndActionsModule.getStateRecord().setAnimationIndex(
					animationList.getSelectedIndex());

		} else {

			RtdaAnimationsAndActionsModule.getStateRecord().setActionIndex(
					animationList.getSelectedIndex());
		}

		AnimationListItem currentSelection = (AnimationListItem) animationList
				.getSelectedValue();
		...
		if (currentSelection != null) {
			...
			animationPanel.setCurrentAnimation(currentSelection
					.getAnimationObject());
	 ...
	 *            the source component where the action occured
	 * @param clickedPoint
	 *            the clicked point
	 */
	protected void showPopup(JComponent sourceComponent, Point clickedPoint) {// TODO

		getAnimationsAndActionsMenu().showPopupMenu(sourceComponent,
				clickedPoint);
	}
	...
	protected void deleteListItem(int index) {// TODO
		...
		if (index >= 0) {
			...
			AnimationListItem item = (AnimationListItem) animationList
			...
			if (item != null) {
				...
				if (animationObject != null) {
					...
					final SVGHandle handle = Editor.getEditor()
							.getHandlesManager().getCurrentHandle();
					final Element animationElement = animationObject
							.getAnimationElement();
					final Element parentElement = (Element) animationElement
							.getParentNode();
					Element sibling = null;

					// getting the animation that can be found after the
					// selected animation
					AnimationListItem nextItem = null;

					if (index + 1 < animationList.getModel().getSize()) {

						nextItem = (AnimationListItem) animationList.getModel()
								.getElementAt(index + 1);

						if (nextItem != null) {

							sibling = nextItem.getAnimationObject()
									.getAnimationElement();
						}
					}
					...
					Runnable executeRunnable = new Runnable() {
						...
						public void run() {
							...
							handle.getSvgDOMListenerManager().fireNodeRemoved(
									parentElement, animationElement);
							handle.getSelection().handleSelection(
									parentElement, false, true);
						}
					};
					...
					Runnable undoRunnable = new Runnable() {
						...
						public void run() {
							...
							if (nextSibling != null) {
								...
								parentElement.insertBefore(animationElement,
										nextSibling);
							...
							handle.getSvgDOMListenerManager().fireNodeInserted(
									parentElement, animationElement);
							handle.getSelection().handleSelection(
									parentElement, false, true);
						}
	...
	protected void raiseItem(int index) {// TODO
		...
		final SVGHandle handle = Editor.getEditor().getHandlesManager()
				.getCurrentHandle();
		AnimationListItem listItem = (AnimationListItem) animationList
				.getSelectedValue();
		final Element currentElement = listItem.getAnimationObject()
				.getAnimationElement();
		...
		if (index + 1 < animationList.getModel().getSize()) {
			...
			AnimationListItem nextListItem = (AnimationListItem) animationList
					.getModel().getElementAt(index + 1);

			if (nextListItem != null) {

				nextElement = nextListItem.getAnimationObject()
						.getAnimationElement();
			}
		}
		...
		if (index - 1 >= 0) {
			...
			AnimationListItem previousListItem = (AnimationListItem) animationList
					.getModel().getElementAt(index - 1);

			if (previousListItem != null) {

				previousElement = previousListItem.getAnimationObject()
						.getAnimationElement();
			}
		}
		...
		if (previousElement != null) {
			...
			executeRunnable = new Runnable() {
				...
				public void run() {
					...
					parentElement
							.insertBefore(currentElement, fpreviousElement);
					handle.getSvgDOMListenerManager().fireNodeInserted(
							parentElement, currentElement);
					handle.getSelection().handleSelection(parentElement, false,
							true);
				}
			};
			...
			undoRunnable = new Runnable() {
				...
				public void run() {
					...
					if (fnextElement != null) {
						...
						parentElement
								.insertBefore(currentElement, fnextElement);
			...
			// adding the undo/redo action
			getAnimationsAndActionsMenu().addUndoRedoAction(executeRunnable,
					undoRunnable, executeRunnable);
		}
	}
	...
	protected void lowerItem(int index) {// TODO
		...
		if (nextElement != null) {
			...
			if (index + 2 < animationList.getModel().getSize()) {
				...
				AnimationListItem nextNextListItem = (AnimationListItem) animationList
						.getModel().getElementAt(index + 2);

				if (nextNextListItem != null) {

					nextNextElement = nextNextListItem.getAnimationObject()
							.getAnimationElement();
				}
			}
			...
			executeRunnable = new Runnable() {
				...
				public void run() {
					...
					if (fnextNextElement != null) {
						...
						// of the next element
						parentElement.insertBefore(currentElement,
								fnextNextElement);

					} else {

						// putting the current element at the end of all the
						// animation nodes
						parentElement.appendChild(currentElement);
					}
