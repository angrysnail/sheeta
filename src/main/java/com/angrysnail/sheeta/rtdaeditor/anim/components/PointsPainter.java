...
import com.angrysnail.sheeta.rtdaeditor.anim.AttributeObject;
...
/**
 * the class of the painter used to paint the points that are obtained from a
 * point chooser
 * 
...
public class PointsPainter {
	 ...
	 */
	private Map<AttributeObject, Point2D> points = new ConcurrentHashMap<AttributeObject, Point2D>();
	...
	public void setCurrentAnimationObjet(final AnimationObject animationObject) {
		...
		if (animationObject != null) {
			...
			if (canvas != null) {
				...
				// checking if the new animation object contains attributes that
				// handles points, and
				// then
				// retrieves the values of the points
				Point2D point2D = null;

				for (final AttributeObject attributeObject : animationObject
						.getAttributesList()) {

					if (attributeObject != null
							&& attributeObject.getType().equals(
									EditableItem.POINT_CHOOSER)) {
						...
						if (point2D != null) {
							...
							String[] colorSplitStrs = colorStr
									.split(EditableItem.separatorRegex);

							// getting the colors corresponding to these strings
							Color color = Editor.getColorChooser().getColor(
									null, colorSplitStrs[0]);
				...
				if (points.size() > 0) {
					...
					canvasPaintListener = new CanvasPainter() {
						...
						/**
						 * the size of each line of the representation of the
						 * point
						 */
						private int size = 4;
						...
						public Set<Rectangle2D> getClip() {
							...
							/*
							 * HashSet<Rectangle2D> clips=new
							 * HashSet<Rectangle2D>(); Rectangle2D rect=null;
							 * 
							 * for(Point2D point : new
							 * HashSet<Point2D>(points.values())){
							 * 
							 * rect=new Rectangle2D.Double(point.getX()-size,
					...
					if (enable) {
						...
						canvas.addLayerPaintListener(SVGCanvas.TOP_LAYER,
								canvasPaintListener, true);
					}

					// creating the dom listener
					domListener = new SVGDOMListener(
							animationObject.getAnimationElement()) {
						...
						protected void handleEvent() {
							...
							if (canvasPaintListener != null) {
								...
								canvas.removePaintListener(canvasPaintListener,
										true);
							}
							...
							for (final AttributeObject attributeObject : animationObject
								...
								if (attributeObject != null
									...
									// getting the point defined by the
									// attribute, and putting it
									// into the map
	...
	/**
	 * computes the point to be displayed on the canvas of the given attribute
	 * object
	 * 
	 * @param attObj
	 *            an attribute object
	 * @return the point to be displayed on the canvas of the given attribute
	 *         object
	 */
	protected Point2D getPoint(AttributeObject attObj) {
		...
		if (attObj != null) {
			...
			String[] defaultSplitValue = defaultValue
					.split(EditableItem.separatorRegex);
	 ...
	 * 
	 * @param enable
	 *            whether this painter is enabled or not
	 */
	...
	/**
	 * set the given point in absolute coordinates according to the upper left
	 * corner of the representation of the parent node
	 * 
	 * @param point
	 *            a point
	...
	protected Point2D setToAbsolute(Point2D point) {
		...
		if (parent != null && parent instanceof Element) {
			...
			Rectangle2D elementBounds = canvas.getSVGHandle()
					.getSvgElementsManager().getNodeBounds((Element) parent);
			...
			if (point != null && elementBounds != null) {
				...
				resPoint = new Point2D.Double(point.getX()
						+ elementBounds.getX(), point.getY()
						+ elementBounds.getY());
			}
		}
