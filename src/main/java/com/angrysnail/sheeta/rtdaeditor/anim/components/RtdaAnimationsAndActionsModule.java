...
import com.angrysnail.sheeta.rtdaeditor.EditorAnimationsToolkit;
import com.angrysnail.sheeta.rtdaeditor.anim.AnimationObject;
import com.angrysnail.sheeta.rtdaeditor.anim.ItemObject;
import com.angrysnail.sheeta.rtdaeditor.anim.disposer.RtdaFrameDisposer;
...
import com.angrysnail.sheeta.rtdaeditor.test.MainDisplayToolkitTest;
import com.angrysnail.sheeta.rtdaeditor.test.display.DialogTest;
import com.angrysnail.sheeta.svgeditor.Editor;
import com.angrysnail.sheeta.svgeditor.ModuleAdapter;
import com.angrysnail.sheeta.svgeditor.actions.toolbar.ToolsFrame;
...
import com.angrysnail.sheeta.svgeditor.display.selection.SelectionChangedListener;
...
public class RtdaAnimationsAndActionsModule extends ModuleAdapter {
	...
	/**
	 * the frame into which the rtda animations and actions panel will be
	 * inserted
	 */
	...
	public RtdaAnimationsAndActionsModule(final Editor editor) {
		...
		AbstractShape shape = Editor.getEditor().getShapeModule(
				"JWidgetManager");
		...
		final HandlesListener svgHandleListener = new HandlesListener() {
			...
			public void handleChanged(SVGHandle currentHandle,
				...
				if (currentHandle != null) {
					...
					manageSelection(new LinkedList<Element>(currentHandle
							.getSelection().getSelectedElements()), true);
			 ...
			 * @param frameChanged
			 *            whether the frame has changed
			 */
			protected void manageSelection(
					LinkedList<Element> selectedElements, boolean frameChanged) {
				...
				if (!frameChanged) {
					...
					try {
						// comparing the old and new selected nodes to check if
						// the selection has
						// changed,
						// in order to know whether the frame should be
						// refreshed or not
						HashSet<Element> oldSel = new HashSet<Element>(
								selectedNodes);
						HashSet<Element> newSel = new HashSet<Element>(
								selectedElements);
		...
		MainDisplay mainDisplay = new MainDisplay(mainDisplayToolkitTest, true,
				false);
		...
		animationsPanel = new AnimationPanel(jwidgetManager,
		...
		try {
			saveDialogTitle = ResourcesManager.bundle
					.getString("label_rtdaanimationsandactionsSaveDialogTitle");
			saveDialogMessage = ResourcesManager.bundle
					.getString("label_rtdaanimationsandactionsSaveDialogMessage");
			frameLabel = ResourcesManager.bundle
					.getString("label_rtdaanimationsandactions");
			animationsPanelLabel = ResourcesManager.bundle
					.getString("label_rtdaanimations");
			actionsPanelLabel = ResourcesManager.bundle
					.getString("label_rtdaactions");
			jwidgetPanelLabel = ResourcesManager.bundle
					.getString("label_jwidgets");
			testLabel = ResourcesManager.bundle
					.getString("label_rtdaAnimationsAndActionTest");
		} catch (Exception ex) {
		}
		...
		rtdaAnimationsAndActionsFrame = new ToolsFrame(editor,
				idRtdaAnimationsAndActions, frameLabel,
				rtdaAnimationsAndActionsPanel);
		...
		rtdaAnimationsAndActionsFrame
				.setVisibilityChangedRunnable(visibilityRunnable);
		...
		tabbedPane.addChangeListener(new ChangeListener() {
			...
			public void stateChanged(ChangeEvent evt) {
				...
				if (animationsPanel != null
						&& animationsPanel.getPointsPainter() != null) {

					animationsPanel.getPointsPainter().setEnabled(
		...
		ImageIcon launchTestIcon = ResourcesManager.getIcon("TestLauncher",
				false);
		ImageIcon launchTestDisabledIcon = ResourcesManager.getIcon(
				"TestLauncher", true);
	 ...
	 * @param nodes
	 *            the list of nodes
	 * @param animationOrActionToSelect
	 *            the animation or action element that should be selected in the
	 *            animations or actions chooser
	 */
	protected void refreshAnimationsAndActions(LinkedList<Element> nodes,
			Element animationOrActionToSelect) {

		// getting the current svg handle
		SVGHandle handle = Editor.getEditor().getHandlesManager()
				.getCurrentHandle();

		// if the frame has already been reinitialized and is not visible,
		// nothing is done
		if (hasBeenReinitialized
				&& ((handle == null && nodes == null) || !rtdaAnimationsAndActionsFrame
						.isVisible())) {

			return;
		}

		// if the frame is not visible and has not been reinitialized, the
		// reinitalization is
		// processed
		if (!rtdaAnimationsAndActionsFrame.isVisible()) {
		...
		if (handle != null && (nodes == null || nodes.size() == 0)) {
			...
			// adding the root element to the list of the nodes that should be
			// animated
		...
		if (nodes != null && nodes.size() == 1) {
			...
			// getting the animation elements that can be found under the
			// selected node
			// and creating the associated animation objects
			for (Node cur = selectedNode.getFirstChild(); cur != null; cur = cur
					.getNextSibling()) {

				if (cur instanceof Element
						&& cur.getNodeName()
								.startsWith(
										com.angrysnail.sheeta.library.Toolkit.rtdaPrefix)
						&& !cur.getNodeName()
								.equals(com.angrysnail.sheeta.library.Toolkit.rtdaPrefix
										+ EditorAnimationsToolkit.widgetDatabaseRootTagName)
						&& !Toolkit.isJWidgetElement((Element) cur)) {

					objects.add(new AnimationObject(jwidgetManager,
							(Element) cur));
				}
			}
		...
		if (objects != null && animationObjects != null
				&& actionObjects != null) {
			...
			// computing the index of the animation and action to select in the
			// animation chooser
			...
			if (animationOrActionToSelect != null) {
				...
				for (AnimationObject animObj : animationObjects) {
					...
					if (animObj.getAnimationElement().equals(
							animationOrActionToSelect)) {
				...
				for (AnimationObject actionObj : actionObjects) {
					...
					if (actionObj.getAnimationElement().equals(
							animationOrActionToSelect)) {
		...
		if (element != null) {
			...
			// checks if the animations, action and jwidget tabs should be
			// enabled or not
			if (Toolkit.hasJWidgetChildElement(element)) {

				Element jwidgetElement = JWidgetManager
						.getJWidgetElement(element);
				jwidgetEdition = jwidgetManager
						.getJWidgetEdition(jwidgetElement
			...
			// getting the tab index that should be selected and selects the
			// corresponding tab
			if (enabledTabIndex.size() > 0
					&& !enabledTabIndex.contains(selectedTab)) {
			...
			animationsPanel.setAnimations(enableAnimations
					? animationObjects
					: null);
			...
			rtdaAnimationsAndActionsPanel.setLayout(new BoxLayout(
					rtdaAnimationsAndActionsPanel, BoxLayout.X_AXIS));
	...
	protected void launchTestDialog(JComponent relativeComponent) {
		...
		if (handle != null && handle.isModified()) {
			...
			// showing the dialog
			int result = JOptionPane.showConfirmDialog(Editor.getParent(),
					saveDialogMessage, saveDialogTitle,
			...
			switch (result) {
				...
				case JOptionPane.YES_OPTION :
					...
					Editor.getEditor()
							.getIOManager()
							.getFileSaveManager()
							.saveHandleDocument(handle, false,
									relativeComponent);
					showTestDialog = true;
					break;
				...
				public void run() {
					...
					SwingUtilities.invokeLater(new Runnable() {
						...
						public void run() {
							...
							mainDisplayToolkitTest.getDialogTest()
