...
import com.angrysnail.sheeta.rtdaeditor.anim.action.ActionConfigurator;
...
public class ActionChooser extends Widget {
	...
	protected void buildWidget() {
		...
		try {
			configureLabel = ResourcesManager.bundle
					.getString("rtdaanim_configure");
		} catch (Exception ex) {
		}
			...
			configureButtonListener = new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					if (currentClass != null) {
						...
						try {
							actionConfigurator = (ActionConfigurator) currentClass
									.newInstance();
						} catch (Exception ex) {
						}
						...
						if (actionConfigurator != null) {
							...
							try {
								currentClass
										.getMethod("configure", classParams)
										.invoke(actionConfigurator, params);
							} catch (Exception ex) {
							}
						}
	...
	protected void setItem(EditableItem item, Runnable validateRunnable) {
		...
		if (isEditor) {
			...
			if (projectFile != null) {
				...
				Set<Class<?>> classesSet = ActionsLoader.getClasses(
						CustomAction.class, projectFile, true);

				// creating the map associating a class name to its class and
				// the sorted list of the
				// class names
			...
			if (value != null && !value.equals("")) {
				...
				selectedActionClass = ActionsLoader.getClass(
						CustomAction.class, projectFile, value, true);
			}
		...
		if (selectedActionClass != null) {
			...
			// getting the name of the class used to configure the selected
			// action
			String configuratorClassName = null;

			try {
				configuratorClassName = (String) selectedActionClass.getMethod(
						"getConfiguratorClassName", new Class[0]).invoke(null,
						new Object[0]);
			} catch (Exception ex) {
				ex.printStackTrace();
			}
			...
			if (configuratorClassName != null
					&& !configuratorClassName.equals("")) {

				// getting the class corresponding to this class name and
				// storing it
				Class<?> configuratorClass = ActionsLoader.getClass(
						ActionConfigurator.class, projectFile,
