...
public class ComplexTagChooser extends Widget {
	...
	protected class TagChooserDialog {
		 ...
		 * the radio buttons
		 */
		private JRadioButton tagRadioButton = new JRadioButton(),
	...
	protected class TagComponent {
				...
				public void actionPerformed(ActionEvent evt) {
					...
					if (getItem() != null) {
						...
						DataBaseNodeFilter filter = new DataBaseNodeFilter(
								tagValue, 0, getItem().getTagType(), false,
								false, null);
		 ...
		 * 
		 * @param enable
		 *            whether this component should be enabled
		 */
	...
	protected class FunctionComponent {
		...
		private JSpinner periodSpinner = new JSpinner(),
				initialValueSpinner = new JSpinner();

		/**
		 * the labels
		 */
		private JLabel functionLbl = new JLabel(""),
				periodLbl = new JLabel(""), initialValueLbl = new JLabel("");
		...
		public FunctionComponent() {
			...
			// getting the labels
			String sinusLabel = "", triangleLabel = "", rampLabel = "", periodLabel = "", initialValueLabel = "", functionNameLabel = "";
			...
			try {
				...
				initialValueLabel = bundle
						.getString("rtdaanim_function_initialValue");
			} catch (Exception ex) {
			}
			...
			// the two spinners
			final SpinnerNumberModel periodSpinnerModel = new SpinnerNumberModel(
					1.0, 0.1, 10000000, 0.1), initialValueSpinnerModel = new SpinnerNumberModel(
					0, 0, 10000000, 0.1);
		...
		protected void handleChanges() {
			...
			if (combo.getSelectedItem() != null) {
				...
				name = ((ComboListItem) combo.getSelectedItem()).getValue()
						.toString();
			}
			...
			try {
				dPeriod = Double.parseDouble(periodSpinner.getValue()
						.toString());
			} catch (Exception ex) {
				dPeriod = 1;
			}
			...
			try {
				dInitialValue = Double.parseDouble(initialValueSpinner
						.getValue().toString());
			} catch (Exception ex) {
				dInitialValue = 0;
			}
			...
			if (name != null && !name.equals("")) {
				...
				functionValue = "function(" + name + "," + period + ","
						+ initialValue + ")";
			}
		}
		...
		public void setValue(String val) {
			...
			// sets the accurate values for the function, period and initial
			// values
			if (value != null && !value.equals("")) {
				...
				periodSpinner
						.setValue(new Double(Double.parseDouble(values[1])));
				initialValueSpinner.setValue(new Double(Double
