...
public class DirectoryChooser extends Widget {
	...
	protected void buildWidget() {
		...
		try {
			directoryChooserLabel = bundle
					.getString("rtdaanim_tagchooserbutton");
			workingDirectoryDescription = bundle
					.getString("rtdaanim_workingdirectorydescription");
		} catch (Exception ex) {
		}
		...
		if (isEditor) {
			...
			moreButton.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent evt) {
					...
					if (Editor.getEditor().getResourcesManager()
					...
					if (returnVal == JFileChooser.APPROVE_OPTION) {
						...
						String path = fileChooser.getSelectedFile().toURI()
								.toASCIIString();
