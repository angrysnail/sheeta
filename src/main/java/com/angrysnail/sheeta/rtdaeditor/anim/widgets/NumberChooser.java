...
public class NumberChooser extends Widget {
	 ...
	 * the spinner model
	 */
	private SpinnerNumberModel spinnerModel = new SpinnerNumberModel(0,
			Double.NEGATIVE_INFINITY + 1, Double.POSITIVE_INFINITY - 1, 1);
	...
	protected void buildWidget() {
		...
		numberEditor = ((JSpinner.NumberEditor) spinner.getEditor())
				.getTextField();
	...
	protected void setItem(EditableItem item, Runnable validateRunnable) {
		...
		numberEditor.setText(FormatStore.displayFormat.format(spinner
				.getModel().getValue()));
