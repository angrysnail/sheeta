...
public class PointChooser extends Widget {
	...
	protected void buildWidget() {
		...
		// the button
		// final ImageIcon
		// pointChooserIcon=SVGResource.getIcon("PointChooserSmall", false);
		...
		if (isEditor) {
			...
			caretListener = new CaretListener() {
				...
				public void caretUpdate(CaretEvent e) {
					...
					getItem().setValue(
							abscissTextField.getText() + EditableItem.separator
									+ ordinateTextField.getText()
									+ EditableItem.separator);
					...
					if (evt instanceof MouseEvent) {
						...
						if (mevt.getID() == MouseEvent.MOUSE_CLICKED) {
							...
							point = SwingUtilities.convertPoint(
									...
									new Point((int) point.getX(), (int) point
											.getY()), canvas);

							// converting the point if the align with rulers
							// mode is activated
							point = canvas.getSVGHandle()
							...
							String xTxt = FormatStore.format(selectedPoint
									.getX());
							String yTxt = FormatStore.format(selectedPoint
									.getY());

							getItem().setValue(
									xTxt + EditableItem.separator + yTxt
											+ EditableItem.separator);
							getItem().validateChanges();
	...
	protected void setItem(EditableItem item, Runnable validateRunnable) {
		...
		Color xcolor = Editor.getColorChooser().getColor(null,
				colorSplitStrs[0]);
		Color ycolor = Editor.getColorChooser().getColor(null,
				colorSplitStrs[1]);
	...
	protected Point2D setToRelative(Point2D point) {
		...
		if (parentNode != null) {
			...
			if (point != null && elementBounds != null) {
				...
				resPoint = handle.getTransformsManager().getScaledPoint(point,
						true);
