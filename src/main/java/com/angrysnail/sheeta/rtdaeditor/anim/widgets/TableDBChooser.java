...
public class TableDBChooser extends Widget {
	...
	protected void setItem(EditableItem item, Runnable validateRunnable) {
		...
		if (!dbName.equals("") && !tableName.equals("")) {
			...
			textField.setText("\"" + tableName + "\" " + inLabel + " \""
					+ dbName + "\"");
		}
	}

	/**
	 * returns an array of two values one for the name of a db, the other for a
	 * table name
	 * 
	 * @param value
	 *            a value
	 * @return an array of two values one for the name of a db, the other for a
	 *         table name
	 */
	...
	/**
	 * the class of the dialog used to choose the method for returning to the
	 * initial value
	 * 
	 * @author ITRIS, Jordi SUC
	 */
	protected class ChooserDialog extends TitledDialog {
		 ...
		 */
		protected LinkedHashMap<String, Set<String>> currentDBAndTableNames = new LinkedHashMap<String, Set<String>>();
		...
		protected JPanel buildContentPanel() {
			...
			// getting the labels
			ResourceBundle bundle = ResourcesManager.bundle;
			String titleMessage = bundle
					.getString("rtdaanim_selectdbtabletitle");
			infoLabel = bundle.getString("rtdaanim_selectdbtablemessage");
			missingDBLabel = bundle.getString("rtdaanim_missingdbmessage");
			missingTableLabel = bundle
					.getString("rtdaanim_missingtablemessage");
				...
				public void actionPerformed(ActionEvent e) {
					...
					if (e.getSource().equals(dbCombo)) {
						...
						if (dbCombo.getSelectedItem() != null) {
							...
							currentDBName = ((ComboListItem) dbCombo
						...
						if (tableCombo.getSelectedItem() != null) {
							...
							currentTableName = ((ComboListItem) tableCombo
									.getSelectedItem()).getValue().toString();
						}
					}
				}
		...
		public int showDialog(String startValue) {
			...
			currentRootElement = DataBaseNodeToolkit
					.getRootDataBaseElement(getItem().getDocument());

			if (currentRootElement != null) {

				LinkedHashMap<String, Set<String>> dbsAndTablesMap = AnimationsToolkit
						.getDBsAndTables(currentRootElement);
			...
			if (currentDBAndTableNames.size() == 0) {
				...
				if (selectedDBComboItem != null) {
					...
					currentDBName = ((ComboListItem) dbCombo.getItemAt(0))
							.getValue().toString();
				}
			}
		...
		protected void refreshTableCombo() {
			...
			if (tableNames == null || tableNames.size() == 0) {
				...
				for (String theTableName : tableNames) {
					...
					tableComboItem = new ComboListItem(theTableName,
							theTableName);
				...
				if (selectedTableComboItem != null) {
					...
					currentTableName = ((ComboListItem) tableCombo.getItemAt(0))
							.getValue().toString();
				}
