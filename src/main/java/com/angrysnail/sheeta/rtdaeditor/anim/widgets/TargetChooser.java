...
public class TargetChooser extends Widget {
	...
	static {
		...
		popupTargetLabel = ResourcesManager.bundle
				.getString("rtdaanim_popupTarget");
	}
	...
	protected void setItem(EditableItem item, Runnable validateRunnable) {
		...
		if (isEditor) {
			...
			Map<String, String> idsToLabel = EditableItem
					.getViewBrowsersIds(item.getSVGElement().getOwnerDocument()
							.getDocumentElement());
			...
			if (isComplex) {
				...
				comboItem = new ComboListItem(popupTargetOption,
						popupTargetLabel);
				combo.addItem(comboItem);
			...
			if (isComplex && id.equals(popupTargetOption)) {
				...
				combo.addItem(new ComboListItem(popupTargetOption,
						popupTargetLabel));
