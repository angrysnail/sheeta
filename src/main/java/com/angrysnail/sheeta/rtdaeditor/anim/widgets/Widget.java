...
import com.angrysnail.sheeta.rtdaeditor.anim.widgets.tageventchooser.TagEventChooser;
...
public abstract class Widget extends JPanel {
	...
	/**
	 * the map of the widgets used to render the table cells it associates the
	 * type of a widget to this widget
	 */
	protected static Map<String, Widget> renderers = new HashMap<String, Widget>();

	/**
	 * the map of the widgets used to edit the table cells it associates the
	 * type of a widget to this widget
	 */
	 ...
	 */
	protected static Widget complexTagChooserWidget = new ComplexTagChooser(
			false),
	...
	static {
		...
		renderers.put(EditableItem.EXTENDED_VIEW_CHOOSER, new ViewChooser(
				false, true));
		...
		renderers.put(EditableItem.COMPLEX_TARGET_CHOOSER, new TargetChooser(
				false, true));
		renderers.put(EditableItem.TARGET_CHOOSER, new TargetChooser(false,
				false));
		renderers.put(EditableItem.EVENT_CHOOSER, new EventChooser(false));
		renderers.put(EditableItem.TAG_EVENT_CHOOSER,
				new TagEventChooser(false));
		renderers.put(EditableItem.CHECK_BOX, new CheckBox(false));
		renderers
				.put(EditableItem.BLINKING_CHOOSER, new BlinkingChooser(false));
		renderers.put(EditableItem.RET_INIT_VAL_CHOOSER,
				new ReturnToInitialValueChooser(false, false));
		renderers.put(EditableItem.SIMPLE_RET_INIT_VAL_CHOOSER,
				new ReturnToInitialValueChooser(false, true));
		renderers.put(EditableItem.TAG_VALUES_CHOOSER, new TagValuesChooser(
				false));
		renderers.put(EditableItem.TAG_VALUES_MULTI_CHOOSER,
				new TagValuesMultiChooser(false));
		...
		renderers.put(EditableItem.POINT_STYLE_CHOOSER, new PointStyleChooser(
				false));
		renderers.put(EditableItem.INTERPOLATION_CHOOSER,
				new InterpolationChooser(false));
		...
		renderers.put(EditableItem.DIRECTORY_CHOOSER, new DirectoryChooser(
				false));
		...
		editors.put(EditableItem.EXTENDED_VIEW_CHOOSER, new ViewChooser(true,
				true));
		...
		editors.put(EditableItem.COMPLEX_TARGET_CHOOSER, new TargetChooser(
				true, true));
		...
		editors.put(EditableItem.RET_INIT_VAL_CHOOSER,
				new ReturnToInitialValueChooser(true, false));
		editors.put(EditableItem.SIMPLE_RET_INIT_VAL_CHOOSER,
				new ReturnToInitialValueChooser(true, true));
		editors.put(EditableItem.TAG_VALUES_CHOOSER, new TagValuesChooser(true));
		editors.put(EditableItem.TAG_VALUES_MULTI_CHOOSER,
				new TagValuesMultiChooser(true));
		...
		editors.put(EditableItem.POINT_STYLE_CHOOSER, new PointStyleChooser(
				true));
		editors.put(EditableItem.INTERPOLATION_CHOOSER,
				new InterpolationChooser(true));
	 ...
	 * @param item
	 *            an editable item
	 * @param validateRunnable
	 *            the runnable used to validate when editing
	 * @param isEditor
	 *            whether the returned widget should be an editor
	 * @param isSelected
	 *            whether the returned widget should be selected
	...
	public static Widget getWidget(EditableItem item,
		...
		if (type.equals(EditableItem.TAG_CHOOSER)
				|| type.equals(EditableItem.LOW_TAG_CHOOSER)
			 ...
			 * (type.equals(EditableItem.TAG_CHOOSER) ||
			 * type.equals(EditableItem.LOW_TAG_CHOOSER))) {
			 * 
			...
			widget = isEditor
					? simpleTagChooserWidgetEditor
					: simpleTagChooserWidget;
			// }
		...
		if (widget != null) {
			...
			widget.setBackground(isSelected
