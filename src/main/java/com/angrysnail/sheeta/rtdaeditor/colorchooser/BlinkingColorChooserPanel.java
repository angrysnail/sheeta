...
public class BlinkingColorChooserPanel extends AbstractColorChooserPanel {
	...
	/**
	 * the list of the runnables used to clear the color chooser panel from its
	 * previous components
	 */
	...
	public BlinkingColorChooserPanel(RtdaColorChooserModule colorChooser) {
		...
		if (bundle != null) {
			...
			try {
				...
				noColorAvailableLabel = bundle
						.getString("noColorAvailableLabel");
			} catch (Exception ex) {
			}
		}
		...
		colorChooser.getSVGEditor().getHandlesManager()
				.addHandlesListener(svgHandleListener);
	}

	@Override
	protected void buildChooser() {

		// clears the color chooser panel before adding new widgets into it, by
		// calling all the
		// dispose runnables
		...
		SVGHandle handle = colorChooser.getSVGEditor().getHandlesManager()
				.getCurrentHandle();
		Collection<Object> colorsList = null;

		if (handle != null) {

			colorsList = new LinkedList<Object>(colorChooser
					.getColorsAndBlinkingsToolkit()
					.getBlinkingColorsMap(
							handle.getScrollPane().getSVGCanvas()
									.getProjectFile()).values());
		}
		...
		if (colorsList != null && colorsList.size() > 0) {
			...
			// creating the list of the panels that will be contained in the
			// memory panel
			...
			// the label panel displaying the name and the corresponding rgb
			// values of a color
			final JLabel colorLabel = new JLabel("", SwingConstants.CENTER);

			int rowNb = (int) (Math.floor(colorsList.size() / colorsNbPerLine) + 1);

			colorsPanel.setLayout(new GridLayout((rowNb > minRowNb)
					? rowNb
					: minRowNb, colorsNbPerLine, 1, 1));
				...
				if (color != null) {
					...
					panel = new JPanel() {
						...
						protected void paintComponent(Graphics g) {
							...
							// draws two colored rectangles representing the two
							// colors of the
							// blinking color
							g.setColor(fcolor);
							g.fillRect(0, 0, panelsSize.width / 2,
									panelsSize.height);

							g.setColor(fcolor.getColor2());
							g.fillRect(panelsSize.width / 2, 0,
									panelsSize.width / 2, panelsSize.height);
						}
					};
					...
					// creating and adding a listener to the clicks on the color
					// panels
					mouseListener = new MouseAdapter() {
						...
						public void mouseClicked(MouseEvent evt) {
							...
							// removes the last color of the last selected
							// colors, if the list is
							// full
							if (lastSelectedColors.size() > 0
									&& lastSelectedColors.size() >= memoryNb) {
							...
							// for each panel contained in the memory, sets its
							// new color and
							// tooltip
							for (int i = 0; i < lastColorPanels.size(); i++) {
								...
								if (thePanel != null) {
									...
									if (i < lastSelectedColors.size()) {
										...
										theColor = (BlinkingColor) lastSelectedColors
												.get(i);

										if (theColor != null) {

											thePanel.setBorder(new LineBorder(
													Color.black, 1));
											thePanel.setBackground(theColor);
											thePanel.setToolTipText(theColor
										...
										thePanel.setBorder(new LineBorder(
												Color.lightGray, 1));
										thePanel.setBackground(getParent()
												.getBackground());
						...
						public void mouseEntered(MouseEvent evt) {
							...
							colorLabel
									.setText(fcolor.getStringRepresentation());
						}
			...
			memoryPanel.setLayout(new GridLayout(memoryRowNb, (int) (Math
					.floor(memoryNb / memoryRowNb)) + 1, 1, 1));
					...
					protected void paintComponent(Graphics g) {
						...
						if (fi < lastSelectedColors.size()) {
							...
							if (bColor != null) {
								...
								// draws two colored rectangles representing the
								// two colors of the
								// blinking color
								g.setColor(bColor);
								g.fillRect(0, 0, panelsSize.width / 2,
			...
			colorsAndMemoryPanel.setLayout(new FlowLayout(FlowLayout.CENTER,
					10, 0));
			colorsAndMemoryPanel.add(colorsPanel);
			colorsAndMemoryPanel.add(memoryPanel);

			// adding the colors and memory panel and the color label widget to
			// the color chooser
			// panel
			...
			colorLabelPanel.setBorder(new CompoundBorder(new EmptyBorder(1, 10,
					1, 10), new SoftBevelBorder(BevelBorder.LOWERED)));
