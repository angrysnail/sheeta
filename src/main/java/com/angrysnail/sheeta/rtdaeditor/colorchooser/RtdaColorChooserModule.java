...
Contact : jordi.suc@itris.fr; philippe.gil@itris.fr
 ...
 =============================================
 */
package com.angrysnail.sheeta.rtdaeditor.colorchooser;
...
import com.angrysnail.sheeta.rtda.colorsblinkings.BlinkingColor;
import com.angrysnail.sheeta.rtda.colorsblinkings.ColorsAndBlinkingsToolkit;
import com.angrysnail.sheeta.rtda.colorsblinkings.PredefinedColor;
...
/**
 * the class of the color chooser that handles specific colors for the rtda
 * animations
 * 
...
public class RtdaColorChooserModule extends ColorChooser implements Module {
	...
	/**
	 * the set containing the handles from which colors and blinking have been
	 * retrieved
	 */
	...
	public RtdaColorChooserModule(Editor editor) {
		...
		HandlesListener svgHandleListener = new HandlesListener() {
			...
			public void handleChanged(SVGHandle currentHandle,
				...
				Set<SVGHandle> storedHandles = new HashSet<SVGHandle>(
						usedHandles);

				// for each stored handle, checks if it is in the handles
		...
		BlinkingColorChooserPanel blinkingColorChooserPanel = new BlinkingColorChooserPanel(
				this);
		addChooserPanel(blinkingColorChooserPanel);

		// creating the color flavors
		try {
			predefinedColorFlavor = new DataFlavor(
					DataFlavor.javaJVMLocalObjectMimeType
							+ ";class=com.angrysnail.sheeta.rtda.colorsblinkings.PredefinedColor");
			blinkingColorFlavor = new DataFlavor(
					DataFlavor.javaJVMLocalObjectMimeType
							+ ";class=com.angrysnail.sheeta.rtda.colorsblinkings.BlinkingColor");
		} catch (Exception ex) {
			predefinedColorFlavor = DataFlavor.stringFlavor;
			blinkingColorFlavor = DataFlavor.stringFlavor;
		}
	}
	...
	public Color getColor(SVGHandle handle, String colorString) {
		...
		if (handle != null && colorString != null && !colorString.equals("")) {
			...
			if (pos != -1) {
				...
				String colorId = colorString.substring(pos + 2,
				...
				if (colorId != null && !colorId.equals("")) {
					...
					Collection<Object> predefinedColors = new LinkedList<Object>(
							colorsAndBlinkingsToolkit.getPredefinedColorsMap(
									projectFile).values());
					Collection<Object> blinkingColors = new LinkedList<Object>(
							colorsAndBlinkingsToolkit.getBlinkingColorsMap(
									projectFile).values());
					...
					for (Object object : predefinedColors) {
						...
						if (predefinedColor != null
								&& predefinedColor.getId().equals(colorId)) {
					...
					if (color == null) {
						...
						for (Object object : blinkingColors) {
							...
							if (blinkingColor != null
									&& blinkingColor.getId().equals(colorId)) {
	...
	public String getColorString(Color color) {
		...
		if (color instanceof PredefinedColor) {
			...
			colorString = ((PredefinedColor) color).getColorValue() + "/*"
					+ ((PredefinedColor) color).getId() + "*/";

		} else if (color instanceof BlinkingColor) {

			colorString = ((BlinkingColor) color).getColorValue() + "/*"
					+ ((BlinkingColor) color).getId() + "*/";
				...
				for (int i = 0; i < attributes.getLength(); i++) {
					...
					if (value != null && !value.equals("")
						...
						if (node.getNodeName().equals("style")) {
							...
							consistentValue = checkStyleValueColor(handle,
									value);

						} else {

							// handling the attributes values
							consistentValue = checkAttributeValueColor(handle,
									value);
						}

						// if one of the color values found in the element
						// attribute are not
						// consistent with the
						// current color values, the value of the attribute is
						// modified
	...
	protected String checkAttributeValueColor(SVGHandle handle, String value) {
		...
		if (handle != null && value != null && !value.equals("")) {
			...
			Map<String, Object> idToColor = colorsAndBlinkingsToolkit
					.getAllColorsMap(handle.getScrollPane().getSVGCanvas()
							.getProjectFile());
			...
			if (idToColor != null) {
				...
				// checks if the current part of the value of the attribute
				// contains a comment
				int pos = attValue.indexOf("/*");

				if (pos != -1) {

					// as the value contains a comment, the consistency of the
					// value of the color is
					// checked//

					// getting the id of the specified color
					foundId = attValue
							.substring(pos + 2, attValue.length() - 2);
					...
					if (foundId != null && !foundId.equals("")) {
						...
						if (color != null) {
							...
							if (color instanceof PredefinedColor) {
								...
								consistentColorValue = ((PredefinedColor) color)
										.getColorValue();

							} else if (color instanceof BlinkingColor) {

								consistentColorValue = ((BlinkingColor) color)
										.getColorValue();
							}

							if (attValue.indexOf(consistentColorValue) == -1) {

								returnValue = consistentColorValue + "/*"
										+ foundId + "*/";
							}
						}
					}
					...
					if (propertyValues[i] != null) {
						...
						if (pos != -1) {
							...
							if (name != null && value != null
								...
								// checking if the color values in the string
								// are consistent, if
								// not, they are modified
								consistentValue = checkAttributeValueColor(
										handle, value);

								if (consistentValue != null) {

									// if the property value has been modified,
	...
	public boolean isColorDataFlavor(DataFlavor flavor) {
		...
		if (flavor != null) {
			...
			isColorDataFlavor = (flavor.isMimeTypeEqual(predefinedColorFlavor)
					|| flavor.isMimeTypeEqual(blinkingColorFlavor) || super
					.isColorDataFlavor(flavor));
		}
	...
	/**
	 * @see com.angrysnail.sheeta.svgeditor.Module#getMenuItems()
	 */
	@Override
	...
	/**
	 * @see com.angrysnail.sheeta.svgeditor.Module#getPopupItems()
	 */
	@Override
	...
	/**
	 * @see com.angrysnail.sheeta.svgeditor.Module#getToolItems()
	 */
	@Override
	...
	/**
	 * @see com.angrysnail.sheeta.svgeditor.Module#initialize()
	 */
	@Override
