...
public class DataBaseNodeChooserPanel extends JPanel {
	...
	/**
	 * the map of the history of the selected tag names in the chooser. the map
	 * associates the type of a tag to its name
	 */
	public static final ConcurrentHashMap<Integer, String> tagNamesHistory = new ConcurrentHashMap<Integer, String>();

	/**
	 * the listeners to the database node chooser
	 */
	protected Set<DataBaseNodeChooserStateListener> listeners = new CopyOnWriteArraySet<DataBaseNodeChooserStateListener>();
	 ...
	 *            the filter for displaying the data base nodes
	 * @param showViewNodes
	 *            shows the nodes only used for the view
	 * @param showClosePopupDialogNode
	 *            shows the node for the "close popup dialog" option
	 */
	...
	protected void build() {
		...
		Element rootDataBaseElement = DataBaseNodeToolkit
				.getRootDataBaseElement(document);
		...
		if (rootDataBaseElement != null) {
			...
			closePopupDialogNode = new DefaultMutableTreeNode(closePopupLabel,
					false);
			...
			treeSelectionListener = new TreeSelectionListener() {
				...
				public void valueChanged(TreeSelectionEvent evt) {
					...
					if (evt.getSource().equals(dataBaseTree)
						...
						// the selected node
						selectedNode = (DataBaseTreeNode) dataBaseTree
								.getLastSelectedPathComponent();

						// removing the tree selection listener from the none
						// tree
			...
			// checking if another value than a node that is in the DB should be
			// selected
			boolean isAnotherValue = false;

			if (showViewNodes
					&& filter.getTagName()
							.equals(DataBaseNodeToolkit.backValue)) {

				otherValuesTree.setSelectionPath(new TreePath(new Object[]{
						rootNode, backNode}));
				isAnotherValue = true;

			} else if (showViewNodes
					&& filter.getTagName()
							.equals(DataBaseNodeToolkit.quitValue)) {
				...
				isAnotherValue = true;
					...
					&& showClosePopupDialogNode
					&& filter.getTagName().equals(
							DataBaseNodeToolkit.closePopupDialogValue)) {

				otherValuesTree.setSelectionPath(new TreePath(new Object[]{
						rootNode, closePopupDialogNode}));
				isAnotherValue = true;
			...
			DataBaseTreeNode rootTreeNode = new DataBaseTreeNode(
					rootDataBaseElement, filter, rootDataBaseElement);
			...
			if (rootTreeNode.getChildCount() > 0) {
				...
				if (!isAnotherValue) {
					...
					DataBaseTreeNode selectedTreeNode = rootTreeNode
							.getTreeNodeForElement(tagElement);

					if (selectedTreeNode != null) {

						// the tree node will be selected
						dataBaseTree.setSelectionPath(new TreePath(
								selectedTreeNode.getPath()));

					} else {

						// the none node of the none tree will be selected
						otherValuesTree.setSelectionPath(new TreePath(
								new Object[]{rootNode, noneNode}));
					}
				}
	...
	public DataBaseNodeInformation getInfo() {
		...
		if (isCorrect) {
			...
			if (selectedNode == null) {
				...
				DefaultMutableTreeNode node = (DefaultMutableTreeNode) otherValuesTree
				...
				// the none node has been selected, the information object that
				// will be returned is
				// created
				info = new DataBaseNodeInformation(TagToolkit.NOT_A_TAG, value,
						new LinkedList<String>(), "");

			} else {

				// recording the current value
				tagNamesHistory.put(filter.getTagType(),
						selectedNode.getTagName());

				// a data base node has been selected, the information object
				// that will be returned
				// is created
				info = new DataBaseNodeInformation(selectedNode.getNodeType(),
						selectedNode.getTagName(),
						selectedNode.getEnumeratedValues(),
						selectedNode.getLocation());
			}
		}
