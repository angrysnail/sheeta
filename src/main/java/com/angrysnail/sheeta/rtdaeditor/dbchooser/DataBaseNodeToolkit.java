...
public class DataBaseNodeToolkit {
	...
	public static String getPath(Element root, Element element) {
		...
		if (element != null) {
			...
			path = EditorAnimationsToolkit.normalizePath(((Element) node)
					.getAttribute("id"));
			node = node.getParentNode();

			// builds the path of the given element by adding the id attribute
			// to each parent
			while (node != null && node instanceof Element
					&& !node.equals(root)) {

				id = EditorAnimationsToolkit.normalizePath(((Element) node)
						.getAttribute("id"));
	...
	/**
	 * creates and returns the root database element corresponding to the given
	 * svg document
	 * 
	 * @param doc
	 *            a document
	...
	public static Element getRootDataBaseElement(Document doc) {
		...
		if (doc != null) {
			...
			if (Toolkit.isDocumentAView(doc)) {
				...
				try {
					canvasPath = new URI(((SVGDocument) doc).getURL())
							.toASCIIString();
				} catch (Exception ex) {
					ex.printStackTrace();
				}

				// getting the reference node and the reference file of the
				// document
				String referenceNode = doc.getDocumentElement().getAttribute(
						"referenceNode");
				String referenceFile = doc.getDocumentElement().getAttribute(
						"referenceFile");
				...
				rootDataBaseElement = DataBaseToolkit
	 ...
	 */
	public static LinkedList<String> getEnumeratedTagValues(Document doc,
			String tagName) {
		...
		if (doc != null && tagName != null) {
			...
			if (dataBaseRoot != null) {
				...
				if (element != null
						&& TagToolkit.getNodeType(element.getNodeName()) == TagToolkit.ENUMERATED) {
	...
	/**
	 * returns a point containing the min and max values of the tag denoted by
	 * the provided name
	 * 
	 * @param doc
	 *            a document
	 * @param tagName
	 *            a tag name
	 * @return a point containing the min and max values of the tag denoted by
	 *         the provided name
	 */
		...
		if (doc != null && tagName != null) {
			...
			if (dataBaseRoot != null) {
				...
				if (element != null) {
					...
					int tagType = TagToolkit.getNodeType(element.getNodeName());
	...
	/**
	 * retrieves and returns the list of the child nodes "name" attribute of the
	 * given element
	 * 
	 * @param element
	 *            an element
	...
	protected static LinkedList<String> getEnumeratedTagValues(Element element) {
		...
		if (element != null && element.hasChildNodes()) {
			...
			for (child = element.getFirstChild(); child != null; child = child
					.getNextSibling()) {
	...
	/**
	 * retrieves the node corresponding to the given tag name in the given data
	 * base
	 * 
	 * @param rootDataBase
	 *            the data base root node
	 * @param tagName
	 *            a tag name
	 * @return the node corresponding to the given tag name in the given data
	 *         base
	 */
	protected static Element getElementForTag(Element rootDataBase,
			String tagName) {
	...
	public static LinkedList<String> getAllAvailableViewPath(Document doc) {
		...
		if (doc != null) {
			...
			if (rootDataBase != null) {
				...
				// getting the nodes that can be found under the root data base
				// and that are view
				NodeList viewNodes = rootDataBase
						.getElementsByTagName(viewNodeName);
