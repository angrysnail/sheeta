...
public class DataBaseTreeNode extends DefaultMutableTreeNode {
	...
	/**
	 * the value of the location attribute of the node if this node has the type
	 * "VIEW"
	 */
	 ...
	 *            the node filter
	 * @param element
	 *            an element of the rtda data base associated with this tree
	 *            node
	 */
	public DataBaseTreeNode(Element rootDataBase, DataBaseNodeFilter filter,
			Element element) {
		...
		boolean isNodeTypeEquivalent = TagToolkit.areTypesEquivalent(
				this.nodeType, this.filter.getTagType(),
				this.filter.isStrictEquality());
		...
		if (!(filter.getExcludedViewPath() != null
				&& !filter.getExcludedViewPath().equals("") && filter
				.getExcludedViewPath().equals(getTagName()))) {

			if (this.nodeType == TagToolkit.VIEW
					&& filter.getTagType() == TagToolkit.VIEW) {

				// computing the absolute location of the view if this node
				...
				if ((filter.getExcludedViewPath() != null
						&& !filter.getExcludedViewPath().equals("") && !filter
						.getExcludedViewPath().equals(absoluteLocation))
						|| (filter.getExcludedViewPath() == null || (filter
								.getExcludedViewPath() != null && filter
								.getExcludedViewPath().equals("")))) {
					...
					|| this.nodeType == TagToolkit.ENUMERATED || this.nodeType == TagToolkit.STRING)
				...
				// computes whether this node can be displayed or not
				if (((isRootNode
						|| (this.filter.isShowOnlyThisLevel() && ((elementLevel < this.filter
								.getLevel() && this.nodeType == TagToolkit.NOT_A_TAG) || (this.filter
								.getLevel() == elementLevel && isNodeTypeEquivalent))) || (!this.filter
						.isShowOnlyThisLevel() && (this.nodeType == TagToolkit.NOT_A_TAG || isNodeTypeEquivalent))))
		...
		// if this node can have children, and can be displayed, the tree nodes
		// are created
		if (this.canBeDisplayed
				&& (this.nodeType == TagToolkit.NOT_A_TAG || this.nodeType == TagToolkit.ENUMERATED)) {
			...
			// the number of the children of the node having the type :
			// NOT_A_TAG
			int notATagChildrenCount = 0;

			for (node = element.getFirstChild(); node != null; node = node
					.getNextSibling()) {
				...
				if (node instanceof Element) {
					...
					if (this.nodeType == TagToolkit.ENUMERATED) {
						...
						childTreeNode = new DataBaseTreeNode(rootDataBase,
								this.filter, (Element) node);
			...
			if (this.nodeType == TagToolkit.NOT_A_TAG) {
				...
				// checking if the tag that has the type : NOT_A_TAG has
				// displayed children,
	...
	public String getTagName() {
		...
		if (this.nodeType != TagToolkit.NOT_A_TAG) {
			...
			this.tagName = EditorAnimationsToolkit
					.normalizePath(DataBaseNodeToolkit.getPath(
							this.rootDataBase, this.element));
		}
	...
	public String getLocation() {
		...
		// if this node represents a view node, the location attribute value is
		// retrieved
	...
	/**
	 * @return the level of the element, i.e.: the number of levels above this
	 *         node (the distance from the root to this node)
	 */
	...
	/**
	 * retrieves and returns the tree node under this tree node corresponding to
	 * the given element
	 * 
	 * @param el
	 *            an element
		...
		if (el != null) {
			...
			if (el.equals(this.element)) {
				...
				while (childrenTreeNodes.hasMoreElements()) {
					...
					childTreeNode = (DataBaseTreeNode) childrenTreeNodes
							.nextElement();
