...
public class DataBaseEditorModule extends ModuleAdapter {
	...
	/**
	 * the constant for the conversion between the widget database and the view
	 * database
	 */
	...
	protected String labelRtdaDatabaseEditor = "",
			labelRtdaDatabaseEditorMenuitemHidden = "",
			labelRtdaDatabaseEditorMenuitemShown = "", noFrameMessage = "",
			noWidgetNodeSelected = "";
	...
	public DataBaseEditorModule(Editor editor) {
		...
		if (bundle != null) {
			...
			try {
				labelRtdaDatabaseEditor = bundle
						.getString("label_rtdadatabaseeditor");
				labelRtdaDatabaseEditorUndoRedo = bundle
						.getString("labelrtdadatabaseeditorundoredo");
				noFrameMessage = bundle
						.getString("rtdaanim_empty_dialog_noframe");
				noWidgetNodeSelected = bundle
						.getString("rtdaanim_empty_dialog_nowidgetnode");
			} catch (Exception ex) {
				ex.printStackTrace();
			}
		}
		...
		HandlesListener svgHandleListener = new HandlesListener() {
			...
			public void handleChanged(SVGHandle currentHandle,
				...
				final SVGHandle handle = getSVGEditor().getHandlesManager()
						.getCurrentHandle();
				...
				if (handle != null) {
					...
					// computing the state
					if (Toolkit.isDocumentAView(handle.getScrollPane()
							.getSVGCanvas().getDocument())) {

						currentType = VIEW_DATABASE;
						manageSelection(new HashSet<Element>(handle
			...
			protected void manageSelection(Set<Element> selectedElements) {
				...
				if (currentType == VIEW_DATABASE
					...
					if (selectedNodes.size() == 1
							&& rtdaDataBaseEditorPanel.isVisible()) {

						Node node = null;
						try {
							node = selectedNodes.getFirst();
						} catch (Exception ex) {
						}
						...
						if (node != null
								...
								&& ((Element) node)
										.hasAttribute(Toolkit.widgetAttribute)) {
		...
		rtdaDataBaseEditorPanel.setLayout(new BoxLayout(
				rtdaDataBaseEditorPanel, BoxLayout.X_AXIS));
	...
	public HashMap<String, JMenuItem> getMenuItems() {
		...
		menuItems.put("ToolFrame_" + this.idrtdaDataBase,
				rtdaDataBaseEditorFrame.getMenuItem());
	...
	public void rtdaDataBaseEditor() {
		...
		if (hasBeenReinitialized
				&& (!rtdaDataBaseEditorFrame.isVisible() || handle == null)) {
		...
		if (handle != null && currentType != UNKNOWN_STATE
				&& currentType != VIEW_DATABASE) {

			if (currentType == WIDGET_DATABASE) {

				// creating and adding the panel editor containing the tree
				// widget displaying the
				// widget database
				editorPanel = new WidgetDataBaseEditorPanel(this, handle
						.getScrollPane().getSVGCanvas().getDocument());

			} else if (currentType == WIDGET_INSERTION_DATABASE) {

				// creating and adding the panel editor containing the tree
				// widget and the swing
				// widgets
				// enabling the user to make the correspondance between the
				// widget database and the
				// view database
				editorPanel = new InsertedWidgetDataBaseEditorPanel(this,
						selectedNodes.getFirst());
			...
			if (editorPanel != null) {
				...
				rtdaDataBaseEditorPanel.setPreferredSize(new Dimension(
			...
			// if we are in an unknow state or in a widget or in a view and no
			// widget image is
			// edited
			if (currentType == VIEW_DATABASE) {
			...
			message = "<html><body align=\"center\">" + message
					+ "</body></html>";
