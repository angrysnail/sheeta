...
/**
 * the widget containing the tree displaying the widget database and the widgets
 * to make the correspondance between the widget data base and the view database
 * 
...
public class InsertedWidgetDataBaseEditorPanel extends DataBaseEditorPanel {
	 ...
	 * the labels
	 */
	protected String topDescriptionLabel = "", chooserLabel = "",
			associatedTagLabel = "", associatedValueLabel = "";
	 ...
	 *            the image node corresponding to the inserted widget
	 */
	public InsertedWidgetDataBaseEditorPanel(DataBaseEditorModule dbEditor,
			final Element widgetImage) {

		this.dbEditor = dbEditor;
		this.widgetImage = widgetImage;

		// getting the root node of the database of the widget under the widget
		// image node
		NodeList children = widgetImage.getElementsByTagName("rtda:"
	...
	protected void buildPanel() {
		...
		model = new InsertedWidgetDataBaseModel(dbEditor, widgetTree,
				widgetDataBaseRoot);
		widgetTree.setModel(model);
		widgetTree
				.setSelectionPath(((InsertedWidgetDataBaseEditorTreeNode) model
						.getRoot()).getTreePath());

		// the cell renderer
		TreeCellRenderer renderer = new InsertedWidgetDataBaseCellRenderer(
				widgetTree);
		...
		final ActionListener buttonListener = new ActionListener() {
			...
			public void actionPerformed(ActionEvent evt) {
				...
				if (selectedPath != null) {
					...
					node = (InsertedWidgetDataBaseEditorTreeNode) selectedPath
					...
					if (node != null) {
						...
						if (node.getType() == TagToolkit.ENUMERATED
							...
							DataBaseNodeFilter filter = new DataBaseNodeFilter(
									node.getTagName(), node.getLevel(),
									node.getType(), true, false, null);

							// getting the information object about the selected
							// tag
							DataBaseNodeChooser nodeChooser = DataBaseNodeChooser
									.getDataBaseNodeChooser(Editor.getParent(),
											widgetImage.getOwnerDocument(),
											filter, false, false);
							nodeChooser.showDialog(((JComponent) evt
									.getSource()));
							nodeChooser.disposeDialog();
							DataBaseNodeInformation info = nodeChooser
									.getInfo();

							if (info != null) {

								// retrieving and setting the new name for the
								// tag
								...
								node.setCorrespondingValue(newTagName,
										newValues);

								String displayedValue = newTagName;
								displayedValue = EditorAnimationsToolkit
										.normalizePath(displayedValue);
								int pos = displayedValue.lastIndexOf("/");

								if (pos != -1) {

									displayedValue = displayedValue.substring(
											pos + 1, displayedValue.length());
								}

								if (displayedValue == null
										|| displayedValue.equals("")) {
		...
		// creating the panel used for selecting a value among several tag
		// values
		...
		final TreeSelectionListener selectionListener = new TreeSelectionListener() {
			...
			public void valueChanged(TreeSelectionEvent evt) {
				...
				node = (InsertedWidgetDataBaseEditorTreeNode) evt.getPath()
						.getLastPathComponent();

				if (node != null) {

					parentNode = (InsertedWidgetDataBaseEditorTreeNode) node
							.getParent();
					...
					if (node.getType() == TagToolkit.ENUMERATED
						...
						// the node corresponds to a tag, therefore, the widgets
						// enabling to select
						// a tag will be displayed
						widgetsPanel.add(tagChooserPanel);

						// setting the text of the textfield with the current
						// corresponding tag of
					...
					} else if (node.getType() == TagToolkit.ENUMERATED_CHILD
							&& parentNode != null) {

						// the node corresponds to the value of an enumerated
						// tag//

						// removing the listener to the combo box
						tagValuesCombo.removeActionListener(comboListener);

						// filling the the combo box enabling to select a tag
						// value among all the
						// available values
						tagValuesCombo.removeAllItems();

						LinkedList<String> values = DataBaseNodeToolkit
								.getEnumeratedTagValues(
										widgetImage.getOwnerDocument(),
										parentNode.getTagName());
