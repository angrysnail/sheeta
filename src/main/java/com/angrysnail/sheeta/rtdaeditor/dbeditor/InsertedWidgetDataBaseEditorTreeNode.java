 ...
 */
public class InsertedWidgetDataBaseEditorTreeNode
		extends
	 ...
	 *            the database editor
	 * @param model
	 *            the widget data base model
	 * @param element
	 *            a dom element
	 */
	public InsertedWidgetDataBaseEditorTreeNode(DataBaseEditorModule dbEditor,
			InsertedWidgetDataBaseModel model, Element element) {

		this.dbEditor = dbEditor;
		this.model = model;
		this.element = element;
		this.svgHandle = dbEditor.getSVGEditor().getHandlesManager()
				.getCurrentHandle();
		this.id = element.getAttribute("id");
		this.correspondingValue = element.getAttribute("value");

		// building the tree path
		LinkedList<InsertedWidgetDataBaseEditorTreeNode> pathList = new LinkedList<InsertedWidgetDataBaseEditorTreeNode>();
		pathList.add(this);
		...
		// getting the int tag type, the name of the icon and the label for the
		// node
		type = TagToolkit.getNodeType(elementName);
		iconName = TagToolkit.getIconName(type);
		typeLabel = TagToolkit.getTagLabel(type);

		// getting the label of the tag node and checking if the tag can have
		// child nodes
		switch (type) {
			...
			if (element.hasChildNodes()) {
				...
				for (int i = 0; i < childrenNodes.getLength(); i++) {
					...
					if (child != null && child instanceof Element) {
						...
						childTreeNode = new InsertedWidgetDataBaseEditorTreeNode(
								dbEditor, model, (Element) child);
						add(childTreeNode);
					}
				}
			}
		...
		this.domListener = new SVGDOMListener(this.element) {
			...
			public void nodeChanged() {
				...
				// checking if the given node equals this tree path element, and
				// modifies the data
				// associated with this node
				if (getNode() != null && getNode() instanceof Element
						&& getNode().equals(treeNode.element)) {

					// updating the data
					treeNode.correspondingValue = ((Element) getNode())
							.getAttribute("value");
			...
			public void structureChanged(Node lastModifiedNode) {
				...
				if (getNode() != null && getNode() instanceof Element
					...
					InsertedWidgetDataBaseEditorTreeNode lastModifiedTreeNode = getTreeNode(
							treeNode, (Element) lastModifiedNode);

					// selecting the last modified node
					treeNode.model.getTree().setSelectionPath(
							lastModifiedTreeNode.getTreePath());
				}
			}
		};
	...
	public void dispose() {
		...
		while (childrenEnum.hasMoreElements()) {
			...
			child = (InsertedWidgetDataBaseEditorTreeNode) childrenEnum
					.nextElement();
	...
	public void add(MutableTreeNode newTreeNode) {
		...
		if (newTreeNode != null
				&& newTreeNode instanceof InsertedWidgetDataBaseEditorTreeNode) {

			childElements
					.add(((InsertedWidgetDataBaseEditorTreeNode) newTreeNode)
							.getElement());
		}
	}
	...
	public void remove(MutableTreeNode oldTreeNode) {
		...
		if (oldTreeNode != null
				&& oldTreeNode instanceof InsertedWidgetDataBaseEditorTreeNode) {

			childElements
					.remove(((InsertedWidgetDataBaseEditorTreeNode) oldTreeNode)
							.getElement());
		}
	}
	...
	/**
	 * returns the tree node corresponding to the given dom node and that can be
	 * found under the root node
	 * 
	 * @param rootNode
	 *            the root node
	 * @param nodeToBeFound
	 *            a dom node
	 * @return the tree node corresponding to the given dom node and that can be
	 *         found under the root node
	 */
	protected InsertedWidgetDataBaseEditorTreeNode getTreeNode(
		...
		if (rootNode != null && nodeToBeFound != null) {
			...
			// checking if the searched tree node corresponds to the given root
			// node
			if (rootNode.getElement().equals(nodeToBeFound)) {
				...
				// searching into the children of the root node, if a
				// corresponding tree node can be
				// found
				...
				while (childrenEnum.hasMoreElements()) {
					...
					childNode = (InsertedWidgetDataBaseEditorTreeNode) childrenEnum
							.nextElement();
	...
	public String getTagName() {
		...
		if (type != TagToolkit.NOT_A_TAG) {
			...
			if (correspondingValue != null && !correspondingValue.equals("")) {
				...
				while (node != null
						&& !node.equals(((InsertedWidgetDataBaseEditorTreeNode) getRoot())
								.getElement())) {

					nodeId = EditorAnimationsToolkit.normalizePath(node
							.getAttribute("value"));
	 ...
	 *            the values for each child of the tag, if it is an enumerated
	 *            tag
	 */
	public void setCorrespondingValue(String value,
			LinkedList<String> childrenValues) {
		...
		if (value != null && !isRoot()) {
			...
			final String newCorrespondingValue = value, oldCorrespondingValue = element
					.getAttribute("value");
			...
			final Runnable executeRunnable = new Runnable() {
				...
				public void run() {
					...
					svgHandle.getSvgDOMListenerManager().fireNodeChanged(
							element);

					// handling the child elements if the tag is an enumerated
					// one
					if (type != TagToolkit.ENUMERATED_CHILD
							&& newChildrenValues.size() > 0) {
						...
						for (Element child : childElements) {
							...
							if (child != null) {
								...
								svgHandle.getSvgDOMListenerManager()
										.fireNodeChanged(child);
							}
	...
	protected String getRepresentationString() {
		...
		if (type == TagToolkit.NOT_A_TAG) {
			...
			if (correspondingValue == null
					|| (correspondingValue != null && correspondingValue
							.equals(""))) {
