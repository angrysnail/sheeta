...
/**
 * the widget containing the tree displaying the widget database nodes and the
 * buttons for creating this database
 * 
...
public class WidgetDataBaseEditorPanel extends DataBaseEditorPanel {
	 ...
	 * the labels
	 */
	protected String topDescriptionLabel = "", insertLabel = "",
			removeLabel = "", errorLabel = "", cannotRemoveTagNodeLabel = "",
			cannotRemoveModuleNodeLabel = "", cannotRemoveValueNodeLabel = "",
			malformedIdLabel = "", duplicatedIdLabel = "";
	...
	protected void buildPanel() {
		...
		if (bundle != null) {
			...
			try {
				topDescriptionLabel = bundle
						.getString("rtdadb_widgetdatabasecreation");
				...
				cannotRemoveTagNodeLabel = bundle
						.getString("rtdadb_cannotdeletetag");
				cannotRemoveModuleNodeLabel = bundle
						.getString("rtdadb_cannotdeletemodule");
				cannotRemoveValueNodeLabel = bundle
						.getString("rtdadb_cannotdeletevalue");
		...
		ImageIcon moduleItemIcon = ResourcesManager.getIcon(
				TagToolkit.getIconName(TagToolkit.NOT_A_TAG), false);

		ImageIcon floatItemIcon = ResourcesManager.getIcon(
				TagToolkit.getIconName(TagToolkit.ANALOGIC_FLOAT), false);

		ImageIcon integerItemIcon = ResourcesManager.getIcon(
				TagToolkit.getIconName(TagToolkit.ANALOGIC_INTEGER), false);

		ImageIcon enumeratedItemIcon = ResourcesManager.getIcon(
				TagToolkit.getIconName(TagToolkit.ENUMERATED), false);

		ImageIcon stringItemIcon = ResourcesManager.getIcon(
				TagToolkit.getIconName(TagToolkit.STRING), false);

		ImageIcon enumeratedValueItemIcon = ResourcesManager.getIcon(
				TagToolkit.getIconName(TagToolkit.ENUMERATED_CHILD), false);
		...
		integerTagItem = new JMenuItem(TagToolkit.integerTagLabel,
				integerItemIcon);
		enumeratedTagItem = new JMenuItem(TagToolkit.enumeratedTagLabel,
				enumeratedItemIcon);
		stringTagItem = new JMenuItem(TagToolkit.stringTagLabel, stringItemIcon);
		enumeratedValueItem = new JMenuItem(TagToolkit.enumeratedValueLabel,
				enumeratedValueItemIcon);
		...
		final TreeSelectionListener selectionListener = new TreeSelectionListener() {
			...
			public void valueChanged(TreeSelectionEvent evt) {
				...
				if (selectedPath != null) {
					...
					WidgetDataBaseEditorTreeNode node = (WidgetDataBaseEditorTreeNode) selectedPath
		...
		final ActionListener insertButtonListener = new ActionListener() {
			...
			public void actionPerformed(ActionEvent evt) {
				...
				if (mousePosition != null) {
					...
					popupMenu.show(insertButton, mousePosition.x,
							mousePosition.y);
				}
			}
		};
			...
			public void mousePressed(final MouseEvent evt) {
				...
				if (isPopUp(evt)) {
					...
					if (path != null) {
						...
						WidgetDataBaseEditorTreeNode node = (WidgetDataBaseEditorTreeNode) path
								.getLastPathComponent();
						...
						if (node != null) {
							...
							widgetTree.setSelectionRow(widgetTree
							...
							SwingUtilities.invokeLater(new Runnable() {
								...
								public void run() {
									...
									if (popupMenu.isEnabled()) {
										...
										popupMenu.show(widgetTree, evt.getX(),
												evt.getY());
									}
								}
							});
				...
				if (currentNode != null && currentNode.getParent() != null
					...
					if (currentNode.canRemoveNode()) {
						...
						JOptionPane.showMessageDialog(Editor.getParent(),
								message, errorLabel, JOptionPane.ERROR_MESSAGE);
					}
				}
			}
		};
			...
			public void actionPerformed(ActionEvent evt) {
				...
				if (currentNode != null) {
					...
					if (evt.getSource().equals(moduleItem)) {
						...
						currentNode
								.insertChildNode(TagToolkit.ENUMERATED_CHILD);
						...
						currentNode
								.insertChildNode(TagToolkit.ANALOGIC_INTEGER);
		...
		JLabel label = new JLabel("<html><body>" + topDescriptionLabel
				+ " :</body></html>");
		...
		SVGHandle handle = dbEditor.getSVGEditor().getHandlesManager()
				.getCurrentHandle();
		...
		if (handle != null) {
			...
			SwingUtilities.invokeLater(new Runnable() {
				...
				public void run() {
					...
					Element widgetDataBaseRoot = DataBaseToolkit
							.getRtdaWidgetDataBase(doc);

					// creating the model for the tree
					model = new WidgetDataBaseModel(dbEditor,
							WidgetDataBaseEditorPanel.this, widgetTree,
							widgetDataBaseRoot);
					widgetTree.setModel(model);

					// the cell renderer
					WidgetDataBaseCellRenderer renderer = new WidgetDataBaseCellRenderer(
							widgetTree);
					widgetTree.setCellRenderer(renderer);

					// the cell editor
					cellEditor = new WidgetDataBaseCellEditor(widgetTree,
							renderer);
					...
					widgetTree
							.setSelectionPath(((WidgetDataBaseEditorTreeNode) model
									.getRoot()).getTreePath());
	 ...
	 *            a node in the tree
	 * @param isPopupEvent
	 *            whether the event that triggered the call of this method is a
	 *            popup event
	 */
	...
	protected void showDuplicatedIdDialog() {
		...
		JOptionPane.showMessageDialog(Editor.getParent(), duplicatedIdLabel,
