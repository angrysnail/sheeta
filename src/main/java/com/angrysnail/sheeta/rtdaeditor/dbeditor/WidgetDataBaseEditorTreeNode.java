...
public class WidgetDataBaseEditorTreeNode extends DefaultMutableTreeNode {
	...
	public WidgetDataBaseEditorTreeNode(DataBaseEditorModule dbEditor,
		...
		this.handle = dbEditor.getSVGEditor().getHandlesManager()
				.getCurrentHandle();
			...
			if (element.hasChildNodes()) {
				...
				for (int i = 0; i < childrenNodes.getLength(); i++) {
					...
					if (child != null && child instanceof Element) {
						...
						childTreeNode = new WidgetDataBaseEditorTreeNode(
		...
		domListener = new SVGDOMListener(this.element) {
			...
			public void nodeInserted(Node insertedNode) {
				...
				if (insertedNode != null
						&& !childElements.contains(insertedNode)) {

					// creating and adding the new tree node
					WidgetDataBaseEditorTreeNode childTreeNode = new WidgetDataBaseEditorTreeNode(
							treeNode.dbEditor, treeNode.model,
							(Element) insertedNode);

					treeNode.model.insertNodeInto(childTreeNode, treeNode,
							getChildCount());
					treeNode.model.getTree().makeVisible(
							childTreeNode.getTreePath());
				}
			}
				...
				if (removedNode != null) {
					...
					if (childTreeNode != null) {
						...
						if (treeNode.getChildCount() > 0) {
							...
							WidgetDataBaseEditorTreeNode firstChild = ((WidgetDataBaseEditorTreeNode) treeNode
									.getFirstChild());

							treeNode.model.getTree().setSelectionPath(
									firstChild.getTreePath());
							treeNode.model.getTree().scrollPathToVisible(
									firstChild.getTreePath());
	...
	public void add(MutableTreeNode newTreeNode) {
		...
		if (newTreeNode != null
				&& newTreeNode instanceof WidgetDataBaseEditorTreeNode) {

			childElements.add(((WidgetDataBaseEditorTreeNode) newTreeNode)
					.getElement());
		}
	}
	...
	public void remove(MutableTreeNode oldTreeNode) {
		...
		if (treeNode != null
				&& oldTreeNode instanceof WidgetDataBaseEditorTreeNode) {

			childElements.remove(((WidgetDataBaseEditorTreeNode) oldTreeNode)
					.getElement());
		}
	}
	...
	public boolean canRemoveNode() {
		...
		if (type == TagToolkit.NOT_A_TAG) {
			...
			// if the parent node is null, then the node is the root node, and
			...
			if (getParent() != null) {
				...
				if (children == null) {
					...
					// checking if the children of the module can be removed, if
					// not, the module
					// cannot be removed
					for (int i = 0; i < children.size(); i++) {

						if (!((WidgetDataBaseEditorTreeNode) children.get(i))
								.canRemoveNode()) {
			...
			// the enumerated value node can be removed if its parent can be
			// removed
			canRemoveNode = ((WidgetDataBaseEditorTreeNode) getParent())
					.canRemoveNode();

		} else {

			canRemoveNode = !(EditorAnimationsToolkit.isTagUsed(element
					.getOwnerDocument().getDocumentElement(), getTagName()));
		}
	...
	public boolean isDuplicatedId(String foundId) {
		...
		if (foundId != null && !foundId.equals("")) {
			...
			if (!isRoot()) {
				...
				// creating the set of the ids of the siblings of the current
				// node
				HashSet<String> siblingIds = new HashSet<String>();
				String currentId = "";
				Set<Element> childEls = ((WidgetDataBaseEditorTreeNode) getParent())
						.getChildElements();
	...
	public void setId(String id) {
		...
		if (id != null && !id.equals(this.id)) {
			...
			if (getParent() != null) {
				...
				parentTagName = ((WidgetDataBaseEditorTreeNode) getParent())
						.getTagName();
			}
			...
			final Runnable executeRunnable = new Runnable() {
				...
				public void run() {
					...
					if (type == TagToolkit.ENUMERATED_CHILD) {
						...
						EditorAnimationsToolkit
								.convertEnumeratedTagValue(element
										.getOwnerDocument()
										.getDocumentElement(), fparentTagName,
										oldId, newId);

					} else {

						EditorAnimationsToolkit.convertTags(element
		...
		if (getAllowsChildren() && child != null) {
			...
			final Runnable executeRunnable = new Runnable() {
				...
				public void run() {
					...
					handle.getSvgDOMListenerManager().fireNodeRemoved(element,
							elementToBeRemoved);
				}
			};
			...
			final Runnable undoRunnable = new Runnable() {
				...
				public void run() {
					...
					handle.getSvgDOMListenerManager().fireNodeInserted(element,
							elementToBeRemoved);
				}
			};
			// adding the undo/redo actions
