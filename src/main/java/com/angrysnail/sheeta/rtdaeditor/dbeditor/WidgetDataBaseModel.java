...
public class WidgetDataBaseModel extends DefaultTreeModel implements TreeModel {
	 ...
	 *            the tree corresponding to the model
	 * @param rootModel
	 *            the root element of the subtree of the widget data base
	 */
	public WidgetDataBaseModel(DataBaseEditorModule dbEditor,
		...
		if (rootModel != null) {
			...
			rootNode = new WidgetDataBaseEditorTreeNode(dbEditor, this,
					rootModel);
			setRoot(rootNode);
		}
	}
	...
	public void insertNodeInto(MutableTreeNode newChild,
		...
		if (parent != null && parent instanceof WidgetDataBaseEditorTreeNode
				&& newChild != null
				&& newChild instanceof WidgetDataBaseEditorTreeNode) {
	...
	public void removeNodeFromParent(MutableTreeNode childNode) {
		...
		if (childNode != null
				&& childNode instanceof WidgetDataBaseEditorTreeNode) {

			((WidgetDataBaseEditorTreeNode) childNode.getParent())
					.getChildElements().remove(
							((WidgetDataBaseEditorTreeNode) childNode)
									.getElement());
		}
			...
			if (node != null) {
				...
				if (node.isDuplicatedId(newValue.toString())) {
					...
					editorPanel.showDuplicatedIdDialog();
						...
						|| (newValue.toString() != null && newValue.toString()
								.equals(""))) {
