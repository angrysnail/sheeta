...
import com.angrysnail.sheeta.rtda.jwidget.PluginLoader;
...
public abstract class JWidgetEdition {
	 ...
	 */
	protected AnimationChooserJWidgetSourceChooser animationsSourceChooser,
			actionsSourceChooser;
	 ...
	 *            the main frame
	 * @param id
	 *            the id of the widget edition object
	 * @param toolItemPosition
	 *            the tool item position
	 */
	public JWidgetEdition(JWidgetManager jwidgetManager, Frame mainFrame,
			String id, int toolItemPosition) {
		...
		animationsAndActionsDocument = getAnimationsAndActionsDocument(
				getClass(), getId() + ".xml");

		// checking if the animations and actions document defines animations
		// and actions
		...
		try {
			bundle = ResourceBundle.getBundle(getClass().getPackage().getName()
					+ ".properties." + getId(), Locale.getDefault(),
		...
		if (bundle == null) {
			...
			try {
				bundle = ResourceBundle.getBundle(getClass().getPackage()
						.getName() + ".properties." + getId());
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
		...
		try {
			...
			undoRedoModificationLabel = ResourcesManager.bundle
					.getString("jwidgeteditionundoredomodify");
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	...
	/**
	 * @param isGrayIcon
	 *            whether the icon should be grayed or not
	 ...
	 * @param jwidgetElement
	 *            the jwidget element
	 * @param imageJWidgetElement
	 *            the image jwidget element
	 * @param size
	 *            the size of the image
	 */
	public void handleRepresentationImage(SVGHandle svgHandle,
			Element jwidgetElement, final Element imageJWidgetElement,
			Dimension size) {
		...
		if (size == null) {
			...
			try {
				width = Double.parseDouble(imageJWidgetElement
						.getAttribute("width"));
				height = Double.parseDouble(imageJWidgetElement
						.getAttribute("height"));
			} catch (Exception ex) {
			}
		...
		Runnable runnable = new Runnable() {
			...
			public void run() {
				...
				imageJWidgetElement.setAttributeNS(EditorToolkit.xmlnsXLinkNS,
						EditorToolkit.xLinkprefix + "href", attributeValue);
			}
		};
	 ...
	 */
	protected abstract BufferedImage createImage(Element jwidgetElement,
			Dimension size);
	 ...
	 * @param refreshElementRepresentation
	 *            whether the representation image of the jwidget element should
	 *            be refreshed
	 */
	public void setProperty(final Element jwidgetElement,
			final String propertyName, final String newValue,
		...
		if (!oldValue.equals(newValue)) {
			...
			Runnable executeRunnable = new Runnable() {
				...
				public void run() {
					...
					if (refreshElementRepresentation) {
						...
						jwidgetManager
								.refreshElementRepresentation(topMostJWidgetElement);
					}
				}
			};
			...
			Runnable undoRunnable = new Runnable() {
				...
				public void run() {
					...
					handle.getSvgDOMListenerManager().fireNodeChanged(
							jwidgetElement);
	 ...
	 * 
	 * @param parentElement
	 *            the parent element to which the jwidget node is added
	 ...
	 * 
	 * @param jwidgetParentElement
	 *            the parent jwidget element
	 * @param name
	 *            the name of the new jwidget sub element
	 * @param widgetLabel
	 *            the label of the sub widget element
	 * @param attributes
	 *            the map associating a property name to its default value
	 * @return a new sub widget element
	 * @param refreshElementRepresentation
	 *            whether the representation image of the jwidget element should
	 *            be refreshed
	 */
	public Element createSubWidgetElement(final Element jwidgetParentElement,
			String name, String widgetLabel,
		...
		Runnable executeRunnable = new Runnable() {
			...
			public void run() {
				...
				handle.getSvgDOMListenerManager().fireNodeInserted(
						jwidgetParentElement, jwidgetElement);

				// refreshing the image representation of the property
				if (refreshElementRepresentation) {

					jwidgetManager
							.refreshElementRepresentation(jwidgetParentElement);
				}
			}
		};
	 ...
	 * 
	 * @param jwidgetParentElement
	 *            the jwidget parent element
	 * @param childElement
	 *            the jwidget child element
	 * @param refreshElementRepresentation
	 *            whether the representation image of the jwidget element should
	 *            be refreshed
	 */
	public void removeSubWidgetElement(final Element jwidgetParentElement,
		...
		Runnable executeRunnable = new Runnable() {
			...
			public void run() {
				...
				handle.getSvgDOMListenerManager().fireNodeRemoved(
						jwidgetParentElement, childElement);
		...
		Runnable undoRunnable = new Runnable() {
			...
			public void run() {
				...
				if (nextElement != null) {
					...
					jwidgetParentElement
							.insertBefore(childElement, nextElement);
	 ...
	 * 
	 * @param jWidgetElement
	 *            a jwidget element node
	 * @param refreshElementRepresentation
	 *            whether the representation image of the jwidget element should
	 *            be refreshed
	 */
	public void putUp(final Element jWidgetElement,
			final boolean refreshElementRepresentation) {
		...
		if (previousElement != null) {
			...
			Runnable executeRunnable = new Runnable() {
				...
				public void run() {
					...
					handle.getSvgDOMListenerManager().fireStructureChanged(
							parentElement, jWidgetElement);
	 ...
	 *            be refreshed
	 */
	public void putDown(final Element jWidgetElement,
			final boolean refreshElementRepresentation) {
			...
			Runnable executeRunnable = new Runnable() {
				...
				public void run() {
					...
					if (secondNextSibling != null) {
						...
						parentElement.insertBefore(jWidgetElement,
								secondNextSibling);
	 ...
	 * 
	 * @param element
	 *            the element before this element
	...
	protected Element getPreviousElement(Element element) {
		...
		for (Node node = element.getPreviousSibling(); node != null; node = node
				.getPreviousSibling()) {
	 ...
	 * 
	 * @param element
	 *            the element after this element
	...
	/**
	 * returns the top most jwidget element that is a parent of the given
	 * jwidget parent element
	 * 
	 * @param element
	 *            a jwidget element
	 * @return the top most jwidget element that is a parent of the given
	 *         jwidget parent element
	 */
		...
		if (element != null) {
			...
			while (parentNode.getParentNode() != null
					&& parentNode.getParentNode().getNodeName()
							.equals(Toolkit.jwidgetElementName)) {
	 ...
	 *            the redo runnable
	 */
	protected void addUndoRedoAction(Runnable executeRunnable,
		...
		UndoRedoAction action = new UndoRedoAction(undoRedoModificationLabel,
				executeRunnable, undoRunnable, redoRunnable,
				new HashSet<Element>());
	 ...
	 * 
	 * @param baseClass
	 *            the base class from which the xml document should be loaded
	 * @param name
	 *            a name
	 * @return the animations and actions document
	 */
	protected Document getAnimationsAndActionsDocument(Class<?> baseClass,
			String name) {
	...
	/**
	 * @param isAnimations
	 *            whether the animations source chooser is required or not
	 * @return the source chooser
	 */
	public AnimationChooserJWidgetSourceChooser getSourceChooser(
	...
	/**
	 * returns an array of two icons : the first one is the regular icon, the
	 * second is the grayed one
	 * 
	 * @param baseClass
	 *            the base class from which the icon should be loaded
	 * @param name
	 *            a name
	 * @return an array of two icons : the first one is the regular icon, the
	 *         second is the grayed one
	 */
	protected static ImageIcon[] getIcons(Class<?> baseClass, String name) {
		...
		if (name != null && !name.equals("")) {
			...
			InputStream in = baseClass.getResourceAsStream("icons/" + name
					+ ".png");
			...
			if (imageIcons[0] != null) {
				...
				imageIcons[1] = new ImageIcon(
						GrayFilter.createDisabledImage(imageIcons[0].getImage()));
