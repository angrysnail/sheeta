...
import com.angrysnail.sheeta.svgeditor.shape.RectangularShape;
...
public class JWidgetManager extends RectangularShape {
	 ...
	 */
	private JWidgetMenuItemsManager itemsManager = new JWidgetMenuItemsManager(
			this);
	...
	/**
	 * the thread used to refresh the representation image of each jwidget
	 * element
	 */
	private JWidgetRepresentationRefreshThread refreshThread = new JWidgetRepresentationRefreshThread(
			this);
	...
	public void initialize() {
		...
		// adding the jwidget edition object to the tools and menu items
		// managers
	...
	public void refresh(Element element) {
		...
		JWidgetEdition jwidgetEdition = getJWidgetEdition(jwidgetElement
		...
		if (jwidgetEdition != null) {
			...
			jwidgetEdition.handleRepresentationImage(handle, jwidgetElement,
					(Element) jwidgetElement.getParentNode(), null);
		}
	}
	...
	public Element createElement(SVGHandle handle, Rectangle2D bounds) {
		...
		if (currentJWidgetEdition != null) {
			...
			final Element imageJWidgetElement = doc.createElementNS(doc
					.getDocumentElement().getNamespaceURI(),
					handledElementTagName);

			imageJWidgetElement.setAttributeNS(null, xAtt,
					FormatStore.format(bounds.getX()));
			imageJWidgetElement.setAttributeNS(null, yAtt,
					FormatStore.format(bounds.getY()));
			imageJWidgetElement.setAttributeNS(null, wAtt,
					FormatStore.format(bounds.getWidth()));
			imageJWidgetElement.setAttributeNS(null, hAtt,
					FormatStore.format(bounds.getHeight()));
			imageJWidgetElement.setAttributeNS(null, "preserveAspectRatio",
					"none meet");
			imageJWidgetElement.setAttributeNS(EditorToolkit.xmlnsXLinkNS,
					EditorToolkit.xLinkprefix + "actuate", "onRequest");

			// creating the jwidget element for this image element
			Element jwidgetElement = currentJWidgetEdition
					.createJWidgetElement(imageJWidgetElement);
			...
			currentJWidgetEdition.handleRepresentationImage(
					...
					jwidgetElement,
					imageJWidgetElement,
					new Dimension((int) (bounds.getWidth() == 0 ? 1 : bounds
							.getWidth()), (int) (bounds.getHeight() == 0
							? 1
							: bounds.getHeight())));
	...
	protected void loadJWidgets() {
		...
		Map<String, Class<?>> jwidgetRuntimeClasses = JWidgetRuntimeManager
				.getJWidgetRuntimeClassesMap();
		...
		for (Class<?> theClass : jwidgetRuntimeClasses.values()) {
			...
			if (theClass != null) {
				...
				try {
					method = theClass.getMethod("getEditionClass",
							(Class<?>[]) null);

					if (method != null) {

						editionClass = (Class<?>) method.invoke(null,
								(Object[]) null);
					}
				} catch (Exception ex) {
					editionClass = null;
