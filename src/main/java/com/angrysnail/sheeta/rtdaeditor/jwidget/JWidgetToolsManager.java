...
public class JWidgetToolsManager {
	...
	public void addJWidgetEditionObjects(
		...
		tooltipLabel = ResourcesManager.bundle.getString(jwidgetManager.getId()
				+ "ItemToolTip");
		...
		if (jwidgetEditionObjectsList.size() > 0) {
			...
			setCurrentJWidgetEditionObject(
					jwidgetEditionObjectsList.getFirst(), false);
		}
	 ...
	 * @param execute
	 *            whether to execute the action (initialization option)
	 */
	protected void setCurrentJWidgetEditionObject(
		...
		if (jwidgetEdition != null) {
			...
			toolButton.setToolTipText(tooltipLabel
					+ jwidgetEdition.getDescription());
	 ...
	 * 
	 * @param selected
	 *            whether the tool button should be selected
	 */
