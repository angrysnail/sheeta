...
import com.angrysnail.sheeta.library.widgets.FileDialog;
...
public abstract class RtdaShape extends RectangularShape {
	 ...
	 */
	protected com.angrysnail.sheeta.library.widgets.FileDialog fileDialog;

	/**
	 * the labels
	 */
	protected String errorLabel = "", insertionFailureLabel = "",
			wrongFileTypeLabel = "";
	...
	protected boolean isDrawingEnabled(SVGHandle handle) {
		...
		if (handle != null) {
			...
			boolean isAView = com.angrysnail.sheeta.library.Toolkit
					.isDocumentAView(handle.getScrollPane().getSVGCanvas()
							.getDocument());
	 ...
	 *            a svg handle
	 * @param location
	 *            the location for the new element to create
	 */
			...
			public void run() {
				...
				if (values != null) {
					...
					if (data != null) {
						...
						final Element newElement = createElement(handle, data,
								location);
	 ...
	 * @param handle
	 *            a svg handle
	 * @return an array of string of three elements : the first one is the
	 *         absolute svg file path, the second one is the relative svg file
	 *         path to the current canvas path, or an empty string if it is not
	 *         used, the third is the xml path of a view or an empty string, if
	 *         the file is not a view
	 */
	 ...
	 * 
	 * @param values
	 *            the array of the values for the svg data
	 ...
	 */
	protected abstract Element createElement(SVGHandle handle,
			ViewOrWidgetData svgData, Point2D location);
		...
		SwingUtilities.invokeLater(new Runnable() {
			...
			public void run() {
				...
				JOptionPane.showMessageDialog(Editor.getParent(),
						insertionFailureLabel, errorLabel,
	...
	/**
	 * shows an error dialog specifying that the file to be inserted has a wrong
	 * type
	 */
	...
	protected class ViewOrWidgetData {
		 ...
		 *            the view xml path
		 */
		protected ViewOrWidgetData(Document document, String path,
				String viewXmlPath) {
			...
			try {
				projectFile = AnimationsToolkit.getProjectFile(new URI(path),
						true);
			} catch (Exception ex) {
			}
		}
		...
		protected String relativize(String basePath) {
			...
			try {
				...
				relativePath = com.angrysnail.sheeta.library.Toolkit
						.getRelativePath(new URI(path).toASCIIString(),
								referenceFile.getParentFile().toURI()
										.toASCIIString());
			} catch (Exception ex) {
			}
