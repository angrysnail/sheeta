...
public class RtdaViewShape extends RtdaShape {
	...
	public RtdaViewShape(Editor editor) {
		...
		wrongFileTypeLabel = bundle
				.getString("RtdaViewShapeInsertedSVGNotView");
	}
	...
	protected String[] getSVGFile(SVGHandle handle) {
		...
		DataBaseNodeFilter filter = new DataBaseNodeFilter("", 0,
				TagToolkit.VIEW, true, false, handle.getName());

		// getting the information object about the selected view
		DataBaseNodeChooser nodeChooser = DataBaseNodeChooser
				.getDataBaseNodeChooser(Editor.getParent(), doc, filter, false,
						false);
		...
		if (info != null) {
			...
			String viewPath = info.getXmlPath(), imagePath = info
					.getViewLocation();
			...
			if (imagePath != null && !imagePath.equals("")) {
				...
				try {
					absolutePath = DataBaseToolkit.getAbsoluteLocation(new URI(
							imagePath), new URI(handle.getName()));
				} catch (Exception ex) {
					imagePath = null;
				}
			}
	...
	@Override
	protected Element createElement(SVGHandle handle, ViewOrWidgetData svgData,
			Point2D location) {
		...
		if (viewPath != null && imagePath != null) {
			...
			// adds the rtda namespace attribute to the the root element if it
			// does not contain it
			...
			EditorToolkit.setAttributeValue(element, wAtt, svgData.getSize()
					.getX());
			EditorToolkit.setAttributeValue(element, hAtt, svgData.getSize()
					.getY());
