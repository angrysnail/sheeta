...
Contact : jordi.suc@itris.fr; philippe.gil@itris.fr
 ...
 =============================================
 */
package com.angrysnail.sheeta.rtdaeditor.shape;
...
public class RtdaWidgetShape extends RtdaShape {
	...
	public RtdaWidgetShape(Editor editor) {
		...
		wrongFileTypeLabel = bundle
				.getString("RtdaWidgetShapeInsertedSVGNotWidget");
	}
			...
			try {
				SwingUtilities.invokeAndWait(new Runnable() {
					...
					public void run() {
						...
						fileDialog.askForFile(extensions, finitialFile,
								new File[]{fcanvasFile});
					}
				});
			} catch (Exception ex) {
			}
	...
	@Override
	public Element createElement(final SVGHandle handle,
			ViewOrWidgetData widgetData, Point2D location) {
		...
		if (widgetData != null) {
			...
			if (size != null) {
				...
				// getting the project name of the canvas and the relativized
				// path of the widget
				String projectName = "";
				...
				if (widgetPath != null && !widgetPath.equals("")) {
					...
					// getting the project name corresponding to the widget svg
					// file
					projectName = widgetData.getProjectName();

					// relativizing the widget path against the current canvas
					// path
					widgetPath = widgetData.relativize(handle.getCanvas()
							.getURI());
				}
				...
				// adding the rtda namespace attribute to the the root element
				// if it does not
				// contain it
				...
				// creating the widget element
				widgetElement = doc.createElementNS(doc.getDocumentElement()
						.getNamespaceURI(), handledElementTagName);
				widgetElement.setAttributeNS(EditorToolkit.xmlnsXLinkNS,
						"xlink:href", widgetPath);
				EditorToolkit.setAttributeValue(widgetElement, xAtt,
						location.getX());
				EditorToolkit.setAttributeValue(widgetElement, yAtt,
						location.getY());
				EditorToolkit.setAttributeValue(widgetElement, wAtt,
						size.getX());
				EditorToolkit.setAttributeValue(widgetElement, hAtt,
						size.getY());
				widgetElement.setAttributeNS(null, Toolkit.widgetAttribute,
						projectName);
				widgetElement.setAttributeNS(null, preserveAtt, "none meet");

				// getting the root of the widget database
				Element dbRoot = DataBaseToolkit
						.getRtdaWidgetDataBase(widgetData.getDocument());
				...
				if (dbRoot != null) {
					...
					String referenceNode = doc.getDocumentElement()
							.getAttribute(referenceNodeAtt);

					// setting the value attribute for the widget database root
					// node
				...
				SwingUtilities.invokeLater(new Runnable() {
					...
					public void run() {
						...
						// inserting the element in the document and handling
						// the undo/redo support
