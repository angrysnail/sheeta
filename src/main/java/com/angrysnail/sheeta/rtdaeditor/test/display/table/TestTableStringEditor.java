 ...
 */
public class TestTableStringEditor extends AbstractCellEditor
		implements
			TableCellEditor {
	 ...
	 *            the map associating
	 */
	protected TestTableStringEditor(JTable table,
			...
			public void actionPerformed(ActionEvent evt) {
				...
				if (theCombo.getSelectedItem() != null) {
					...
					if (item instanceof TestTableSVGComboItem) {
						...
						currentValue = ((TestTableSVGComboItem) item)
								.getValue();
		...
		textFieldListener = new CaretListener() {
			...
			public void caretUpdate(CaretEvent evt) {
				...
				try {
					db = Double.parseDouble(((JTextField) evt.getSource())
							.getText());
				} catch (Exception ex) {
					db = Double.NaN;
				}
	...
	protected void fireEditingStopped() {
		...
		if (lastComponent != null && lastComponent instanceof JTextField) {
			...
			((JTextField) lastComponent)
					.removeCaretListener(textFieldStringListener);
		}
	...
	public Component getTableCellEditorComponent(JTable table, Object value,
		...
		if (row < testSimulationValues.size() && value != null) {
			...
			if (testValues != null) {
				...
				if (value instanceof Number) {
				...
				} else if (((TestTableModel) table.getModel()).getColumnId(col)
						.equals("value")
					...
					if (testValues.getTagType() == TagToolkit.STRING) {
						...
						if (testValues.getValue() != TestSimulationTagValues
								.getInvalidLabel()) {
						...
						list.addAll(testValues.getEnumeratedValuesMap()
								.values());
						...
						for (String enumValue : list) {
							...
							if (enumValue != null) {
								...
								item = new TestTableSVGComboItem(enumValue,
										enumValue, col, testValues);
					...
					lastComponent = currentCombo;
						...
						.equals("type") && value instanceof String) {
					...
					for (String name : functions.keySet()) {
						...
						if (name != null && !name.equals("")) {
							...
							if (label != null && !label.equals("")) {
								...
								item = new TestTableSVGComboItem(name, label,
										col, testValues);

							} else {

								item = new TestTableSVGComboItem(name, name,
										col, testValues);
							}
