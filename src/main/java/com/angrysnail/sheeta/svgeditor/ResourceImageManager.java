...
public class ResourceImageManager {
	...
	public ResourceImageManager(Editor editor) {
		...
		editor.getHandlesManager().addHandlesListener(new HandlesListener() {
			...
			public void handleChanged(SVGHandle currentHandle,
				...
				for (SVGHandle handle : new LinkedList<SVGHandle>(
						handleToIdToImages.keySet())) {
	...
	public ResourceRepresentation getResourceRepresentation(
		...
		if (svgHandle != null && resourceId != null) {
			...
			Runnable runnable = new Runnable() {
				...
				public void run() {
					...
					ImageRepresentation imageRepresentation = getResourceImage(
							fhandle, fresourceId);
		...
		return getRepresentation(svgHandle, resourceId, useSmallImage,
				componentToRefresh);
	}
	 ...
	 *            the id of a resource node
	 */
	public void invalidateResourceRepresentation(SVGHandle svgHandle,
			String resourceId) {
		...
		if (svgHandle != null) {
			...
			if (handleToIdToImages.containsKey(svgHandle)) {
				...
				if (idToImageMap != null) {
					...
					Map<String, Element> resources = svgHandle
							.getSvgResourcesManager().getResourcesFromDefs(
									svgHandle.getCanvas().getDocument(),
									resourceNames);

					for (String id : new LinkedList<String>(
							idToImageMap.keySet())) {
	...
	protected void createNewImage(SVGHandle svgHandle, String resourceId) {
		...
		if (svgHandle != null && resourceId != null && !resourceId.equals("")) {
			...
			try {
				resourceElement = svgHandle.getScrollPane().getSVGCanvas()
						.getDocument().getElementById(resourceId);
			} catch (Exception ex) {
			}
			...
			if (resourceElement != null) {
				...
				canvas.setDocumentState(AbstractJSVGComponent.ALWAYS_STATIC);
				canvas.setDisableInteractions(true);

				// creating the new document
				final String svgNS = SVGDOMImplementation.SVG_NAMESPACE_URI;
				final SVGDocument doc = (SVGDocument) resourceElement
						.getOwnerDocument().cloneNode(false);

				// creating the root element
				final Element root = (Element) doc.importNode(resourceElement
						.getOwnerDocument().getDocumentElement(), false);
				doc.appendChild(root);

				// removing all the attributes of the root element
				NamedNodeMap attributes = doc.getDocumentElement()
						.getAttributes();
				...
				for (int i = 0; i < attributes.getLength(); i++) {
					...
					if (attributes.item(i) != null) {
						...
						doc.getDocumentElement().removeAttribute(
				...
				final Element defs = (Element) doc.importNode(
						resourceElement.getParentNode(), true);
				root.appendChild(defs);

				if (resourceElement.getNodeName().equals("linearGradient")
						|| resourceElement.getNodeName().equals(
								"radialGradient")
					...
					if (clonedResourceElement != null) {
						...
						// element
						Element initialRoot = resourceElement
						...
						if (resourceElement.getNodeName().equals(
								"linearGradient")) {

							if (resourceElement.getAttributeNS(null,
									"gradientUnits").equals("userSpaceOnUse")) {
								...
								// the rectangle
								try {
									x1 = Double.parseDouble(resourceElement
											.getAttributeNS(null, "x1"));
									y1 = Double.parseDouble(resourceElement
											.getAttributeNS(null, "y1"));
									x2 = Double.parseDouble(resourceElement
											.getAttributeNS(null, "x2"));
									y2 = Double.parseDouble(resourceElement
											.getAttributeNS(null, "y2"));
								...
								clonedResourceElement.setAttributeNS(null,
										"x1", format.format(x1));
								clonedResourceElement.setAttributeNS(null,
										"y1", format.format(y1));
								clonedResourceElement.setAttributeNS(null,
						...
						} else if (resourceElement.getNodeName().equals(
								"radialGradient")) {
							...
							if (resourceElement.getAttributeNS(null,
								...
								// the rectangle
								try {
									cx = Double.parseDouble(resourceElement
											.getAttributeNS(null, "cx"));
									cy = Double.parseDouble(resourceElement
											.getAttributeNS(null, "cy"));
									r = Double.parseDouble(resourceElement
											.getAttributeNS(null, "r"));
									fx = Double.parseDouble(resourceElement
											.getAttributeNS(null, "fx"));
									fy = Double.parseDouble(resourceElement
											.getAttributeNS(null, "fy"));
								...
								clonedResourceElement.setAttributeNS(null,
										"cy", format.format(cy));
								clonedResourceElement.setAttributeNS(null, "r",
										format.format(r));
								clonedResourceElement.setAttributeNS(null,
										"fx", format.format(fx));
								clonedResourceElement.setAttributeNS(null,
							...
							if (resourceElement.getAttributeNS(null,
									"patternUnits").equals("userSpaceOnUse")) {
								...
								clonedResourceElement.setAttributeNS(null, "x",
										format.format(x));
								clonedResourceElement.setAttributeNS(null, "y",
										format.format(y));
								clonedResourceElement.setAttributeNS(null,
										"width", format.format(w));
								clonedResourceElement.setAttributeNS(null,
										"height", format.format(h));
							}
						}
					}
					...
					line.setAttributeNS(null, "style",
							"stroke:none;fill:none;marker-start:url(#" + id
									+ ");");
				}
				...
				GVTTreeRendererAdapter gVTTreeRendererAdapter = new GVTTreeRendererAdapter() {
					...
					public void gvtRenderingCompleted(GVTTreeRendererEvent evt) {
						...
						if (bufferedImage != null) {
							...
							Color borderColor = MetalLookAndFeel
									.getSeparatorForeground();

							g.setColor(borderColor);
							g.drawRect(0, 0, imageSize.width - 1,
									imageSize.height - 1);
						}
	 ...
	 */
	protected ResourceRepresentation getRepresentation(SVGHandle svgHandle,
			String resourceId, boolean useSmallImage,
			Component componentToRefresh) {
		...
		if (svgHandle != null && resourceId != null && !resourceId.equals("")) {
			...
			rep = new ResourceRepresentation(svgHandle, resourceId,
					useSmallImage, componentToRefresh);
		}
	 ...
	 */
	public ImageRepresentation getResourceImage(SVGHandle svgHandle,
			String resourceId) {
	 ...
	 */
	public Image getImage(SVGHandle svgHandle, String resourceId,
		...
		if (svgHandle != null && resourceId != null && !resourceId.equals("")) {
			...
			if (handleToIdToImages.containsKey(svgHandle)) {
				...
				if (idToImageMap.containsKey(resourceId)) {
					...
					ImageRepresentation imageRepresentation = idToImageMap
							.get(resourceId);
	...
	public class ResourceRepresentation extends JPanel {
		...
		protected ResourceRepresentation(SVGHandle svgHandle,
			...
			if (useSmallImage) {
				...
				setPreferredSize(new Dimension(smallImageSize.width + 2,
						smallImageSize.height + 2));

			} else {

				setPreferredSize(new Dimension(imageSize.width + 2,
						imageSize.height + 2));
			}

			ImageRepresentation imageRepresentation = getResourceImage(
					svgHandle, resourceId);
			Image image = null;
			...
			if (imageRepresentation != null) {
				...
				if (useSmallImage) {
					...
					image = new ImageIcon(imageRepresentation.getSmallImage())
							.getImage();

				} else {

					image = new ImageIcon(imageRepresentation.getLargeImage())
							.getImage();
				}
			}
		...
		protected void refreshRepresentation() {// TODO
			...
			if (imageRepresentation != null) {
				...
				if (useSmallImage
						&& imageRepresentation.getSmallImage() != null) {

					synchronized (this) {
						resourceImage = new ImageIcon(
								imageRepresentation.getSmallImage()).getImage();
					}
					...
					synchronized (this) {
						resourceImage = new ImageIcon(
								imageRepresentation.getLargeImage()).getImage();
					}
				}
	...
	protected class ImageRepresentation {
		...
		protected ImageRepresentation(Image image) {
			...
			this.smallImage = image.getScaledInstance(smallImageSize.width,
					smallImageSize.height, Image.SCALE_SMOOTH);
		}
