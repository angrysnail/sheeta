				...
				for (Element current : sourceNodesToPaste) {
					...
					if (current != null) {
						...
						if (!samePosition) {
							...
							shapeModule = ShapeToolkit
									.getShapeModule(clonedElement);
									...
									if (mousePosition) {
										...
										if (mousePoint != null) {
											...
											translationFactors.x = -shapeBounds
													.getX() + mousePoint.getX();
											translationFactors.y = -shapeBounds
													.getY() + mousePoint.getY();
										}
				...
				for (Element res : resNodes) {
					...
					if (res != null && resId != null) {
						...
						if (!handle.getSvgElementsManager().checkId(resId)) {
							...
							// creating the new id
							newId = handle.getSvgElementsManager().getId(resId,
									null);

							// modifying the id of the resource
							res.setAttribute("id", newId);
							modifyReferencedResourceId(handle,
									new HashSet<Element>(nodesToPaste), resId,
									newId);
						}

						// adding the resource node to the list of the resource
						// nodes
		...
		if (selectedNodes.size() > 0) {
			...
			for (Element el : childNodesList) {
				...
				if (clonedSelectedNodesSet.contains(el)) {
					...
					nextNodesMap.put(el, (((i + 1) < childNodesList.size())
							? childNodesList.get(i + 1)
							: null));
				}
			...
			// adding the undo/redo action list
			UndoRedoAction action = new UndoRedoAction(isDelete
					? undoRedoDeleteLabel
					: undoRedoCutLabel, executeRunnable, undoRunnable,
					executeRunnable, new HashSet<Element>(elementsToDelete));
