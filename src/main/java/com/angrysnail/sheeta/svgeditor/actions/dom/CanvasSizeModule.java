...
public class CanvasSizeModule extends DomActionsModule {
	...
	public CanvasSizeModule(Editor editor) {
		...
		if (Editor.getParent() instanceof Frame) {
			...
			canvasSizeDialog = new CanvasSizeDialog(this,
	...
	protected void doAction(SVGHandle handle, Set<Element> elements, int index,
		...
		switch (type) {
			...
			case RESIZE_CANVAS :
				...
				Point2D currentSize = handle.getCanvas()
						.getGeometryCanvasSize();

				// showing the dialog
				canvasSizeDialog.showDialog((JComponent) evt.getSource(),
						currentSize);
				break;
	...
	protected void changeCanvasSize(final Point2D newSize) {
		...
		if (newSize != null) {
			...
			final SVGHandle currentHandle = Editor.getEditor()
					.getHandlesManager().getCurrentHandle();

			if (currentHandle != null) {

				// getting the current size of the canvas
				final Point2D currentSize = currentHandle.getCanvas()
						.getGeometryCanvasSize();
	...
	/**
	 * resizes the canvas denoted by the provided handle so that all whitespaces
	 * around the content shape are removed
	 * 
	 * @param handle
	 *            a svg handle
	 */
	protected void resizeToContent(final SVGHandle handle) {
		...
		for (int i = 0; i < childNodes.getLength(); i++) {
			...
			if (node instanceof Element
				...
				bounds = handle.getSvgElementsManager().getSensitiveBounds(
						element);
		...
		if (wholeBounds != null) {
			...
			// translating each shape element so that the upper left corner of
			// the
			// bounds matches the origin of the svg file
			AbstractShape shapeModule = null;
			Set<UndoRedoAction> undoRedoActions = new HashSet<UndoRedoAction>();
			...
			for (Element el : modifiedElements) {
				...
				if (shapeModule != null) {
					...
					elts.add(el);
					action = shapeModule.translate(handle, elts,
							translationFactors);
			...
			// resizing the canvas
			final Point2D currentSize = handle.getCanvas()
					.getGeometryCanvasSize();
			...
			// creating the undo/redo action
			action = new UndoRedoAction("", executeRunnable, undoRunnable,
					executeRunnable, new HashSet<Element>());
