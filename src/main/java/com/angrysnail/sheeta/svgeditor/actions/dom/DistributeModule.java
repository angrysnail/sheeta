...
public class DistributeModule extends DomActionsModule {
	...
	protected void doAction(SVGHandle handle, Set<Element> elements, int index,
		...
		Map<Element, Rectangle2D> elementsToBounds = getBounds(handle, elements);

		// getting the translation factors for each element//
		Map<Element, Point2D> translationFactors = new HashMap<Element, Point2D>();

		// getting the union of all the bounds
		Rectangle2D unionRect = union(new HashSet<Rectangle2D>(
				elementsToBounds.values()));
		...
		ElementComparator comparator = new ElementComparator(elementsToBounds,
				index == 0);
		...
		switch (type) {
			...
			case H_DISTRIBUTE :
				...
				double leftSpace = getLeftSpace(elementsToBounds.values(),
						unionRect, true);
					...
					for (Element element : sortedElements) {
						...
						if (i != sortedElements.size() - 1) {
							...
							if (i != 0) {
								...
								translationFactors.put(
										element,
										new Point2D.Double(currentOffset
												- (bounds.getX() - unionRect
														.getX()), 0));
							}
			...
			case V_DISTRIBUTE :
				...
				// computing the left space between the rectangles
				leftSpace = getLeftSpace(elementsToBounds.values(), unionRect,
						false);
					...
					for (Element element : sortedElements) {
						...
						if (i != sortedElements.size() - 1) {
							...
							if (i != 0) {
								...
								translationFactors.put(
										element,
										new Point2D.Double(0, currentOffset
												- (bounds.getY() - unionRect
														.getY())));
							}
	...
	/**
	 * returns the space that is unused between the elements denoted by the
	 * provided bounds
	 * 
	 * @param elementsBounds
	 *            the collection of the bounds of elements
	 * @param unionRect
	 *            the union of all the provided bounds
	 * @param isHorizontal
	 *            whether the computed left space should be the one along the
	 *            horizontal axis
	 * @return the space that is unused between the elements denoted by the
	 *         provided bounds
	 */
	protected double getLeftSpace(Collection<Rectangle2D> elementsBounds,
		...
		// creating the set of the union of the rectangles that intersect one
		// another
		...
		for (Rectangle2D bounds : startSet) {
			...
			for (Rectangle2D theBounds : startSet) {
				...
				if (bounds != theBounds
						&& intersects(bounds, theBounds, isHorizontal)) {
	 ...
	 *            a rectangle
	 * @param isHorizontal
	 *            whether the intersection should be computed horizontally or
	 *            vertically
	 * @return whether the two bounds intersect horizontally or vertically
	 */
	protected boolean intersects(Rectangle2D rect1, Rectangle2D rect2,
