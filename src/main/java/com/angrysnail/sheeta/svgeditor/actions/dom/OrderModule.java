...
public class OrderModule extends DomActionsModule {
	 ...
	 * the constants
	 */
	protected static final int ORDER_TOP = 0, ORDER_UP = 1, ORDER_DOWN = 2,
			ORDER_BOTTOM = 3;
	...
	public OrderModule(Editor editor) {
		...
		int[] types = {ORDER_TOP, ORDER_UP, ORDER_DOWN, ORDER_BOTTOM};
		actionsTypes = types;
	...
	@Override
	protected void doAction(SVGHandle handle, final Set<Element> elements,
		...
		final LinkedList<Element> initialChildNodesList = Toolkit
				.getChildrenElements(parentElement);

		// creating the initial map associating an element to its next element
		final Map<Element, Element> initialNextElementMap = new HashMap<Element, Element>();
		...
		final Map<Element, Element> newNextElementMap = new HashMap<Element, Element>();
		...
		for (Element element : newChildNodesList) {
			...
			if (elements.contains(element)) {
				...
				if (i < newChildNodesList.size() - 1) {
					...
					newNextElementMap
							.put(element, newChildNodesList.get(i + 1));
