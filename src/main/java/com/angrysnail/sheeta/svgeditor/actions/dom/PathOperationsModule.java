package com.angrysnail.sheeta.svgeditor.actions.dom;
...
import com.angrysnail.sheeta.svgeditor.ColorManager;
...
public class PathOperationsModule extends DomActionsModule {
	 ...
	 * the constants
	 */
	protected static final int CONVERT_TO_PATH = 0, UNION = 1, SUBTRACTION = 2,
			INTERSECTION = 3, CLOSE_PATH = 4;
	...
	public PathOperationsModule(Editor editor) {
		...
		int[] types = {CONVERT_TO_PATH, UNION, SUBTRACTION, INTERSECTION,
				CLOSE_PATH};
		actionsTypes = types;
	...
	@Override
	protected void doAction(final SVGHandle handle, Set<Element> elements,
		...
		if (type == CONVERT_TO_PATH) {
			...
			final LinkedList<Element> sortedElementsList = new LinkedList<Element>();
			...
			final Map<Element, Element> elementToNextMap = new HashMap<Element, Element>();
			...
			final LinkedList<Element> reversedElementsList = new LinkedList<Element>(
					sortedElementsList);
			...
			switch (type) {
				...
				case UNION :
					...
					newElement = doUnionAction(handle, parentElement, elements,
							index);
					break;

				case SUBTRACTION :

					newElement = doSubtractionAction(handle, parentElement,
							elements, index);
					break;

				case INTERSECTION :

					newElement = doIntersectionAction(handle, parentElement,
							elements, index);
					break;
			}
			...
			executeRunnable = new Runnable() {
				...
				public void run() {
					...
					handle.getSelection().handleSelection(fnewElement, false,
							true);
				}
			};
	 ...
	 */
	protected void doConvertToPathAction(final SVGHandle handle,
		...
		final Map<Element, Element> newElementsMap = convertPathsToElement(
				handle, elementToPathMap);
		...
		executeRunnable = new Runnable() {
			...
			public void run() {
				...
				handle.getSelection().clearSelection();
				handle.getSelection().handleSelection(newElementsMap.keySet(),
						true, true);
			}
		};
	 ...
	 *            the index of the action
	 */
	protected void doClosePathAction(SVGHandle handle, Set<Element> elements,
			int index) {
		...
		Map<Element, Path> elementToPathManagerMap = getPathManagers(handle,
				elements);

		// creating the initial and new maps assocating an
		// element to its d attribute value
		final Map<Element, String> initialElementToDAttribute = new HashMap<Element, String>();
		final Map<Element, String> newElementToDAttribute = new HashMap<Element, String>();
	 ...
	 * @param elements
	 *            a set of elements used to compute the union
	 * @param index
	 *            the index of the action
	 * @return the new element computing from the union of the provided elements
	 */
	protected Element doUnionAction(SVGHandle handle, Element parentElement,
			Set<Element> elements, int index) {
		...
		if (area != null) {
			...
			newElement = createElement(parentElement, new Path(
					new ExtendedGeneralPath(newPath)));
		}
	 ...
	 *            the parent element of the element to be created
	 * @param elements
	 *            a set of elements used to compute the intersection
	 * @param index
	 *            the index of the action
	 * @return the new element computing from the intersection of the provided
	 *         elements
	 */
	protected Element doSubtractionAction(SVGHandle handle,
			Element parentElement, Set<Element> elements, int index) {

		Element newElement = null;

		// getting the ordered list of the selected items
		LinkedList<Element> orderedSelectedElements = new LinkedList<Element>(
	 ...
	 *         elements
	 */
	protected Element doIntersectionAction(SVGHandle handle,
			Element parentElement, Set<Element> elements, int index) {
	...
	/**
	 * converts the given elements into general paths and returns the map
	 * associating an element to the related general path
	 * 
	 * @param handle
	 *            a svg handle
	 * @param elements
	 ...
	 */
	protected Map<Element, GeneralPath> getPaths(SVGHandle handle,
			Set<Element> elements) {

		Map<Element, GeneralPath> paths = new HashMap<Element, GeneralPath>();
		...
		for (Element element : elements) {
			...
			outline = handle.getSvgElementsManager().getOutline(element);
	...
	/**
	 * converts the given elements into path managers and returns the map
	 * associating an element to the related path manager
	 * 
	 * @param handle
	 *            a svg handle
	 * @param elements
	 ...
	 */
	protected Map<Element, Path> getPathManagers(SVGHandle handle,
			Set<Element> elements) {

		Map<Element, Path> pathManagers = new HashMap<Element, Path>();
	 ...
	 *            a svg handle
	 * @param elementToPath
	 *            an element to its path
	 * @return the map associating a paths element to the element it should be
	 *         inserted before
	 */
	protected Map<Element, Element> convertPathsToElement(SVGHandle handle,
			Map<Element, GeneralPath> elementToPath) {

		// getting the map associating a general path to its element
		Map<GeneralPath, Element> pathToElementMap = new HashMap<GeneralPath, Element>();
		GeneralPath path = null;
		...
		Map<Element, Element> pathElements = new HashMap<Element, Element>();
		...
		for (GeneralPath thePath : pathToElementMap.keySet()) {
			...
			if (element != null) {
				...
				pathElement = createElement(parentElement, new Path(
						new ExtendedGeneralPath(thePath)));
	...
	/**
	 * creating a path element whose properties are defined by the provided path
	 * manager
	 * 
	 * @param parentElement
	 *            the parent element
	 ...
	 */
	protected Element createElement(Element parentElement, Path pathManager) {
