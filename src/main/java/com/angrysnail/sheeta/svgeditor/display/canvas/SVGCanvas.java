...
import com.angrysnail.sheeta.svgeditor.display.canvas.grid.Grid;
import com.angrysnail.sheeta.svgeditor.display.canvas.zoom.Zoom;
...
public class SVGCanvas extends JPanel {
	 ...
	 */
	protected static int[] layers = {GRID_LAYER, BOTTOM_LAYER, SELECTION_LAYER,
	...
	public void newDocument(String width, String height) {
		...
		svgRoot.setAttribute("viewBox",
				"0 0 " + EditorToolkit.getPixelledNumber(width) + " "
		...
		try {
			...
			if (doc != null) {
				...
				Dimension scaledCanvasSize = getScaledCanvasSize(doc
						.getDocumentElement());
				svgHandle.getSvgDOMNormalizer()
						.normalize(doc, scaledCanvasSize);
				Toolkit.checkRtdaXmlns(doc);
	 ...
	 *            the areas to redraw
	 */
	protected void refreshCanvasContent(boolean repaintSVGContent,
				...
				if (usedRectangle != null
					...
					if (scrollY > 0) {
						...
						tmpImage = tempCanvasOffscreenImage.getSubimage(0, 0,
								...
								renderedRectangle.height + scrollY);
						g2.drawImage(tmpImage, 0, -scrollY,
								tmpImage.getWidth(), tmpImage.getHeight(), null);
					}
					...
					if (scrollX > 0) {
						...
						tmpImage = tempCanvasOffscreenImage.getSubimage(0, 0,
								...
								renderedRectangle.height);
						g2.drawImage(tmpImage, -scrollX, 0,
								tmpImage.getWidth(), tmpImage.getHeight(), null);
					}
				...
				if (isScrollAction) {
					...
					if (baseScrollY > 0) {
						...
						svgRectangle = new Rectangle2D.Double(
								baseRectangle.getX(), baseRectangle.getY()
					...
					if (baseScrollX > 0) {
						...
						svgRectangle = new Rectangle2D.Double(
								baseRectangle.getX() + basedImageSize.getX()
										- baseScrollX, baseRectangle.getY(),
	...
	protected void handleRepaint(Rectangle2D clipZone) {
		...
		if (repaintZone != null) {
			...
			Rectangle2D rZone = new Rectangle2D.Double(repaintZone.getX(),
					repaintZone.getY(), repaintZone.getWidth(),
		...
		if (clip == null) {
			...
			repaintManager.addDirtyRegion(this, clip.getBounds().x,
					clip.getBounds().y, clip.getBounds().width,
