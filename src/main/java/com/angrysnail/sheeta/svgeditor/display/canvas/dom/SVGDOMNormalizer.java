package com.angrysnail.sheeta.svgeditor.display.canvas.dom;
...
import com.angrysnail.sheeta.library.geom.PolyShape2D;
import com.angrysnail.sheeta.library.geom.Polygon2D;
import com.angrysnail.sheeta.library.geom.Polyline2D;
...
public class SVGDOMNormalizer {
	...
	private static final String styleAttributeName = "style", gTagName = "g",
			textNodeName = "#text", textTagName = "text",
			tspanTagName = "tspan",
			/* viewBoxAtt="viewBox", */defsTagName = "defs",
			styleAtt = "style", stopOpacityAtt = "stop-opacity",
			stopColorAtt = "stop-color", lineTagName = "line",
			polygonTagName = "polygon", polylineTagName = "polyline",
			x1Att = "x1", x2Att = "x2", y1Att = "y1", y2Att = "y2",
			pointsAtt = "points", pathTagName = "path", dAtt = "d",
			transformAtt = "transform", circleTagName = "circle",
			ellipseTagName = "ellipse", rxAtt = "rx", ryAtt = "ry", rAtt = "r";
	...
	public void normalize(Document doc, Dimension scaledSize) {
		...
		if (doc != null) {
			...
			if (root != null) {
				...
				// computing the defs element, by merging all the existing defs
				// elements
				computeDefs(doc);

				// getting the map associating the id of a resource to the
				// resource node
				Map<String, Element> resources = handle
						.getSvgResourcesManager().getResourcesFromDefs(doc,
								resourceNames);
	 ...
	 * 
	 * @param resources
	 *            the map associating the id of a resource to the resource node
	 * @param element
	 *            an element
	 */
	protected void handleResources(Element element,
		...
		if (style != null && !style.equals("")) {
			...
			// for each resource id, checks if it is contained in the style
			// attribute
			LinkedList<Element> nodesList = null;

			for (String id : resources.keySet()) {

				nodesList = handle.getSvgResourcesManager().getUsedResources()
						.get(id);
				...
				if (id != null && !id.equals("")
					...
					if (nodesList == null) {
						...
						handle.getSvgResourcesManager().getUsedResources()
								.put(id, nodesList);
					}
			...
			if (defsList.size() > 1) {
				...
				for (Element aDefs : defsList) {
					...
					while (aDefs.hasChildNodes()) {
						...
						defs.appendChild(aDefs.removeChild(aDefs
								.getFirstChild()));
					}
				}
	 ...
	 * 
	 * @param node
	 *            the node on which the changes will be made
	 */
	...
	/**
	 * removes the attributes specified in the properties.xml and adds them to
	...
	protected void translateFromAttributesToStyle(Node node) {
		...
		if (node != null && node instanceof Element
			...
			// the list of the attributes to be removed and added to the style
			// attribute
			HashSet<String> styleProperties = Editor.getEditor()
					.getResourcesManager().getAttributesToTranslate();
			...
			for (int i = 0; i < attributes.getLength(); i++) {
				...
				if (styleProperties.contains(att.getNodeName())) {
					...
					if (!value.equals("") && style.indexOf(name + ":") == -1) {
						...
						// if the attribute is not in the style value, it is
						// added to the style
						// value
			...
			// removes the attributes that have been added to the style
			// attribute
	...
	protected boolean shouldBeConverted(Element element) {
		...
		return tagName.equals(lineTagName) || tagName.equals(polygonTagName)
				|| tagName.equals(polylineTagName)
				|| tagName.equals(circleTagName);
	}

	/**
	 * converts the provided element to another one that is supported by the
	 * application and modifying the matrix transform of its children
	 * 
	 * @param element
	 *            an element
	 */
	protected void convertElements(Element element) {
		...
		if (tagName.equals(lineTagName) || tagName.equals(polygonTagName)
				|| tagName.equals(polylineTagName)) {
			...
			if (tagName.equals(lineTagName)) {
				...
				try {
					PolyShape2D polygon = new Polygon2D(
							PolyShape2D.getCoordinates(element
									.getAttribute(pointsAtt)));
			...
			if (path != null) {
				...
				Element pathElement = doc.createElementNS(doc
						.getDocumentElement().getNamespaceURI(), pathTagName);
				...
				// appending all the child nodes of the element to the path
				// element
			...
			Element ellipseElement = doc.createElementNS(doc
					.getDocumentElement().getNamespaceURI(), ellipseTagName);
	...
	/**
	 * normalizes a group element, setting the transform of this node to
	 * identity and modifying the matrix transform of its children
	 * 
	 * @param g
	 *            a group element
	 */
	...
	protected String getText(Node node) {
		...
		if (node != null
				&& (node.getNodeName().equals(textTagName) || node
						.getNodeName().equals(tspanTagName))) {
