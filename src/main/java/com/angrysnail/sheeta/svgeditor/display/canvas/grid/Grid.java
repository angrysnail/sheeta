		...
		CanvasPainter paintListener = new CanvasPainter() {
			...
			public void paintToBeDone(Graphics2D g) {
				...
				if (handler.isGridEnabled()) {
					...
					Rectangle canvasBounds = canvas.getScrollPane()
							.getCanvasBounds();
					Rectangle viewportRectangle = canvas.getScrollPane()
							.getViewPortBounds();

					Rectangle2D scaledCanvasBounds = canvas
							.getSVGHandle()
							.getTransformsManager()
							.getScaledRectangle(
									new Rectangle2D.Double(0, 0,
											canvasBounds.width,
											canvasBounds.height), true);
					Rectangle2D innerRectangle = canvas
							.getSVGHandle()
							.getTransformsManager()
							.getScaledRectangle(
									new Rectangle2D.Double(-canvasBounds.x,
											-canvasBounds.y,
											viewportRectangle.width,
											viewportRectangle.height), true);
					...
					double startx = (Math.floor(resultRect.x
							/ horizontalDistance) + 1)
							* horizontalDistance;
					double starty = (Math
							.floor(resultRect.y / verticalDistance) + 1)
							* verticalDistance;

					if (clip != null) {

						for (double i = startx; i < resultRect.x
								+ resultRect.width; i += horizontalDistance) {

							point = canvas
									.getSVGHandle()
									.getTransformsManager()
									.getScaledPoint(new Point2D.Double(i, 0),
											false);

							if (point.getX() >= clip.x
									&& point.getX() <= (clip.x + clip.width)) {

								g2.drawLine((int) point.getX(), 0,
										(int) point.getX(), canvasBounds.height);
							}
						}

						for (double i = starty; i < resultRect.y
								+ resultRect.height; i += verticalDistance) {

							point = canvas
									.getSVGHandle()
									.getTransformsManager()
									.getScaledPoint(new Point2D.Double(0, i),
											false);

							if (point.getY() >= clip.y
									&& point.getY() <= (clip.y + clip.height)) {

								g2.drawLine(0, (int) point.getY(),
										canvasBounds.width, (int) point.getY());
							}
						}
