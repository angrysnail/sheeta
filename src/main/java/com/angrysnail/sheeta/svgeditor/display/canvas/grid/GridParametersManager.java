...
public class GridParametersManager {
	...
	protected static final BasicStroke defaultGridStroke = new BasicStroke(1,
			BasicStroke.CAP_BUTT, BasicStroke.JOIN_BEVEL, 0, new float[]{2, 3},
			0);
	...
	protected void initializeParameters() {
		...
		try {
			gridEnabled = Boolean.parseBoolean(PreferencesStore.getPreference(
					null, GRID_ENABLED_PREF_ID));
		} catch (Exception ex) {
	...
	protected void updatePreferences() {
		...
		PreferencesStore.setPreference(null, GRID_ENABLED_PREF_ID,
				Boolean.toString(gridEnabled));
		...
		PreferencesStore.setPreference(null, COLOR_PREF_ID, Editor
				.getColorChooser().getColorString(gridColor));
