package com.angrysnail.sheeta.svgeditor.display.canvas.rulers;
...
import com.angrysnail.sheeta.svgeditor.display.canvas.SVGScrollPane;
...
public class Ruler extends JPanel {
	...
	public void refreshRange() {
		...
		double scale = scrollpane.getSVGCanvas().getZoomManager()
				.getCurrentScale();
	...
	protected void drawCursor(Graphics g) {
		...
		if (isHorizontal) {
			...
			cursor = cursorShape.createTransformedShape(AffineTransform
					.getTranslateInstance(currentPosition, getHeight()
							- cursorShape.getBounds().height));

		} else {

			cursor = cursorShape.createTransformedShape(AffineTransform
					.getTranslateInstance(getWidth()
							- cursorShape.getBounds().width, currentPosition));
		}
	...
	protected void drawRuler(Graphics2D g) {
		...
		if (range.getRanges().size() > 0) {
			...
			if (canvasLength > viewPortLength) {
				...
				startRangePoint = Math.floor(startPoint / smallestRange)
						* smallestRange;
				...
				if (isHorizontal) {
					...
					while (cur <= endPoint) {
						...
						scrollPanePosition = (startRangePoint + cur) * scale
								- viewPortOrigin;

						// checking if the item that will be painted belongs to
						// the clip rectangle
						if (scrollPanePosition >= clip.x
								&& scrollPanePosition <= (clip.x + clip.width)) {
					...
					while (cur <= endPoint) {
						...
						// checking if the item that will be painted belongs to
						// the clip rectangle
						if (scrollPanePosition >= clip.y
								&& scrollPanePosition <= (clip.y + clip.height)) {
	 ...
	 *            the graphics
	 * @param canvasPos
	 *            the position of the item in the unscaled canvas coordinates
	 * @param scrollpanePos
	 *            the position of the item in the scrollpane coordinates
	 */
	protected void paintItem(Graphics2D g, double canvasPos,
			double scrollpanePos) {

		double scale = scrollpane.getSVGCanvas().getZoomManager()
				.getCurrentScale();

		// determining which range should be used for this item and then paints
		// the item
		if (range.getRanges().size() == 3) {

			if (canvasPos % range.getRanges().getFirst() == 0) {

				paintRawItem(g, canvasPos, scrollpanePos, 0, range.getRanges()
						.getFirst() * scale);

			} else if (canvasPos % range.getRanges().get(1) == 0) {

				paintRawItem(g, canvasPos, scrollpanePos, 1, range.getRanges()
						.get(1) * scale);

			} else {

				paintRawItem(g, canvasPos, scrollpanePos, 2, range.getRanges()
						.getLast() * scale);
			}
	 ...
	 *            the graphics
	 * @param canvasPos
	 *            the position of the item in the canvas
	 * @param pos
	 *            the position of the item
	 * @param index
	 *            the kind of representation for this item
	 * @param availableSize
	 *            the available size until the next item of the same index
	 */
	protected void paintRawItem(Graphics2D g, double canvasPos, double pos,
			int index, double availableSize) {
		...
		switch (index) {
			...
			case 0 :
				...
				if (isHorizontal) {
					...
					g.draw(new Line2D.Double(pos, getHeight() - 8, pos,
							getHeight()));
					...
					// drawing the strings
					char[] tab = new String((int) (canvasPos) + "")
							.toCharArray();
					...
					g.draw(new Line2D.Double(getWidth() - 8, pos, getWidth(),
							pos));
				}
			...
			case 1 :
				...
				if (isHorizontal) {
					...
					g.draw(new Line2D.Double(pos, getHeight() - 6, pos,
							getHeight()));

				} else {

					g.draw(new Line2D.Double(getWidth() - 6, pos, getWidth(),
							pos));
				}
			...
			case 2 :
				...
				if (isHorizontal) {
					...
					g.draw(new Line2D.Double(pos, getHeight() - 4, pos,
							getHeight()));

				} else {

					g.draw(new Line2D.Double(getWidth() - 4, pos, getWidth(),
							pos));
				}
	...
	protected class Range {
		...
		protected Range(double distance, double scale) {
			...
			while (true) {
				...
				// checking if the middle of the available space can be a part
				// of the ranges
			...
			// handling the list of the ranges so that only the last three
			// ranges remain in it
			if (ranges.size() > 3) {

				ranges = new LinkedList<Double>(ranges.subList(
						ranges.size() - 3, ranges.size()));
			}
		}
		...
		/**
		 * computes and returns the closest number from the given number that
		 * belongs to a range
		 * 
		 * @param pos
		 *            a number
		 * @return the closest number from the given number that belongs to a
		 *         range
		 */
	...
	/**
	 * the class of the listener to the mouse move and drag actions on the svg
	 * canvas
	 * 
	 * @author ITRIS, Jordi SUC
	 */
		...
		protected void handleEvent(Point point) {
			...
			if (isHorizontal) {
				...
				rect = new Rectangle(newPosition - cursorLength / 2,
						getHeight() - cursorLength / 2, cursorLength,
						cursorLength / 2);

				// computing the old bounds of the cursor and merging them to
				// the previously
				// computed ones
				...
				// computing the new bounds of the cursor
				rect = new Rectangle(getWidth() - cursorLength / 2, newPosition
						- cursorLength / 2, cursorLength / 2, cursorLength);

				// computing the old bounds of the cursor and merging them to
				// the previously
				// computed ones
				rect.add(new Rectangle(getWidth() - cursorLength / 2,
						oldPosition - cursorLength / 2, cursorLength / 2,
						cursorLength));
			}
