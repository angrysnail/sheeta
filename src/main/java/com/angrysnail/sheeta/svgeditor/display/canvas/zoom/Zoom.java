...
public class Zoom {
	 ...
	 */
	public static final double[] defaultZoomFactors = {0.05, 0.1, 0.2, 0.5,
			0.75, 1, 2, 5, 7.5, 10, 20};
	...
	public void scaleTo(double scale) {
		...
		Dimension newScrollValues = new Dimension(
				(int) Math.round(newCenterPoint.getX()
						- viewPortBounds.getWidth() / 2),
	...
	public void fitImageToViewport() {
		...
		if (canvasSize != null) {
			...
			Rectangle2D imageBounds = new Rectangle2D.Double(0, 0,
					canvasSize.getX(), canvasSize.getY());
		...
		for (Element element : selectedNodes) {
			...
			if (element != null) {
				...
				if (wholeBounds == null) {
					...
					wholeBounds = new Rectangle2D.Double(bounds.getX(),
