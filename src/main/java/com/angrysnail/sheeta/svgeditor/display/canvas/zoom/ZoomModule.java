package com.angrysnail.sheeta.svgeditor.display.canvas.zoom;
...
public class ZoomModule extends ModuleAdapter {
	 ...
	 */
	protected String[] ids = {"ZoomIn", "ZoomOut", "ZoneZoom",
			"FitImageToViewport", "FitContentToViewport",
	...
	protected void createMenuAndToolItems() {
		...
		for (int i = 0; i < ids.length; i++) {
			...
			// getting the labels
			itemLabels[i] = ResourcesManager.bundle.getString(ids[i]
					+ "ItemLabel");
			...
			listener = new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					if (handle != null) {
						...
						handle.getCanvas().getZoomManager()
								.scaleTo(Zoom.defaultZoomFactors[index]);
					}
				}
			};
		...
		if (handle != null) {
			...
			switch (index) {
				...
				case 2 :
					...
					Editor.getEditor()
							.getSelectionManager()
							.setSelectionMode(SelectionInfoManager.ZOOM_MODE,
									null);
					break;
