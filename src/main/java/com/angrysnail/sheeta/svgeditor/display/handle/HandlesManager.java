...
import com.angrysnail.sheeta.svgeditor.display.canvas.grid.GridParametersManager;
...
public class HandlesManager {
		...
		Runnable runnable = new Runnable() {
			...
			public void run() {
				...
				if (currentHandle != null) {
					...
					currentHandle.getSVGFrame().getFrameMenuItem()
							.setSelected(true);
				}
			}
		};
	...
	protected void addHandleInMenu(SVGHandle handle) {
		...
		handle.getSVGFrame().getFrameMenuItem()
		...
		if (menubar != null) {
			...
			if (handleFigure < 10) {
				...
				handle.getSVGFrame()
						.getFrameMenuItem()
						.setAccelerator(
								KeyStroke.getKeyStroke("ctrl NUMPAD"
										+ handleFigure));
				handleFigure++;
			}
	...
	public void removeHandle(String name) {
		...
		if (handle != null) {// TODO
			...
			if (editor.getClipboardManager() != null
					...
					&& editor.getClipboardManager().getSourceHandle()
							.equals(handle)) {
