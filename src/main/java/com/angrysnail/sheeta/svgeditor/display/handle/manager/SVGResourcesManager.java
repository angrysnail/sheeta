...
/**
 * the class used to handle the resources and the elements using them
 * 
 * @author Jordi SUC
 */
public class SVGResourcesManager {
	...
	/**
	 * the map associating the id of a resource to the list of the nodes using
	 * this resource
	 */
	private HashMap<String, LinkedList<Element>> usedResources = new HashMap<String, LinkedList<Element>>();
	...
	/**
	 * adds the given id of a resource and its associated node to the map of the
	 * nodes using a resource
	 * 
	 * @param resourceId
	 *            the id of a resource
	 * @param node
	 *            the node using the given resource
	 */
	public void addNodeUsingResource(String resourceId, Node node) {
		...
		if (resourceId != null && !resourceId.equals("") && node != null) {
			...
			if (nodesList == null) {
				...
				// if the id was not contained in the map, creates a new list of
				// nodes
	 ...
	 *            the id of a resource
	 * @param list
	 *            the nodes using the given resource
	 */
	public void addNodesUsingResource(String resourceId,
	...
	/**
	 * removes the given id of a resource and its associated node to the map of
	 * the nodes using a resource
	 * 
	 * @param resourceId
	 *            the id of a resource
	 * @param node
	 ...
	 *            the nodes using the given resource
	 */
	public void removeNodesUsingResource(String resourceId,
			LinkedList<Element> list) {
	 ...
	 * 
	 * @param newId
	 *            the new id for the resource
	 * @param lastId
	 *            the previous id for the resource
	 */
	public void modifyResourceId(String newId, String lastId) {

		if (newId != null && !newId.equals("") && lastId != null
				&& !lastId.equals("") && !lastId.equals(newId)
				&& usedResources.containsKey(lastId)) {
			...
			if (nodesList != null) {
				...
				// for each node, replaces the last id of the resource with the
				// new id
	...
	/**
	 * checks if the given node is using a resource and registers it on the
	 * canvas if this is such a case
	 * 
	 * @param node
	 *            a node
	 */
	public void registerUsedResource(Node node) {
		...
		if (node != null && node instanceof Element) {
			...
			// registers all the used resources that could be found in the
			// children nodes of this
			// node
	...
	/**
	 * checks if the given node is using a resource and unregisters it in the
	 * canvas if this is such a case
	 * 
	 * @param node
	 *            a node
	...
	public void unregisterAllUsedResource(Node node) {
		...
		if (node != null && node instanceof Element) {
			...
			// unregisters all the used resources that could be found in the
			// children nodes of this
			// node
			for (Node cur = node.getFirstChild(); cur != null; cur = cur
					.getNextSibling()) {
	 ...
	 * @param doc
	 *            the document
	 * @param resourceTagNames
	 *            the list of the tag names of the resources that should appear
	 *            in the returned list
	 * @return the map associating an id to a resource node
	 */
	public HashMap<String, Element> getResourcesFromDefs(Document doc,
			LinkedList<String> resourceTagNames) {

		HashMap<String, Element> idResources = new HashMap<String, Element>();

		// getting the list of the resources that can be found in the defs
		// elements
		if (doc != null && resourceTagNames != null
				&& resourceTagNames.size() > 0) {

			// the list of the available defs nodes that can be found in the
			// document
			...
			if (defsNodes != null && defsNodes.getLength() > 0) {
				...
				for (int i = 0; i < defsNodes.getLength(); i++) {
					...
					if (defs != null) {
						...
						// for each child of the "defs" element, adds its id to
						// the map
						for (cur = defs.getFirstChild(); cur != null; cur = cur
								.getNextSibling()) {

							if (cur instanceof Element
									&& resourceTagNames.contains(cur
											.getNodeName())) {
	...
	public Element getDefsElement() {
		...
		NodeList defsNodes = handle.getCanvas().getDocument()
	...
	/**
	 * @return the map associating an id to the linkedlist of the elements
	 *         associated with it
	 */
