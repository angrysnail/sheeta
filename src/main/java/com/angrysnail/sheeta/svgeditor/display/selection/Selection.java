...
public class Selection {
	 ...
	 */
	private SelectionInfoManager selectionManager = Editor.getEditor()
			.getSelectionManager();
	...
	public void setSelectionSubMode(int selectionSubMode) {
		...
		if (Editor.getEditor().getSelectionManager().getSelectionMode() == SelectionInfoManager.ITEMS_ACTION_MODE) {

			// refreshing the selection
			selectionLevel = SELECTION_LEVEL_ITEMS_ACTION;
			refreshSelection(true);

		} else if (Editor.getEditor().getSelectionManager().getSelectionMode() == SelectionInfoManager.DRAWING_MODE) {
			...
			if (current instanceof Element) {
				...
				if (EditorToolkit.isElementAShape(element)) {
					...
					if (r2 != null) {
						...
						Rectangle2D r3 = new Rectangle2D.Double(r2.getX(),
								r2.getY(), r2.getWidth() + 1,
								r2.getHeight() + 1);
	...
	public void drawingAction(Point2D point, int modifier, int type) {
		...
		com.angrysnail.sheeta.svgeditor.shape.AbstractShape drawingShape = selectionManager
				.getDrawingShape();
	...
	public void handleZoomZone(Point2D currentPoint, int type) {
		...
		switch (type) {
			...
			case SELECTION_ZONE_MOUSE_RELEASED :
				...
				// clicked points
				Rectangle2D rect = EditorToolkit.getComputedRectangle(
						firstPoint, currentPoint);
				rect = svgHandle.getTransformsManager().getScaledRectangle(
						rect, true);
				...
				firstPoint = null;
				canvas.removePaintListener(ghostSelectionZoneCanvasPainter1,
						true);
				canvas.removePaintListener(ghostSelectionZoneCanvasPainter2,
						true);
				canvas.removePaintListener(ghostSelectionZoneCanvasPainter3,
						true);
				canvas.removePaintListener(ghostSelectionZoneCanvasPainter4,
						true);
				...
				if (!Editor.getEditor().getRemanentModeManager()
						.isRemanentMode()) {
			...
			case SELECTION_ZONE_MOUSE_DRAGGED :
				...
				rect = EditorToolkit.getComputedRectangle(firstPoint,
						currentPoint);
				...
				ghostSelectionZoneCanvasPainter1.setPoints(new Point2D.Double(
						rect.getX(), rect.getY()),
		...
		if (!actionDisabled && !isSelectionLocked()) {
			...
			if (firstPoint == null) {
				...
				switch (selectionSubMode) {
					...
					case TRANSLATION_SUB_MODE :
						...
						actionSelectionManager = (translationElements.size() > 1
			...
			switch (selectionSubMode) {
				...
				case TRANSLATION_SUB_MODE :
					...
					actionSelectionManager.doTranslateAction(
				...
				switch (selectionSubMode) {
					...
					case ACTION_SUB_MODE :
						...
						actionSelectionManager.validateAction(
								...
								firstPoint, scaledPoint, actionItem);
						break;
				}
			}
	...
	public void handleCursor(Point currentMousePoint) {
		...
		if (currentItem != null) {
			...
			svgHandle.getScrollPane().getSVGCanvas()
					.setSVGCursor(currentItem.getCursor());
			svgHandle.getScrollPane().getViewport()
					.setCursor(currentItem.getCursor());

		} else {

			int cursorType = Cursor.DEFAULT_CURSOR;
			int selectionMode = Editor.getEditor().getSelectionManager()
					.getSelectionMode();
	 ...
	 * @author ITRIS, Jordi SUC
	 */
	protected static class GhostSelectionZoneCanvasPainter
			extends
				CanvasPainter {
