...
public class SelectionListener extends MouseAdapter
	...
	public void mousePressed(MouseEvent evt) {
		...
		if (selectionMode != SelectionInfoManager.NONE_MODE && !isPopUp(evt)) {
			...
			Editor.getEditor().getSVGModuleLoader().getPopupManager()
					.showPopup(selection.getSVGHandle(), evt.getPoint());
		}
	}
				...
				case SelectionInfoManager.DRAWING_MODE :
					...
					if (lastMouseReleasedActionTime != 0
						...
						Rectangle2D oldZone = new Rectangle2D.Double(
								...
								zoneForDoubleClick.getX(),
								zoneForDoubleClick.getY());

						if (deltaTime < doubleClickThreshold
								&& oldZone
										.contains(lastMouseReleasedActionPoint)) {
			...
			switch (selectionMode) {
				...
				case SelectionInfoManager.REGULAR_MODE :
					...
					if (!evt.isShiftDown()) {
						...
						selection.doAction(dragStarted
								? point
								: initialDragPoint);
					}
				...
				case SelectionInfoManager.ZONE_MODE :
					...
					boolean isMultiSelectionEnabled = isMultiSelectionEnabled(evt);
					selection.handleSelectionZone(dragStarted
							? point
							: initialDragPoint,
				...
				case SelectionInfoManager.ZOOM_MODE :
					...
					selection.handleZoomZone(dragStarted
							? point
							: initialDragPoint,
							Selection.SELECTION_ZONE_MOUSE_DRAGGED);
					break;
				...
				case SelectionInfoManager.DRAWING_MODE :
					...
					selection.drawingAction(dragStarted
							? point
							: initialDragPoint, evt.getModifiers(),
