...
import com.angrysnail.sheeta.svgeditor.shape.MultiAbstractShape;
...
public abstract class SelectionManager {
	 ...
	 * 
	 * @param elements
	 *            the set of the elements that are handled
	 * @param firstPoint
	 *            the first clicked point
	 * @param currentPoint
	 *            the current position of the mouse
	 * @param item
	 *            the selection item for this action
	 */
	public abstract void doAction(Set<Element> elements, Point2D firstPoint,
			Point2D currentPoint, SelectionItem item);

	/**
	 * validates an action according to the selection item that was selected by
	 * the user and the point where the mouse can be found
	 * 
	 * @param elements
	 *            the set of the elements that are handled
	 * @param firstPoint
	 ...
	 *            the selection item for this action
	 */
	public abstract void validateAction(Set<Element> elements,
	 ...
	 *            the current position of the mouse
	 */
	public abstract void doTranslateAction(Set<Element> elements,
	 ...
	 *            the current position of the mouse
	 */
	public abstract void validateTranslateAction(Set<Element> elements,
			Point2D firstPoint, Point2D currentPoint);
	 ...
	 */
	public Set<SelectionItem> getLockedSelectionItems(SVGHandle handle,
		...
		Rectangle2D wholeBounds = MultiAbstractShape.getElementsBounds(handle,
				elements);

		// scaling the bounds in the canvas space
		Rectangle2D area = handle.getTransformsManager().getScaledRectangle(
				wholeBounds, false);
		...
		SelectionItem item = new SelectionItem(handle, elements, new Point(
				...
				SelectionItem.LOCKED_POINT, SelectionItem.POINT_STYLE, 0,
				false, null);
		items.add(item);

		items.add(new SelectionItem(handle, elements, new Point((int) area
				.getX(), (int) area.getMaxY()), SelectionItem.LOCKED_POINT,
				SelectionItem.POINT_STYLE, 0, false, null));

		items.add(new SelectionItem(handle, elements, new Point((int) area
				.getMaxX(), (int) area.getMaxY()), SelectionItem.LOCKED_POINT,
				SelectionItem.POINT_STYLE, 0, false, null));

		items.add(new SelectionItem(handle, elements, new Point((int) area
				.getMaxX(), (int) area.getY()), SelectionItem.LOCKED_POINT,
				SelectionItem.POINT_STYLE, 0, false, null));
