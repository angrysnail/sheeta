...
Contact : jordi.suc@itris.fr; philippe.gil@itris.fr
 ...
 =============================================
 */
package com.angrysnail.sheeta.svgeditor.display.undoredo;
...
/**
 * @author ITRIS, Jordi SUC the class that manages the undo/redo stack that
 *         contains the actions to call when undoing or redoing
 */
public class UndoRedo extends ModuleAdapter {
	 ...
	 * 
	 * @param actionList
	 *            the actionList that describes what has to be done when calling
	 *            the undo or redo action
	 * @param refreshSelection
	 *            whether the selection should be refreshed
	 */
	public void addActionList(final UndoRedoActionList actionList,
			boolean refreshSelection) {

		// checking if the undo stack is full, if true, removes the first added
		// list
		...
		// getting the list of all the elements that are handled in the action
		// list
		...
		svgHandle.enqueue(executeRunnable, allElements,
				actionList.isInvokeInAWTThread());
	...
	public void redoLastAction() {
		...
		if (redoStack.size() > 0) {
			...
			// getting the lastly entered action list, and removes it from the
			// stack
			...
			// getting the list of all the elements that are handled in the
			// action list
			Set<Element> allElements = new HashSet<Element>();
	...
	/**
	 * @return the label of the last entered undo action list, or null if no
	 *         action is available
	 */
	...
	/**
	 * @return the label of the last entered redo action list, or null if no
	 *         action is available
	 */
