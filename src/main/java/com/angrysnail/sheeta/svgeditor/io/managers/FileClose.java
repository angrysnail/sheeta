...
public class FileClose {
	...
	public FileClose(IOManager ioManager) {
		...
		warningTitle = ResourcesManager.bundle
				.getString("FileCloseWarningTitle");
		warningMessage = ResourcesManager.bundle
				.getString("FileCloseWarningMessage");
	}

	/**
	 * closes the provided handle, some verifications are done before closing so
	 * that no modifications are lost
	 * 
	 * @param handle
	 *            a svg handle
	 * @param relativeComponent
	 *            the component relatively to which the save dialog will be
	 *            shown
	 */
	public void closeHandle(SVGHandle handle, JComponent relativeComponent) {
		...
		if (handle.isModified()) {
			...
			// checking whether the file should be saved, or the close action
			// cancelled
			int returnValue = JOptionPane.showConfirmDialog(Editor.getParent(),
					warningMessage, warningTitle,
					JOptionPane.YES_NO_CANCEL_OPTION);

			if (returnValue == JOptionPane.YES_OPTION) {

				boolean saveActionLaunched = ioManager.getFileSaveManager()
	...
	public void closeAll() {
		...
		Collection<SVGHandle> handles = Editor.getEditor().getHandlesManager()
				.getHandles();
	...
	public void close(SVGHandle handle) {
		...
		Editor.getEditor().getHandlesManager().removeHandle(handle.getName());
	}
}
