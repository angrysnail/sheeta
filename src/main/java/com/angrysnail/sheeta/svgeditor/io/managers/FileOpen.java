...
import com.angrysnail.sheeta.svgeditor.io.managers.dialog.FileChooserDialog;
import com.angrysnail.sheeta.svgeditor.io.managers.dialog.SVGFileFilter;
...
public class FileOpen {
	...
	public FileOpen(IOManager ioManager) {
		...
		if (Editor.getParent() instanceof Frame) {
			...
			fileChooserDialog = new FileChooserDialog(
					...
					FileChooserDialog.OPEN_FILE_MODE);
	...
	public void askUserForFile(JComponent relativeComponent) {
		...
		if (selectedFiles != null && selectedFiles.length > 0) {
			...
			for (File file : selectedFiles) {
				...
				monitor = new OpenMonitor(file, Editor.getParent(),
						relativeComponent, 0, 100);
		...
		if (file != null && file.exists()) {
			...
			if (handle != null) {
				...
				Editor.getEditor().getResourcesManager()
						.addRecentFile(file.toURI().toASCIIString());
			...
			JOptionPane.showMessageDialog(Editor.getParent(), message,
	...
	public LinkedList<File> getRecentFiles() {
		...
		// files
		Collection<String> filePaths = Editor.getEditor().getResourcesManager()
				.getRecentFiles();
