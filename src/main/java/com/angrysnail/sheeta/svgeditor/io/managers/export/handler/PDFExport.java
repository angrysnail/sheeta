...
import com.angrysnail.sheeta.svgeditor.io.managers.export.handler.dialog.PDFExportDialog;
import com.angrysnail.sheeta.svgeditor.io.managers.export.image.SVGDocumentImageCreator;
...
public class PDFExport extends Export {
	 ...
	 */
	protected void writeImage(final org.w3c.dom.Document document,
			final File destFile) {
		...
		Thread thread = new Thread() {
			...
			public void run() {
				...
				try {
					...
					ctx = new BridgeContext(userAgent, null,
					...
					if (gvt != null) {
						...
						final com.itextpdf.text.Document pdfDoc = new com.itextpdf.text.Document(
								rect, margins.left, margins.right, margins.top,
								margins.bottom) {
							@Override
							public void open() {
						...
						final PdfTemplate tp = cb.createTemplate(
								rect.getWidth(), rect.getHeight());
						tp.setWidth(rect.getWidth());
						tp.setHeight(rect.getHeight());

						Graphics2D g2 = tp.createGraphics(rect.getWidth(),
								rect.getHeight(), new DefaultFontMapper());
