...
/**
 * the class of the dialog used to choose the parameters of the pdf export
 * action
 * 
...
public class PDFExportDialog extends ExportDialog {
	 ...
	 */
	protected String title = "", author = "", subject = "", keywords = "",
			creator = "";
	...
	/**
	 * @return the panel containing the widgets used to modify the parameters of
	 *         the page
	 */
	protected JPanel getPageParametersPanel() {
		...
		// creating the spinners that will be used to choose the custom size
		// chooser
		final JSpinner widthSpinner = new JSpinner(), heightSpinner = new JSpinner();

		SpinnerNumberModel widthSpinnerModel = new SpinnerNumberModel(0, 0,
				Double.MAX_VALUE, 1);
		SpinnerNumberModel heightSpinnerModel = new SpinnerNumberModel(0, 0,
				Double.MAX_VALUE, 1);
		...
		widthSpinner.addChangeListener(new ChangeListener() {
			...
			public void stateChanged(ChangeEvent evt) {
				...
				if (pageSize == null) {
					...
					pageSize = new com.itextpdf.text.Rectangle(
							(float) ((Double) widthSpinner.getValue())
									.doubleValue(), 0.0f);
					...
					pageSize = new com.itextpdf.text.Rectangle(
							(float) ((Double) widthSpinner.getValue())
									.doubleValue(), pageSize.getHeight());
				}
			}
		});
		...
		heightSpinner.addChangeListener(new ChangeListener() {
			...
			public void stateChanged(ChangeEvent evt) {
				...
				if (pageSize == null) {
					...
					pageSize = new com.itextpdf.text.Rectangle(0.0f,
							(float) ((Double) heightSpinner.getValue())
									.doubleValue());

				} else {

					pageSize = new com.itextpdf.text.Rectangle(pageSize
							.getWidth(), (float) ((Double) heightSpinner
		...
		// creating the labels
		JLabel lbw = new JLabel(widthLabel.concat(" : ")), lbh = new JLabel(
				heightLabel.concat(" : ")), pxw = new JLabel("px"), pxh = new JLabel(
				"px");
		...
		combo.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent evt) {
				...
				if (combo.getSelectedItem() != null) {
					...
					try {
						pageSize = ((PredefinedPageSizeItem) combo
		...
		// creating the radio buttons used to choose between a predefined and a
		// personnalized size
		// chooser
		final JRadioButton predefinedRadioButton = new JRadioButton(predefLabel), customRadioButton = new JRadioButton(
				customLabel);
		...
		ChangeListener radioButtonsListener = new ChangeListener() {
			...
			public void stateChanged(ChangeEvent evt) {
				...
				if (lastSelectedButton == null
						|| (lastSelectedButton != null && !lastSelectedButton
								.equals(evt.getSource()))) {

					if (evt.getSource().equals(predefinedRadioButton)
							&& predefinedRadioButton.isSelected()) {
						...
						try {
							...
							if (combo.getSelectedItem() != null) {
								...
								combo.setSelectedItem(PredefinedPageSizeItem
										.getItem(PageSize.A4));
							}
					...
					} else if (evt.getSource().equals(customRadioButton)
		...
		initializers.add(new Runnable() {
			...
			public void run() {
				...
				if (pageSize != null) {
					...
					PredefinedPageSizeItem item = PredefinedPageSizeItem
							.getItem(pageSize);
		...
		sizeChooserPanel.setBorder(new CompoundBorder(new TitledBorder(
				exportSizeLabel), new EmptyBorder(0, 0, 4, 0)));
		...
		final JRadioButton portraitRadioButton = new JRadioButton(portraitLabel), landscapeRadioButton = new JRadioButton(
				landscapeLabel);
		...
		final JSpinner topSpinner = new JSpinner(), bottomSpinner = new JSpinner(), leftSpinner = new JSpinner(), rightSpinner = new JSpinner();

		SpinnerNumberModel topSpinnerModel = new SpinnerNumberModel(0, 0,
				Double.MAX_VALUE, 1), bottomSpinnerModel = new SpinnerNumberModel(
				0, 0, Double.MAX_VALUE, 1), leftSpinnerModel = new SpinnerNumberModel(
				0, 0, Double.MAX_VALUE, 1), rightSpinnerModel = new SpinnerNumberModel(
				0, 0, Double.MAX_VALUE, 1);
		...
		((JSpinner.DefaultEditor) topSpinner.getEditor()).getTextField()
				.setColumns(3);
		((JSpinner.DefaultEditor) bottomSpinner.getEditor()).getTextField()
				.setColumns(3);
		((JSpinner.DefaultEditor) leftSpinner.getEditor()).getTextField()
				.setColumns(3);
		((JSpinner.DefaultEditor) rightSpinner.getEditor()).getTextField()
				.setColumns(3);
		...
		ChangeListener changeListener = new ChangeListener() {
			...
			public void stateChanged(ChangeEvent evt) {
				...
				int value = (int) ((Double) widthSpinner.getValue())
						.doubleValue();
		...
		JLabel topLbl = new JLabel(topLabel + " : "), bottomLbl = new JLabel(
				bottomLabel + " : "), leftLbl = new JLabel(leftLabel + " : "), rightLbl = new JLabel(
				rightLabel + " : "), topPx = new JLabel("px"), bottomPx = new JLabel(
				"px"), leftPx = new JLabel("px"), rightPx = new JLabel("px");
	...
	/**
	 * @return the panel containing the widgets for entering information on the
	 *         page
	 */
	protected JPanel getPageInformationPanel() {
		...
		JLabel titleLbl = new JLabel(titleLabel + " : "), authorLbl = new JLabel(
				authorLabel + " : "), subjectLbl = new JLabel(subjectLabel
				+ " : "), keywordsLbl = new JLabel(keywordsLabel + " : "), creatorLbl = new JLabel(
				creatorLabel + " : ");
		...
		final JTextField titleTxt = new JTextField(25), authorTxt = new JTextField(
				25), subjectTxt = new JTextField(25), keywordsTxt = new JTextField(
				25), creatorTxt = new JTextField(25);
	...
	/**
	 * the class of the items of the combo box used for choosing a predefined
	 * size
	 */
	protected static class PredefinedPageSizeItem {
		 ...
		 */
		protected static LinkedHashMap<com.itextpdf.text.Rectangle, String> sizeToLabel = new LinkedHashMap<com.itextpdf.text.Rectangle, String>();

		/**
		 * the labels of the predefined sizes
		 */
		protected static String label_11X17 = "", labelA0 = "", labelA1 = "",
				labelA10 = "", labelA2 = "", labelA3 = "", labelA4 = "",
				labelA5 = "", labelA6 = "", labelA7 = "", labelA8 = "",
				labelA9 = "", labelARCH_A = "", labelARCH_B = "",
				labelARCH_C = "", labelARCH_D = "", labelARCH_E = "",
				labelB0 = "", labelB1 = "", labelB2 = "", labelB3 = "",
				labelB4 = "", labelB5 = "", labelFLSA = "", labelFLSE = "",
				labelHALFLETTER = "", labelLEDGER = "", labelLEGAL = "",
				labelLETTER = "", labelNOTE = "";
		...
		static {
			...
			if (bundle != null) {
				...
				try {
					...
					labelHALFLETTER = bundle
							.getString("labelexportpdfHALFLETTER");
			...
			LinkedList<com.itextpdf.text.Rectangle> sizes = new LinkedList<com.itextpdf.text.Rectangle>(
					sizeToLabel.keySet());

			// creating all the items according to the size objects that can be
			// found in the map
		 ...
		 */
		protected static PredefinedPageSizeItem getItem(
			...
			if (pageSize != null) {
				...
				// getting the item that has got the given page size for its
				// size
				for (int i = 0; i < items.length; i++) {

					if (items[i] != null && items[i].getSize() != null
							&& items[i].getSize().equals(pageSize)) {
