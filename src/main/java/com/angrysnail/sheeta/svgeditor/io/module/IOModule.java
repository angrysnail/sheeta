...
public class IOModule extends ModuleAdapter {
	 ...
	 */
	protected String[] ids = {"FileNew", "FileOpen", "FileOpenRecent",
			"FileSave", "FileSaveAs", "FileExport", "FilePrint", "FileClose",
			"EditorExit"};

	protected String[] accelerators = {"ctrl N", "ctrl O", null, "ctrl S",
			"ctrl shift S", null, "ctrl P", "ctrl W", "Esc"};
	 ...
	 */
	private OpenRecentMenuManager openRecentMenuManager = new OpenRecentMenuManager();

	/**
	 * the export menu manager
	 */
	private ExportMenuManager exportMenuManager = new ExportMenuManager();
		...
		switch (index) {
			...
			case 3 :
				...
				Editor.getEditor()
						.getIOManager()
						.getFileSaveManager()
						.saveHandleDocument(currentHandle, false,
								menuItems[index]);
			...
			case 6 :
				...
				Editor.getEditor().getIOManager().getFilePrint()
						.print(currentHandle);
				break;
