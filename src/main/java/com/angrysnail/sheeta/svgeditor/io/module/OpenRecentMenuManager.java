...
import com.angrysnail.sheeta.svgeditor.io.managers.monitor.OpenMonitor;
import com.angrysnail.sheeta.svgeditor.resources.RecentFilesListener;

/**
 * the class of the manager used to build and handle the state of the open
 * recent menu items of the open recent menu
 * 
 * @author Jordi SUC
 */
public class OpenRecentMenuManager {
	...
	public OpenRecentMenuManager() {
		...
		RecentFilesListener listener = new RecentFilesListener() {
			...
			@Override
			public void recentFilesListChanged(LinkedList<String> filesPathsList) {
	...
	protected void updateMenuItems() {
		...
		LinkedList<String> recentFilesList = Editor.getEditor()
		...
		if (recentFilesList.size() == 0) {
			...
			for (String recentFilePath : recentFilesList) {
				...
				// getting the file corresponding to the path and checking its
				// existence
				try {
				...
				if (file != null/* && file.exists() */) {
					...
					listener = new ActionListener() {
						...
						public void actionPerformed(ActionEvent evt) {
							...
							Monitor monitor = new OpenMonitor(ffile,
									Editor.getParent(), menuItem, 0, 100);
					...
					// as the file does not exist, it is removed from the list
					// of the recent files
					Editor.getEditor().getResourcesManager()
