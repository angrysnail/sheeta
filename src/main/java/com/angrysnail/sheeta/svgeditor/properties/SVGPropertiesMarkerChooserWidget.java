...
public class SVGPropertiesMarkerChooserWidget extends SVGPropertiesWidget {
	...
	protected void buildComponent() {
		...
		if (propertyValue != null && !propertyValue.equals("")
				&& !propertyValue.equals("none")) {

			propertyValue = com.angrysnail.sheeta.library.Toolkit
					.toUnURLValue(propertyValue);
		}
		...
		if (handle != null) {
			...
			resources = handle.getSvgResourcesManager().getResourcesFromDefs(
					doc, resourceTagNames);
		}
		...
		for (Iterator it = values.keySet().iterator(); it.hasNext();) {
			...
			try {
				...
				item = new SVGComboResourceItem(handle, value,
						(String) labels.get(cur), resource);

				// checks if the current possible value is equals to the current
				// value of the
				// property
		...
		final ActionListener comboListener = new ActionListener() {
			...
			public void actionPerformed(ActionEvent evt) {
				...
				// gets the new value and adds it, and registers the node to the
				// resource manager
				String value = "";

				if (combo.getSelectedItem() != null) {

					value = ((SVGComboResourceItem) combo.getSelectedItem())
							.getValue();
				}
				...
				if (value != null && !value.equals("")) {
					...
					if (!value.equals("none")) {
						...
						value = com.angrysnail.sheeta.library.Toolkit
								.toURLValue(value);
					}
