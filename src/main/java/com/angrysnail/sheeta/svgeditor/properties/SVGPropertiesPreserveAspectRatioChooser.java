...
import com.angrysnail.sheeta.svgeditor.visualresources.SVGComboItem;

/**
 * @author ITRIS, Jordi SUC
 */
public class SVGPropertiesPreserveAspectRatioChooser
		extends
			SVGPropertiesWidget {
	...
	protected void buildComponent() {
		...
		final ActionListener comboAndMeetOrSliceListener = new ActionListener() {
			...
			public void actionPerformed(ActionEvent evt) {
				...
				if (combo.getSelectedItem() != null) {
					...
					propertyItem.changePropertyValue(((SVGComboItem) combo
							.getSelectedItem()).getValue() + " " + meetOrSlice);
				}
			}
		};
		...
		configure = new Runnable() {
			...
			public void run() {
				...
				meetOrSliceCheckBox
						.removeActionListener(comboAndMeetOrSliceListener);
				...
				if (value != null) {
					...
					for (int i = 0; i < splitValue.length; i++) {
						...
						if (splitValue[i].equals("meet")
								|| splitValue[i].equals("slice")) {
				...
				combo.addActionListener(comboAndMeetOrSliceListener);
				meetOrSliceCheckBox
						.addActionListener(comboAndMeetOrSliceListener);
			}
		};
