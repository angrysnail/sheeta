...
public class SVGPropertiesRenderChooserWidget extends SVGPropertiesWidget {
	...
	protected void buildComponent() {
		...
		// the color represented by this string (if this string represents a
		// color)
		...
		if (currentColor != null) {
			...
			if ("none".equals(value)) {
				...
				values[1] = com.angrysnail.sheeta.library.Toolkit
						.toUnURLValue(value);
			}
		...
		final JToggleButton colorPickerButton = new JToggleButton(
				colorPickerIcon);
			...
			actionListeners[i] = new ActionListener() {
				...
				public void actionPerformed(ActionEvent evt) {
					...
					for (int j = 0; j < 2; j++) {
						...
						// selects or deselects the radio buttons and enables or
						// disables the
						// associated widgets
						if (fi != j) {
							...
							if (j == 0) {
								...
								colorPickerButton
										.setIcon(colorPickerDisabledIcon);
					...
					// unregisters the nodes with the last value that each of
					// them had
					String val = null;
					...
					for (Element el : nodesList) {
						...
						if (el != null) {
							...
							val = com.angrysnail.sheeta.library.Toolkit
									.toUnURLValue(val);
					...
					if (fi == 1) {
						...
						SVGComboResourceItem itm = (SVGComboResourceItem) combo
								.getSelectedItem();
						...
						if (itm != null) {
							...
							if (itm.getValue().equals("none")) {
								...
								propertyItem
										.changePropertyValue(itm.getValue());

							} else {

								propertyItem
										.changePropertyValue(com.angrysnail.sheeta.library.Toolkit
												.toURLValue(itm.getValue()));

								// registers the nodes and the id of the
								// resource
								if (!itm.equals("")) {

									handle.getSvgResourcesManager()
											.addNodesUsingResource(
													itm.getValue(), nodesList);
								}
							}
						}
					...
					public void eventDispatched(AWTEvent event) {
						...
						if (event instanceof MouseEvent) {
							...
							if (mevt.getID() == MouseEvent.MOUSE_PRESSED) {
								...
								Color col = editor.getSVGToolkit().pickColor(
										point);

								if (col != null) {

									// setting the new color value
									String scl = Editor.getColorChooser()
											.getColorString(col);
									propertyItem.changePropertyValue(scl);
									values[0] = scl;
									colorPanel.setBackground(col);
									Editor.getSVGColorManager()
											.setCurrentColor(col);
								}
		...
		final ActionListener comboListener = new ActionListener() {
			...
			public void actionPerformed(ActionEvent evt) {
				...
				// unregisters the nodes with the last value that each of them
				// had
				String val = null;
				...
				if (combo.getSelectedItem() != null) {
					...
					newValue = ((SVGComboResourceItem) combo.getSelectedItem())
							.getValue();
				}
				...
				if (newValue != null && !newValue.equals("")) {
					...
					if (newValue.equals("none")) {
						...
						handle.getSvgResourcesManager().addNodesUsingResource(
								newValue, nodesList);
					}
