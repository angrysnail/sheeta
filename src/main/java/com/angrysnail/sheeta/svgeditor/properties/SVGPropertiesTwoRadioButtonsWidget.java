	...
	protected void buildComponent() {
		...
		if (propertyItem.getPropertyValuesMap().size() == 2) {
			...
			for (Iterator it = propertyItem.getPropertyValuesMap().keySet()
				...
				try {
					key = (String) it.next();
					values[i] = (String) propertyItem.getPropertyValuesMap()
							.get(key);
					labels[i] = (String) propertyItem
							.getPropertyValuesLabelMap().get(key);
					i++;
				} catch (Exception ex) {
					break;
				}
			...
			if (values[0] != null && labels[0] != null && values[1] != null
					&& labels[1] != null) {
