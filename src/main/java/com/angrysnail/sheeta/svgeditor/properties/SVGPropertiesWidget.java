...
public abstract class SVGPropertiesWidget {
	...
	static {
		...
		// gets the graphics environment and the array of the platform available
		// fonts
		GraphicsEnvironment ge = GraphicsEnvironment
				.getLocalGraphicsEnvironment();
		Font[] fontTab = ge.getAllFonts();

		// removes all the duplicated font families and creates the list of
		// fonts
		Font cfont = null;
		...
		for (int i = 0; i < fontTab.length; i++) {
			...
			if (fontTab[i] != null
					&& !fontFamilyList.contains(fontTab[i].getFamily())) {
