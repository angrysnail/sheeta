package com.angrysnail.sheeta.svgeditor.selection;
...
import com.angrysnail.sheeta.svgeditor.display.selection.Selection;
...
public class SelectionModule extends ModuleAdapter {
	...
	/**
	 * the tool item that is displayed in the toolbar to handle regular
	 * selections
	 */
	...
	protected void createMenuAndToolItems() {
		...
		// getting the label
		regularSelectionLabel = ResourcesManager.bundle
				.getString(regularSelectionId + "ItemLabel");
		zoneSelectionLabel = ResourcesManager.bundle.getString(zoneSelectionId
				+ "ItemLabel");
		allSelectionLabel = ResourcesManager.bundle.getString(allSelectionId
				+ "ItemLabel");
		allDeselectionLabel = ResourcesManager.bundle
				.getString(allDeselectionId + "ItemLabel");
		enterGroupLabel = ResourcesManager.bundle.getString(enterGroupId
				+ "ItemLabel");
		exitGroupLabel = ResourcesManager.bundle.getString(exitGroupId
				+ "ItemLabel");
			...
			public void actionPerformed(ActionEvent evt) {
				...
				if (evt.getModifiers() == InputEvent.BUTTON1_DOWN_MASK
					...
					Editor.getEditor().getSelectionManager()
							.setSelectionMode(mode, null);
				}
			}
		};

		// creating the listener to the all selection and deselection menu and
		// tool items
		...
		regularSelectionIcon = ResourcesManager.getIcon(regularSelectionId,
				false);
		regularSelectionDisabledIcon = ResourcesManager.getIcon(
				regularSelectionId, true);
		zoneSelectionIcon = ResourcesManager.getIcon(zoneSelectionId, false);
		zoneSelectionDisabledIcon = ResourcesManager.getIcon(zoneSelectionId,
				true);
		allSelectionIcon = ResourcesManager.getIcon(allSelectionId, false);
		allSelectionDisabledIcon = ResourcesManager.getIcon(allSelectionId,
				true);
		allDeselectionIcon = ResourcesManager.getIcon(allDeselectionId, false);
		allDeselectionDisabledIcon = ResourcesManager.getIcon(allDeselectionId,
				true);
		...
		regularSelectionMenuItem = new JMenuItem(regularSelectionLabel,
				regularSelectionIcon);
		...
		zoneSelectionMenuItem = new JMenuItem(zoneSelectionLabel,
				zoneSelectionIcon);
		...
		allSelectionMenuItem = new JMenuItem(allSelectionLabel,
				allSelectionIcon);
		...
		allSelectionMenuItem.setAccelerator(KeyStroke.getKeyStroke(
				KeyEvent.VK_A, java.awt.Toolkit.getDefaultToolkit()
						.getMenuShortcutKeyMask(), false));
		allSelectionMenuItem.setEnabled(false);

		// creating the all deselection menu item
		allDeselectionMenuItem = new JMenuItem(allDeselectionLabel,
				allDeselectionIcon);
		...
		allDeselectionMenuItem.setAccelerator(KeyStroke.getKeyStroke(
				KeyEvent.VK_D, java.awt.Toolkit.getDefaultToolkit()
						.getMenuShortcutKeyMask(), false));
		...
		exitGroupMenuItem
				.setAccelerator(KeyStroke.getKeyStroke("ctrl shift E"));
		...
		Editor.getEditor().getSelectionManager()
				.setRegularButton(regularSelectionToolItem);
		...
		svgHandleManager.addHandlesListener(new HandlesListener() {
			...
			public void handleChanged(SVGHandle currentHandle,
				...
				if (currentHandle != null) {
					...
					manageSelection(new HashSet<Element>(currentHandle
	...
	protected void updateMenuItems() {
		...
		if (handle != null) {
			...
			if (parentElement != null
					&& !parentElement.equals(parentElement.getOwnerDocument()
							.getDocumentElement())) {
	...
	protected void exitGroup() {
		...
		if (handle != null) {
			...
			if (parentElement != null && parentElement.getParentNode() != null) {
				...
				selection.setParentElement(
						(Element) parentElement.getParentNode(), true);
			}
		}
	}
	...
	public Collection<PopupItem> getPopupItems() {
		...
		PopupItem groupEnterItem = new PopupItem(Editor.getEditor(),
				enterGroupId, enterGroupLabel, enterGroupId) {
			...
			public JMenuItem getPopupItem(LinkedList<Element> nodes) {
				...
				if (nodes != null && nodes.size() == 1) {
					...
					if (current != null
							&& Selection.isParentElementEligible(current)) {
		...
		PopupItem groupExitItem = new PopupItem(Editor.getEditor(),
				exitGroupId, exitGroupLabel, exitGroupId) {
			...
			public JMenuItem getPopupItem(LinkedList<Element> nodes) {
				...
				if (handle != null) {
					...
					// getting the current parent element
					Element parentElement = handle.getSelection()
							.getParentElement();

					// checking if the menu item can be enabled
					if (parentElement != null
							&& !parentElement.equals(handle.getCanvas()
		...
		PopupItem lockItem = new PopupItem(Editor.getEditor(), lockId,
				lockLabel, lockId) {
			...
			public JMenuItem getPopupItem(LinkedList<Element> nodes) {
				...
				if (handle != null) {
					...
					Set<Element> selectedElements = selection
							.getSelectedElements();

					// checking the state of the lock item
					if (selectedElements.size() > 0
							&& !selection.isAllSelectionLocked()) {
		...
		PopupItem unLockItem = new PopupItem(Editor.getEditor(), unLockId,
