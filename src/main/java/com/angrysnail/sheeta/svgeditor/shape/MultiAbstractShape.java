...
public abstract class MultiAbstractShape extends AbstractShape {
			...
			case 1 :
				...
				if (item.getType() != SelectionItem.CENTER) {
					...
					if (item.getType() == SelectionItem.NORTH_WEST
						...
						undoRedoAction = rotate(handle, elementsSet,
								centerPoint, angle);
	 ...
	 * 
	 * //creating the union of all the shapes corresponding to the set of
	 * elements GeneralPath path=new GeneralPath(); Area area=null;
	 * LinkedList<Shape> lineShapes=new LinkedList<Shape>(); AbstractShape
	 * shapeModule=null; Shape shp=null;
	 * 
	 * for(Element element : elementsSet){
	 * 
	...
	public UndoRedoAction translate(SVGHandle handle, Set<Element> elementSet,
		...
		return getUndoRedoAction(undoRedoActionSet, elementSet,
