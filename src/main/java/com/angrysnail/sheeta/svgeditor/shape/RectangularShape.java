...
public abstract class RectangularShape extends AbstractShape {
		...
		switch (type) {
			...
			case DRAWING_MOUSE_RELEASED :
				...
				if (Editor.getEditor().getSquareModeManager().isSquareMode()) {
					...
					rect = EditorToolkit.getComputedSquare(firstDrawingPoint,
							point);

				} else {

					rect = EditorToolkit.getComputedRectangle(
							firstDrawingPoint, point);
				}
	...
	public UndoRedoAction translate(
		...
		if (initialShape != null) {
			...
			Point2D translationCoefficients = ShapeToolkit
					.getTranslationFactors(handle, element,
							translationFactors.getX(),
							translationFactors.getY());
	...
	protected Point2D getRadius(SVGHandle handle, Element element,
		...
		switch (item.getIndex()) {
			...
			case 0 :
				...
				factors = new Point2D.Double(factor, factors.getY() == 0
						? factor
						: factors.getY());
				break;
			...
			case 1 :
				...
				// creating the point that will be returned
				factors = new Point2D.Double(factors.getX() == 0
						? factor
						: factors.getX(), factor);
	...
	protected Shape getModifiedRadiusShape(SVGHandle handle, Element element,
		...
		rect.setRoundRect(rect.getX(), rect.getY(), rect.getWidth(),
				rect.getHeight(), newFactors.getX() * 2, newFactors.getY() * 2);
		...
		public void paintToBeDone(Graphics2D g) {
			...
			if (shape != null) {
				...
				if (shape instanceof RoundRectangle2D) {
					...
					g.drawRoundRect((int) roundRect.getX(),
							(int) roundRect.getY(), (int) roundRect.getWidth(),
							(int) roundRect.getHeight(),
							(int) roundRect.getArcWidth(),
							(int) roundRect.getArcHeight());
