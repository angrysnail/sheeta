...
public class ShapeToolkit {
	...
	public static AbstractShape getShapeModule(Element element) {
		...
		if (element != null) {
			...
			Set<AbstractShape> modules = Editor.getEditor()
					.getSVGModuleLoader().getShapeModules();

			// the set of all the modules that match the provided element
			Set<AbstractShape> possibleModules = new HashSet<AbstractShape>();
			...
			if (possibleModules.size() == 1) {
				...
				for (AbstractShape shapeModule : possibleModules) {
					...
					if (shapeModule.isPrioritary()
							|| i == possibleModules.size() - 1) {
	...
	/**
	 * applies the element's transformation to the given shape and returns the
	 * resulting shape
	 * 
	 * @param svgHandle
	 *            a svg handle
	 ...
	 */
	public static Shape getTransformedShape(SVGHandle svgHandle,
			Element element, Shape shape) {

		Shape transformedShape = shape;

		// getting the element's affine transform
		AffineTransform af = svgHandle.getSvgElementsManager().getTransform(
				element);
	...
	/**
	 * returns the translation coefficients taking into account the element's
	 * transform
	 * 
	 * @param handle
	 ...
	 * @param deltaX
	 *            the absciss factor of the translation
	 * @param deltaY
	 *            the ordinate factor of the translation
	 * @return the translation coefficients taking into account the element's
	 *         transform
	 */
	public static Point2D getTranslationFactors(SVGHandle handle,
			Element element, double deltaX, double deltaY) {
	 ...
	 *            the center point
	 * @param firstPoint
	 *            the point defining the first segment whose origin is the
	 *            center point
	 * @param secondPoint
	 *            the point the point defining the second segment whose origin
	 *            is the center point
	 * @return an angle the point defining the second segment with the center
	 *         point
	 */
	...
	/**
	 * returns the angle in the trigonometric circle of the segment whose center
	 * is the center point and the second point is the given point
	 * 
	 * @param centerPoint
	 *            the center point
	 * @param point
	 *            the other point og the segment
	 * @return an angle
	 */
	public static double getAngle(Point2D centerPoint, Point2D point) {
		...
		Point2D signedPoint = new Point2D.Double(point.getX()
				- centerPoint.getX(), point.getY() - centerPoint.getY());

		// putting the point into the upper right part of the trigonometric
		// circle
		Point2D normalizedPoint = new Point2D.Double(Math.abs(signedPoint
				.getX()), Math.abs(signedPoint.getY()));

		// computing the angle
		angle = Math.abs(Math.atan2(normalizedPoint.getY(),
				normalizedPoint.getX()));
	 ...
	 * @param elements
	 *            the set of the elements to modify
	 */
	public static void addUndoRedoAction(SVGHandle handle, String actionLabel,
			Runnable executeRunnable, Runnable undoRunnable,
			Set<Element> elements) {

		// creating the undo/redo action and inserting it into the undo/redo
		// stack
		UndoRedoAction action = getUndoRedoAction(actionLabel, executeRunnable,
				undoRunnable, elements);

		UndoRedoActionList actionlist = new UndoRedoActionList(actionLabel,
				false);
	 ...
	 */
	public static UndoRedoAction getUndoRedoAction(String actionLabel,
			Runnable executeRunnable, Runnable undoRunnable,
			Set<Element> elements) {
	 ...
	 *            a svg element
	 * @param delta
	 *            the difference between the first and the last point of the
	 *            relative coordinate
	 * @param centerPoint
	 *            the center point
	 * @param isHorizontal
	 *            whether the skew action is horizontal or vertical
	 * @return the skew affine transform corresponding to the given elements
	 */
	public static AffineTransform getSkewAffineTransform(SVGHandle svgHandle,
			Element element, Point2D centerPoint, double delta,
		...
		if (centerPoint == null) {
			...
			centerPoint = new Point2D.Double(bounds.getCenterX(),
					bounds.getCenterY());
		}
