package com.angrysnail.sheeta.svgeditor.shape.path;
...
import com.angrysnail.sheeta.svgeditor.display.selection.SelectionItem;
import com.angrysnail.sheeta.svgeditor.shape.AbstractShape;
import com.angrysnail.sheeta.svgeditor.shape.ShapeToolkit;
import com.angrysnail.sheeta.svgeditor.shape.path.segments.ActionSeg;
import com.angrysnail.sheeta.svgeditor.shape.path.segments.CubicToSeg;
import com.angrysnail.sheeta.svgeditor.shape.path.segments.LineToSeg;
import com.angrysnail.sheeta.svgeditor.shape.path.segments.MoveToSeg;
import com.angrysnail.sheeta.svgeditor.shape.path.segments.QuadToSeg;
...
public class DrawingHandler {
		...
		if (segments.size() == 0 && points.size() == 0) {
			...
			if (item != null && item.getElements().size() == 1) {
				...
				if (element != null
					...
					drawingPathToConnect = new PathToConnect(element,
							item.getIndex());
			...
			if (Editor.getEditor().getConstraintLinesModeManager()
					.constraintLines()) {
			...
			path.quadTo((float) scControlPoint.getX(),
	...
	public void mouseReleased(SVGHandle handle, Point2D point) {
		...
		if (points.size() == 1
			...
			path.curveTo((float) scControlPoint1.getX(),
					(float) scControlPoint1.getY(),
					(float) scControlPoint2.getX(),
					(float) scControlPoint2.getY(), (float) scEndPoint.getX(),
					(float) scEndPoint.getY());
		}
	}
	...
	public void mouseMoved(SVGHandle handle, Point2D point) {
		...
		if (points.size() == 0 && segments.size() > 0) {
			...
			points.add(new ActionPoint(AbstractShape.DRAWING_MOUSE_MOVED, point));
	...
	public void mouseDoubleClicked(SVGHandle handle, Point2D point) {
		...
		if (segments.size() > 0) {
			...
			if (drawingPathToConnect != null) {
				...
				if (Editor.getEditor().getClosePathModeManager()
						.shouldClosePath()) {
	...
	protected void initializeDrawingPath() {
		...
		if (endPoint != null) {
			...
			controlPointsPath.moveTo((float) endPoint.getX(),
					(float) endPoint.getY());
		}
	}
	...
	protected Point2D computeLinePointWhenLineConstraintModeActive(
		...
		if (startPoint != null && basePoint != null) {
			...
			Point2D.Double pt1 = new Point2D.Double(startPoint.getX(),
					startPoint.getY());
			Point2D.Double pt2 = new Point2D.Double(basePoint.getX(),
					basePoint.getY());
