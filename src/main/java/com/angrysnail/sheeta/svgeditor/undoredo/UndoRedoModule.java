package com.angrysnail.sheeta.svgeditor.undoredo;
...
public class UndoRedoModule extends ModuleAdapter {
	 ...
	 */
	private static String idundoredo = "UndoRedo", idundo = "Undo",
			idredo = "Redo";
	...
	public Collection<PopupItem> getPopupItems() {
		...
		PopupItem undoItem = new PopupItem(Editor.getEditor(), idundo,
				labelundo, "Undo") {
			...
			public JMenuItem getPopupItem(LinkedList<Element> nodes) {
				...
				if (handle != null) {
					...
					String undoLabel = handle.getUndoRedo()
							.getUndoActionListLabel();
		...
		PopupItem redoItem = new PopupItem(Editor.getEditor(), idredo,
				labelredo, "Redo") {
			...
			public JMenuItem getPopupItem(LinkedList<Element> nodes) {
				...
				if (handle != null) {
					...
					// getting the undo and redo labels
					String redoLabel = handle.getUndoRedo()
							.getRedoActionListLabel();
