...
/**
 * the class of the widget that will be displayed in the properties dialog, and
 * enabling to modify the color values of the children of a gradient resource
 * node
 * 
 * @author ITRIS, Jordi SUC, Maciej Wojtkiewicz
 */
public class SVGVisualResourceGradientColorChooserChildWidget
		extends
			SVGVisualResourceChildWidget {

	/**
	 * the constructor of the class
	 * 
	 * @param resourceObject
	 *            the resource object whose children will be modified by this
	 *            widget
	 */
	public SVGVisualResourceGradientColorChooserChildWidget(
	...
	protected void buildComponent() {
		...
		if (resourceObject != null
				&& resourceObject.getChildren().getFirst() != null) {

			final SVGVisualResourceObject fresourceObject = resourceObject;
			final Editor svgEditor = resourceObject.getResourceModel()
					.getVisualResources().getSVGEditor();
			SVGVisualResourceObjectChild childObject = (SVGVisualResourceObjectChild) resourceObject
			...
			if (bundle != null) {
				...
				try {
					...
					stopProperties = bundle
							.getString("vresource_stopproperties");
			...
			final Comparator comparator = new Comparator() {
				...
				public int compare(Object arg0, Object arg1) {
					...
					if (arg0 != null && arg1 != null
							&& arg0 instanceof SVGGradientChooserItem
							&& arg1 instanceof SVGGradientChooserItem) {

						SVGGradientChooserItem item0 = (SVGGradientChooserItem) arg0, item1 = (SVGGradientChooserItem) arg1;
			...
			// the dimension of the arrows panel
			final Dimension gradientPanelSize = new Dimension(200 + itemWidth,
					42);
			...
			final JPanel gradientPanel = new JPanel() {
				...
				protected void paintComponent(Graphics g) {
					...
					int sq = 8, lx = (gradientPanelSize.width - itemWidth) / sq, ly = gradientPanelSize.height
							/ sq;
					...
					for (int i = 0; i <= lx; i++) {
						...
						for (int j = 0; j <= ly; j++) {
							...
							g.fillRect(
									i * sq + itemWidth / 2,
									j * sq,
									((i * sq + itemWidth / 2) > (gradientPanelSize.width - itemWidth)
											? sq - (i * sq + itemWidth / 2)
											: sq), sq);
						}
					}
					...
					for (Iterator it = gradientItems.iterator(); it.hasNext();) {
						...
						if (item != null) {
							...
							item.paintGradient((Graphics2D) g, previousItem,
									it.hasNext());
							previousItem = item;
						}
					}
					...
					g.drawRect(itemWidth / 2, 0, gradientPanelSize.width
							- itemWidth, gradientPanelSize.height - 1);
				}
			};
			...
			final JLabel stopColorLbl = new JLabel(stopColor.concat(" :")), stopOpacityLbl = new JLabel(
					stopOpacity.concat(" :")), offsetLbl = new JLabel(
					offset.concat(" :"));

			// the panel containing the add and remove button
			JPanel buttonPanel = new JPanel();

			svgEditor.getResourcesManager();
			svgEditor.getResourcesManager();
			svgEditor
			.getResourcesManager();
			// the icons
			final ImageIcon addIcon = ResourcesManager.getIcon(
					"New", false), deleteIcon = ResourcesManager
					.getIcon("Delete", false), deleteDisabledIcon = ResourcesManager.getIcon("Delete", true);

			// the add and remove buttons
			final JButton addBt = new JButton(addIcon), deleteBt = new JButton(
					deleteIcon);
			...
			positionValueTxt.setAlignmentX(Component.RIGHT_ALIGNMENT);
			...
			displayAndSlider.setLayout(new BoxLayout(displayAndSlider,
					BoxLayout.X_AXIS));
			...
			final SVGGradientChooserArrowsListener arrowsListener = new SVGGradientChooserArrowsListener() {
					...
					if (item != null) {
						...
						if (item.getColor() != null) {
							...
							positionValueTxt.setText(item.getXPositionPercent()
									+ "%");
							displayedValue
									.setText((int) (item.getOpacity() * 100)
											+ "%");
				 ...
				 */
				@Override
				public void createGradientItem(
						SVGVisualResourceObjectChild resChild) {

					if (resChild != null) {

						SVGGradientChooserItem item = new SVGGradientChooserItem(
								resChild, arrowsPanelSize, gradientPanelSize);
				...
				public void removeCurrentGradientItem() {
					...
					if (currentItem != null) {
						...
						SVGVisualResourceObjectChild resChild = currentItem
								.getResourceObjectChild();
				...
				public void mousePressed(MouseEvent e) {
					...
					if (point != null) {
						...
						if (item != null) {
							...
							// creating a new item
							SVGVisualResourceObjectChild resChild = resourceObject
									.createNewChild();

							if (resChild != null) {

								item = new SVGGradientChooserItem(resChild,
										arrowsPanelSize, gradientPanelSize);
				...
				public void mouseReleased(MouseEvent e) {
					...
					if (currentItem != null) {
						...
						if (e.getClickCount() == 2) {
							...
							Color color = Editor.getColorChooser()
							...
							if (color != null && currentItem != null) {
								...
								Editor.getSVGColorManager().setCurrentColor(
										color);
				...
				public void mouseDragged(MouseEvent e) {
					...
					if (currentItem != null) {
						...
						if (point != null && canDrag) {
							...
							positionValueTxt.setText(currentItem
									.getXPositionPercent() + "%");
							refresh();
						}
					}
				...
				protected SVGGradientChooserItem getClickedItem(Point point) {
					...
					if (point != null) {
						...
						for (int i = gradientItems.size() - 1; i >= 0; i--) {
							...
							try {
								item = (SVGGradientChooserItem) gradientItems
										.get(i);
							} catch (Exception ex) {
								item = null;
							}
				...
				protected void selectGradientChooserItem(
						SVGGradientChooserItem gradientChooserItem) {
					...
					if (gradientChooserItem != null) {
						...
						for (Iterator it = gradientItems.iterator(); it
								.hasNext();) {
			...
			for (Iterator it = resourceObject.getChildren().iterator(); it
					.hasNext();) {
			...
			final ActionListener colorBtListener = new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					Color color = Editor.getColorChooser()
							.showColorChooserDialog(colorPanel.getBackground());

					if (color != null && arrowsListener != null
							&& arrowsListener.getCurrentItem() != null) {
			...
			final MouseAdapter sliderListener = new MouseAdapter() {
				...
				public void mouseReleased(MouseEvent evt) {
					...
					arrowsListener.getCurrentItem().setOpacity(
							((double) slider.getValue()) / 100);
					arrowsListener.getCurrentItem().applyChanges();
					gradientPanel.repaint();
				}
			};
			...
			// the panel containing the widgets to modify the properties of an
			// item
			...
			Insets insetsLbl0 = new Insets(0, 0, 2, 2), insets0 = new Insets(0,
					2, 0, 2);
			...
			disposer = new Runnable() {
				...
				public void run() {
					...
					final SVGHandle handle = svgEditor.getHandlesManager()
							.getCurrentHandle();

					// sorts the child nodes according to the "offset" attribute
					// value
					if (handle != null && fresourceObject != null
							&& arrowsListener.isModified()) {
						...
						for (cur = fresourceObject.getResourceNode()
								.getFirstChild(); cur != null; cur = cur
								.getNextSibling()) {
						...
						for (Iterator it = gradientItems.iterator(); it
							...
							if (cur != null) {
								...
								fresourceObject.getResourceNode().appendChild(
										cur);
							}
						}
					}
					...
					// removes the listener to the mouse events on the arrows
					// panel
	 ...
	 */
	protected interface SVGGradientChooserArrowsListener
			extends
				MouseListener,
				MouseMotionListener {
		...
		/**
		 * the method used to refresh the display of the arrows and the gradient
		 * panel
		 */
		public void refresh();
		...
		/**
		 * @return the boolean telling if the gradient items have been modified
		 *         or not
		 */
		public boolean isModified();
	...
	protected class SVGGradientChooserItem {
		...
		private final Color ITEM_COLOR = Color.BLUE,
				...
				BORDER_HIGHLIGHT_COLOR = MetalLookAndFeel
		...
		/**
		 * the opacity for the color of the item, a double in the [0..1]
		 * interval
		 */
		 ...
		 * @param resChild
		 *            a resource child
		 * @param arrowsPanelSize
		 *            the size of of the arrows panel
		 * @param gradientPanelSize
		 *            the size of of the gradient panel
		 */
		protected SVGGradientChooserItem(SVGVisualResourceObjectChild resChild,
				Dimension arrowsPanelSize, Dimension gradientPanelSize) {

			this.resChild = resChild;

			if (arrowsPanelSize != null) {

				this.yPosition = arrowsPanelSize.height;
			}
			...
			if (resChild != null) {
				...
				if (attributes != null) {
					...
					// for each attribute, checks if it has one of the following
					// names and then
					// stores it
					for (int i = 0; i < attributes.size(); i++) {

						try {
							att = (SVGVisualResourceObjectAttribute) attributes
									.get(i);
						} catch (Exception ex) {
							att = null;
						}
		...
		protected int computeXPosition() {
			...
			if (posResAttribute != null && arrowsPanelSize != null) {
				...
				if (val.indexOf("%") != -1) {
					...
					pos = (pos2 * (arrowsPanelSize.width - itemWidth)) / 100
							+ itemWidth / 2;

				} else {

					double pos2 = Double.parseDouble(val);
					pos = (int) (pos2 * (arrowsPanelSize.width - itemWidth))
							+ itemWidth / 2;
				}
			}
		...
		protected int getXPositionPercent() {
			...
			if (arrowsPanelSize != null) {
				...
				double percent = 100 * (xPosition - itemWidth / 2)
						/ (arrowsPanelSize.width - itemWidth);
				xPos = (int) percent;
			}
		...
		protected Color computeColor() {
			...
			if (styleResAttribute != null) {
				...
				if (ind != -1) {
					...
					Editor editor = resChild.getChildModel()
							.getVisualResources().getSVGEditor();
		...
		protected double computeOpacity() {
			...
			if (styleResAttribute != null) {
				...
				if (ind != -1) {
					...
					op = resChild.getChildModel().getVisualResources()
							.getSVGEditor().getSVGToolkit()
							.getDoubleValue(val, true) / 100;
				}
			}
		 ...
		 * 
		 * @param opacity
		 *            the value for the opacity a double in the [0..1] interval
		 */
		...
		protected void applyColor() {
			...
			if (color != null && styleResAttribute != null) {
				...
				if (ind != -1) {
					...
					// getting the value of the color among the style string
					String val1 = val1 = val.substring(0, ind), val2 = val
							.substring(ind, val.length());
				...
				val = val.concat(prop.concat(":".concat(
						Editor.getColorChooser().getColorString(color)).concat(
						";")));
		...
		protected void applyOpacity() {
			...
			if (styleResAttribute != null) {
				...
				// creating the new value of the style attribute
				val = val.concat(prop.concat(":".concat(format.format(opacity))
						.concat(";")));
		...
		/**
		 * @param point
		 *            th position of the mouse
		 * @return true if the x position of the mouse is contained in the item
		 *         interval
		 */
		protected boolean belongsToThisItem(Point point) {
			...
			if (point != null) {
				...
				Rectangle shapeBounds = shape.getBounds(), rectBounds = rect
						.getBounds(), allBounds = new Rectangle(shapeBounds.x,
						rectBounds.y, rectBounds.width, rectBounds.height
								+ shapeBounds.height);
		...
		protected void paintArrow(Graphics2D g) {
			...
			g.drawLine(bounds.x + bounds.width - 1, bounds.y, bounds.x
					+ bounds.width - 1, bounds.y + bounds.height - 1);
			g.drawLine(bounds.x + 1, bounds.y + bounds.height - 1, bounds.x
					+ bounds.width - 1, bounds.y + bounds.height - 1);

			g.setColor(BORDER_HIGHLIGHT_COLOR);
			g.drawLine(bounds.x + 1, bounds.y + 1, bounds.x + 1, bounds.y
					+ bounds.height - 1);
			g.drawLine(bounds.x + 1, bounds.y + 1, bounds.x + bounds.width - 1,
					bounds.y + 1);
			g.drawLine(bounds.x + bounds.width, bounds.y, bounds.x
					+ bounds.width, bounds.y + bounds.height);
			g.drawLine(bounds.x, bounds.y + bounds.height, bounds.x
					+ bounds.width, bounds.y + bounds.height);
		}
		 ...
		 *            a graphics
		 * @param previousItem
		 *            the item previously displayed on the gradient panel
		 * @param hasNextItem
		 *            true if another item takes place after this current one in
		 *            the gradient items list
		 */
		protected void paintGradient(Graphics2D g,
				SVGGradientChooserItem previousItem, boolean hasNextItem) {
			...
			if (color != null) {
				...
				preColor = new Color(color.getRed(), color.getGreen(),
						color.getBlue(), (int) (opacity * 255));
				tColor = new Color(color.getRed(), color.getGreen(),
						color.getBlue(), (int) (opacity * 255));
			}
			...
			if (previousItem != null) {
				...
				if (pc != null) {
					...
					preColor = new Color(pc.getRed(), pc.getGreen(),
							pc.getBlue(),
							(int) (previousItem.getOpacity() * 255));
				}
			}
			...
			if (gradientPanelSize != null && preColor != null && tColor != null) {
				...
				GradientPaint gradient = new GradientPaint(preXPosition, 0,
						preColor, xPosition, 0, tColor);
				g.setPaint(gradient);
				g.fillRect(preXPosition, 0, xPosition - preXPosition,
						gradientPanelSize.height);

				// if the current item is the last item in the gradient items
				// list
				if (!hasNextItem) {

					g.setColor(tColor);
					g.fillRect(xPosition, 0,
							(gradientPanelSize.width - itemWidth) - xPosition
									+ itemWidth / 2, gradientPanelSize.height);
				}
			}
		}
