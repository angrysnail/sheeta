...
import com.angrysnail.sheeta.svgeditor.ResourceImageManager;
...
public class SVGVisualResourceListsPanel extends JPanel {
	 ...
	 */
	private final Map vresourceModels = Collections
	 ...
	 * a small font
	 */
	private static final Font smallFont = new Font("smallFont",
			Font.ROMAN_BASELINE, 9);

	/**
	 * the font
	 */
	private static final Font theFont = new Font("theFont",
	...
	/**
	 * the map associating the id of a resource to the panel representing this
	 * resource in the list
	 */
	private Map<SVGVisualResourceObject, JComponent> cellPanels = Collections
			.synchronizedMap(new HashMap<SVGVisualResourceObject, JComponent>());
	 ...
	 * 
	 * @param visualResources
	 *            the visual resources module
	 * @param vresourceModels
	 *            the list of the visual resource items
	 */
	public SVGVisualResourceListsPanel(SVGVisualResources visualResources,
			LinkedList vresourceModels) {

		this.visualResources = visualResources;
		this.svgHandle = visualResources.getSVGEditor().getHandlesManager()
				.getCurrentHandle();
			...
			changeListener = new ChangeListener() {
				...
				public void stateChanged(ChangeEvent arg0) {
					...
					if (tabPanel != null) {
						...
						resourceState.setSelectedTabId(tabPanel
	 ...
	 * 
	 * @param type
	 *            the type of the action that has triggered this action to be
	 *            called
	 * @param resourceType
	 *            the type of the resource
	 * @param currentElement
	 *            the resource element that is currently used
	 * @param oldElement
	 *            a resource element that has been removed or modified
	 */
	public void refreshResources(int type, String resourceType,
			Element currentElement, Element oldElement) {

		if (resourceType != null && !resourceType.equals("")) {

			// getting the map associating the id of a resource to the resource
			// node
			LinkedList resourcesNames = new LinkedList();
			resourcesNames.add(resourceType);
			String resourceId = "", oldResourceId = "";

			SVGVisualResourceModel model = (SVGVisualResourceModel) vresourceModels
					.get(resourceType);
			ResourceImageManager resImgManager = Editor.getEditor()
					.getResourceImageManager();
				...
				if (listModel != null) {
					...
					if (type == RESOURCE_NEW && currentElement != null) {
						...
						if (!resourceId.equals("")) {
							...
							resObj = model
									.createVisualResourceObject(currentElement);

							// creates the list item
							SVGVisualResourceListItem item = new SVGVisualResourceListItem(
									svgHandle, resObj);
						...
						if (oldResourceId != null && !oldResourceId.equals("")) {
							...
							resImgManager.invalidateResourceRepresentation(
									svgHandle, oldResourceId);
						}

						// removes the resource object linked with the old
						// element
						model.removeVisualResourceObject(oldResourceId);

						// getting the list item linked with the resource
						// element
						item = (SVGVisualResourceListItem) itemListMap
								.get(oldResourceId);
					...
					} else if (type == RESOURCE_MODIFIED
							&& currentElement != null && oldElement != null) {
						...
						// getting the old list item linked with the resource
						// element
						item = (SVGVisualResourceListItem) itemListMap
								.get(oldResourceId);
						...
						if (item != null) {
							...
							// retrieving the index of the list item of the old
						...
						if (!resourceId.equals("")) {
							...
							// creates the list item
							item = new SVGVisualResourceListItem(svgHandle,
									resObj);
							itemListMap.put(resourceId, item);
	...
	protected void removeAllCellPanels() {
		...
		for (Iterator it = new LinkedList(cellPanels.values()).iterator(); it
				.hasNext();) {
	 ...
	 *         values of the visual resources
	 */
	protected JTabbedPane createTabbedPane() {
		...
		for (it = new LinkedList(vresourceModels.values()).iterator(); it
				.hasNext();) {
	...
	/**
	 * @param vresourceModel
	 *            a visual resource model
	 * @return a JPanel containing the widgets allowing to modify the visual
	 *         resources
	 */
	protected JPanel createTabPanel(SVGVisualResourceModel vresourceModel) {
		...
		/*
		 * //adding a drag gesture listener to the color panel final
		 * DragGestureListener dragGestureListener=new DragGestureListener(){
		 * 
		 * public void dragGestureRecognized(DragGestureEvent evt) {
		 * 
		 * int selectedIndex=list.locationToIndex(evt.getDragOrigin()); final
		 * Object obj=list.getModel().getElementAt(selectedIndex);
		 * list.setSelectedIndex(selectedIndex);
		 * 
		 * if(obj!=null){
		 * 
		 * final String
		 * resourceId=((SVGVisualResourceListItem)obj).getVisualResourceObject
		 * ().getResourceId();
		 * 
		 * //the transferable final Transferable transferable=new
		 * Transferable(){
		 * 
		 ...
		 * 
		 * DataFlavor[] dfs=new DataFlavor[1]; dfs[0]=DataFlavor.stringFlavor;
		 * 
		 * return dfs; }
		 * 
		 ...
		 * res=((SVGVisualResourceListItem)obj).getVisualResourceObject();
		 * 
		 * if(res.canBeModified()){
		 * 
		 * Image
		 * cursorImage=getVisualResources().getSVGEditor().getResourceImageManager
		 * ().getImage(frame, res.getResourceId(), false); DragSource
		 * dragSource=SVGEditor.getSVGEditor().getDragSource(); Cursor
		 * cursor=SVGEditor
		 * .getSVGEditor().getSVGToolkit().createCursorFromImage(cursorImage);
		 * 
		 * dragSource.startDrag(evt, cursor, transferable, null);
		 * evt.getTriggerEvent().consume(); } } } };
		 * 
		 * //adding a drag gesture tokenizer to this component final
		 * DragGestureRecognizer
		 * dragGestureRecognizer=getVisualResources().getSVGEditor
		 * ().getDragSource(). createDefaultDragGestureRecognizer(list,
		 ...
		 * dragGestureRecognizer.setComponent(null); list.setDropTarget(null); }
		 * });
		 */
		...
		String newLabel = "", duplicateLabel = "", deleteLabel = "", importLabel = "", propLabel = "", deleteMessage = "", deleteTitle = "", errorTitle = "", cantDeleteMessage = "";
		...
		if (bundle != null) {
			...
			cantDeleteMessage = bundle
					.getString("vresource_cantdeleteresource");
		}

		final String ferrorTitle = errorTitle, fcantDeleteMessage = cantDeleteMessage;

		final ImageIcon newIcon = ResourcesManager.getIcon("New", false), duplIcon = ResourcesManager
				.getIcon("Duplicate", false), duplDisIcon = ResourcesManager
				.getIcon("Duplicate", true), delIcon = ResourcesManager
				.getIcon("Delete", false), delDisIcon = ResourcesManager
				.getIcon("Delete", true), impIcon = ResourcesManager.getIcon(
				"Import", false), impDisIcon = ResourcesManager.getIcon(
				"Import", true), propIcon = ResourcesManager.getIcon(
				"Properties", false), propDisIcon = ResourcesManager.getIcon(
				"Properties", true);

		// the buttons used to make actions on the resources contained in the
		// tab
		final JButton newBt = new JButton(newIcon), duplicateBt = new JButton(
				duplDisIcon), deleteBt = new JButton(delDisIcon), importBt = new JButton(
				impDisIcon), propBt = new JButton(propDisIcon);
		...
		final ListSelectionListener listSelectionListener = new ListSelectionListener() {
			...
			public void valueChanged(ListSelectionEvent e) {
				...
				if (list.getModel().getSize() > 0) {
					...
					try {
						item = (SVGVisualResourceListItem) list
								.getSelectedValue();
					} catch (Exception ex) {
					}
				}
				...
				if (item != null) {
					...
					if (resObj != null) {
						...
						resourceState.setSelectedItemId(resObj
								.getResourceModel().getName(), resObj
								.getResourceId());
					}
		...
		final ActionListener duplicateBtListener = new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				SVGVisualResourceListItem item = (SVGVisualResourceListItem) list
						.getSelectedValue();
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (resObj != null
					...
					int returnVal = JOptionPane.showConfirmDialog(
							Editor.getParent(), fdeleteMessage, fdeleteTitle,
					...
					JOptionPane.showMessageDialog(Editor.getParent(),
							fcantDeleteMessage, ferrorTitle,
		...
		final ActionListener importBtListener = new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (svgHandle != null && defs != null && item != null) {
					...
					// exports the resource node to the "defs" node if it is
					// contained in the
					// resource store
					if (resObj != null) {
	 ...
	 *            a resource model
	 */
	protected void createNewResource(Element parentElement,
			SVGVisualResourceModel vresourceModel) {

		if (svgHandle != null && parentElement != null
				&& vresourceModel != null) {
			...
			// if the resource contains shape nodes as children, the id of the
			// shape node is
			// returned by a chooser
			if (vresourceModel.isShapedResource()) {

				shapeId = SVGVisualResourceShapeIdChooser
						.showShapeChooserIdDialog(svgHandle);
			}

			if (!vresourceModel.isShapedResource()
					|| (vresourceModel.isShapedResource() && shapeId != null && !shapeId
							.equals(""))) {

				final String fshapeId = shapeId;

				// creating the new resource node
				final Element newElement = getVisualResources()
						.getVisualResourcesToolkit().createVisualResource(
				...
				if (newElement != null) {
					...
					Runnable executeRunnable = new Runnable() {
						...
						public void run() {
							...
							getVisualResources().getVisualResourcesToolkit()
									.appendVisualResource(svgHandle,
											fparentElement, newElement);
							resourceState.setSelectedItemId(name, id);
							refreshResources(RESOURCE_NEW, name, newElement,
									null);
						}
					};
					...
					Runnable undoRunnable = new Runnable() {
						...
						public void run() {
							...
							getVisualResources()
									.getVisualResourcesToolkit()
									.removeVisualResource(svgHandle, newElement);
							resourceState.setSelectedItemId(name, "");
							refreshResources(RESOURCE_DELETED, name, null,
									newElement);
						}
					};

					// create the undo/redo action and insert it into the
					// undo/redo stack
					UndoRedoAction action = new UndoRedoAction(
							getVisualResources().undoredoresourcesnew,
	...
	protected void duplicateResource(SVGVisualResourceObject resourceObject) {
		...
		if (svgHandle != null && resourceObject != null
				&& resourceObject.canBeModified()
				&& resourceObject.getParentNode() != null) {

			final Element fparentElement = (Element) resourceObject
					.getParentNode();
			...
			// creating the new resource node
			final Element newElement = getVisualResources()
					.getVisualResourcesToolkit().duplicateVisualResource(
							svgHandle, resourceNode);
			...
			if (newElement != null) {
				...
				// create the undo/redo action and insert it into the undo/redo
				// stack
				UndoRedoAction action = new UndoRedoAction(
						getVisualResources().undoredoresources,
						executeRunnable, undoRunnable, executeRunnable,
						new HashSet<Element>());
		...
		if (svgHandle != null && resourceObject != null
			...
			Runnable undoRunnable = new Runnable() {
				...
				public void run() {
					...
					getVisualResources().getVisualResourcesToolkit()
							.appendVisualResource(svgHandle, fparentElement,
									resourceElement);
					resourceState.setSelectedItemId(name, id);
			...
			// stack
			UndoRedoAction action = new UndoRedoAction(
					getVisualResources().undoredoresourcesremove,
					executeRunnable, undoRunnable, executeRunnable,
					new HashSet<Element>());
	 ...
	 *            a resource object
	 */
	protected void importResource(Element parentElement,
			SVGVisualResourceObject resourceObject) {

		if (svgHandle != null && resourceObject != null
				&& !resourceObject.canBeModified() && parentElement != null) {

			final Element fparentElement = parentElement, resourceNode = resourceObject
					.getInitialNode();
			final String name = resourceObject.getResourceModel().getName();

			// creating the new resource node
			final Element newElement = getVisualResources()
					.getVisualResourcesToolkit().importVisualResource(
							svgHandle, fparentElement, resourceNode);
	...
	protected void modifyProperties(SVGVisualResourceObject resourceObject) {
		...
		if (svgHandle != null && resourceObject != null
			...
			final Element parentElement = (Element) resourceObject
					.getParentNode();
			final String name = resourceObject.getResourceModel().getName();

			// displays the dialog
			int returnValue = SVGVisualResourceResourceModifier
					.showPropertyModifierDialog(fresourceObject);
			...
			if (returnValue == SVGVisualResourceResourceModifier.OK) {
				...
				// getting the ids
				final String oldId = oldElement.getAttribute("id"), newId = fresourceObject
						.getResourceId();

				// getting all the elements that used the resource
				Set<Element> elements = svgHandle.getSvgResourcesManager()
						.getNodeUsingResource(oldId);
				...
				Runnable executeRunnable = new Runnable() {
					...
					public void run() {
						...
						// removing the initial node and adding the modified
						// node
						getVisualResources().getVisualResourcesToolkit()
								.removeVisualResource(svgHandle, oldElement);
						getVisualResources().getVisualResourcesToolkit()
								.appendVisualResource(svgHandle, parentElement,
										newElement);

						if (!newId.equals(oldId)) {

							// modifying the id of the resource in the map of
							// the resources in the
							// canvas
							svgHandle.getSvgResourcesManager()
									.modifyResourceId(newId, oldId);
						}

						// refreshing the nodes that use the resource
						svgHandle.getSvgResourcesManager()
								.refreshNodesUsingResource(newId);

						// refreshing the resources lists
						resourceState.setSelectedItemId(name, newId);
						refreshResources(RESOURCE_MODIFIED, name, newElement,
								oldElement);
					}
				};
				...
				Runnable undoRunnable = new Runnable() {
					...
					public void run() {
						...
						// removing the modified node and adding the initial
						// node
						getVisualResources().getVisualResourcesToolkit()
								.removeVisualResource(svgHandle, newElement);
						getVisualResources().getVisualResourcesToolkit()
						...
						// modifying the id of the resource in the map of the
						// resources in the
						// canvas
						if (!newId.equals(oldId)) {

							svgHandle.getSvgResourcesManager()
									.modifyResourceId(oldId, newId);
						}

						// refreshing the nodes that use the resource
						svgHandle.getSvgResourcesManager()
								.refreshNodesUsingResource(oldId);

						// refreshing the resources lists
						resourceState.setSelectedItemId(name, oldId);
						refreshResources(RESOURCE_MODIFIED, name, oldElement,
								newElement);
					}
				};
				...
				fresourceObject.getResourceModel().getResourceObjectsList()
						.remove(fresourceObject);

				refreshResources(RESOURCE_MODIFIED, name, oldElement,
						fresourceObject.getResourceNode());
			}
		}
	}
	 ...
	 * @author ITRIS, Jordi SUC
	 */
	protected class SVGVisualResourcesListCellRenderer
			implements
				ListCellRenderer {
		...
		public Component getListCellRendererComponent(JList list, Object item,
			...
			if (item != null) {
				...
				SVGVisualResourceObject resObj = ((SVGVisualResourceListItem) item)
						.getVisualResourceObject();
				panel.add(resourceComponent, BorderLayout.WEST);

				JLabel label = new JLabel(((SVGVisualResourceListItem) item)
						.getVisualResourceObject().getResourceId());
				panel.add(label, BorderLayout.CENTER);

				if (isSelected) {

					if (((SVGVisualResourceListItem) item)
							.getVisualResourceObject().canBeModified()) {
						...
						panel.setBackground(MetalLookAndFeel
								.getPrimaryControlDarkShadow());
						label.setBackground(MetalLookAndFeel
								.getPrimaryControlDarkShadow());
						label.setForeground(list.getSelectionForeground());
					}
