...
public class SVGVisualResourceModel {
	 ...
	 *            the element describing a resource
	 */
	public SVGVisualResourceModel(SVGVisualResources visualResources,
			Element resourceElement) {

		this.visualResources = visualResources;
		SVGHandle handle = visualResources.getSVGEditor().getHandlesManager()
				.getCurrentHandle();
		...
		if (resourceElement != null && handle != null) {
			...
			// the name of the type of the resource
			name = visualResources.getNormalizedString(resourceElement
					.getAttribute("name"));

			// gets the name of the tab
			if (visualResources.getBundle() != null) {

				defaultId = resourceElement
						.getAttribute("vresource_newresource");
			...
			for (cur = resourceElement.getFirstChild(); cur != null; cur = cur
					.getNextSibling()) {

				if (cur instanceof Element) {

					if (cur.getNodeName().equals("attribute")
							|| cur.getNodeName().equals("group")) {

						// creates the attributes models
						attr = new SVGVisualResourceModelAttribute(
								visualResources, (Element) cur);
						attributes.add(attr);

					} else if (cur.getNodeName().equals("child")) {

						// creates the child model
						childModel = new SVGVisualResourceModelChild(
								visualResources, (Element) cur);
			...
			// creating the resource objects for the nodes contained in the
			// "defs" node
			if (defs != null) {
			...
			Document visualResourceStore = visualResources
					.getVisualResourceStore();

			// creating the resource objects for the nodes contained in the
			// resource store
			if (visualResourceStore != null
					&& visualResourceStore.getDocumentElement() != null) {

				String id = "";

				for (cur = visualResourceStore.getDocumentElement()
	...
	protected void removeVisualResourceObject(String resourceId) {
		...
		if (resourceId != null && !resourceId.equals("")) {
			...
			for (Iterator it = new LinkedList(resourceObjectsList).iterator(); it
					.hasNext();) {
	...
	protected SVGVisualResourceObject createVisualResourceObject(Element element) {
		...
		if (element != null && element.getOwnerDocument() != null
				&& defs != null && defs.getOwnerDocument() != null
		...
		if (element != null) {
			...
			for (Iterator it = attributes.iterator(); it.hasNext();) {
				...
				if (attModel != null) {
					...
					resObjAttr = attModel
							.createVisualResourceObjectAttribute(cloneElement);
			...
			// creates the list of the resource objects of the children of the
			// resource node
			...
			if (childModel != null) {
				...
				if (cloneElement.hasChildNodes()) {
					...
					for (Node cur = cloneElement.getFirstChild(); cur != null; cur = cur
							.getNextSibling()) {

						if (cur instanceof Element) {

							resObjChild = childModel
									.createVisualResourceObjectChild((Element) cur);
					...
					resObjChild = childModel
							.createVisualResourceObjectChild(child);
			...
			// creates the resource object
			resObj = new SVGVisualResourceObject(cloneElement, element, this,
					resObjAttrList, resObjChildList, canBeModified);
