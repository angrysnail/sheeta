...
public class SVGVisualResourceModelAttribute {
	...
	/**
	 * the list containing the attributes of this group if this attribute is a
	 * group attribute
	 */
	private final LinkedList groupAttributes = new LinkedList();

	/**
	 * the map associating the name of a the description of the content of the
	 * attribute to its default value
	 */
	private final LinkedHashMap contents = new LinkedHashMap();

	/**
	 * the map of the possible values for the attribute, associating the name to
	 * the value
	 */
	 ...
	 *            the node of the description of the attribute
	 */
	public SVGVisualResourceModelAttribute(SVGVisualResources visualResources,
		...
		if (attributeElement != null) {
			...
			name = visualResources.getNormalizedString(attributeElement
					.getAttribute("name"));
			...
			// checks if this node is a group node and then creates the
			// attribute objects linked to
			// it
			if (attributeElement.getNodeName().equals("group")) {

				SVGVisualResourceModelAttribute attr = null;

				for (Node cur = attributeElement.getFirstChild(); cur != null; cur = cur
						.getNextSibling()) {
				...
				// adds the values in the "contents" map and in the possible
				// values map
	...
	/**
	 * @return the list of the attributes contained in this node, if this is a
	 *         group attribute
	 */
	...
	/**
	 * @return the map associating the name of a the description of the content
	 *         of the attribute to its default value
	 */
	 ...
	 * 
	 * @param element
	 *            a resource node or a child node
	 * @return a resource object
	 */
	protected SVGVisualResourceObjectAttribute createVisualResourceObjectAttribute(
			Element element) {
		...
		if (element != null) {
			...
			if (isGroupAttribute()) {
				...
				for (Iterator it = groupAttributes.iterator(); it.hasNext();) {
					...
					if (attModel != null) {
						...
						resObjAttr2 = attModel
								.createVisualResourceObjectAttribute(element);
			...
			SVGHandle svgHandle = visualResources.getSVGEditor()
			...
			if (!isGroupAttribute()) {
				...
				attr = visualResources.getVisualResourcesToolkit()
						.getVisualResourceAttributeNode(svgHandle, element,
								name, defaultValue);
			}

			// creates the resource object
			resObjAttr = new SVGVisualResourceObjectAttribute(attr, this,
					resObjAttrList);
		}
