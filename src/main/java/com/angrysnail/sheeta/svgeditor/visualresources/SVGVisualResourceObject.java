...
public class SVGVisualResourceObject {
	 ...
	 * 
	 * @param resourceNode
	 *            the node of the resource in the svg document
	 * @param initialNode
	 *            the initial resource node
	 * @param resourceModel
	 *            the model of this resource
	 * @param attributes
	 *            the list of the attributes
	 ...
	 *            the list of the children
	 * @param canBeModified
	 *            true if the resource object can be modified
	 */
	public SVGVisualResourceObject(Element resourceNode, Element initialNode,
			SVGVisualResourceModel resourceModel, LinkedList attributes,
			LinkedList children, boolean canBeModified) {
		...
		canCreateChildren = (!resourceNode.hasAttributeNS(
	...
	/**
	 * modifies the values of the nodes of the attributes of the resource node
	 * and of its children
	 */
	protected void applyChanges() {
		...
		// modifies the value of the attribute nodes of the children nodes (if
		// they exist)
	...
	protected SVGVisualResourceObjectChild createNewChild() {
		...
		if (canCreateChildren) {
			...
			SVGVisualResourceModelChild childModel = resourceModel
					.getChildModel();

			if (childModel != null && resourceNode != null
					&& resourceNode.getOwnerDocument() != null) {

				final SVGHandle svgHandle = resourceModel.getVisualResources()
						.getSVGEditor().getHandlesManager().getCurrentHandle();

				// creates a new child node
				final Element childElement = resourceModel
						.getVisualResources()
						.getVisualResourcesToolkit()
						.createVisualResourceChildStructure(svgHandle,
								resourceNode);
				...
				if (childElement != null) {
					...
					child = childModel
							.createVisualResourceObjectChild(childElement);
	...
	protected void removeChildResource(SVGVisualResourceObjectChild resChild) {
		...
		if (canCreateChildren && resChild != null && svgHandle != null) {
			...
			resourceModel
					.getVisualResources()
					.getVisualResourcesToolkit()
					.removeVisualResourceChild(svgHandle,
							fresChild.getChildElement());
