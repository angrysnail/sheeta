 ...
 */
public class SVGVisualResourcePatternSizeChooserAttributeWidget
		extends
	 ...
	 */
	public SVGVisualResourcePatternSizeChooserAttributeWidget(
	...
	protected void buildComponent() {
		...
		if (resourceObjectAttribute != null) {
			...
			SVGVisualResourceObjectAttribute xAtt = null, yAtt = null, wAtt = null, hAtt = null, patternUnitsAtt = null, att = null;
			...
			final SVGVisualResourceObjectAttribute fxAtt = xAtt, fyAtt = yAtt, fwAtt = wAtt, fhAtt = hAtt, fpatternUnitsAtt = patternUnitsAtt;

			// getting the bounds values//
			Rectangle2D.Double bounds = new Rectangle2D.Double(0, 0, 0, 0), defBounds = new Rectangle2D.Double(
					0, 0, 0, 0), otherBounds = new Rectangle2D.Double(0, 0, 0,
					0);

			// the object containing the values for this chooser
			SVGPatternSizeChooserValues patternChValues = null;

			if (patternUnitsAtt != null && xAtt != null && yAtt != null
					&& wAtt != null && hAtt != null) {

				boolean isPatternUnitsDefault = (!patternUnitsAtt.getValue()
						.equals(patternUnitsOther));

				bounds.x = svgEditor.getSVGToolkit().getDoubleValue(
						xAtt.getValue(), isPatternUnitsDefault);
				bounds.y = svgEditor.getSVGToolkit().getDoubleValue(
						yAtt.getValue(), isPatternUnitsDefault);
				bounds.width = svgEditor.getSVGToolkit().getDoubleValue(
						wAtt.getValue(), isPatternUnitsDefault);
				bounds.height = svgEditor.getSVGToolkit().getDoubleValue(
						hAtt.getValue(), isPatternUnitsDefault);
				...
				if (isPatternUnitsDefault) {
					...
					defBounds = new Rectangle2D.Double(svgEditor
							.getSVGToolkit().getDoubleValue(
									xAtt.getModel().getDefaultValue(), true),
							svgEditor.getSVGToolkit().getDoubleValue(
									yAtt.getModel().getDefaultValue(), true),
							svgEditor.getSVGToolkit().getDoubleValue(
									wAtt.getModel().getDefaultValue(), true),
							svgEditor.getSVGToolkit().getDoubleValue(
									hAtt.getModel().getDefaultValue(), true));
				}

				patternChValues = new SVGPatternSizeChooserValues(defBounds,
						otherBounds, isPatternUnitsDefault);
			}

			// the object containing the values for this chooser
			final SVGPatternSizeChooserValues fpatternChValues = patternChValues;

			if (patternChValues != null && xAtt != null && yAtt != null
					&& wAtt != null && hAtt != null) {
				...
				final LinkedHashMap possibleValues = patternUnitsAtt.getModel()
						.getPossibleValues();
				...
				for (Iterator it = possibleValues.keySet().iterator(); it
					...
					try {
						...
						// the property
						if (val != null
								&& val.equals(patternUnitsAtt.getValue())) {
				...
				if (fpatternUnitsAtt != null) {
					...
					if (bundle != null) {
						...
						try {
							patternUnitsLabel = bundle
									.getString(fpatternUnitsAtt.getModel()
											.getAbsoluteName());
						} catch (Exception ex) {
						}
					}
				...
				final JTextField xTxt = new JTextField(format.format(bounds.x)), yTxt = new JTextField(
						format.format(bounds.y)), wTxt = new JTextField(
						format.format(bounds.width)), hTxt = new JTextField(
						format.format(bounds.height));

				// getting the labels
				String strX = xAtt.getModel().getName(), strY = yAtt.getModel()
						.getName(), strW = wAtt.getModel().getName(), strH = hAtt
						.getModel().getName();
				...
				final JLabel xLbl = new JLabel(strX.concat("=")), yLbl = new JLabel(
						strY.concat("=")), wLbl = new JLabel(strW.concat("=")), hLbl = new JLabel(
						strH.concat("="));
				...
				final JLabel xUnitLbl = new JLabel(unitSign), yUnitLbl = new JLabel(
						unitSign), wUnitLbl = new JLabel(unitSign), hUnitLbl = new JLabel(
						unitSign);
				...
				final ActionListener comboListener = new ActionListener() {
					...
					public void actionPerformed(ActionEvent evt) {
						...
						if (val != null && !val.equals("")) {
							...
							String xStr = xTxt.getText(), yStr = yTxt.getText(), wStr = wTxt
									.getText(), hStr = hTxt.getText();
							...
							theBounds.x = svgEditor.getSVGToolkit()
									.getDoubleValue(
											xStr,
											fpatternChValues
													.isDefaultPatternUnits());
							theBounds.y = svgEditor.getSVGToolkit()
									.getDoubleValue(
											yStr,
											fpatternChValues
													.isDefaultPatternUnits());
							theBounds.width = svgEditor.getSVGToolkit()
									.getDoubleValue(
											wStr,
											fpatternChValues
													.isDefaultPatternUnits());
							theBounds.height = svgEditor.getSVGToolkit()
									.getDoubleValue(
											hStr,
											fpatternChValues
													.isDefaultPatternUnits());
							...
							fpatternChValues
									.setDefaultPatternUnits(!fpatternUnitsAtt
											.getValue().equals(
													patternUnitsOther));
							...
							if (fpatternChValues.isDefaultPatternUnits()) {
								...
								xStr = format.format(fpatternChValues
										.getDefBounds().x);
								yStr = format.format(fpatternChValues
										.getDefBounds().y);
								wStr = format.format(fpatternChValues
										.getDefBounds().width);
								hStr = format.format(fpatternChValues
										.getDefBounds().height);
								...
								yStr = format.format(fpatternChValues
										.getOtherBounds().y);
								wStr = format.format(fpatternChValues
										.getOtherBounds().width);
								hStr = format.format(fpatternChValues
										.getOtherBounds().height);
	...
	protected class SVGPatternSizeChooserValues {
		 ...
		 * the default bounds
		 */
		private Rectangle2D.Double defBounds = new Rectangle2D.Double(0, 0, 0,
				0);

		/**
		 * the other bounds
		 */
		private Rectangle2D.Double otherBounds = new Rectangle2D.Double(0, 0,
				0, 0);
		 ...
		 * 
		 * @param defBounds
		 *            the default bounds of the pattern
		 * @param otherBounds
		 *            the other bounds of the pattern
		 * @param isDefaultPatternUnits
		 *            the boolean telling if the default units are used or not
		 */
		protected SVGPatternSizeChooserValues(Rectangle2D.Double defBounds,
				Rectangle2D.Double otherBounds, boolean isDefaultPatternUnits) {
