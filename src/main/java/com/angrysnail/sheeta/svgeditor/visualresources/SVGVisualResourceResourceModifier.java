...
public class SVGVisualResourceResourceModifier {
	 ...
	 * a small font
	 */
	protected static final Font smallFont = new Font("smallFont",
			Font.ROMAN_BASELINE, 9);

	/**
	 * the font
	 */
	protected static final Font theFont = new Font("theFont",
	 ...
	 */
	public static int showPropertyModifierDialog(
		...
		if (resourceObject != null) {
			...
			if (bundle != null) {
				...
				try {
					title = bundle
							.getString(resourceObject
									.getResourceModel()
									.getVisualResources()
									.getAbsoluteString(
											resourceObject.getResourceModel()
													.getName()));
					title = title.concat(" ".concat(bundle
							.getString("vresource_properties")));
				} catch (Exception ex) {
				}
			}
			...
			// the buttons
			final JButton okBt = new JButton(okLabel), cancelBt = new JButton(
					cancelLabel);
			...
			dialog.setLocation(
					(int) (Toolkit.getDefaultToolkit().getScreenSize()
							.getWidth() / 2 - dialog.getSize().width / 2),
					(int) (Toolkit.getDefaultToolkit().getScreenSize()
							.getHeight() / 2 - dialog.getSize().height / 2));
	 ...
	 *         object
	 */
	protected static SVGVisualResourceWidget getChildWidget(
			SVGVisualResourceObject resourceObject) {
		...
		if (resourceObject.getChildren().size() > 0) {
			...
			try {
				child = (SVGVisualResourceObjectChild) resourceObject
	...
	protected static SVGVisualResourceWidget getAttributeWidget(
		...
		if (resourceObjectAttribute != null
				&& resourceObjectAttribute.getModel() != null) {

			SVGVisualResourceModelAttribute attributeModel = resourceObjectAttribute
					.getModel();
			...
			if (type != null) {
				...
				if (type.equals("combo")) {
					...
					widget = new SVGVisualResourceComboAttributeWidget(
					...
					widget = new SVGVisualResourceEntryAttributeWidget(
							resourceObjectAttribute);

				} else if (type.equals("vectorchooser")
						|| type.equals("circlechooser")) {
					...
					widget = new SVGVisualResourceSliderAttributeWidget(
							resourceObjectAttribute);
