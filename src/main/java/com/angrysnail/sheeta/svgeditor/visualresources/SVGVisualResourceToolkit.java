...
public class SVGVisualResourceToolkit {
	 ...
	 *            the parentNode
	 * @param nodeName
	 *            the name of the node of the accurate resource
	 * @param idShapeToBeAppended
	 *            the id of the shape to be appended as a child of the resource
	 * @return the element corresponding to the given name of the resource
	 */
	protected Element createVisualResource(SVGHandle handle,
			Element parentNode, String nodeName, String idShapeToBeAppended) {

		Element resource = null;
		ResourceBundle bundle = ResourcesManager.bundle;

		if (handle != null && parentNode != null && nodeName != null
				&& !nodeName.equals("")) {
			...
			if (idShapeToBeAppended != null && !idShapeToBeAppended.equals("")) {
				...
				Element elt = null;
				Document doc = handle.getScrollPane().getSVGCanvas()
						.getDocument();
				...
				if (doc != null) {
					...
					if (elt != null) {
						...
						// setting a transform value to put the upper left
						// corner of the document
						AffineTransform af = handle.getSvgElementsManager()
								.getTransform(childElement);
						...
						if (af != null) {
							...
							af.preConcatenate(AffineTransform
									.getTranslateInstance(e, f));
							handle.getSvgElementsManager().setTransform(
									childElement, af);
						}
					}
				}
			...
			String svgNS = parentNode.getOwnerDocument().getDocumentElement()
					.getNamespaceURI();
			resource = parentNode.getOwnerDocument().createElementNS(svgNS,
					nodeName);
			...
			if (nodeName.equals("linearGradient")) {
				...
				resource.setAttributeNS(null, "gradientUnits",
						"objectBoundingBox");

				Element child = parentNode.getOwnerDocument().createElementNS(
						svgNS, "stop");
				child.setAttributeNS(null, "offset", "0%");
				child.setAttributeNS(null, "style",
				...
				resource.setAttributeNS(null, "patternUnits",
						"objectBoundingBox");
				resource.setAttributeNS(null, "patternContentUnits",
						"userSpaceOnUse");
	 ...
	 */
	protected Element duplicateVisualResource(SVGHandle handle,
		...
		if (resourceNode != null) {
			...
			if (parentNode != null) {
				...
				// getting the id of the resource node and creating the id for
				// the cloned node
				String suffix = "";
				...
				if (bundle != null) {
					...
					try {
						suffix = bundle
								.getString("vresource_duplicatednodesuffix");
					} catch (Exception ex) {
					}
				}
	 ...
	 *            a resource node
	 */
	protected void appendVisualResource(SVGHandle handle, Element parentNode,
			Element resourceNode) {
	 ...
	 */
	protected boolean canRemoveVisualResource(SVGHandle handle,
			Element resourceNode) {
	 ...
	 * @param parentNode
	 *            the parentNode
	 * @param nodeToBeImported
	 *            the node that should be imported
	 * @return the imported node
	 */
	protected Element importVisualResource(SVGHandle handle,
			Element parentNode, Element nodeToBeImported) {

		Element resourceNode = null;
		Document visualResourceStore = getVisualResources()
				.getVisualResourceStore();
		ResourceBundle bundle = getVisualResources().getBundle();

		if (handle != null && parentNode != null && visualResourceStore != null
				&& nodeToBeImported != null) {
	...
	/**
	 * gets the attribute node of the parent node given its name or creates it
	 ...
	 * @param value
	 *            the value of the attribute if it is created
	 * @return the attribute node
	 */
	public Node getVisualResourceAttributeNode(SVGHandle handle,
			Element parentNode, String name, String value) {

		Node attNode = null;

		if (handle != null && parentNode != null && name != null
				&& !name.equals("")) {
	 ...
	 *            the current svg handle
	 * @param parentNode
	 *            the parentNode to which the child will be appended
	 * @return the element corresponding to a child of a resource whose name is
	 *         nodeName
	 */
	protected Element createVisualResourceChildStructure(SVGHandle handle,
			Node parentNode) {
		...
		if (handle != null && parentNode != null) {
			...
			if (parentNode.getNodeName().equals("linearGradient")
					|| parentNode.getNodeName().equals("radialGradient")) {

				String svgNS = SVGDOMImplementation.SVG_NAMESPACE_URI;
				resourceChild = parentNode.getOwnerDocument().createElementNS(
						svgNS, "stop");
				resourceChild.setAttributeNS(null, "offset", "0%");
				resourceChild.setAttributeNS(null, "style",
						"stop-color:#000000;stop-opacity:1.0;");
			}
		}
	 ...
	 *            the parentNode to which the child will be appended
	 * @param childNode
	 *            the child of a resource node
	 */
	protected void appendVisualResourceChild(SVGHandle handle, Node parentNode,
			Node childNode) {
