 ...
 * enabling to modify the properties of an attribute (or a group of attributes)
 * of a resource node.
 * 
 * @author ITRIS, Jordi SUC
 */
public class SVGVisualResourceVectorCircleChooserAttributeWidget
		extends
	 ...
	 */
	public SVGVisualResourceVectorCircleChooserAttributeWidget(
	...
	protected void buildComponent() {
		...
		if (resourceObjectAttribute != null) {
			...
			SVGVisualResourceObjectAttribute px1Att = null, py1Att = null, px2Att = null, py2Att = null, radiusAtt = null, gradientUnitsAtt = null, att = null;

			final String gradientUnitsOther = "userSpaceOnUse";
			String nm = "";

			for (Iterator it = resourceObjectAttribute.getGroupAttributes()
					.iterator(); it.hasNext();) {
			...
			final SVGVisualResourceObjectAttribute fpx1Att = px1Att, fpy1Att = py1Att, fpx2Att = px2Att, fpy2Att = py2Att, fradiusAtt = radiusAtt, fgradientUnitsAtt = gradientUnitsAtt;

			// getting the points value//
			Point2D.Double point1 = new Point2D.Double(0, 0), point2 = new Point2D.Double(
					0, 0);
			double radius = 0;

			Point2D.Double defPoint1 = new Point2D.Double(0, 0), defPoint2 = new Point2D.Double(
					0, 0);
			double defRadius = 0;

			Point2D.Double otherPoint1 = new Point2D.Double(0, 0), otherPoint2 = new Point2D.Double(
					0, 0);
			...
			if (gradientUnitsAtt != null && px1Att != null && py1Att != null
					&& px2Att != null && py2Att != null) {

				boolean isGradientUnitsDefault = (!gradientUnitsAtt.getValue()
						.equals(gradientUnitsOther));

				point1.x = svgEditor.getSVGToolkit().getDoubleValue(
						px1Att.getValue(), isGradientUnitsDefault);
				point1.y = svgEditor.getSVGToolkit().getDoubleValue(
						py1Att.getValue(), isGradientUnitsDefault);
				point2.x = svgEditor.getSVGToolkit().getDoubleValue(
						px2Att.getValue(), isGradientUnitsDefault);
				point2.y = svgEditor.getSVGToolkit().getDoubleValue(
						py2Att.getValue(), isGradientUnitsDefault);

				if (radiusAtt != null) {

					radius = svgEditor.getSVGToolkit().getDoubleValue(
							radiusAtt.getValue(), isGradientUnitsDefault);
				}
				...
				if (isGradientUnitsDefault) {
					...
					defPoint1.x = svgEditor.getSVGToolkit().getDoubleValue(
							px1Att.getModel().getDefaultValue(), true);
					defPoint1.y = svgEditor.getSVGToolkit().getDoubleValue(
							py1Att.getModel().getDefaultValue(), true);
					defPoint2.x = svgEditor.getSVGToolkit().getDoubleValue(
							px2Att.getModel().getDefaultValue(), true);
					defPoint2.y = svgEditor.getSVGToolkit().getDoubleValue(
							py2Att.getModel().getDefaultValue(), true);

					if (radiusAtt != null) {

						defRadius = svgEditor.getSVGToolkit().getDoubleValue(
								radiusAtt.getModel().getDefaultValue(), true);
					}
				}

				vectChValues = new SVGVectorCircleChooserValues(defPoint1,
						defPoint2, defRadius, otherPoint1, otherPoint2,
						otherRadius, isGradientUnitsDefault,
						type.equals("vectorchooser"));
			}

			// the object containing the values for this chooser
			final SVGVectorCircleChooserValues fvectChValues = vectChValues;

			if (vectChValues != null && px1Att != null && py1Att != null
					&& px2Att != null && py2Att != null) {
				...
				final LinkedHashMap possibleValues = gradientUnitsAtt
						.getModel().getPossibleValues();
				...
				for (Iterator it = possibleValues.keySet().iterator(); it
						.hasNext();) {
					...
					try {
						...
						// the property
						if (val != null
								&& val.equals(gradientUnitsAtt.getValue()))
							selectedItm = itm;
				...
				if (fgradientUnitsAtt != null) {
					...
					if (bundle != null) {
						...
						try {
							gradientUnitsLabel = bundle
									.getString(fgradientUnitsAtt.getModel()
											.getAbsoluteName());
						} catch (Exception ex) {
						}
					}
				...
				final SVGVectorCircleChooserItems items = new SVGVectorCircleChooserItems(
						fvectChValues.getDefaultPoint1(),
						fvectChValues.getDefaultPoint2(),
				...
				// the entries which will be used to manually set the
				// coordinates of the vector
				final JTextField px1Txt = new JTextField(
						format.format(point1.x)), py1Txt = new JTextField(
						format.format(point1.y)), px2Txt = new JTextField(
						format.format(point2.x)), py2Txt = new JTextField(
						format.format(point2.y)), radiusTxt = new JTextField(
						format.format(radius));

				// the labels corresponding to the entries
				final JLabel px1Lbl = new JLabel(px1Att.getModel().getName()
						.concat("=")), py1Lbl = new JLabel(py1Att.getModel()
						.getName().concat("=")), px2Lbl = new JLabel(px2Att
						.getModel().getName().concat("=")), py2Lbl = new JLabel(
						py2Att.getModel().getName().concat("=")), radiusLbl = new JLabel();

				if (radiusAtt != null) {

					radiusLbl.setText(radiusAtt.getModel().getName()
							.concat("="));
				}
				...
				final JLabel px1UnitLbl = new JLabel(unitSign), py1UnitLbl = new JLabel(
						unitSign), px2UnitLbl = new JLabel(unitSign), py2UnitLbl = new JLabel(
						unitSign), radiusUnitLbl = new JLabel(unitSign);
				...
				final SVGVectorCirclePanelListener vectorPanelListener = new SVGVectorCirclePanelListener() {
					...
					/**
					 * the integer describing the point on which a click has
					 * been done
					 */
					...
					public void mouseDragged(MouseEvent e) {
						...
						if (vectorPanelSize != null) {
							...
							// if a point of the vector has been hit, sets the
							// new value while
							// dragging
							if (currentPoint == SVGVectorCircleChooserItems.POINT1
								...
								point.x = 100 * e.getPoint().x
										/ vectorPanelSize.width;
								point.y = 100 * e.getPoint().y
										/ vectorPanelSize.height;
								...
								if (currentPoint == SVGVectorCircleChooserItems.POINT1) {
									...
									if (point1 != null) {
										...
										double cy = 100 * point.y
												/ vectorPanelSize.height;
										double rad = Math.abs(point1.y - cy);

										items.setRadius(rad);

										radiusTxt.setText(format.format(items
												.getRadius()));
										fvectChValues.setDefaultRadius(items
												.getRadius());
									}
								}
					...
					/**
					 * applies the changes made on the values of the attributes
					 * to the attributes
					 */
					protected void applyChanges() {

						if (items != null
								&& fvectChValues.isDefaultGradientUnits) {

							Point2D.Double point1 = items.getPoint1(), point2 = items
									.getPoint2();
							...
							if (fradiusAtt != null) {
								...
								fradiusAtt.setValue(format.format(items
										.getRadius()) + "%");
							}
						}
					}
					...
					public void caretUpdate(CaretEvent evt) {
						...
						if (evt.getSource().equals(px1Txt)) {
							...
							if (fvectChValues.isDefaultGradientUnits()) {
								...
								val = svgEditor.getSVGToolkit().getDoubleValue(
										px1Txt.getText().concat("%"),
										fvectChValues.isDefaultGradientUnits());
								fvectChValues.getDefaultPoint1().x = val;
								items.setPoint(
										fvectChValues.getDefaultPoint1(),
										SVGVectorCircleChooserItems.POINT1);
							...
							if (fvectChValues.isDefaultGradientUnits()) {
								...
								val = svgEditor.getSVGToolkit().getDoubleValue(
										px2Txt.getText().concat("%"),
										fvectChValues.isDefaultGradientUnits());
								fvectChValues.getDefaultPoint2().x = val;
								items.setPoint(
										fvectChValues.getDefaultPoint2(),
										SVGVectorCircleChooserItems.POINT2);
						...
						} else if (evt.getSource().equals(radiusTxt)
								&& fradiusAtt != null) {

							if (fvectChValues.isDefaultGradientUnits()) {

								val = svgEditor.getSVGToolkit().getDoubleValue(
										radiusTxt.getText().concat("%"),
										fvectChValues.isDefaultGradientUnits());
								fvectChValues.setDefaultRadius(val);
								items.setRadius(fvectChValues
										.getDefaultRadius());
				...
				vectChooserPanel.setLayout(new BoxLayout(vectChooserPanel,
						BoxLayout.X_AXIS));
				...
				// sets the layout
				textfields
						.setLayout(new BoxLayout(textfields, BoxLayout.Y_AXIS));
				...
				// the panel containing the vector panel and the textfields
				// panel
				...
				final ActionListener comboListener = new ActionListener() {
					...
					public void actionPerformed(ActionEvent evt) {
						...
						if (val != null && !val.equals("")) {
							...
							String px1Str = px1Txt.getText(), py1Str = py1Txt
									.getText(), px2Str = px2Txt.getText(), py2Str = py2Txt
									.getText(), radiusStr = radiusTxt.getText();
							...
							point1.x = svgEditor.getSVGToolkit()
									.getDoubleValue(
											px1Str,
											fvectChValues
													.isDefaultGradientUnits());
							point1.y = svgEditor.getSVGToolkit()
									.getDoubleValue(
											py1Str,
											fvectChValues
													.isDefaultGradientUnits());
							point2.x = svgEditor.getSVGToolkit()
									.getDoubleValue(
											px2Str,
											fvectChValues
													.isDefaultGradientUnits());
							point2.y = svgEditor.getSVGToolkit()
									.getDoubleValue(
											py2Str,
											fvectChValues
													.isDefaultGradientUnits());
							...
							fvectChValues
									.setDefaultGradientUnits(!fgradientUnitsAtt
											.getValue().equals(
													gradientUnitsOther));
							...
							if (fvectChValues.isDefaultGradientUnits()) {
								...
								px1Str = format.format(fvectChValues
										.getDefaultPoint1().x);
								py1Str = format.format(fvectChValues
										.getDefaultPoint1().y);
								px2Str = format.format(fvectChValues
										.getDefaultPoint2().x);
								py2Str = format.format(fvectChValues
										.getDefaultPoint2().y);
								radiusStr = format.format(fvectChValues
										.getDefaultRadius());
								...
								// sets the color for the labels
								px1Lbl.setForeground(fvectChValues
										.getPoint1Color());
								py1Lbl.setForeground(fvectChValues
										.getPoint1Color());
								px2Lbl.setForeground(fvectChValues
										.getPoint2Color());
								py2Lbl.setForeground(fvectChValues
										.getPoint2Color());
								radiusLbl.setForeground(fvectChValues
										.getRadiusColor());
								...
								// adds the listener to the vector panel
								vectorPanel
										.addMouseListener(vectorPanelListener);
								vectorPanel
										.addMouseMotionListener(vectorPanelListener);
								...
								vectorPanel
										.removeMouseListener(vectorPanelListener);
								vectorPanel
										.removeMouseMotionListener(vectorPanelListener);
							}
	 ...
	 *
	 */
	protected interface SVGVectorCirclePanelListener
			extends
				MouseListener,
				MouseMotionListener {
	...
	protected class SVGVectorCircleChooserValues {
		 ...
		 * @param otherRadius
		 *            the other radius
		 * @param isDefaultGradientUnits
		 *            the boolean telling if the default units are used or not
		 * @param isVectorChooser
		 *            the boolean telling if the vector chooser is used
		 */
		protected SVGVectorCircleChooserValues(Point2D.Double defaultPoint1,
				Point2D.Double defaultPoint2, double defaultRadius,
				Point2D.Double otherPoint1, Point2D.Double otherPoint2,
				double otherRadius, boolean isDefaultGradientUnits,
				boolean isVectorChooser) {
	...
	public class SVGVectorCircleChooserItems {
		 ...
		 */
		private Point2D.Double point1 = new Point2D.Double(0, 0),
		 ...
		 * 
		 * @param point1
		 *            the first point in percentage values
		 * @param point2
		 *            the second point in percentage values
		 * @param radius
		 *            the radius in percentage values
		 * @param vectorPanelSize
		 *            the size of the vector panel
		 * @param isVectorChooser
		 *            the boolean telling the state of the items
		 * @param vectCircleChooser
		 *            the object containing information on the vector and circle
		 *            chooser
		 */
		public SVGVectorCircleChooserItems(Point2D.Double point1,
				Point2D.Double point2, double radius,
		 ...
		 *            a point
		 * @param type
		 *            the type of the point (point1 or point2)
		 */
		...
		/**
		 * tells whether the given point corresponds to one of the vector's
		 * point
		 * 
		 * @param point
		 *            a point
		...
		protected int hasHitAPoint(Point point) {
			...
			if (point != null) {
				...
				Rectangle2D.Double rect1 = new Rectangle2D.Double(vPoint1.x
						- average, vPoint1.y - average, 2 * average,
						2 * average), rect2 = new Rectangle2D.Double(vPoint2.x
						- average, vPoint2.y - average, 2 * average,
						2 * average), rectRadius = new Rectangle2D.Double(
						vPoint1.x - average, vPoint1.y - vRadius - average,
						2 * average, 2 * average);
		...
		protected void paintVectorOrCircle(Graphics2D g) {
			...
			if (g != null) {
				...
				if (enabled) {
					...
					// the elements that will be painted in the vector chooser
					// version
					if (isVectorChooser) {

						g.setColor(Color.darkGray);
						g.drawLine((int) vPoint1.x, (int) vPoint1.y,
								(int) vPoint2.x, (int) vPoint2.y);

						g.setColor(vectCircleChooser.getPoint1Color());
						g.drawLine((int) (vPoint1.x - average),
								(int) (vPoint1.y - average),
								(int) (vPoint1.x + average),
								(int) (vPoint1.y + average));
						g.drawLine((int) (vPoint1.x + average),
								(int) (vPoint1.y - average),
								(int) (vPoint1.x - average),
						...
						g.drawOval((int) (vPoint2.x - average),
								(int) (vPoint2.y - average), 2 * average,
								2 * average);

					} else {

						// the elements that will be painted in the circle
						// chooser version
						double vRadius = getPanelRadius();

						g.setColor(vectCircleChooser.getPoint2Color());
						g.drawLine((int) (vPoint2.x - average),
								(int) (vPoint2.y - average),
								(int) (vPoint2.x + average),
								(int) (vPoint2.y + average));
						g.drawLine((int) (vPoint2.x + average),
								(int) (vPoint2.y - average),
								(int) (vPoint2.x - average),
						...
						g.setColor(Color.darkGray);
						g.drawOval((int) (vPoint1.x - vRadius),
								(int) (vPoint1.y - vRadius),
								(int) (2 * vRadius), (int) (2 * vRadius));

						g.setColor(vectCircleChooser.getRadiusColor());
						g.drawLine((int) vPoint1.x, (int) vPoint1.y,
								(int) vPoint1.x, (int) (vPoint1.y - vRadius));
						g.drawLine((int) (vPoint1.x - average / 2),
								(int) (vPoint1.y - vRadius - average / 2),
								(int) (vPoint1.x + average / 2),
								(int) (vPoint1.y - vRadius + average / 2));
						g.drawLine((int) (vPoint1.x + average / 2),
								(int) (vPoint1.y - vRadius - average / 2),
								(int) (vPoint1.x - average / 2),
					...
					if (vectorPanelSize != null) {
						...
						g.setColor(Color.darkGray);
						g.fillRect(0, 0, vectorPanelSize.width,
								vectorPanelSize.height);
					}
				}
			}
