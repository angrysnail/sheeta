...
public class SVGVisualResources extends ModuleAdapter {
	...
	public SVGVisualResources(Editor editor) {
		...
		if (bundle != null) {
			...
			try {
				...
				undoredoresourcesremove = bundle
						.getString("undoredoresourcesremove");
			} catch (Exception ex) {
			}
		}
		...
		final HandlesListener svgHandlesListener = new HandlesListener() {
			...
			public void handleChanged(SVGHandle currentHandle,
				...
				for (Iterator it = new LinkedList(stateMap.keySet()).iterator(); it
						.hasNext();) {
		...
		visualResourcesPanel.setLayout(new BoxLayout(visualResourcesPanel,
				BoxLayout.Y_AXIS));
		...
		visualResourcesFrame = new ToolsFrame(editor, idvisualresources,
				labelresources, visualResourcesPanel);
		...
		Runnable visibilityRunnable = new Runnable() {
			...
			public void run() {
				...
				SVGHandle svgHandle = getSVGEditor().getHandlesManager()
						.getCurrentHandle();
	...
	public HashMap getMenuItems() {
		...
		menuItems.put("ToolFrame_" + this.idvisualresources,
				visualResourcesFrame.getMenuItem());
	...
	protected void handleVisualResources(SVGHandle handle) {
		...
		if (hasBeenReinitialized
				&& (!visualResourcesFrame.isVisible() || handle == null)) {
		...
		if (handle != null && visualResourcesFrame.isVisible()) {
			...
			visualResourceStore = editor.getResourcesManager()
					.getResourceStore();
			...
			visualResourcesPanel.setPreferredSize(new Dimension(
		...
		if (bundle != null) {
			...
			try {
				message = bundle
						.getString("visualresources_empty_dialog_noframe");
			} catch (Exception ex) {
			}
