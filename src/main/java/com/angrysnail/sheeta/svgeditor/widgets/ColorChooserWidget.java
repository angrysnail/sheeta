package com.angrysnail.sheeta.svgeditor.widgets;
	...
	public void init(Object objectValue) {
		...
		if (objectValue != null) {
			...
			if (objectValue instanceof Color) {
				...
				currentValue = Editor.getColorChooser().getColorString(
						currentColor);
				...
				if (colorString.equals("")) {
					...
					if (handle != null) {
						...
						Color color = Editor.getColorChooser().getColor(handle,
								colorString);
	...
	protected void build() {
		...
		button = new JButton() {
			...
			protected void paintComponent(Graphics g) {
				...
				if (currentColor != null) {
					...
					g.fillRect(diff, diff, prefSize.width - 2 * diff,
							prefSize.height - 2 * diff);
				}
			}
		};
		...
		colorButtonListener = new ActionListener() {
			...
			public void actionPerformed(ActionEvent evt) {
				...
				Color color = Editor.getColorChooser().showColorChooserDialog(
						currentColor);
