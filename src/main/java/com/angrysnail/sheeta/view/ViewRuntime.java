...
public class ViewRuntime {
	 ...
	 */
	public ViewRuntime(String path, boolean isDebugMode,
	...
	protected void buildMainFrame() {
		...
		svgView = new View(regularFrame, regularFrame, true, true, isDebugMode);

		regularFrame.getContentPane().setLayout(
				new BoxLayout(regularFrame.getContentPane(), BoxLayout.Y_AXIS));
		...
		if (svgView.getBundle() != null) {
			...
			try {
				mainFrameTitle = svgView.getBundle()
						.getString("mainFrameTitle");
			} catch (Exception ex) {
			}
		}
		...
		if (isFullScreenMode) {
			...
			Dimension preferredDimension = new Dimension(300,
					regularFrame.getHeight()
							- regularFrame.getContentPane().getHeight());

			// computing the location
			Dimension screenSize = Toolkit.getDefaultToolkit().getScreenSize();
			Point location = new Point(
					(screenSize.width - preferredDimension.width) / 2,
